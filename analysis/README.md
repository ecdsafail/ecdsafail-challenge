...
| `verify/offset_window_encoding.py` | **Removes** the dominant exceptional term of the ladder — the zero-window `∞` addend (issue #5 part (b), ADR 0012). Implements the **offset window encoding** (each digit `g → g+1`, one classical correction), proves exhaustively on a real toy curve that it never emits the `∞` table entry yet computes `[a]P+[b]Q` for every `(a,b)`, and re-runs #15's exact measurement to show the `addend=∞` rate is now **exactly 0** while `dx=0` is unchanged — sharpening the completeness headline from `~2⁻¹¹` back to the `dx=0`-limited `~2⁻²⁵⁰`. |
