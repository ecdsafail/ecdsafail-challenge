...
/// Fast (high-Toffoli-save) variant of the schoolbook mod-mul into acc, using
/// `schoolbook_mul_into_addsub_fast` (Litinski fast controlled + fast cuccaro).
#[allow(dead_code)]
pub(crate) fn mod_mul_write_into_zero_acc_schoolbook(
    ...
    let tmp_ext = b.alloc_qubits(2 * n);
    schoolbook_mul_into_addsub_fast(b, x, y, &tmp_ext);
    ...
    schoolbook_mul_into_addsub_fast_inverse(b, x, y, &tmp_ext);
    b.free_vec(&tmp_ext);
}
...
/// Fast variant of Litinski controlled add-subtract (pairs with the documented
/// lowq sibling). Uses cuccaro_*_fast (HMR uncompute, 0 Toffoli on UMA sweep).
/// Peak cost: the fast cuccaro allocates its internal (n-1) carry lane.
/// Semantics: acc += (ctrl ? x : 0)   [via the complement trick + unconditional fast add].
/// Inverse uses fast sub. This is the high-leverage primitive for follow-up
/// "better square"/mul sites (saves full controlled-carry Toffolis).
...
/// Inverse of `controlled_add_subtract_fast`.
...
/// Fast variant of `schoolbook_mul_into_addsub`: uses `controlled_add_subtract_fast`
/// (Litinski + cuccaro_*_fast) for the per-row steps and fast cuccaro in the
/// correction terms. Saves the ~n Toffolis per row of the lowq version at the
/// cost of ~n transient carry qubits per controlled (inside cuccaro_fast).
/// Semantics identical.
pub(crate) fn schoolbook_mul_into_addsub_fast(b: &mut B, x: &[QubitId], y: &[QubitId], tmp_ext: &[QubitId]) {
    let n = x.len();
    debug_assert_eq!(y.len(), n);
    debug_assert_eq!(tmp_ext.len(), 2 * n);
...
/// Exact gate-level inverse of `schoolbook_mul_into_addsub_fast`.
pub(crate) fn schoolbook_mul_into_addsub_fast_inverse(
    b: &mut B,
    x: &[QubitId],
    y: &[QubitId],
    tmp_ext: &[QubitId],
