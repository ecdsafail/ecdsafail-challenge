...
/// Top-cleaned borrowed-carry form of
/// [`cuccaro_add_fast_low_to_ext_borrowed_carries`].  The top `clean_top`
/// source bits are folded in-place via Cuccaro MAJ/UMA (carry propagates
/// through `a[i]` itself), so only `n - clean_top` borrowed carry lanes are
/// live at peak.  Value-exact: identical sum, narrower transient footprint.
/// This mirrors [`cuccaro_add_fast_low_to_ext_topclean`] but consumes the
/// caller-supplied (borrowed) carry lanes instead of allocating them.
...
/// Top-cleaned borrowed-carry form of
/// [`cuccaro_sub_fast_low_to_ext_borrowed_carries`].  Mirror of
/// [`cuccaro_sub_fast_low_to_ext_topclean`] consuming caller-supplied carries.
