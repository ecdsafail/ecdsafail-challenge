...
fn verify_control_constancy(
    ...
    for (i, d) in decisions.iter().enumerate() {
        match *d {
            ...
            Decision::DropComplementCtrls { a, b } => {
                // Verify a and b are complementary: a XOR b should equal cond_mask (all-ones)
                // meaning they differ in every bit. We record both with expected=0
                // (live should be 0 after XOR, since we XOR with cond_mask on RHS).
                flagged.push((i, vec![(a, 0), (b, 0)]));
            }
            Decision::FoldEqualCtrls { a, b, .. } => {
                // Verify a and b are equal: a XOR b should be 0 (live should be 0).
                flagged.push((i, vec![(a, 0), (b, 0)]));
            }
        }
    }
