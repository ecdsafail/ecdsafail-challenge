...
fn add_mf_fold_chunked(circ: &mut B, e: &QubitId, d: &QubitId, y: &[QubitId], s_chunk: usize) {
    ...
    let drop_penultimate_zero_cout =
        std::env::var("TLM_FOLD_DROP_PENULTIMATE_ZERO_COUT")
            .ok()
            .as_deref()
            == Some("1");
    let fuse_penultimate_final_carry =
        std::env::var("TLM_FOLD_FUSE_PENULTIMATE_FINAL_CARRY")
            .ok()
            .as_deref()
            == Some("1");
    ...
    for j in 0..nch {
        ...
        if drop_cout {
            ...
            let cin = if j == 0 {
                cin0.as_ref().expect("penultimate chunk cannot be chunk zero")
            } else {
                &boundary[j - 1]
            };
            fold_chunk_clean_drop_cout(circ, &ctl[lo..hi], &y[lo..hi], cin);
