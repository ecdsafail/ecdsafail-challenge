...
pub struct B {
    pub ops: Vec<Op>,
    pub count_only: bool,
    pub(crate) op_sink: Option<Box<dyn FnMut(&Op)>>,
...
impl B {
    ...
    pub(crate) fn new_with_op_sink(op_sink: Box<dyn FnMut(&Op)>) -> Self {
        let mut b = Self::new();
        b.op_sink = Some(op_sink);
        b
    }
    ...
    fn push_op(&mut self, op: Op) {
        ...
        if !self.count_only {
            if let Some(op_sink) = &mut self.op_sink {
                op_sink(&op);
                return;
            }
            self.ops.push(op);
    ...
    fn current_ops_len(&self) -> usize {
        if self.count_only || self.op_sink.is_some() {
            self.counted_ops
        } else {
            self.ops.len()
        }
...
pub fn build() -> Vec<Op> {
    ...
    if std::env::var("POINT_ADD_DIALOG_ROUTE").ok().as_deref() != Some("1") {
        if source_stream_trailmix_build_requested() {
            match stream_trailmix_build_to_ops_bin() {
                Ok(emitted) => {
                    eprintln!("POINT_ADD_SOURCE_STREAM_BUILD emitted_ops={emitted}");
                    std::process::exit(0);
                }
                Err(error) => {
                    eprintln!("POINT_ADD_SOURCE_STREAM_BUILD failed: {error}");
                    std::process::exit(2);
                }
            }
        }
...
fn source_stream_trailmix_build_requested() -> bool {
    if std::env::var("POINT_ADD_SOURCE_STREAM_BUILD").ok().as_deref() == Some("0") {
        return false;
    }
    if std::env::var("POINT_ADD_COUNT_ONLY").ok().as_deref() == Some("1") {
        return false;
    }
    if std::env::vars().any(|(key, _)| key.starts_with("TRACE_")) {
        return false;
    }
    true
}

fn stream_trailmix_build_to_ops_bin() -> std::io::Result<usize> {
    ...
    fn write_op_record<W: Write>(writer: &mut W, op: &Op) -> std::io::Result<()> {
    ...
    let sink_encoder = Rc::clone(&encoder);
    let sink_error = Rc::clone(&write_error);
    let builder = trailmix_port::build_builder_to_op_sink(Box::new(move |op| {
        if sink_error.borrow().is_some() {
            return;
        }
        let result = {
            let mut encoder = sink_encoder.borrow_mut();
            let encoder = encoder
                .as_mut()
                .expect("streaming encoder should be active during TrailMix build");
            write_op_record(encoder, op)
        };
        if let Err(error) = result {
            *sink_error.borrow_mut() = Some(error);
        }
    }));
    let emitted_ops = builder.counted_ops;
    drop(builder);
    ...
    let encoder = encoder
        .borrow_mut()
        .take()
        .expect("streaming encoder should be present after TrailMix build");
    ...
    file.write_all(&(emitted_ops as u64).to_le_bytes())?;
    file.flush()?;
    drop(file);
