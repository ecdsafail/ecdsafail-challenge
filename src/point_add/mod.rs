...
/// Drop Toffoli gates that provably never fire (current_base_condition == 0
/// for all shots) by simulating the classical condition stack.
/// These are conditional CCX/CCZ ops nested inside always-false `if` branches
/// generated by the GCD comparison logic.
fn drop_dead_toffoli(mut ops: Vec<Op>) -> Vec<Op> {
    eprintln!("DROP_DEAD_CCZ: entry, processing {} ops", ops.len());
    use std::collections::HashMap;
    use crate::circuit::NO_BIT;
    use crate::circuit::OperationType::{BitInvert, BitStore0, BitStore1, CCX, CCZ, PopCondition, PushCondition};

    const NUM_SHOTS: u64 = 64;
    const ALL_SHOTS: u64 = if NUM_SHOTS == 64 {
        u64::MAX
    } else {
        (1u64 << NUM_SHOTS) - 1
    };
    ...
    let mut bit_state: HashMap<u64, u64> = HashMap::new();
    let mut current_base: u64 = ALL_SHOTS;
    ...
    for op in ops.drain(..) {
        match op.kind {
            PushCondition => {
                ...
                let cc = op.c_condition.0;
                let bit_val = *bit_state.get(&cc).unwrap_or(&0);
                current_base &= bit_val;
                kept.push(op);
            }
            PopCondition => {
                if let Some(base) = condition_stack.pop() {
                    current_base = base;
                }
                kept.push(op);
            }
            BitStore0 => {
                ...
                if ct != NO_BIT.0 {
                    let entry = bit_state.entry(ct).or_insert(0);
                    *entry &= !current_base;
                }
                kept.push(op);
            }
    ...
    if dropped > 0 {
        eprintln!(
            "DROP_DEAD_CCZ: filtered {} dead Toffoli from {} ops -> {} kept",
            dropped,
            total_ops,
            kept.len(),
        );
