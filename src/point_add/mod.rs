...
pub fn build() -> Vec<Op> {
    if std::env::var("DUMP_PLATEAU").ok().as_deref() == Some("1") {
        let b = trailmix_port::build_builder();
        for p in &b.named_peak_plateaus {
            eprintln!(
                "PLATEAU phase={} trigger={} ops_idx={}..{} occ={}",
                p.phase, p.trigger_allocation, p.first_ops_idx, p.last_ops_idx, p.occurrences
            );
            for (name, lanes) in &p.live_components {
                eprintln!("    {:>4}  {}", lanes, name);
            }
        }
        return b.ops;
    }
