...
impl Circuit {
    ...
    pub fn hmr_ghost(&mut self, q: &QReg) -> Ghost {
        ...
        self.ghost_event_log.push(crate::point_add::trailmix_port::circuit::GhostEvent {
            op_idx: hmr_op_idx,
            create: true,
            id,
            anchor_id,
    ...
    pub fn resolve_ghost(&mut self, mut g: Ghost, r: &QReg) {
        ...
        if r_mask != g.mask_at_hmr {
            ...
            if std::env::var("DEBUG_ON_FAIL").is_ok() {
                ...
                let mut dbg = crate::point_add::trailmix_port::tracker::debugger::Debugger::attach(self);
                dbg.goto(last_op);
                dbg.repl();
            }
    ...
    pub fn destroy_sim_ghosts(
        ...
        ghosts: Vec<Ghost>,
    ) -> (crate::point_add::trailmix_port::circuit::DestroyedSimState, Vec<crate::point_add::trailmix_port::circuit::QReg>) {
