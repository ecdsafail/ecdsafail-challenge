...
pub struct Circuit {
    ...
    next_ghost_id: u64,
    gate_only_depth: usize,
    gate_only_allocations: Vec<u32>,
}
...
impl Circuit {
    ...
    pub fn alloc_qreg(&mut self, _name: &str) -> QReg {
        ...
        if self.gate_only_depth > 0 {
            let id = q.0 as u32;
            if !self.gate_only_allocations.contains(&id) {
                self.gate_only_allocations.push(id);
            }
        }
        QReg {
            id: q.0 as u32,
    ...
    pub fn alloc_bit(&mut self) -> Cbit {
        assert_eq!(
            self.gate_only_depth, 0,
            "classical bit allocated inside a gate-only block"
        );
    ...
    pub fn flush_pending_frees(&mut self) {
        ...
        for q in pending {
            self.free_qubit(QubitId(q.into()));
        }
    }

    pub fn zero_and_free(&mut self, mut q: QReg) {
        self.flush_pending_frees();
        self.free_qubit(QubitId(q.id.into()));
        q.detached = true;
    }

    fn free_qubit(&mut self, q: QubitId) {
        if self.gate_only_depth > 0 {
            self.b.free_clean(q);
        } else {
            self.b.free(q);
    ...
    pub(crate) fn begin_gate_only_block(&mut self) {
        self.flush_pending_frees();
        assert_eq!(self.gate_only_depth, 0, "nested gate-only block");
        assert!(
            self.gate_only_allocations.is_empty(),
            "stale gate-only allocation record"
        );
        self.gate_only_depth = 1;
    }

    pub(crate) fn finish_gate_only_block(&mut self) -> Vec<u32> {
        assert_eq!(self.gate_only_depth, 1, "gate-only block is not active");
        self.flush_pending_frees();
        self.gate_only_depth = 0;
        std::mem::take(&mut self.gate_only_allocations)
    }
