...
impl Circuit {
    ...
    pub fn new_with_ops_capacity(ops_capacity: usize) -> Self {
        ...
        Self {
            ...
            named_peak_target: std::env::var("TRACE_NAMED_PEAK_TARGET")
                .ok()
                .map(|value| value.parse::<u32>().expect("TRACE_NAMED_PEAK_TARGET integer")),
    ...
    pub fn alloc_qreg(&mut self, name: &str) -> QReg {
        ...
        if self.named_peak_target.is_some() {
            assert!(
    ...
    fn record_named_peak_plateau(&mut self, trigger_allocation: &str) {
        let Some(target) = self.named_peak_target else {
            return;
        };
        if self.b.active_qubits != target {
            return;
        }
        assert_eq!(
