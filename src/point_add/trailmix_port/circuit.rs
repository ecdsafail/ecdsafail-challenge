...
pub struct Circuit {
    ...
    next_ghost_id: u64,
    active_names: BTreeMap<u32, String>,
    live_reports: HashSet<(String, u32)>,
}
...
impl Circuit {
    ...
    pub fn alloc_qreg(&mut self, name: &str) -> QReg {
        ...
        self.active_names.insert(q.0 as u32, name.to_string());
        self.maybe_report_live_labels();
        QReg {
            id: q.0 as u32,
    ...
    fn maybe_report_live_labels(&mut self) {
        if std::env::var("TRAILMIX_LIVE_LABELS").ok().as_deref() != Some("1") {
            return;
        }
        let min_active = std::env::var("TRAILMIX_LIVE_LABELS_MIN")
            .ok()
            .and_then(|s| s.parse::<u32>().ok())
            .unwrap_or(self.b.peak_qubits);
        if self.b.active_qubits < min_active {
            return;
        }
        if let Ok(filter) = std::env::var("TRAILMIX_LIVE_LABELS_PHASE") {
            if !filter.is_empty() && !self.current_section.contains(&filter) {
                return;
            }
        }
        let key = (self.current_section.clone(), self.b.active_qubits);
        if !self.live_reports.insert(key) {
            return;
        }

        let mut counts: BTreeMap<String, usize> = BTreeMap::new();
        for name in self.active_names.values() {
            let family = name.split_once('[').map_or(name.as_str(), |(head, _)| head);
            *counts.entry(family.to_string()).or_insert(0) += 1;
        }
        let mut rows: Vec<_> = counts.into_iter().collect();
        rows.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)));
        let top_n = std::env::var("TRAILMIX_LIVE_LABELS_TOP")
            .ok()
            .and_then(|s| s.parse::<usize>().ok())
            .unwrap_or(24);
        eprintln!(
            "TRAILMIX_LIVE_LABELS active={} phase='{}' families={}",
            self.b.active_qubits,
            self.current_section,
            rows.len()
        );
        for (name, count) in rows.into_iter().take(top_n) {
            eprintln!("TRAILMIX_LIVE_LABEL {:>5} {}", count, name);
        }
    }
}
