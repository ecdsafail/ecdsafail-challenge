...
use crate::point_add::trailmix_port::compat::{num_bigint, rand};
...
impl ContractSimView<'_> {
    ...
    pub fn ghost(
        &self,
        id: u64,
    ) -> Option<&crate::point_add::trailmix_port::tracker::ghost::GhostRecord> {
    ...
    pub fn read_u256_shot(
        &self,
        reg: &[QReg],
        shot: usize,
    ) -> crate::point_add::trailmix_port::compat::num_bigint::BigUint {
...
pub struct Circuit {
    ...
    #[doc(hidden)]
    pub dump_hooks: std::collections::HashMap<
        String,
        crate::point_add::trailmix_port::tracker::debugger::DumpHook,
    >,
...
impl Circuit {
    ...
    pub fn new() -> Self {
        ...
        let hmr_seed: u64 = 0xEC_D5_AF_A1_1B_1D_64;
        Circuit {
    ...
    pub fn disable_sim_contracts(&mut self) {
        self.sim = None;
        self.sim_bits = None;
    }
    ...
    where
        F: Fn(&crate::point_add::trailmix_port::tracker::debugger::Debugger, &[&str]) -> String
            + Send
            + Sync
            + 'static,
    {
                ...
                Some(last) => {
                    if last < last_alloc {
                        ...
                        if std::env::var("DEBUG_ON_FAIL").is_ok() {
                            ...
                            let mut d =
                                crate::point_add::trailmix_port::debugger::Debugger::attach(self);
...
mod declare_xor_of_tests {
    ...
    /// Canary for the TrailMix low-Q streamed product comparator route:
    /// product_bit = carry_lsb XOR (x_i AND y_j), then HMR product_bit and
    /// discharge the phase against the live carry and AND witness before
    /// MBU-clearing the witness. This is the atomic pattern needed by the
    /// secp256k1-shape product compare; wider columns repeat this pattern.
    #[test]
    fn circuit_hmr_product_bit_xor_and_discharge_64lane() {
        let mut c = Circuit::new();
        let carry = c.alloc_input_qreg("carry_lsb");
        let x = c.alloc_input_qreg("x_i");
        let y = c.alloc_input_qreg("y_j");
        let and_xy = c.alloc_qreg("and_xy");
        let product_bit = c.alloc_qreg("product_bit");

        use rand::Rng;
        let mut rng = rand::rngs::OsRng;
        for shot in 0..64 {
            if rng.gen::<bool>() {
                c.sim_load_reg_bytes_shot(std::slice::from_ref(&carry), &[1u8], shot);
            }
            if rng.gen::<bool>() {
                c.sim_load_reg_bytes_shot(std::slice::from_ref(&x), &[1u8], shot);
            }
            if rng.gen::<bool>() {
                c.sim_load_reg_bytes_shot(std::slice::from_ref(&y), &[1u8], shot);
            }
        }

        c.ccx(&x, &y, &and_xy);
        c.cx(&carry, &product_bit);
        c.cx(&and_xy, &product_bit);
        c.declare_xor_of(&product_bit, &carry, &and_xy);

        let bit = c.alloc_bit();
        c.hmr(&product_bit, bit);
        c.z_if_bit(&carry, bit);
        c.z_if_bit(&and_xy, bit);
        c.free_bit(bit);
        c.zero_and_free(product_bit);

        c.clear_and(&and_xy, &x, &y);
        c.zero_and_free(and_xy);
        c.assert_phase_clean();
        let _ = c.destroy_sim(vec![carry, x, y]);
    }
}
