...
fn configure_lowq_trailmix_route() {
    // Default to the safer static shrunken-PZ schedule for the first low-Q
    // submission. The dynamically generated sub-1000 schedule validates locally,
    // but its much larger op stream already failed server-side under resource
    // pressure. This route is still below the 1050q reference while emitting
    // substantially fewer operations.
    set_default_env("TRAILMIX_THIN_SCHEDULE", "0");
...
pub fn build_builder() -> crate::point_add::B {
    ...
    search_tail_nonce(&builder, tail_q0, tail_q1);
