...
fn configure_sub1000_trailmix_route() {
    ...
    // sub-846 FRONT P2: shrink the bit-length direct-prefix-AND loan from nine
    // clean lanes (5 KG + 3 increment + 1 flag) to eight by eliminating the
    // materialized flag. Frees one owned `rs.dynamic-bitlen.fused-prefix-scratch`
    // lane on the P2 co-peak plateau -> measured peak 846 -> 845. Value-proven
    // (KG_ONLY_PREFIX_POSITIVE 160). NOTE: paired with
    // LOWQ_KG_ONLY_PREFIX_BORROWED_FLAG below, the 2-control prefix callbacks use
    // the cheap clean-flag path (NOT the dirty double-control), so this is a
    // pure lane saving with no op inflation.
    set_default_env("LOWQ_KG_ONLY_PREFIX_SCRATCH", "1");
    // Lend the swap promised-support lane (already on the 845 plateau) to the
    // KG-only bit-length prefix as a clean-|0> flag, restoring the cheap
    // `ccx; clean-CINC; clear_and` two-control prefix callback. This eliminates
    // the O(n)-firing dirty `dirty_controlled_inc_suffix` (~4.55M calls,
    // ~842M ops) while keeping the qubit peak at 845: op count 1.98B -> 1.13B.
    set_default_env("LOWQ_KG_ONLY_PREFIX_BORROWED_FLAG", "1");
    ...
    // C1 (sub-851): host the swap-scan prefix-AND all-ones-detector MCX on a
    // borrowed DIRTY lane (idle persistent `phase2`) instead of the clean
    // `mcxk_t3` ancilla that set peak=851. Removes that top transient qubit ->
    // structural peak 851 -> 850. The dirty-MCX restores `phase2` exactly per
    // call; validated by the per-shot mcx_dirty contract checks during build.
    set_default_env("LOWQ_Q845_SWAP_DIRTY_MCX", "1");
    // C1 extension (sub-850): drop the persistent Khattar-Gidney prefix-AND tree
    // (`uls_anc`, 3 clean qubits) on the n=9 swap scan. The all-ones detector is
    // emitted as a single full-width `mcx_dirty_ladder` over all 9 `address`
    // bits, borrowing 7 dirty lenders from the idle `l_q` length register (never
    // threaded into the swap step => provably untouched; restored exactly by the
    // Bennett cascade). Removes uls_anc[0..2] -> structural peak 850 -> 847.
    set_default_env("LOWQ_Q845_SWAP_LADDER_DETECTOR", "1");
    // uls_gate fusion: XOR the swap-scan all-ones detector directly into the
    // `active` flag via the ladder (no clean uls_gate alloc) -> peak 847 -> 846.
    set_default_env("LOWQ_Q846_ULS_GATE_FUSE", "1");
    // sub-846 P1 cut: free the 8-lane clean cursor-scratch of the swap-step
    // coefficient kernel by routing ALL of its clean v-chain / ripple / constant
    // consumers (increment, fixed-sign event, nonzero, geq comparator, the
    // truncated address constant-subtract) onto the idle l_q DIRTY lenders that
    // already serve the swap-scan detector. The detector's mcx_dirty_ladder
    // restores its lenders before the iterate body runs, so l_q is free during
    // the body; every consumer borrow is itself an mcx_dirty_ladder (restored
    // exactly). Requires the in-place reverse boundary (the other clean-zero pin
    // on cursor-scratch). Value-proven by SWAP_ONLY_LQ_DIRTY_PROOF (267392
    // dirty-path checks: data-transparent, l_q restored, reversible). P1 -8.
    set_default_env("LOWQ_Q838_INPLACE_BOUNDARY", "1");
    set_default_env("LOWQ_Q838_CURSOR_BORROW", "1");
