...
fn configure_sub1000_trailmix_route() {
    ...
    set_default_env("LOWQ_Q824_COEFFICIENT_DIRTY_FUSED_TMP", "0");
    set_default_env("LOWQ_Q824_ROTATED_SWAP_LQ_HIGH_HOST", "0");
    set_default_env("TRACE_Q823_ALLOW_DIRTY_COUNT_HOST", "0");
    set_default_env("TRACE_Q824_DIRTY_FUSED_SIGN_PATCH", "none");
    set_default_env("TRACE_Q824_DIRTY_FUSED_COUNTER_ROUTE", "dirty_ladder");
    set_default_env("TRACE_Q823_COMPACT_REMAINDER_CONSTANT", "0");
    set_default_env("TRACE_Q823_ROTATED_CONDITION7", "0");
    // Keep the env-free build on the remote-complete Q824 remainder-only
    // fallback. Q818 remains selectable for research, but its current 5.4B-op
    // stream exceeds the trusted evaluator's 4B input cap; the 822 fallback
    // validates locally but failed the remote runner before metrics.
