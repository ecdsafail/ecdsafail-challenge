...
#[cfg(test)]
mod q826_source_baked_route_tests {
    ...
    #[test]
    fn env_free_q826_route_is_baked_and_overrides_are_hashed() {
        let _lock = ROUTE_ENV_LOCK.lock().expect("Q826 route environment lock");
...
pub fn build_builder() -> crate::point_add::B {
    ...
    // Reserve the source-baked Q826 three-class host-cover stream exactly. The
    // composed host route emits 741,854,295 records;
    ...
    let mut circ = circuit::Circuit::new_with_ops_capacity(741_854_295);
    ...
    if let Ok(target) = std::env::var("TRACE_NAMED_PEAK_TARGET") {
        eprintln!(
            "TRAILMIX_NAMED_PEAK_CENSUS target={} families={}",
            target,
        ...
        for (index, plateau) in builder.named_peak_plateaus.iter().enumerate() {
            let components = plateau
                ...
                .join(",");
            eprintln!(
                "TRAILMIX_NAMED_PEAK family={} phase={} trigger={} occurrences={} allocation_serial={}..{} ops_idx={}..{} live={}",
                index,
                plateau.phase,
