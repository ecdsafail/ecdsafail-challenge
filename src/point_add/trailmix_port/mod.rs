...
fn configure_sub1000_trailmix_route() {
    ...
    // Trusted-clean q975 route: combine the target-684 CLZ fold with exact
    // elimination of fixed-one/flag workspace and the A-scan collapse.
    ...
    // A-scan collapse (Claude Opus 4.8): hoist the redundant per-step A bit-length
    // scan in the division/cofactor substeps. Value-exact, peak-neutral (975).
    set_default_env("TRAILMIX_FUSE_DIV_CLZ_A", "1");
    set_default_env("TRAILMIX_TAIL_NONCE", "2001699");
}
