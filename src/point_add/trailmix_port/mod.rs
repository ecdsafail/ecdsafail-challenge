...
fn configure_sub1000_trailmix_route() {
    ...
    // Fold the all-zero source correction into the existing bit-length prefix
    // traversal. This preserves the position-deposit arithmetic while removing
    // the separate zero flag, carry chain, and duplicated prefix traversal.
    set_default_env("LOWQ_FUSED_ZERO_PREFIX_BITLEN", "1");
    // Emit every register-shared decrement as the exact reversed increment
    // stream. This removes redundant X brackets without changing action,
    // peak width, or Toffoli count.
    set_default_env("LOWQ_Q873_REVERSE_DECREMENT_STREAM", "1");
    // Source-bake the three independently proved Q873 X-only cuts. Explicit
    // 0/1 overrides remain available to the focused proof binaries.
    set_default_env("LOWQ_Q873_CHAINED_EQUALITY_MASK", "1");
    set_default_env("LOWQ_Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT", "1");
    set_default_env("LOWQ_Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL", "1");
...
pub fn q949_route_identity() -> String {
    const ROUTE_ENV: &[&str] = &[
        ...
        "LOWQ_Q873_CHAINED_EQUALITY_MASK",
    ...
    q949_hash_component(
        ...
        if inversion::register_shared_eea_reference::Q873_FUSED_COEFFICIENT_COMPARE_SOURCE_BAKE {
            b"1"
        } else {
            b"0"
        },
...
pub fn q925_register_shared_schedule_identity() -> String {
    use inversion::register_shared_eea_reference::{
        reference_active_windows, REFERENCE_LENGTH_WIDTH, REFERENCE_STEPS,
    };

    let mut hasher = sha3::Shake256::default();
    hasher.update(b"q925-register-shared-schedule-identity-v1");
...
pub fn build_builder() -> crate::point_add::B {
    ...
    // Exact reservation for the source-baked three-cut Q873 operation stream.
    // This avoids Vec's growth cliff and never reserves the 894M baseline.
    let mut circ = circuit::Circuit::new_with_ops_capacity(858_585_087);
