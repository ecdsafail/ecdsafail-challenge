...
fn configure_sub1000_trailmix_route() {
    ...
    // Re-hunted for the Q850 dirty-borrow detector stream (834,829,613 records).
    // The borrow cut changes the op stream, so the original Q851 nonce 4114534 no
    // longer keeps this stream support-clean (model: 18 miss / 200 repair). Nonce
    // 1049 gives model 8 miss / 21 repair -- below the validated Q851 baseline of
    // 13 miss / 43 repair. (Count-only hunt: HASH_OPS_LEN=834829709.)
    set_default_env("TRAILMIX_TAIL_NONCE", "1049");
}
...
pub fn build_builder() -> crate::point_add::B {
    ...
    // Reserve the source-baked stream exactly. The Q850 dirty-borrow detector
    // route emits 834,829,613 records (down from the Q851 838,845,965 baseline);
    // staying tight keeps the build inside the 64 GiB submission-server envelope.
    // Vec can still grow for an explicitly overridden experimental route.
    let mut circ = circuit::Circuit::new_with_ops_capacity(834_829_613);
