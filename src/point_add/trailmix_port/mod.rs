...
pub const Q949_WIDTH_CONTEXTS_PER_FACTOR: usize = 3
...
fn configure_sub1000_trailmix_route() {
    ...
    set_default_env("LOWQ_Q824_COEFFICIENT_LQ_HIGH_HOST", "1");
    set_default_env("LOWQ_Q824_COEFFICIENT_DIRTY_FUSED_TMP", "1");
    set_default_env("LOWQ_Q824_ROTATED_SWAP_LQ_HIGH_HOST", "1");
    set_default_env("TRACE_Q823_ALLOW_DIRTY_COUNT_HOST", "1");
    set_default_env("TRACE_Q824_DIRTY_FUSED_SIGN_PATCH", "none");
    set_default_env("TRACE_Q824_DIRTY_FUSED_COUNTER_ROUTE", "dirty_ladder");
    set_default_env("TRACE_Q823_COMPACT_REMAINDER_CONSTANT", "1");
    set_default_env("TRACE_Q823_ROTATED_CONDITION7", "1");
    // Keep the env-free build on the validated Q825/Q822 fallback.  The Q818
    // one-bit l_q route remains selectable for research, but its current
    // 5.4B-op stream exceeds the trusted evaluator's 4B input cap.
    set_default_env("LOWQ_Q825_SEVEN_BIT_L_Q", "1");
    set_default_env("LOWQ_Q822_FIVE_BIT_L_Q", "0");
    set_default_env("LOWQ_Q821_FOUR_BIT_L_Q", "0");
    set_default_env("LOWQ_Q820_THREE_BIT_L_Q", "0");
    set_default_env("LOWQ_Q819_TWO_BIT_L_Q", "0");
    set_default_env("LOWQ_Q818_ONE_BIT_L_Q", "0");
...
fn support_report_for_xof_limited(
    ...
    let trace_coordinates = std::env::var("TRAILMIX_SUPPORT_COORDS").ok().as_deref() == Some("1");
    let audit_lq7 = std::env::var("TRAILMIX_LQ7_SUPPORT_CHECK").ok().as_deref() == Some("1");
    let l_q_violation_threshold = env_usize("TRAILMIX_LQ_SUPPORT_THRESHOLD", 128);
    for draw in 0..target_draws {
        ...
        for (label, factor) in [("dx", dx), ("qx_minus_rx", c)] {
            if audit_lq7 && !factor.is_zero() {
                ...
                let audit =
                    inversion::register_shared_eea::audit_secp256k1_factor(factor, |observation| {
                        ...
                        if observation.l_q >= l_q_violation_threshold && factor_violation.is_none()
                        {
            ...
            let repairs = inversion::shrunken_pz_schedule::thin_factor_repairs_u256(factor);
            if trace_coordinates && repairs > 0 {
                let coordinates =
                    inversion::shrunken_pz_schedule::thin_factor_repair_coordinates_u256(factor);
...
fn print_trailmix_support_report(prefix: &str, draws: usize, report: &TrailMixSupportReport) {
    eprintln!(
        "{prefix} draws={} accepted={} miss_factors={} repair_entries={} first_miss={:?}",
        draws, report.accepted_shots, report.miss_factors, report.repair_entries, report.first_miss
    );
    if std::env::var("TRAILMIX_LQ7_SUPPORT_CHECK").ok().as_deref() == Some("1") {
        eprintln!(
            "{prefix}_LQ draws={} accepted={} factors={} observations={} threshold={} max_l_q={} max_l_t={} max_l_s={} max_l_r_prime={} max_work1_used={} max_work2_used={} length_updates={} max_lenupd_lrp_before={} max_lenupd_lrp_after={} max_lenupd_transient_lrp={} max_lenupd_l_t={} max_lenupd_work1_used={} max_lenupd_work2_used={} max_lenupd_draw={:?} max_lenupd_label={:?} max_lenupd_step={:?} max_draw={:?} max_label={:?} max_step={:?} violations={} first_violation={:?}",
            draws,
            report.accepted_shots,
            report.lq7_factors_audited,
            report.lq7_observations,
            env_usize("TRAILMIX_LQ_SUPPORT_THRESHOLD", 128),
            report.maximum_l_q,
            report.maximum_l_t,
            report.maximum_l_s,
            report.maximum_l_r_prime,
...
fn report_current_support(builder: &crate::point_add::B) {
    ...
    let Some(hasher) = builder.clone_fiat_hash() else {
        eprintln!("TRAILMIX_SUPPORT no hash stream; set POINT_ADD_HASH_OPS_LEN in count-only mode");
        return;
    };
    let draws = env_usize("TRAILMIX_SUPPORT_SHOTS", TRAILMIX_NUM_TESTS);
    let report = support_report_for_xof(hasher.finalize_xof(), draws);
    print_trailmix_support_report("TRAILMIX_SUPPORT", draws, &report);
}
...
pub fn q949_schedule_identity() -> String {
    use inversion::shrunken_pz_schedule::{
        q949_effective_reg_los, q949_effective_reg_widths, shift_bounds, SHRUNKEN_PZ_NSTEPS,
    };
...
pub const Q949_ROUTE_ENV: &[&str] = &[
    ...
    "LOWQ_Q824_COEFFICIENT_DIRTY_FUSED_TMP",
...
pub fn q925_register_shared_schedule_identity() -> String {
    use inversion::register_shared_eea_reference::{
        reference_active_windows, REFERENCE_LENGTH_WIDTH, REFERENCE_R_LENGTH_WIDTH, REFERENCE_STEPS,
    };
...
#[doc(hidden)]
pub fn q925_committed_factor_corpus(builder: &crate::point_add::B) -> Q925CommittedFactorCorpus {
...
impl Q949JointWidthDemandAccumulator {
    ...
    fn finish(self) -> Q949JointWidthDemand {
        ...
        Q949JointWidthDemand {
            ...
            component_witnesses: self
                .component_witnesses
                .map(|witness| witness.expect("Q949 joint-width maximum is missing its witness")),
        }
    }
}
...
impl Q949ClzLowBoundAccumulator {
    ...
    fn record(
        ...
        assert_eq!(
            ...
            observation.observed_widths[register]
        );
    ...
    fn finish(self) -> Q949ClzLowBound {
        Q949ClzLowBound {
            ...
            safe_low_upper_bound: self.minimum_nonzero_observed_width.map(|width| width - 1),
...
fn q949_width_phase_label(phase: inversion::shrunken_pz_schedule::Q949WidthPhase) -> &'static str {
...
fn q949_expected_width_context_keys() -> Vec<(
    ...
    use inversion::shrunken_pz_schedule::{Q949TraceDirection, Q949WidthPhase, SHRUNKEN_PZ_NSTEPS};
...
fn q949_validate_census_report(report: &Q949SupportedTraceReport) {
    use inversion::shrunken_pz_schedule::{
        q949_effective_reg_los, q949_effective_reg_widths, Q949TraceDirection, Q949WidthPhase,
        SHRUNKEN_PZ_NSTEPS,
    };

    assert_eq!(
        Q949_CENSUS_DIAGNOSTICS_SCHEMA, "q949-census-diagnostics-v2",
    ...
    assert!(report.width_misses <= report.entry_width_checks * 3 + report.boundary_width_checks);
    ...
    for bound in &report.clz_low_bounds {
        ...
        assert_eq!(
            bound.current_low,
            q949_effective_reg_los(bound.row)[register]
        );
        assert_eq!(
...
fn q949_census_status(report: &Q949SupportedTraceReport) -> &'static str {
    if report.requested_draws == Q949_PROOF_DRAWS
        ...
        && report.max_counter
            == inversion::shrunken_pz_schedule::SHRUNKEN_PZ_NSTEPS - report.earliest_terminal_row
    {
        "pass"
    } else {
        "reject"
...
fn q949_census_diagnostics_json(
    ...
    let mut output = String::with_capacity(
        2_048
            + report.joint_width_demands.len() * 1_024
...
pub fn q944_gate_host_feasibility_check(
    ...
    for draw in 0..requested_draws {
        ...
        for (factor_label, factor) in [
            ...
            ("qx_minus_rx", sub_mod_p(other.0, result.0, curve.modulus)),
        ] {
            let phase = q945_phase_for_factor(factor_label);
            let certificate =
                inversion::shrunken_pz_schedule::q949_affine_trace_certificate_u256(factor);
...
pub fn q945_host_support_check(builder: &crate::point_add::B) -> Q945HostSupportReport {
        ...
        for (factor_label, factor) in [
            ...
            if first_narrow_compare_miss.is_none() {
                first_narrow_compare_miss =
                    certificate.first_narrow_compare_miss.map(|coordinate| {
                        Q945SupportedNarrowCompareMiss {
                            draw,
                            factor_label,
                            factor,
                            phase,
            ...
            for observation in certificate.q945_carry_host_observations {
                ...
                let row364 = observation.row == 364 && observation.substep == Q945Substep::Division;
                if row364 {
                    row364_checks += 1;
                ...
                let row374 = observation.row == 374 && observation.substep == Q945Substep::Division;
                if row374 {
                ...
                let failed =
                    host_failed || !semantic || (row374 && observation.q24_noncarry_touches != 0);
                if first_host_miss.is_none() && failed {
    ...
    assert!(hclz_sites
        .values()
        .all(|site| site.checks == accepted_draws));
    assert!(carry_sites
        .values()
        .all(|site| site.checks == accepted_draws));
    ...
    assert_eq!(
        first_host_miss.is_some(),
        hclz_host_misses + carry_host_misses + carry_semantic_misses != 0
            || row374_q24_noncarry_touches != 0
    );
    assert_eq!(
        first_narrow_compare_miss.is_some(),
...
pub fn q949_supported_trace_check(builder: &crate::point_add::B) -> Q949SupportedTraceReport {
        ...
        for (factor_label, factor) in [
            ...
            for observation in &certificate.width_observations {
                ...
                match joint_width_demands.entry(key) {
                    Entry::Vacant(entry) => {
                        entry.insert(Q949JointWidthDemandAccumulator::new(*observation, witness));
                    }
                    Entry::Occupied(mut entry) => entry.get_mut().record(*observation, witness),
                }
            }
            assert_eq!(
            ...
            if first_width_miss.is_none() {
                first_width_miss =
                    certificate
                        .first_width_miss
                        .map(|coordinate| Q949SupportedWidthMiss {
                            draw,
                            factor_label,
                            factor,
                            coordinate,
            ...
            if first_clz_window_miss.is_none() {
                first_clz_window_miss = certificate.first_clz_window_miss.map(|coordinate| {
                    Q949SupportedClzWindowMiss {
                        draw,
                        factor_label,
                        factor,
                        coordinate,
    ...
    assert_eq!(
        report.reverse_row_380_relation_checks, report.factors_checked,
    ...
    assert_eq!(
        report.boundary_width_checks,
        Q949_PROOF_FACTORS * 2 * (inversion::shrunken_pz_schedule::SHRUNKEN_PZ_NSTEPS - 1) * 5
    );
    assert_eq!(
        report
    ...
    assert!(report.earliest_terminal_row <= report.latest_terminal_row);
    assert!(report.latest_terminal_row < inversion::shrunken_pz_schedule::SHRUNKEN_PZ_NSTEPS);
    assert_eq!(
        report.max_counter,
        inversion::shrunken_pz_schedule::SHRUNKEN_PZ_NSTEPS - report.earliest_terminal_row,
    ...
    assert!(
        report.max_counter < 256,
        "Q949 affine terminal counter overflow"
    );
    report
}
...
fn search_tail_nonce(builder: &crate::point_add::B, q0: u32, q1: u32) {
    ...
    let results: Vec<(Option<(u64, TrailMixSupportReport)>, Option<u64>)> = std::thread::scope(
        |scope| {
...
fn build_builder_with_op_sink(op_sink: Option<Box<dyn FnMut(&Op)>>) -> crate::point_add::B {
    configure_sub1000_trailmix_route();
    if std::env::var("TRACE_TRAILMIX_SUPPORT_AUDIT_ONLY")
        .ok()
        .as_deref()
        == Some("1")
    {
        let draws = env_usize("TRAILMIX_SUPPORT_SHOTS", TRAILMIX_NUM_TESTS);
        let mut hasher = sha3::Shake256::default();
        hasher.update(b"trailmix-support-audit-v1");
        hasher.update(&(draws as u64).to_le_bytes());
        hasher.update(&env_usize("TRAILMIX_LQ_SUPPORT_THRESHOLD", 128).to_le_bytes());
        let report = support_report_for_xof(hasher.finalize_xof(), draws);
        print_trailmix_support_report("TRAILMIX_SUPPORT_AUDIT", draws, &report);
        std::process::exit(0);
    }
    if std::env::var("TRACE_Q822_LQ5_DIRECT_SWAP_COMPARE_ONLY")
        .ok()
        .as_deref()
        == Some("1")
    {
        let (mismatches, phase_dirty, ancilla_dirty, first) =
            inversion::register_shared_eea_reference::q822_lq5_direct_swap_compare_diagnostic();
        eprintln!(
            "TRAILMIX_Q822_LQ5_DIRECT_SWAP_COMPARE mismatches={mismatches} phase_dirty={phase_dirty} ancilla_dirty={ancilla_dirty} first={first:?}"
        );
        std::process::exit(0);
    }
    if std::env::var("TRACE_Q822_LQ5_STAGE_COMPARE_ONLY")
        .ok()
        .as_deref()
        == Some("1")
    {
        let report = inversion::register_shared_eea_reference::diagnose_q822_lq5_composition();
        eprintln!("TRAILMIX_Q822_LQ5_STAGE_COMPARE {report:?}");
        std::process::exit(0);
    }
    if std::env::var("TRACE_Q824_DIRTY_FUSED_STAGE_COMPARE_ONLY")
        .ok()
        .as_deref()
        == Some("1")
    {
        let report = inversion::register_shared_eea_reference::diagnose_q824_dirty_fused_composition();
        eprintln!("TRAILMIX_Q824_DIRTY_FUSED_STAGE_COMPARE {report:?}");
        std::process::exit(0);
    }
    if std::env::var("TRACE_Q82X_LQ_STAGE_COMPARE_ONLY")
        .ok()
        .as_deref()
        == Some("1")
    {
        let width = env_usize("TRACE_Q82X_LQ_WIDTH", 1);
        let report =
            inversion::register_shared_eea_reference::diagnose_q82x_lq_composition(width);
        eprintln!("TRAILMIX_Q82X_LQ_STAGE_COMPARE width={width} {report:?}");
        std::process::exit(0);
    }
    if std::env::var("TRACE_Q818_REMAINDER_PREDICATE_SCAN_ONLY")
        .ok()
        .as_deref()
        == Some("1")
    {
        let report =
            inversion::register_shared_eea_reference::diagnose_q818_remainder_predicate_scan();
        eprintln!("TRAILMIX_Q818_REMAINDER_PREDICATE_SCAN {report:?}");
        std::process::exit(0);
    }
    if std::env::var("TRACE_Q818_REMAINDER_RANGE_STATS_ONLY")
        .ok()
        .as_deref()
        == Some("1")
    {
        let report =
            inversion::register_shared_eea_reference::diagnose_q818_remainder_range_stats();
        eprintln!("TRAILMIX_Q818_REMAINDER_RANGE_STATS {report:?}");
        std::process::exit(0);
    }
    ...
    if std::env::var("TRACE_Q824_COEFFICIENT_HOST_PROOF")
        .ok()
        .as_deref()
        == Some("1")
    {
        let (lq5_zero_cases, lq5_zero_mismatches, lq5_one_cases, lq5_one_mismatches) =
            inversion::register_shared_eea_reference::q824_coefficient_lq_high_host_cleanliness_diagnostic();
        eprintln!(
            "TRAILMIX_Q824_COEFFICIENT_HOST_PROOF lq5_zero_cases={lq5_zero_cases} lq5_zero_mismatches={lq5_zero_mismatches} lq5_one_cases={lq5_one_cases} lq5_one_mismatches={lq5_one_mismatches}"
        );
        if std::env::var("TRACE_Q824_COEFFICIENT_HOST_PROOF_ONLY")
            .ok()
            .as_deref()
            == Some("1")
        {
    ...
    if std::env::var("TRACE_Q824_DIRTY_FUSED_Q822_PROOF")
        .ok()
        .as_deref()
        == Some("1")
    {
        let (cases, mismatches, internal_dirty) =
            inversion::register_shared_eea_reference::q824_dirty_fused_q822_diagnostic();
        eprintln!(
            "TRAILMIX_Q824_DIRTY_FUSED_Q822_PROOF cases={cases} mismatches={mismatches} internal_dirty={internal_dirty}"
        );
        if std::env::var("TRACE_Q824_DIRTY_FUSED_Q822_PROOF_ONLY")
            .ok()
            .as_deref()
            == Some("1")
        {
    ...
    // Default production route emits about 1.04B records; reserve the validated
    // envelope used by the submitted Q822 fallback.  Explicit Q818 experiments
    // should use the streaming op sink instead of materializing the Vec.
    let mut circ = match op_sink {
        Some(op_sink) => circuit::Circuit::new_with_op_sink(op_sink),
        None => circuit::Circuit::new_with_ops_capacity(751_661_003),
    };
    ...
    if std::env::var("TRACE_STRUCTURAL_COUNT").ok().as_deref() == Some("1") {
        let structural_t = builder.counted_kind_ops[OperationType::CCX as usize]
            + builder.counted_kind_ops[OperationType::CCZ as usize];
        eprintln!(
            "TRAILMIX_STRUCTURAL_COUNT qubits={} ops={} toffoli={}",
            builder.peak_qubits,
            builder.current_ops_len(),
            structural_t
        );
    ...
    if std::env::var("TRACE_BUILD_EXIT_BEFORE_WRITE")
        .ok()
        .as_deref()
        == Some("1")
    {
    ...
    if std::env::var("TRACE_PHASE_OPS").is_ok() {
        ...
        let mut by_phase: BTreeMap<&'static str, crate::point_add::PhaseResource> = BTreeMap::new();
        for row in &builder.counted_phase_rows {
            let entry = by_phase
                .entry(row.phase)
                .or_insert(crate::point_add::PhaseResource {
...
pub fn build_builder() -> crate::point_add::B {
    build_builder_with_op_sink(None)
}

pub fn build_builder_to_op_sink(op_sink: Box<dyn FnMut(&Op)>) -> crate::point_add::B {
    build_builder_with_op_sink(Some(op_sink))
}
