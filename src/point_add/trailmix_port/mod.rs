...
fn configure_sub1000_trailmix_route() {
    ...
    // Baked Q953 record route. The source embeds the exact schedule used by
    // trusted job 71141 and pins both ownership cutovers to its certificate.
    ...
    set_default_env("LOWQ_Q953_EMBEDDED_SCHEDULE", "1");
    set_default_env(
        "LOWQ_Q953_SCHEDULE_SHA3_256",
        "5a56fd48a3b85ef5b3d890af983cde7add0eac74531b1c9c79b669ff57612677",
    );
    set_default_env("TRAILMIX_SROT_W", "3");
    set_default_env("TRAILMIX_DEFER_Y_MATERIALIZE", "1");
    set_default_env("TRAILMIX_ZERO_DY_NEWDX_ROUTE", "1");
    set_default_env("TRAILMIX_TAIL_NONCE", "154463457858599");
}
...
fn report_current_support(builder: &crate::point_add::B) {
    ...
    let Some(hasher) = builder.clone_fiat_hash() else {
        assert!(!q953, "Q953 support proof requires POINT_ADD_HASH_OPS_LEN");
        eprintln!(
            "TRAILMIX_SUPPORT no hash stream; set POINT_ADD_HASH_OPS_LEN in count-only mode"
        );
        return;
    };
    let q953_schedule = q953.then(|| {
        inversion::shrunken_pz_state_machine::q953_srot_counter67_schedule_certificate()
    });
    let draws = env_usize("TRAILMIX_SUPPORT_SHOTS", TRAILMIX_NUM_TESTS);
    let report = support_report_for_xof(hasher.finalize_xof(), draws);
    if q953 {
        assert_eq!(draws, TRAILMIX_NUM_TESTS, "Q953 support proof shot count");
        assert_eq!(report.miss_factors, 0, "Q953 fresh nonce has support misses");
        assert_eq!(report.repair_entries, 0, "Q953 fresh nonce needs repairs");
        let schedule = q953_schedule.expect("Q953 schedule certificate");
        let first_terminal_capable_row = schedule.first_terminal_capable_row;
        assert!(
            report
                .min_terminal_row
                .is_some_and(|row| row >= first_terminal_capable_row),
            "Q953 support terminal trace violates the schedule lower bound"
        );
    }
    eprintln!(
...
pub fn build_builder() -> crate::point_add::B {
    ...
    builder.finish_direct_stream_and_exit_if_enabled();
    builder
}
