...
fn configure_sub1000_trailmix_route() {
    ...
    // Baked Q952 coherent+direct route. The source embeds the exact certified
    // schedule and defaults to the route trusted for this nonce.
    set_default_env("LOWQ_Q956_OFF_BORROW", "1");
    set_default_env("LOWQ_Q955_OFF_CANONICAL", "1");
    set_default_env("LOWQ_Q954_SROT_COUNTER7", "1");
    set_default_env("LOWQ_Q953_SROT_COUNTER67", "1");
    set_default_env("LOWQ_Q952_SROT_COUNTER567", "1");
    set_default_env("LOWQ_Q952_BORROWED_ARITH", "1");
    set_default_env("LOWQ_Q953_COHERENT_TEMP_MUL", "1");
    set_default_env("LOWQ_Q953_DIRECT_COUNTER_COMPARE", "1");
    set_default_env("LOWQ_Q952_EMBEDDED_SCHEDULE", "1");
    set_default_env(
        "LOWQ_Q952_SCHEDULE_SHA3_256",
        "5a56fd48a3b85ef5b3d890af983cde7add0eac74531b1c9c79b669ff57612677",
    );
    set_default_env("TRAILMIX_SROT_W", "2");
    set_default_env("TRAILMIX_DEFER_Y_MATERIALIZE", "1");
    set_default_env("TRAILMIX_ZERO_DY_NEWDX_ROUTE", "1");
    set_default_env("TRAILMIX_TAIL_NONCE", "154463484622665");
}
...
struct TrailMixSupportReport {
    ...
    min_terminal_row: Option<usize>,
    max_terminal_row: Option<usize>,
}
...
fn support_report_for_xof_limited(
    ...
    for draw in 0..target_draws {
        ...
        for (label, factor) in [("dx", dx), ("qx_minus_rx", c)] {
            if !factor.is_zero() {
                let terminal_row =
                    inversion::shrunken_pz_schedule::terminal_row_u256(factor);
                report.min_terminal_row = Some(
                    report
                        .min_terminal_row
                        .map_or(terminal_row, |old| old.min(terminal_row)),
                );
                report.max_terminal_row = Some(
                    report
                        .max_terminal_row
                        .map_or(terminal_row, |old| old.max(terminal_row)),
                );
            }
            let repairs =
...
fn report_current_support(builder: &crate::point_add::B) {
    ...
    let q952 = std::env::var("LOWQ_Q952_SROT_COUNTER567").ok().as_deref() == Some("1");
    let q953 = std::env::var("LOWQ_Q953_SROT_COUNTER67").ok().as_deref() == Some("1");
    let sealed_srot_route = q952 || q953;
    let Some(hasher) = builder.clone_fiat_hash() else {
        assert!(
            !sealed_srot_route,
            "Q952/Q953 support proof requires POINT_ADD_HASH_OPS_LEN"
        );
        eprintln!(
    ...
    if sealed_srot_route {
        assert_eq!(
            draws, TRAILMIX_NUM_TESTS,
            "Q952/Q953 support proof shot count"
        );
        assert_eq!(
            report.miss_factors, 0,
            "Q952/Q953 fresh nonce has support misses"
        );
        assert_eq!(
            report.repair_entries, 0,
            "Q952/Q953 fresh nonce needs repairs"
        );
        let first_terminal_capable_row = if q952 {
            inversion::shrunken_pz_state_machine::
                q952_srot_counter567_schedule_certificate()
                .first_terminal_capable_row
        } else {
            inversion::shrunken_pz_state_machine::
                q953_srot_counter67_schedule_certificate()
                .first_terminal_capable_row
        };
        assert!(
            report
                .min_terminal_row
                .is_some_and(|row| row >= first_terminal_capable_row),
            "Q952/Q953 support terminal trace violates the schedule lower bound"
        );
    }
    eprintln!(
        "TRAILMIX_SUPPORT draws={} accepted={} miss_factors={} repair_entries={} first_miss={:?} min_terminal_row={:?} max_terminal_row={:?}",
        draws,
        report.accepted_shots,
        report.miss_factors,
        report.repair_entries,
        report.first_miss,
        report.min_terminal_row,
        report.max_terminal_row,
    );
}

#[doc(hidden)]
pub fn operation_stream_shake256_256(builder: &crate::point_add::B) -> [u8; 32] {
    let mut xof = builder
        .clone_fiat_hash()
        .expect("operation-stream digest requires POINT_ADD_HASH_OPS_LEN")
        .finalize_xof();
