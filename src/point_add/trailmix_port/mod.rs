...
fn configure_sub1000_trailmix_route() {
    ...
    set_default_env("TRAILMIX_THIN_CLZ_WINDOW", "84");
    set_default_env("TRAILMIX_THIN_MARGIN", "12");
    set_default_env("TRAILMIX_THIN_VALIDATE", "500000");
    set_default_env("TRAILMIX_THIN_SHIFT_MARGIN", "3");
    ...
    set_default_env("TRAILMIX_TAIL_NONCE", "5");
}
...
fn support_report_for_xof_limited(
    ...
    for draw in 0..target_draws {
        ...
        for (label, factor) in [("dx", dx), ("qx_minus_rx", c)] {
            let (repairs, clz_misses) = if label == "qx_minus_rx" {
                let _cancel_profile = inversion::shrunken_pz_schedule::enter_cancel_profile();
                (
                    inversion::shrunken_pz_schedule::thin_factor_repairs_u256(factor),
                    inversion::shrunken_pz_schedule::thin_factor_clz_window_misses_u256(factor),
                )
            } else {
                (
                    inversion::shrunken_pz_schedule::thin_factor_repairs_u256(factor),
            ...
            let total_misses = repairs + clz_misses;
            if total_misses > 0 {
                report.miss_factors += 1;
                report.repair_entries += total_misses;
                if report.first_miss.is_none() {
                    report.first_miss = Some((draw, label, total_misses));
                }
...
fn search_tail_nonce(builder: &crate::point_add::B, q0: u32, q1: u32) {
    ...
    let confirm_draws = env_usize("TRAILMIX_TAIL_NONCE_CONFIRM_SHOTS", 0);
    let confirm_max_misses = env_usize("TRAILMIX_TAIL_NONCE_CONFIRM_MAX_MISSES", usize::MAX);
            ...
            for tid in 0..threads {
                ...
                handles.push(scope.spawn(move || {
                    ...
                    while off < limit {
                        ...
                        let mut report = support_report_for_xof_limited(
                            &mut xof,
                            draws,
                        ...
                        if report.miss_factors == 0 {
                            if confirm_draws > draws {
                                let hasher = hash_tail_nonce(base_hasher.clone(), nonce, q0, q1);
                                let mut xof = hasher.finalize_xof();
                                report = support_report_for_xof_limited(
                                    &mut xof,
                                    confirm_draws,
                                    (confirm_max_misses != usize::MAX).then_some(confirm_max_misses),
                                );
                                if trace_clean
                                    || report.miss_factors <= confirm_max_misses
                                {
                                    eprintln!(
                                        "TRAILMIX_TAIL_SEARCH_CONFIRM nonce={} accepted={} miss_factors={} repair_entries={} first_miss={:?}",
                                        nonce,
                                        report.accepted_shots,
                                        report.miss_factors,
                                        report.repair_entries,
                            ...
                            if report.miss_factors == 0 && !continue_after_clean {
                                break;
                            }
                        }
                        off += threads;
