...
fn configure_sub1000_trailmix_route() {
    ...
    // Re-searched after the `bit_length_lean` peak-reduction restructure (peak
    // 999 -> 997) reordered the emitted op stream: nonce=55 gives miss_factors=0
    // over all 9024 Fiat-Shamir draws at the new stream (hash ops len 136844164).
    set_default_env("TRAILMIX_TAIL_NONCE", "55");
}
