...
fn configure_sub1000_trailmix_route() {
    ...
    // Baked Q954 submission route: compose the support-preserving off borrow,
    // exact canonical inverse Horner cleanup, and certified counter[7]/s_rot
    // alias. The trusted stream below is replayed without environment flags.
    set_default_env("LOWQ_Q956_OFF_BORROW", "1");
    set_default_env("LOWQ_Q955_OFF_CANONICAL", "1");
    set_default_env("LOWQ_Q954_SROT_COUNTER7", "1");
    set_default_env("TRAILMIX_SROT_W", "4");
    set_default_env("TRAILMIX_DEFER_Y_MATERIALIZE", "1");
    set_default_env("TRAILMIX_ZERO_DY_NEWDX_ROUTE", "1");
    set_default_env("TRAILMIX_TAIL_NONCE", "4618586");
}
