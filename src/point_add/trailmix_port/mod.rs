...
fn configure_sub1000_trailmix_route() {
    ...
    set_default_env("TRAILMIX_THIN_TRAIN", "65536");
    ...
    set_default_env("TRAILMIX_THIN_REPAIR_MARGIN", "0");
    ...
    set_default_env("LOWQ_EXACT_WINDOW_COMPARE", "1");
    ...
    set_default_env("LOWQ_Q956_OFF_BORROW", "1");
    set_default_env("LOWQ_Q955_OFF_CANONICAL", "1");
    ...
    // Baked Q948 route: affine counter, robust support envelope, borrowed
    // transcript, direct HCLZ guard, and exact full-width comparisons. Census
    // binaries use proof mode, which is intentionally exclusive with the
    // production fresh-support certificates.
    let proof_mode = std::env::var("LOWQ_Q949_PROOF_MODE").ok().as_deref() == Some("1");
    set_default_env("LOWQ_Q949_AFFINE_COUNTER", "1");
    set_default_env("LOWQ_Q949_ROBUST_SYMMETRIC_SCHEDULE", "1");
    if !proof_mode {
        set_default_env("LOWQ_Q949_ROBUST_FRESH_SUPPORT_CERTIFIED", "1");
    }
    set_default_env("LOWQ_PASSENGER_TOP_LIFETIME_EXPERIMENT", "1");
    set_default_env("LOWQ_BORROWED_TRANSCRIPT_EXPERIMENT", "1");
    if !proof_mode {
        set_default_env("LOWQ_BORROWED_TRANSCRIPT_FRESH_SUPPORT_CERTIFIED", "1");
    }
    set_default_env("LOWQ_REVERSE_CA255_RELATIONAL_LOAN_EXPERIMENT", "0");
    set_default_env("LOWQ_Q948_DIRECT_HCLZ_PEAK_GUARD", "1");
    set_default_env("LOWQ_Q947_PASSENGER_DIRECT_HCLZ", "1");
    set_default_env("LOWQ_Q947_FRESH_SUPPORT_CERTIFIED", "0");
    set_default_env("LOWQ_Q946_SECOND_OWNERSHIP_RELEASE", "1");
    set_default_env("LOWQ_Q946_FRESH_SUPPORT_CERTIFIED", "1");
    ...
    set_default_env("TRAILMIX_TAIL_NONCE", "990030016442");
}
...
pub fn q949_source_identity() -> String {
    ...
    hasher.update(b"q949-source-identity-v4");
    for (name, bytes) in q949_source_manifest::SOURCES {
        q949_hash_component(&mut hasher, name, bytes);
    }
...
pub fn q949_route_identity() -> String {
    ...
    if std::env::var("LOWQ_EXACT_WINDOW_COMPARE").ok().as_deref() == Some("1") {
        q949_hash_component(&mut hasher, "LOWQ_EXACT_WINDOW_COMPARE", b"1");
    }
    q949_finish_identity(hasher)
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Q949SupportedNarrowCompareMiss {
    pub draw: usize,
    pub factor_label: &'static str,
    pub factor: U256,
...
pub struct Q949SupportedTraceReport {
    ...
    pub first_narrow_compare_miss: Option<Q949SupportedNarrowCompareMiss>,
...
fn q949_validate_census_report(report: &Q949SupportedTraceReport) {
    ...
    assert!(report.narrow_compare_misses <= report.narrow_compare_checks);
    assert_eq!(
        report.narrow_compare_checks,
        report.division_offset_compare_checks
            + report.multiply_offset_cleanup_compare_checks
    );
    assert_eq!(
        report.narrow_compare_misses,
        report.division_offset_compare_misses
            + report.multiply_offset_cleanup_compare_misses
    );
    assert_eq!(
        report.first_narrow_compare_miss.is_some(),
        report.narrow_compare_misses != 0,
        "Q949 narrowed-compare first-miss accounting drift"
    );
...
fn q949_emit_census_diagnostics(
    ...
    eprintln!(
        concat!(
            ...
            "narrow_compare_checks={} narrow_compare_misses={}"
        ),
        Q949_CENSUS_DIAGNOSTICS_SCHEMA,
        status,
        report.accepted_shots,
...
/// This is deliberately separate from profiling: width, CLZ, and narrowed
/// comparison misses are retained while the complete census runs, then any
/// miss rejects the proof.
#[doc(hidden)]
pub fn q949_supported_trace_check(builder: &crate::point_add::B) -> Q949SupportedTraceReport {
        ...
        for (factor_label, factor) in [
            ...
            if first_narrow_compare_miss.is_none() {
                first_narrow_compare_miss = certificate.first_narrow_compare_miss.map(
                    |coordinate| Q949SupportedNarrowCompareMiss {
                        draw,
                        factor_label,
                        factor,
                        coordinate,
            ...
            assert_eq!(
                certificate.narrow_compare_miss_coordinates.len(),
                certificate.narrow_compare_misses,
                "Q949 factor narrow-compare coordinate count drift"
            );
            assert_eq!(
    ...
    if !diagnostic_rejection_report {
        ...
        assert_eq!(
            report.narrow_compare_misses, 0,
            "Q949 narrowed-compare semantic census failed; first={:?}",
            report.first_narrow_compare_miss,
        );
    }
    assert!(report.earliest_terminal_row <= report.latest_terminal_row);
    assert!(
...
fn search_tail_nonce(builder: &crate::point_add::B, q0: u32, q1: u32) {
    ...
    let candidate_nonces: Option<Vec<u64>> =
        std::env::var("TRAILMIX_TAIL_NONCE_CANDIDATES_FILE")
            .ok()
            .and_then(|path| {
                let contents = std::fs::read_to_string(&path).unwrap_or_else(|err| {
                    panic!("failed to read TRAILMIX_TAIL_NONCE_CANDIDATES_FILE={path}: {err}")
                });
                let mut out = Vec::new();
                for line in contents.lines() {
                    ...
                    let token = trimmed
                        ...
                        .unwrap_or("");
                    if !token.is_empty() {
                        out.push(token.parse::<u64>().unwrap_or_else(|err| {
                            panic!("bad tail nonce token '{token}' in {path}: {err}")
                        }));
                    }
                }
                out.sort_unstable();
                out.dedup();
                eprintln!(
                    "TRAILMIX_TAIL_SEARCH_CANDIDATES loaded={} file={path}",
                    out.len()
                );
                Some(out)
            });
    let work_len = candidate_nonces.as_ref().map_or(limit, |v| {
        if limit == 0 {
            v.len()
        } else {
            limit.min(v.len())
    ...
    let threads = env_usize("TRAILMIX_TAIL_NONCE_THREADS", default_threads)
        .max(1)
        .min(work_len.max(1));
    let candidates = candidate_nonces.as_deref();
        ...
        std::thread::scope(|scope| {
            ...
            for tid in 0..threads {
                ...
                handles.push(scope.spawn(move || {
                    ...
                    let mut off = tid;
                    while off < work_len {
                        let nonce = candidates
                            .map(|v| v[off])
                            .unwrap_or_else(|| start.wrapping_add(off as u64));
