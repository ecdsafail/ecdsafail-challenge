...
fn configure_sub1000_trailmix_route() {
    ...
    // materialized flag (dirty double-control for the rare 2-control prefix
    // callbacks). Frees one owned `rs.dynamic-bitlen.fused-prefix-scratch` lane
    // on the P2 co-peak plateau -> measured peak 846 -> 845. Value-proven
    // (KG_ONLY_PREFIX_POSITIVE 160 + DIRECT_PREFIX_BITLEN_PROOF 16320). The dirty
    // double-control fires only at log* layer boundaries and the candidate pool
    // is capped, so op inflation is modest (1.13B -> 1.98B, eval-feasible).
