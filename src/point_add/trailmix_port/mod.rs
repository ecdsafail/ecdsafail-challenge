...
fn configure_sub1000_trailmix_route() {
    ...
    set_default_env("TRAILMIX_TAIL_NONCE", "800178108");
}
...
// Emit the PZ thin-screen schedule tables as CUDA __constant__ arrays for the
// GPU prefilter (prefilter_pz.cuh). Gated by TRAILMIX_DUMP_TABLES=1 so it never
// affects a real build. Universal caps (SHRUNKEN_PZ_*) + the trained per-step
// widths (reg_widths, post-margin/pre-q-adjust — the device replays the q-budget).
fn dump_pz_tables() {
    use inversion::shrunken_pz_schedule as sch;
    let n = sch::SHRUNKEN_PZ_NSTEPS;
    let arrs: [(&str, &[u16]); 5] = [
        ("A", &sch::SHRUNKEN_PZ_A),
        ("B", &sch::SHRUNKEN_PZ_B),
        ("CA", &sch::SHRUNKEN_PZ_CA),
        ("CB", &sch::SHRUNKEN_PZ_CB),
        ("Q", &sch::SHRUNKEN_PZ_Q),
    ];
    println!("// auto-generated by TRAILMIX_DUMP_TABLES; NSTEPS={n}");
    println!("#define PZ_NSTEPS {n}");
    for (name, arr) in arrs {
        print!("__device__ __constant__ unsigned short PZ_UNIV_{name}[PZ_NSTEPS] = {{");
    ...
    // PZ_WIDTH = trained widths with the q-budget applied (mirrors the tmp logic
    // in thin_factor_repairs_u256: target=TRAILMIX_Q_TARGET, cap=TRAILMIX_Q_CAP,
    // guard=0, ab_cap/cacb_cap unset). Device then just compares against this.
    let target: u16 = env_usize("TRAILMIX_Q_TARGET", 684).min(u16::MAX as usize) as u16;
    let cap: u16 = env_usize("TRAILMIX_Q_CAP", 99).min(u16::MAX as usize) as u16;
    print!("__device__ __constant__ unsigned short PZ_WIDTH[PZ_NSTEPS][5] = {{");
    for i in 0..n {
        let (a, b, ca, cb, q0) = sch::reg_widths(i);
        let (a, b, ca, cb) = (a as u16, b as u16, ca as u16, cb as u16);
        let other = 2 * a.max(b) + 2 * ca.max(cb);
        let budget = target.saturating_sub(other).max(1);
        let q = q0 as u16 ;
        let q = q.min(budget).min(cap).max(1);
        if i > 0 {
            print!(",");
        }
        print!("{{{a},{b},{ca},{cb},{q}}}");
...
pub fn build_builder() -> crate::point_add::B {
    ...
    if std::env::var("TRAILMIX_DUMP_TABLES").ok().as_deref() == Some("1") {
        dump_pz_tables();
        std::process::exit(0);
    }

    // Ground-truth (factor limbs, repairs) pairs for the GPU predicate cross-check.
    // Factors = x-coords of i*G (realistic field elements). Gated; exits early.
    if std::env::var("TRAILMIX_DUMP_FACTORS").ok().as_deref() == Some("1") {
        let curve = secp256k1();
        for i in 1u64..30000 {
            let pt = curve.mul(curve.gx, curve.gy, U256::from(i));
            let f = pt.0;
            let r = inversion::shrunken_pz_schedule::thin_factor_repairs_u256(f);
            let l = f.as_limbs();
            println!("FACTOR {} {} {} {} {}", l[0], l[1], l[2], l[3], r);
        }
        std::process::exit(0);
    }
