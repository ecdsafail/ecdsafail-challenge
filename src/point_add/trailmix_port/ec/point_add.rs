...
mod tests {
    ...
    fn sub_mod_u256(a: U256, b: U256, p: U256) -> U256 {
        crate::weierstrass_elliptic_curve::sub_mod(a, b, p)
    }

    #[test]
    fn shrunken_pz_cancel_denominator_batches_pre_inverse() {
        let curve = secp256k1();
        let p = curve.modulus;

        for i in 1..=32u64 {
            let pp = curve.mul(curve.gx, curve.gy, U256::from(i * 17 + 3));
            let qq = curve.mul(curve.gx, curve.gy, U256::from(i * 29 + 11));
            if pp.0 == qq.0 {
                continue;
            }

            let dx = sub_mod_u256(qq.0, pp.0, p);
            let dy = sub_mod_u256(qq.1, pp.1, p);
            let inv_dx = dx.inv_mod(p).expect("generic dx is invertible");
            let lambda = dy.mul_mod(inv_dx, p);
            let rr = curve.add(pp.0, pp.1, qq.0, qq.1);
            let new_dx = sub_mod_u256(qq.0, rr.0, p);
            let new_dy = qq.1.add_mod(rr.1, p);

            // new_dx = qx - rx
            //        = qx - (lambda^2 - px - qx)
            //        = 3*qx - dx - dy^2/dx^2
            // so D := (3*qx - dx)*dx^2 - dy^2 is available before inversion and
            // satisfies D = new_dx*dx^2. That lets a batch route invert dx and D
            // with one inverse of dx*D, then derive 1/new_dx = dx^2/D.
            let dx2 = dx.mul_mod(dx, p);
            let three_qx_minus_dx = sub_mod_u256(qq.0.mul_mod(U256::from(3u64), p), dx, p);
            let d = sub_mod_u256(three_qx_minus_dx.mul_mod(dx2, p), dy.mul_mod(dy, p), p);
            assert_eq!(d, new_dx.mul_mod(dx2, p), "D relation failed for case {i}");

            let inv_batch = dx.mul_mod(d, p).inv_mod(p).expect("generic dx*D is invertible");
            let inv_dx_from_batch = d.mul_mod(inv_batch, p);
            let inv_d_from_batch = dx.mul_mod(inv_batch, p);
            assert_eq!(inv_dx_from_batch, inv_dx, "batched 1/dx failed for case {i}");

            let inv_new_dx_from_batch = dx2.mul_mod(inv_d_from_batch, p);
            let inv_new_dx = new_dx.inv_mod(p).expect("generic new_dx is invertible");
            assert_eq!(
                inv_new_dx_from_batch, inv_new_dx,
                "batched 1/new_dx failed for case {i}"
            );
            assert_eq!(
                new_dy.mul_mod(inv_new_dx_from_batch, p),
                lambda,
                "alt-witness lambda failed for case {i}"
            );
            assert_eq!(
                lambda.mul_mod(new_dx, p),
                new_dy,
                "new_dy=lambda*new_dx failed for case {i}"
            );
        }
    }
