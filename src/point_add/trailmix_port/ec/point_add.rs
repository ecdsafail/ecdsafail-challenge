...
pub fn horner_forward(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg]) -> QReg {
    ...
    crate::point_add::trailmix_port::arith::rfold_mbu::reduce_once_secp256k1_from_rfold(circ, result, &flag);
...
pub fn horner_reverse(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg], flag: QReg) {
    ...
    crate::point_add::trailmix_port::arith::rfold_mbu::reduce_once_secp256k1_from_rfold_reverse(circ, result, &flag);
    circ.zero_and_free(flag);
    circ.set_section(&prev);
    for i in 0..n - 1 {
        crate::point_add::trailmix_port::arith::rfold_mbu::controlled_mod_sub_rfold_mbu(circ, &b[i], result, a);
        crate::point_add::trailmix_port::arith::rfold_mbu::mod_halve_rfold_mbu(circ, result);
    }
    crate::point_add::trailmix_port::arith::rfold_mbu::controlled_mod_sub_rfold_mbu(circ, &b[n - 1], result, a);
}
...
pub fn mod_mac_inplace(circ: &mut Circuit, a: &[QReg], b: &[QReg], c: &[QReg]) {
    ...
    for _ in 0..(n - 1) {
        crate::point_add::trailmix_port::arith::rfold_mbu::mod_halve_rfold_mbu(circ, a);
    }
...
pub fn mod_msc_inplace(circ: &mut Circuit, a: &[QReg], b: &[QReg], c: &[QReg]) {
    ...
    // then canonicalize.
    for _ in 0..(n - 1) {
        crate::point_add::trailmix_port::arith::rfold_mbu::mod_double_rfold_mbu(circ, a);
    }
    ...
    crate::point_add::trailmix_port::arith::rfold_mbu::reduce_once_secp256k1_from_rfold(circ, a, &post_flag);
    horner_canonical_flag_consume(circ, a, post_flag);
    circ.pop_section(&prev);
}
...
pub fn ec_add_inplace_shrunken_pz(
    ...
    *ty = dy_vec;
    ...
    mod_sub_creg_pointadd_w(circ, &tx[..], ox);
    ...
    mod_sub_creg_pointadd_w(circ, &tx[..], ox); // tx -= ox => tx = dx_diff
    circ.set_section("ec3.new_y.build");
    mod_mac_inplace(circ, &ty[..], &lambda, &tx[..]); // ty += lambda*dx_diff
    mod_sub_creg_pointadd_w(circ, &ty[..], oy); // ty = new_y
    circ.set_section("ec3.new_y.dx_diff_clean");
    mod_add_creg_pointadd_w(circ, &tx[..], ox); // tx += ox (canonical)
    mod_msc_inplace(circ, &tx[..], &lambda, &lambda); // tx -= lambda^2 => tx = -2*new_x (canonical)
    crate::point_add::trailmix_port::arith::rfold_mbu::mod_halve_rfold_mbu(circ, &tx[..]); // tx = -new_x (halve of even -2*new_x)
    mod_neg_inplace_w(circ, &tx[..]); // tx = new_x

    // Phase 7: cancel lambda via the alt-witness lambda = new_dy/new_dx.
    circ.set_section("ec3.alt.new_dy");
    mod_add_creg_pointadd_w(circ, &ty[..], oy); // ty := new_y + oy = new_dy
    circ.set_section("ec3.alt.new_dx");
    mod_sub_from_creg_pointadd_w(circ, &tx[..], ox); // tx := ox - new_x = new_dx
    ...
    *ty = ndy_vec;
    ...
    mod_sub_from_creg_pointadd_w(circ, &tx[..], ox); // tx := ox - new_dx = new_x
    circ.set_section("ec3.alt.new_y_restore");
    mod_sub_creg_pointadd_w(circ, &ty[..], oy); // ty := new_dy - oy = new_y
...
fn shrunken_pz_pointadd_use_qload() -> bool {
    std::env::var("SHRUNKEN_PZ_POINTADD_USE_QLOAD")
        .ok()
        .map(|v| v != "0")
        .unwrap_or(false)
}
...
pub fn mod_sub_creg_w(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    ...
    circ.cx(&flag, &a[n - 1]);
    let r = crate::point_add::trailmix_port::arith::mod_arith::secp256k1_r_le();
...
pub fn mod_add_creg_w(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    ...
    p_le33[..32].copy_from_slice(&crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE);
...
pub fn mod_neg_inplace_w(circ: &mut Circuit, a: &[QReg]) {
    ...
    p_plus_1[..32].copy_from_slice(&crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE);
...
pub fn mod_add_creg_qload_w(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    let temp = load_creg_temp(circ, c);
    crate::point_add::trailmix_port::arith::mod_arith::mod_add_mbu(circ, a, &temp, &crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE);
    unload_creg_temp(circ, temp, c);
}

fn mod_add_creg_pointadd_w(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    if shrunken_pz_pointadd_use_qload() {
...
pub fn mod_sub_creg_qload_w(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    let temp = load_creg_temp(circ, c);
    crate::point_add::trailmix_port::arith::mod_arith::mod_sub_mbu(circ, a, &temp, &crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE);
    unload_creg_temp(circ, temp, c);
}

fn mod_sub_creg_pointadd_w(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    if shrunken_pz_pointadd_use_qload() {
        mod_sub_creg_qload_w(circ, a, c);
    } else {
        mod_sub_creg_w(circ, a, c);
    }
}
...
fn mod_sub_from_creg_pointadd_w(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    if shrunken_pz_pointadd_use_qload() {
        mod_sub_from_creg_qload_w(circ, a, c);
    } else {
        mod_sub_from_creg_w(circ, a, c);
    }
}
...
pub fn controlled_mod_add_deferred_w(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    let n = a.len();
    assert_eq!(b.len(), n);
    crate::point_add::trailmix_port::arith::rfold_mbu::controlled_mod_add_rfold_mbu(circ, ctrl, a, b);
}
...
mod tests {
    ...
    fn mod_mac_inplace_random_secp() {
        use rand::RngCore;
        let p = BigUint::from_bytes_le(&crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE);
