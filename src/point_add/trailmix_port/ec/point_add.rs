...
pub fn horner_forward(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg]) -> QReg {
    ...
    crate::point_add::trailmix_port::arith::rfold_mbu::reduce_once_secp256k1_from_rfold(
        circ, result, &flag,
    );
...
pub fn horner_reverse(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg], flag: QReg) {
    ...
    crate::point_add::trailmix_port::arith::rfold_mbu::reduce_once_secp256k1_from_rfold_reverse(
        circ, result, &flag,
    );
    circ.zero_and_free(flag);
    circ.set_section(&prev);
    for i in 0..n - 1 {
        crate::point_add::trailmix_port::arith::rfold_mbu::controlled_mod_sub_rfold_mbu(
            circ, &b[i], result, a,
        );
...
pub fn mod_add_creg_w(
    ...
    p_le33[..32]
        .copy_from_slice(&crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE);
    crate::point_add::trailmix_port::arith::const_add::controlled_sub_const(
        circ, &flag, a, &p_le33,
    );
...
pub fn mod_add_creg_qload_w(
    circ: &mut Circuit,
    a: &[QReg],
    c: &[crate::point_add::trailmix_port::circuit::Cbit],
) {
    let temp = load_creg_temp(circ, c);
    crate::point_add::trailmix_port::arith::mod_arith::mod_add_mbu(
        circ,
        a,
        &temp,
        &crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE,
    );
    unload_creg_temp(circ, temp, c);
}

fn mod_add_creg_pointadd_w(
    circ: &mut Circuit,
    a: &[QReg],
    c: &[crate::point_add::trailmix_port::circuit::Cbit],
) {
...
pub fn mod_sub_creg_qload_w(
    circ: &mut Circuit,
    a: &[QReg],
    c: &[crate::point_add::trailmix_port::circuit::Cbit],
) {
    let temp = load_creg_temp(circ, c);
    crate::point_add::trailmix_port::arith::mod_arith::mod_sub_mbu(
        circ,
        a,
        &temp,
        &crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE,
...
pub fn mod_sub_from_creg_qload_w(
    circ: &mut Circuit,
    a: &[QReg],
    c: &[crate::point_add::trailmix_port::circuit::Cbit],
) {
...
fn mod_sub_from_creg_pointadd_w(
    circ: &mut Circuit,
    a: &[QReg],
    c: &[crate::point_add::trailmix_port::circuit::Cbit],
) {
...
pub fn controlled_mod_add_deferred_w(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    let n = a.len();
    assert_eq!(b.len(), n);
    crate::point_add::trailmix_port::arith::rfold_mbu::controlled_mod_add_rfold_mbu(
        circ, ctrl, a, b,
    );
}
