...
pub fn horner_forward(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg]) -> QReg {
    ...
    crate::point_add::trailmix_port::rfold_mbu::reduce_once_secp256k1_from_rfold(
        circ, result, &flag,
    );
...
pub fn horner_reverse(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg], flag: QReg) {
    ...
    crate::point_add::trailmix_port::rfold_mbu::reduce_once_secp256k1_from_rfold_reverse(
        circ, result, &flag,
    );
    circ.zero_and_free(flag);
    circ.set_section(&prev);
    for i in 0..n - 1 {
        crate::point_add::trailmix_port::rfold_mbu::controlled_mod_sub_rfold_mbu(
            circ, &b[i], result, a,
        );
...
pub fn ec_add_inplace_shrunken_pz(
    ...
    use crate::point_add::trailmix_port::inversion::shrunken_pz_state_machine::{
        shrunken_pz_divide_cancel, shrunken_pz_divide_forward, shrunken_pz_product_undo,
    };
    ...
    let register_shared = register_shared_eea_enabled();
    let (dx_inner, dy_vec, lambda) = if register_shared {
    ...
    *ty = dy_vec;
    ...
    let (ndx_inner, ndy_vec) = if register_shared {
