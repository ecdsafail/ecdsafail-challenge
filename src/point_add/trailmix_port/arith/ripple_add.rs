    ...
    }
    {
        ...
        circ.contract_capture(
            ...
            > {
                ...
                let modulus =
                    crate::point_add::trailmix_port::num_bigint::BigUint::from(1u32) << n_cap;
...
pub fn controlled_add(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    ...
    if do_contract {
        ...
        circ.contract_capture(
            ...
            > {
                let read =
                    |regs: &[&QReg]| -> crate::point_add::trailmix_port::num_bigint::BigUint {
                        let mut v =
                            crate::point_add::trailmix_port::num_bigint::BigUint::from(0u32);
                        for (i, q) in regs.iter().enumerate() {
                            if view.contract_read_bit_shot(q, shot) {
                                v |= crate::point_add::trailmix_port::num_bigint::BigUint::from(
                                    1u32,
                                ) << i;
                            }
                        }
