...
use crate::point_add::trailmix_port::compat::num_bigint;
...
pub fn add_creg(
    ...
    }
    {
        ...
        circ.contract_capture(
            ...
            > {
                let mut a_pre =
                    crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(0u32);
                for (i, q) in a_for_capture.iter().enumerate() {
                    if view.contract_read_bit_shot(q, shot) {
                        a_pre |= crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(
                            1u32,
                        ) << i;
                    }
                }
                ...
                for (i, id) in c_ids.iter().enumerate() {
                    if (view.bit_mask(*id) >> shot) & 1 == 1 {
                        c_val |= crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(
                            1u32,
                        ) << i;
                    }
                }
                let modulus =
                    crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(1u32)
                        << n_cap;
    ...
    for i in 0..lim {
        ...
        crate::point_add::trailmix_port::arith::khattar_gidney::cinc_gidney_halving(
            circ, sub, &ctrl,
        );
        circ.x_if_bit(&ctrl, bit);
    ...
    }
    {
        ...
        let n_cap = n;
        circ.contract_pop_and_check::<(crate::point_add::trailmix_port::compat::num_bigint::BigUint, crate::point_add::trailmix_port::compat::num_bigint::BigUint), _>(
            "poc_arith.add_creg",
            move |captured, view, shot| -> Result<(), String> {
                let (a_pre, c_val) = captured;
                let mut a_post = crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(0u32);
                for (i, q) in a_for_check.iter().enumerate() {
                    if view.contract_read_bit_shot(q, shot) {
                        a_post |= crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(1u32) << i;
                    }
                }
                let modulus = crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(1u32) << n_cap;
...
pub fn add_const(circ: &mut Circuit, a: &[QReg], val: &[u8]) {
    ...
    if pop == 1 {
        ...
        crate::point_add::trailmix_port::arith::khattar_gidney::inc_khattar_gidney(
            circ,
            &a[lo_bit..],
        );
        return;
...
pub fn controlled_add(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], b: &[QReg]) {
    ...
    if do_contract {
        ...
        circ.contract_capture(
            ...
            |view, shot| -> Result<(crate::point_add::trailmix_port::compat::num_bigint::BigUint, crate::point_add::trailmix_port::compat::num_bigint::BigUint, bool), String> {
    ...
    if !aliases_a && !aliases_b {
        ...
        crate::point_add::trailmix_port::arith::cuccaro::controlled_add_cuccaro_3n(
            circ, ctrl, a, b,
        );
    } else {
    ...
    if do_contract {
        let a_for_check: Vec<&QReg> = a.iter().collect();
        let n_cap = a.len();
        circ.contract_pop_and_check::<(crate::point_add::trailmix_port::compat::num_bigint::BigUint, crate::point_add::trailmix_port::compat::num_bigint::BigUint, bool), _>(
            "poc_arith.controlled_add",
            move |captured, view, shot| -> Result<(), String> {
