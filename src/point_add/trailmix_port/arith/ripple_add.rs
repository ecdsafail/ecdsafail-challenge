...
pub fn add_creg(circ: &mut Circuit, a: &[QReg], c: &[crate::point_add::trailmix_port::circuit::Cbit]) {
    ...
    }
    {
        ...
        circ.contract_capture(
            "poc_arith.add_creg",
            move |view, shot| -> Result<(crate::point_add::trailmix_port::compat::num_bigint::BigUint, crate::point_add::trailmix_port::compat::num_bigint::BigUint), String> {
                let mut a_pre = crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(0u32);
                for (i, q) in a_for_capture.iter().enumerate() {
                    if view.contract_read_bit_shot(q, shot) {
                        a_pre |= crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(1u32) << i;
                    }
                }
                let mut c_val = crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(0u32);
                for (i, id) in c_ids.iter().enumerate() {
                    if (view.bit_mask(*id) >> shot) & 1 == 1 {
                        c_val |= crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(1u32) << i;
                    }
                }
