...
pub fn mcx_dirty(circ: &mut Circuit, ctrls: &[&QReg], target: &QReg, psi: &QReg) {
    ...
    match k {
        ...
        6..=8 => {
            // k>5 via doubled C^(k-1) X:
            //   CCX(c0,c1,psi);
            //   C^(k-1)X(psi,c2..→target) with c0 dirty;
            //   CCX(c0,c1,psi);
            //   C^(k-1)X again.
            // This is expensive but keeps the one-dirty/no-clean shape for the
            // small high-control gates used by the low-qubit branch.
            let (c0, c1) = (ctrls[0], ctrls[1]);
            let mut inner_ctrls: Vec<&QReg> = Vec::with_capacity(ctrls.len() - 1);
            inner_ctrls.push(psi);
            inner_ctrls.extend_from_slice(&ctrls[2..]);
            circ.ccx(c0, c1, psi);
            mcx_dirty(circ, &inner_ctrls, target, c0);
            circ.ccx(c0, c1, psi);
            mcx_dirty(circ, &inner_ctrls, target, c0);
        }
        _ => {
            // The Barenco-style constants above are derived only for
            // k <= 8. Callers needing k >= 9 must route through
            // `mcx_dirty_any_k` (Theorem 3 recursion via `mcx_clean_k`);
            // a direct call here violates that contract.
            panic!("mcx_dirty supports k <= 8 controls (got k = {k}); use mcx_dirty_any_k for k >= 9");
        }
    }
...
/// C^kX with 1 dirty ancilla for any k. Recurses via Theorem 3 when k >= 9.
///
/// Base case: k <= 8 uses existing `mcx_dirty` (Barenco-style constants).
/// Recursive case: k >= 9 falls back to `mcx_clean_k` (O(log k) clean
/// ancillae). The dirty qubit is ignored for k >= 9; callers that need
/// strict dirty-only semantics should use k <= 8.
...
/// last gate-touch. For k <= 8, `target` is freed after the last
/// `mcx_dirty` gate. For k >= 9, uses `mcx_clean_k_uncompute_consume`
