...
/// `target ^= AND(ctrls)` using `k-2` borrowed dirty lanes and no clean lane.
///
/// Let `V` be the full dirty v-chain, including the first link
/// `dirty[0] ^= ctrls[0] & ctrls[1]`, and let `V0` omit that first link. `V`
/// toggles the target by the full control product plus all terms seeded by the
/// unknown dirty values. `V0` toggles exactly those seed terms. Thus `V; V0`
/// leaves only the full product, and both chains unwind every dirty lane.
///
/// For `k >= 3`, cost is `(2k-3) + (2k-5) = 4k-8` CCX. The construction is a
/// phase-free basis permutation.
pub fn mcx_dirty_ladder(
    ...
    assert!(
        dirty.len() >= k - 2,
        "mcx_dirty_ladder needs k-2 restored dirty lanes"
    );
    let dirty = &dirty[..k - 2];
    let aliases = |q: &QReg| {
        ctrls.iter().any(|c| std::ptr::eq(*c, q)) || std::ptr::eq(target, q)
    };
    assert!(
        !dirty.iter().any(|q| aliases(q)),
        "mcx_dirty_ladder dirty lane aliases a control or target"
    );
    for i in 0..dirty.len() {
        assert!(
            !dirty[..i].iter().any(|q| std::ptr::eq(*q, dirty[i])),
            "mcx_dirty_ladder duplicate dirty lane"
        );
    }
