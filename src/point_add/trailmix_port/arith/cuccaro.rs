...
/// Controlled 3n Cuccaro add using a caller-owned, conditionally clean carry.
///
/// This emits the same forward/reverse skeleton as
/// [`controlled_add_cuccaro_3n_refs`], but does not allocate or free a carry
/// lane. Its contract is deliberately asymmetric:
///
/// - `ctrl=0`: `carry` may be arbitrary; the complete operation is identity.
/// - `ctrl=1`: `carry` must be zero; `a := a+b (mod 2^n)`.
///
/// In both branches `carry`, `b`, and `ctrl` are restored. The carry must be a
/// distinct, nonoperand lane; every operand lane must also be structurally
/// distinct. These assertions make accidental use of an output or aliased
/// counter lane fail at circuit construction time.
pub fn controlled_add_cuccaro_3n_refs_with_carry(
    circ: &mut Circuit,
    ctrl: &QReg,
    a: &[&QReg],
    b: &[&QReg],
    ...
    assert_eq!(
        b.len(),
        n,
        "controlled_add_cuccaro_3n_refs_with_carry: a/b length mismatch"
    );
    assert!(
        !std::ptr::eq(ctrl, carry),
        "controlled_add_cuccaro_3n_refs_with_carry: ctrl aliases carry"
    );
    for (i, &q) in a.iter().enumerate() {
        assert!(
            !std::ptr::eq(q, ctrl) && !std::ptr::eq(q, carry),
            "controlled_add_cuccaro_3n_refs_with_carry: a[{i}] aliases ctrl/carry"
        );
        assert!(
            !a[..i].iter().any(|&prior| std::ptr::eq(prior, q)),
            "controlled_add_cuccaro_3n_refs_with_carry: duplicate a lane"
        );
    }
    for (i, &q) in b.iter().enumerate() {
        assert!(
            !std::ptr::eq(q, ctrl) && !std::ptr::eq(q, carry),
            "controlled_add_cuccaro_3n_refs_with_carry: b[{i}] aliases ctrl/carry"
        );
        assert!(
            !b[..i].iter().any(|&prior| std::ptr::eq(prior, q)),
            "controlled_add_cuccaro_3n_refs_with_carry: duplicate b lane"
        );
        assert!(
            !a.iter().any(|&aq| std::ptr::eq(aq, q)),
            "controlled_add_cuccaro_3n_refs_with_carry: a/b operands overlap"
        );
    }

    if n > 0 {
        let a_for_capture = a.to_vec();
        let b_for_capture = b.to_vec();
        let ctrl_ref = ctrl;
        let carry_ref = carry;
        circ.contract_capture(
            "mbu.controlled_add_cuccaro_3n.borrowed.pre",
    ...
    if n == 1 {
        circ.ccx(ctrl, b[0], a[0]);
        controlled_add_cuccaro_3n_borrowed_post_check(circ, ctrl, a, b, carry);
        return;
    }
...
fn controlled_add_cuccaro_3n_borrowed_post_check(
    circ: &mut Circuit,
    ctrl: &QReg,
    a: &[&QReg],
    b: &[&QReg],
    ...
    let n = a.len();
    let a_for_check = a.to_vec();
    let b_for_check = b.to_vec();
    let ctrl_ref = ctrl;
    let carry_ref = carry;
    ...
    >(
        "mbu.controlled_add_cuccaro_3n.borrowed.pre",
        move |cap, view, shot| -> Result<(), String> {
            let (a_pre, b_pre, ctrl_pre, carry_pre) = cap;
            if *ctrl_pre && *carry_pre {
                return Err(
                    "borrowed Cuccaro carry must be zero whenever ctrl=1".to_string(),
                );
            }
            let read = |regs: &[&QReg]| {
                let mut value =
                ...
                for (i, q) in regs.iter().enumerate() {
                    if view.contract_read_bit_shot(q, shot) {
                        value |=
                            crate::point_add::trailmix_port::num_bigint::BigUint::from(1u32) << i;
                    }
                }
                value
            };
            let a_post = read(&a_for_check);
            let b_post = read(&b_for_check);
            let ctrl_post = view.contract_read_bit_shot(ctrl_ref, shot);
            let carry_post = view.contract_read_bit_shot(carry_ref, shot);
            let expected = if *ctrl_pre {
                let modulus =
                    crate::point_add::trailmix_port::num_bigint::BigUint::from(1u32) << n;
                (a_pre + b_pre) % modulus
            } else {
                a_pre.clone()
            };
            if a_post != expected {
                return Err(format!(
                    "borrowed ctrl_add_cuccaro_3n: a_post={a_post:#x}, expected={expected:#x}"
                ));
            }
            if &b_post != b_pre {
                return Err(format!(
                    "borrowed ctrl_add_cuccaro_3n: b changed {b_pre:#x}->{b_post:#x}"
                ));
            }
            if ctrl_post != *ctrl_pre || carry_post != *carry_pre {
                return Err(format!(
                    "borrowed ctrl_add_cuccaro_3n: ctrl/carry changed ({},{})->({},{})",
                    u8::from(*ctrl_pre),
                    u8::from(*carry_pre),
                    u8::from(ctrl_post),
                    u8::from(carry_post),
...
pub struct BorrowedCarryExhaustiveReport {
    pub widths_checked: usize,
    pub states_checked: usize,
    pub active_clean_states_checked: usize,
    pub inactive_clean_states_checked: usize,
    pub inactive_dirty_states_checked: usize,
    pub max_internal_extra_qubits: usize,
    pub max_operation_increase: usize,
    pub max_toffoli_increase: usize,
}

/// Exhaustively simulate the emitted borrowed-carry gate stream through width
/// five, including every dirty-carry inactive state.
#[doc(hidden)]
pub fn exhaustive_controlled_add_cuccaro_3n_borrowed_carry_check(
) -> BorrowedCarryExhaustiveReport {
    ...
    fn apply(ops: &[Op], mut state: u64) -> u64 {
        ...
        for op in ops {
            match op.kind {
                ...
                other => panic!("borrowed Cuccaro proof emitted unexpected gate {other:?}"),
            }
        }
        state
    }
    ...
    let mut active_clean_states_checked = 0usize;
    let mut inactive_clean_states_checked = 0usize;
    let mut inactive_dirty_states_checked = 0usize;
    let mut max_internal_extra_qubits = 0usize;
    let mut max_operation_increase = 0usize;
    let mut max_toffoli_increase = 0usize;

    for width in 1..=5usize {
        let mut borrowed = Circuit::new();
        assert!(
            !borrowed.b.count_only,
            "borrowed Cuccaro proof requires emitted operations"
        );
        let ctrl = borrowed.alloc_qreg("borrowed-cuccaro.ctrl");
        let carry = borrowed.alloc_qreg("borrowed-cuccaro.carry");
        let a = borrowed.alloc_qreg_bits("borrowed-cuccaro.a", width);
        let b = borrowed.alloc_qreg_bits("borrowed-cuccaro.b", width);
        let ar: Vec<&QReg> = a.iter().collect();
        let br: Vec<&QReg> = b.iter().collect();
        controlled_add_cuccaro_3n_refs_with_carry(
            &mut borrowed,
            &ctrl,
            &ar,
            &br,
        ...
        let borrowed_builder = borrowed.into_builder();
        let extra = borrowed_builder.peak_qubits as usize - external;
        max_internal_extra_qubits = max_internal_extra_qubits.max(extra);
        assert_eq!(extra, 0, "width={width}: borrowed Cuccaro allocated");
        assert_eq!(
            borrowed_builder.ops.len(),
            if width == 1 { 1 } else { 7 * width },
            "width={width}: borrowed Cuccaro operation count drift"
        );
        assert_eq!(
            toffoli_count(&borrowed_builder.ops),
            if width == 1 { 1 } else { 3 * width },
            "width={width}: borrowed Cuccaro Toffoli count drift"
        );

        let mut owned = Circuit::new();
        let owned_ctrl = owned.alloc_qreg("owned-cuccaro.ctrl");
        let owned_a = owned.alloc_qreg_bits("owned-cuccaro.a", width);
        let owned_b = owned.alloc_qreg_bits("owned-cuccaro.b", width);
        let owned_ar: Vec<&QReg> = owned_a.iter().collect();
        let owned_br: Vec<&QReg> = owned_b.iter().collect();
        controlled_add_cuccaro_3n_refs(&mut owned, &owned_ctrl, &owned_ar, &owned_br);
        let owned_builder = owned.into_builder();
        max_operation_increase = max_operation_increase.max(
            borrowed_builder
                .ops
                .len()
                .saturating_sub(owned_builder.ops.len()),
        );
        max_toffoli_increase = max_toffoli_increase.max(
            toffoli_count(&borrowed_builder.ops)
                .saturating_sub(toffoli_count(&owned_builder.ops)),
        );
        ...
        for input in 0..(1u64 << external) {
            let ctrl_pre = input & 1;
            let carry_pre = (input >> 1) & 1;
            if ctrl_pre == 1 && carry_pre == 1 {
                continue;
            }
            states_checked += 1;
            match (ctrl_pre, carry_pre) {
                (1, 0) => active_clean_states_checked += 1,
                (0, 0) => inactive_clean_states_checked += 1,
                (0, 1) => inactive_dirty_states_checked += 1,
                _ => unreachable!(),
            }
            let a_pre = word(input, a_start, width);
            let b_pre = word(input, b_start, width);
            let a_post = a_pre.wrapping_add(ctrl_pre * b_pre) & mask;
            let expected = (input & !(mask << a_start)) | (a_post << a_start);
            let got = apply(&borrowed_builder.ops, input);
            assert_eq!(
                got, expected,
                "borrowed Cuccaro width={width} input={input}"
            );
        }
    }

    assert_eq!(max_internal_extra_qubits, 0);
    assert_eq!(max_operation_increase, 0);
    assert_eq!(max_toffoli_increase, 0);
    BorrowedCarryExhaustiveReport {
        widths_checked: 5,
        states_checked,
        active_clean_states_checked,
        inactive_clean_states_checked,
        inactive_dirty_states_checked,
        max_internal_extra_qubits,
        max_operation_increase,
        max_toffoli_increase,
    }
}
