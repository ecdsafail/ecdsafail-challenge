...
pub fn controlled_add_const(circ: &mut Circuit, ctrl: &QReg, a: &[QReg], val: &[u8]) {
    ...
    if pop == 1 {
        ...
        crate::point_add::trailmix_port::arith::khattar_gidney::cinc_khattar_gidney(
            circ,
            &a[lo_bit..],
            ctrl,
        );
    ...
    if runs.len() <= 8 && segment_count <= 15 {
        ...
        crate::point_add::trailmix_port::arith::khattar_gidney::controlled_classical_quantum_add(
            circ, ctrl, a, val,
        );
    }
}
...
fn xor_all_ones_consume_free(circ: &mut Circuit, block: &[QReg], target: QReg) {
    let block_refs: Vec<&QReg> = block.iter().collect();
    crate::point_add::trailmix_port::arith::mcx::mcx_clean_k_uncompute_consume(
        circ,
        &block_refs,
        target,
    );
...
pub fn controlled_add_const_refs(circ: &mut Circuit, ctrl: &QReg, a: &[&QReg], val: &[u8]) {
    ...
    crate::point_add::trailmix_port::arith::khattar_gidney::controlled_classical_quantum_add_refs(
        circ, ctrl, a, val,
    );
}
