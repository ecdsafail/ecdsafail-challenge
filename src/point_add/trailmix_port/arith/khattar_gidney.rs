...
use crate::point_add::trailmix_port::arith::mcx::{mcx_clean_k, mcx_dirty_any_k, mcx_dirty_any_k_consume, mcx_dirty_ladder};
...
where
    F: FnMut(&mut Circuit, usize, &QReg),
{
    unary_iterate_log_star_borrowed(circ, c, n_iters, &[], None, body)
}

/// Like [`unary_iterate_log_star`], but when enough `ladder_lenders` are
/// supplied (>= n-2 live lanes, disjoint from the counter `c` and the internal
/// gate) the per-iteration all-ones detector is realized as a single flat
/// *dirty-borrowed* MCX over all `n` counter bits ([`mcx_dirty_ladder`], which
/// borrows and exactly restores `k-2` dirty lenders). This removes the entire
/// clean KG prefix-AND ancilla block (`uls_anc`) from the inversion peak,
/// trading extra Toffoli for `kg_prefix_ancilla_count(n-1)` fewer peak qubits.
/// The lenders are restored on every detector edge (forward and uncompute) and
/// may be live scratch the body itself uses, since each ladder call is
/// self-contained. With no lenders it falls back to the clean KG prefix path.
pub fn unary_iterate_log_star_borrowed<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    ladder_lenders: &[&QReg],
    direct_target: Option<(&QReg, bool)>,
    mut body: F,
) where
    F: FnMut(&mut Circuit, usize, &QReg),
{
    ...
    // Direct-target gateless path: when a `direct_target` accumulator is given
    // and enough ladder lenders are available, apply the all-ones detector
    // straight onto the target (`target ^= (c == i)`) with NO `uls_gate` qubit
    // and a single dirty-ladder MCX per iteration (gate-mode needs a
    // compute+uncompute pair). Removes the peak-top gate qubit entirely. The
    // caller's body must NOT re-toggle the target from the detector (the toggle
    // already happened); it just consumes the accumulated target.
    if let Some((target, body_first)) = direct_target {
        let lenders_ok = ladder_lenders.len() >= n - 2
            && ladder_lenders.iter().all(|&l| {
                !std::ptr::eq(l, target) && !c.iter().any(|&ci| std::ptr::eq(l, ci))
            });
        let target_ok = !c.iter().any(|&ci| std::ptr::eq(ci, target));
        if n >= 3 && lenders_ok && target_ok {
            let all_ctrls: Vec<&QReg> = c.to_vec();
            let lenders: Vec<&QReg> = ladder_lenders[..n - 2].to_vec();
            for i in 0..n_iters {
                // `body_first` (reverse sweep) consumes the accumulator before
                // this edge's toggle; otherwise (forward) toggle then consume.
                if body_first {
                    body(circ, i, target);
                    mcx_dirty_ladder(circ, &all_ctrls, target, &lenders);
                } else {
                    mcx_dirty_ladder(circ, &all_ctrls, target, &lenders);
                    body(circ, i, target);
                }
            ...
            // restore c = v (same tail as the gate-mode path).
            let last = n_iters - 1;
            for (j, q) in c.iter().enumerate() {
                if (last >> j) & 1 == 0 {
                    circ.x(q);
    ...
    if nb == 1 {
        ...
        && ladder_lenders.len() >= n - 2
        && ladder_lenders
            .iter()
            .all(|&l| !std::ptr::eq(l, &gate) && !c.iter().any(|&ci| std::ptr::eq(l, ci)))
    {
        // Flat dirty-ladder all-ones detector over ALL n counter bits. No clean
        // KG prefix-AND ancilla (the `uls_anc` block is eliminated): the k-2
        // dirty lenders are borrowed from already-live scratch and restored on
        // every detector edge by `mcx_dirty_ladder`. Each ladder call is
        // self-contained, so lenders the body itself touches are fine.
        let all_ctrls: Vec<&QReg> = c.to_vec();
        let lenders: Vec<&QReg> = ladder_lenders[..n - 2].to_vec();
        for i in 0..n_iters {
            mcx_dirty_ladder(circ, &all_ctrls, &gate, &lenders);
            body(circ, i, &gate);
            mcx_dirty_ladder(circ, &all_ctrls, &gate, &lenders);
            if i + 1 < n_iters {
                let b = (i ^ (i + 1)).count_ones() as usize;
