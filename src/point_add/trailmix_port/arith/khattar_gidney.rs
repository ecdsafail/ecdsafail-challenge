    ...
    F: FnMut(&mut Circuit, usize, &QReg),
{
    ...
    if nb == 1 {
        ...
        for i in 0..n_iters {
            ...
            gc.push(c0);
            let dirty_mcx3 = std::env::var(SUB800_ULS_DIRTY_MCX3_FLAG).ok().as_deref() == Some("1")
                && gc.len() == 3;
            let dirty = if dirty_mcx3 {
                anc.iter()
                    .copied()
                    .find(|candidate| gc.iter().all(|control| control.id() != candidate.id()))
            } else {
                None
            };
    ...
    F: FnMut(&mut Circuit, usize),
{
    ...
    if nb == 1 {
        ...
        let anc_owned = circ.alloc_qreg_bits("uls_anc", ancilla_count - clean_lenders.len());
        let mut anc: Vec<&QReg> = anc_owned.iter().collect();
    ...
    F: FnMut(&mut Circuit, usize),
{
    ...
    assert!(
        n >= 2,
        "direct unary iteration needs at least two counter bits"
    );
    assert!(
        n_iters >= 1,
        "direct unary iteration needs at least one step"
    );
    assert!(
        n >= 63 || n_iters <= (1usize << n),
    ...
    let toggle = |circ: &mut Circuit| match c.len() {
        ...
        count => {
            ...
            for index in 2..count - 1 {
                circ.ccx(c[index], clean_scratch[index - 2], clean_scratch[index - 1]);
            }
...
fn verify_sub800_dirty_mcx3_primitive() -> (usize, usize) {
    ...
    for value in 0u64..32 {
        for (bit, (&baseline_id, &candidate_id)) in
            baseline_ids.iter().zip(&candidate_ids).enumerate()
        {
            if (value >> bit) & 1 != 0 {
    ...
    for (bit, (&baseline_id, &candidate_id)) in baseline_ids.iter().zip(&candidate_ids).enumerate()
    {
...
pub fn exhaustive_sub800_uls_dirty_mcx3_check() -> Sub800UlsDirtyMcx3ProofReport {
    ...
    for counter_width in 5usize..=9 {
        ...
        let (simulator_cases, simulator_phases) =
            verify_sub800_uls_simulator_equivalence(&baseline, &candidate, counter_width, N_ITERS);
...
pub fn exhaustive_sub800_uls_clean_lender_check() -> Sub800UlsCleanLenderProofReport {
    ...
    for counter_width in 5usize..=9 {
        let baseline = build_sub800_uls_clean_lender_proof_harness(counter_width, N_ITERS, false);
        let candidate = build_sub800_uls_clean_lender_proof_harness(counter_width, N_ITERS, true);
        assert_eq!(baseline.external_ids, candidate.external_ids);
        assert_eq!(baseline.external_mask, candidate.external_mask);
        assert_eq!(
            candidate.builder.peak_qubits + 1,
...
fn sub800_uls_direct_expected_state(
    ...
    let mut fingerprint = (value >> (counter_width + 1)) & ((1u64 << result_width) - 1);
    for iteration in 0..n_iters {
        if mode == Sub800UlsFusedTargetProofMode::Forward && counter == iteration {
            active = !active;
        }
...
fn verify_sub800_uls_direct_selector_equivalence(
    ...
    let expected_external = counter_width + 1 + result_width + 1 + counter_width.saturating_sub(1);
    ...
    for batch_start in (0..states).step_by(64) {
        ...
        for &(start, end) in &candidate.callback_ranges {
            assert!(
                candidate_cursor <= start && start <= end && end <= candidate.builder.ops.len()
            );
...
fn verify_sub800_uls_dirty_selector_primitive() -> (usize, usize) {
    ...
    for batch_start in (0..BASIS_STATES).step_by(64) {
        ...
        assert_eq!(
            ...
            0,
            "dirty selector left phase garbage"
        );
        ...
        for (bit, &id) in dirty_ids.iter().enumerate() {
            ...
            for shot in 0..shots {
                expected |=
                    ((((batch_start + shot) >> (CONTROL_WIDTH + 1 + bit)) & 1) as u64) << shot;
            }
            assert_eq!(simulator.qubit(QubitId(u64::from(id))) & live, expected);
        }
    }
...
pub fn cdec_khattar_gidney_refs_with_anc_exact_reverse(
    ...
    inc_khattar_gidney_refs_inner_with_anc_exact_reverse(
        circ, &combined, /*skip_lsb_x=*/ true, anc_refs,
    );
