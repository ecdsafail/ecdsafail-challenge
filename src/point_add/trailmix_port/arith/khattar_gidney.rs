...
use crate::point_add::trailmix_port::arith::mcx::{mcx_clean_k, mcx_dirty, mcx_dirty_any_k, mcx_dirty_any_k_consume};
...
/// All-ones detector edge for `unary_iterate_log_star`: `gate ^= AND(gc)`.
/// Uses a dirty-borrowed MCX (no fresh clean ancilla) when a valid `borrow`
/// lender is supplied and the control count is in `mcx_dirty`'s supported
/// range (3..=5); otherwise falls back to the clean-ancilla MCX.
fn detector_mcx(circ: &mut Circuit, gc: &[&QReg], gate: &QReg, borrow: Option<&QReg>) {
    if let Some(psi) = borrow {
        let k = gc.len();
        let lender_ok = !std::ptr::eq(psi, gate)
            && !gc.iter().any(|&control| std::ptr::eq(psi, control));
        if (3..=5).contains(&k) && lender_ok {
            mcx_dirty(circ, gc, gate, psi);
            return;
        }
    }
...
/// Like [`unary_iterate_log_star`], but when `borrow` is `Some(psi)` the
/// per-iteration all-ones detector is realized with a *dirty-borrowed* MCX
/// ([`mcx_dirty`], k<=5) instead of [`mcx_clean_k`], which allocates a fresh
/// clean ancilla (`mcxk_t3`). `psi` must be a live qubit that is NOT one of the
/// detector controls (`c[1..]` prefix + `c[0]`) and NOT the internal gate; it is
/// restored exactly on every detector edge (enforced by `mcx_dirty`'s contract
/// checks). This removes the top scratch qubit at the inversion peak, trading a
/// few extra Toffoli for one fewer peak qubit.
    ...
    F: FnMut(&mut Circuit, usize, &QReg),
{
    ...
    if nb == 1 {
        ...
        for i in 0..n_iters {
            ...
            gc.push(c0);
            detector_mcx(circ, &gc, &gate, borrow);
            body(circ, i, &gate);
            detector_mcx(circ, &gc, &gate, borrow); // uncompute
            if i + 1 < n_iters {
                let b = (i ^ (i + 1)).count_ones() as usize;
