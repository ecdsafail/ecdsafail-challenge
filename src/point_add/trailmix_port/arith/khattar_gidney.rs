...
where
    F: FnMut(&mut Circuit, usize, &QReg),
{
    unary_iterate_log_star_dirty(circ, c, n_iters, None, body)
}

/// As [`unary_iterate_log_star`], but when `dirty` is `Some(psi)` the inner
/// all-ones-detector multi-controlled-X is emitted on the BORROWED dirty lane
/// `psi` (via `mcx_dirty_any_k`) instead of allocating a fresh clean
/// `mcxk_t3` ancilla. `psi` must be a live qubit that is NOT one of `c`,
/// NOT `gate`, and whose value the caller does not rely on being preserved
/// across the call (the dirty-MCX restores `psi` exactly per invocation).
/// This is the sub-851 swap-comparator transient collapse (C1): it removes the
/// `mcxk_t3[0]` lane that sets peak=851 on the register-shared EEA swap scan.
pub fn unary_iterate_log_star_dirty<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    dirty: Option<&QReg>,
    ...
    F: FnMut(&mut Circuit, usize, &QReg),
{
    match dirty {
        Some(psi) => {
            let bank = [psi];
            unary_iterate_log_star_dirty_bank(circ, c, n_iters, Some(&bank[..]), body)
        }
        None => unary_iterate_log_star_dirty_bank(circ, c, n_iters, None, body),
    }
}

/// As [`unary_iterate_log_star_dirty`], but `dirty_bank` may carry MORE than
/// one borrowed dirty lane. When the bank holds at least `n-2` idle lenders
/// (where `n = c.len()`), the all-ones detector is emitted as a SINGLE
/// `mcx_dirty_ladder` over all `n` counter bits directly into `gate`,
/// borrowing `n-2` lenders from the bank — this drops the persistent
/// Khattar-Gidney prefix-AND ladder (`uls_anc`, `kg_prefix_ancilla_count(n-1)`
/// clean qubits) entirely. The bank lanes are restored EXACTLY by
/// `mcx_dirty_ladder` (Bennett cascade), so they may hold arbitrary live
/// values that the caller depends on (e.g. an idle length register). When the
/// bank is `None`, exactly one lane, or fewer than `n-2` lanes, behaviour is
/// IDENTICAL to [`unary_iterate_log_star_dirty`] (the KG-prefix path, with the
/// single-lane lane used as the small-k detector borrow).
///
/// This is the sub-851 swap-comparator transient collapse (C1 extension): the
/// `None`/single-lane path removed `mcxk_t3` (851->850); the multi-lane bank
/// path additionally removes `uls_anc[0..nb-1]` (-> 847 on the n=9 swap scan).
pub fn unary_iterate_log_star_dirty_bank<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    ...
    F: FnMut(&mut Circuit, usize, &QReg),
{
    ...
    // The KG-prefix path borrows at most one dirty lane for the small-k
    // detector MCX; the ladder path needs n-2 idle lenders for the direct
    // full-width detector. Validate lender disjointness from c / gate below.
    let ladder_lenders: Option<&[&QReg]> = match dirty_bank {
        Some(bank) if bank.len() >= n.saturating_sub(2) && n >= 3 => Some(bank),
        _ => None,
    };
    let dirty: Option<&QReg> = dirty_bank.and_then(|bank| bank.first().copied());
    ...
    if let Some(bank) = ladder_lenders {
        // --- LADDER DETECTOR PATH (no persistent KG prefix-AND tree) ---
        //
        // The all-ones detector `gate ^= AND(c)` is emitted fresh each
        // iteration as a single `mcx_dirty_ladder` over ALL n counter bits,
        // borrowing n-2 lenders from `bank`. Because there is no persistent
        // prefix tree, between iterations we only apply the gray-code flip
        // c ^= (i ^ (i+1)) on the affected low bits (no partial-layer patch).
        //
        // SAFETY: mcx_dirty_ladder restores every lender EXACTLY (Bennett
        // cascade, see mcx.rs), so the bank lanes may hold arbitrary live
        // values. The asserts below enforce lender ∉ c ∪ {gate}.
        let lenders: Vec<&QReg> = bank[..n - 2].to_vec();
        for &lend in &lenders {
            assert!(
                !std::ptr::eq(lend, &gate),
                "unary_iterate ladder: dirty lender aliases gate"
            );
            assert!(
                !c.iter().any(|&cq| std::ptr::eq(cq, lend)),
                "unary_iterate ladder: dirty lender aliases a counter bit"
            );
        }
        let c_all: Vec<&QReg> = c.to_vec();
        for i in 0..n_iters {
            // gate = AND(c) = (c == all_ones) = (v == i).
            mcx_dirty_ladder(circ, &c_all, &gate, &lenders);
            body(circ, i, &gate);
            mcx_dirty_ladder(circ, &c_all, &gate, &lenders); // uncompute gate
            if i + 1 < n_iters {
                let b = (i ^ (i + 1)).count_ones() as usize;
                for j in 0..b.min(n) {
                    circ.x(c[j]); // gray-code: flip the changed low bits
                }
            }
        }
        circ.zero_and_free(gate);
        ...
        for i in 0..n_iters {
            ...
            gc.push(c0);
            // C1: prefer a borrowed DIRTY ancilla (no clean mcxk_t3 alloc) when
            // the caller supplied an idle lane and the control count is in the
            // dirty-MCX domain (k <= 5). Falls back to the clean construction
            // otherwise so behaviour is unchanged for k >= 6 or no borrow.
            match dirty {
                Some(psi) if gc.len() <= 5 => mcx_dirty_any_k(circ, &gc, &gate, psi),
                _ => mcx_clean_k(circ, &gc, &gate),
            }
            body(circ, i, &gate);
            match dirty {
                Some(psi) if gc.len() <= 5 => mcx_dirty_any_k(circ, &gc, &gate, psi), // uncompute
                _ => mcx_clean_k(circ, &gc, &gate),                                   // uncompute
            }
            if i + 1 < n_iters {
                let b = (i ^ (i + 1)).count_ones() as usize;
...
/// Fused variant of the ladder-detector path in
/// [`unary_iterate_log_star_dirty_bank`] that ELIMINATES the clean `uls_gate`
/// ancilla. The all-ones detector `(c == i)` is XORed DIRECTLY into the
/// caller-provided `target` register via a single `mcx_dirty_ladder` over all
/// `n` counter bits (borrowing `n-2` lenders from `dirty_bank`), instead of
/// being computed into a fresh clean `gate` and then driving `cx(gate, target)`.
///
/// This is sound precisely because the two production swap callers consume the
/// detector as exactly one `circ.cx(gate, active)` and the body then reads the
/// (flipped) `active`/`target`. Fusing `ladder(c -> gate); cx(gate, target)`
/// into `ladder(c -> target)` produces the identical net `target ^= AND(c)`
/// while saving one qubit (peak 847 -> 846 on the n=9 swap scan).
///
/// `flip_after` controls the order relative to `body`:
/// * `false` (forward): `target ^= AND(c)` THEN `body(i)` — matches the
///   forward closure (`cx` then body).
/// * `true` (reverse): `body(i)` THEN `target ^= AND(c)` — matches the reverse
///   closure (body then `cx`).
///
/// `dirty_bank` MUST carry at least `n-2` lenders (the ladder requirement);
/// callers gate on this. Lenders are restored EXACTLY by `mcx_dirty_ladder`,
/// so they may hold arbitrary live values. `target` MUST be disjoint from `c`
/// and from every lender (asserted). The net effect on `c` (gray-code walk and
/// final restore to `v`) is bit-identical to the non-fused ladder path.
#[allow(clippy::too_many_arguments)]
pub fn unary_iterate_log_star_ladder_fused_cx<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    dirty_bank: &[&QReg],
...
{
    let n = c.len();
    assert!(n >= 3, "unary_iterate ladder-fused: need n >= 3 counter bits");
    assert!(n_iters >= 1, "unary_iterate ladder-fused: n_iters >= 1");
    assert!(
        n >= 63 || n_iters <= (1usize << n),
        "unary_iterate ladder-fused: n_iters {n_iters} exceeds 2^{n}"
    );
    assert!(
        dirty_bank.len() >= n - 2,
        "unary_iterate ladder-fused: lender shortage (need {}, got {})",
        n - 2,
        dirty_bank.len()
    );
    let lenders: Vec<&QReg> = dirty_bank[..n - 2].to_vec();
    for &lend in &lenders {
        assert!(
            !std::ptr::eq(lend, target),
            "unary_iterate ladder-fused: dirty lender aliases target"
        );
        assert!(
            !c.iter().any(|&cq| std::ptr::eq(cq, lend)),
            "unary_iterate ladder-fused: dirty lender aliases a counter bit"
        );
    }
    assert!(
        !c.iter().any(|&cq| std::ptr::eq(cq, target)),
        "unary_iterate ladder-fused: target aliases a counter bit"
    );
    ...
    for i in 0..n_iters {
        if !flip_after {
            mcx_dirty_ladder(circ, &c_all, target, &lenders); // target ^= AND(c)
        }
        body(circ, i);
        if flip_after {
            mcx_dirty_ladder(circ, &c_all, target, &lenders); // target ^= AND(c)
...
mod cond_inc_tests {
    ...
    /// Same validation as `run_unary_ls_case` but routes the inner all-ones
    /// detector through the BORROWED-DIRTY MCX (`unary_iterate_log_star_dirty`
    /// with `Some(psi)`). Asserts identical res/fired/counter outputs AND that
    /// the dirty lane `psi` is restored to its initial value `dirty_init` for
    /// every initial value, with zero residual phase. This is the C1
    /// (sub-851) swap-comparator dirty-MCX collapse value-check.
    fn run_unary_ls_dirty_case(n: usize, v: u64, n_iters: usize, dirty_init: u64) {
        use super::unary_iterate_log_star_dirty;
        let mut circ = Circuit::new();
        let c: Vec<QReg> = (0..n)
            .map(|i| circ.alloc_qreg(&format!("c{}", i)))
            .collect();
        let res: Vec<QReg> = (0..n)
            ...
            .collect();
        let fired = circ.alloc_qreg("fired");
        let psi = circ.alloc_qreg("dirty_psi");
        let mut bytes = vec![0u8; n.div_ceil(8)];
        for i in 0..n {
            if (v >> i) & 1 == 1 {
                bytes[i / 8] |= 1u8 << (i % 8);
        ...
        circ.sim_load_reg_bytes_shot(&c, &bytes, 0);
        if dirty_init & 1 == 1 {
            circ.sim_load_reg_bytes_shot(std::slice::from_ref(&psi), &[1u8], 0);
        }
        let c_refs: Vec<&QReg> = c.iter().collect();
        let res_refs: Vec<&QReg> = res.iter().collect();
        let fired_ref = &fired;
        let psi_ref = &psi;
        unary_iterate_log_star_dirty(
            &mut circ,
            &c_refs,
            n_iters,
            Some(psi_ref),
        ...
        let expect_res = if in_range { v } else { 0 };
        assert_eq!(res_v, expect_res, "uls-dirty n={} v={} res", n, v);
        assert_eq!(
            sim.qubit_mask(fired_d) & 1,
            in_range as u64,
            "uls-dirty n={} v={} fired",
        ...
        assert_eq!(c_out, v, "uls-dirty n={} v={} counter not restored", n, v);
        assert_eq!(
            sim.qubit_mask(psi_d) & 1,
            dirty_init & 1,
            "uls-dirty n={} v={} dirty psi not restored (init={})",
            n,
            v,
            dirty_init & 1
        );
        assert_eq!(sim.phase_mask(), 0, "uls-dirty n={} v={} phase", n, v);
    }


    /// Value-check for the uls_gate FUSION path
    /// (`unary_iterate_log_star_ladder_fused_cx`): the all-ones detector is
    /// XORed DIRECTLY into `target` (no clean uls_gate). Asserts the fused
    /// `target ^= AND(c)` per iteration produces identical net `fired`/`res`/
    /// counter-restore as the gate-based path AND that every borrowed dirty
    /// lender is restored exactly, for arbitrary lender init, zero residual
    /// phase. `flip_after` exercises both the forward (false) and reverse
    /// (true) orderings used by the swap step.
    fn run_unary_ls_ladder_fused_case(
        n: usize,
        v: u64,
        n_iters: usize,
        lender_init: u64,
        ...
        use super::unary_iterate_log_star_ladder_fused_cx;
        assert!(n >= 3);
        let n_lenders = n - 2;
        let mut circ = Circuit::new();
        let c: Vec<QReg> = (0..n)
        ...
        let res: Vec<QReg> = (0..n)
            .map(|i| circ.alloc_qreg(&format!("r{}", i)))
            .collect();
        // `target` is the swap `active`-like flag the detector flips directly.
        let target = circ.alloc_qreg("target");
        let lenders: Vec<QReg> = (0..n_lenders)
            .map(|i| circ.alloc_qreg(&format!("lend{}", i)))
            .collect();
        let mut bytes = vec![0u8; n.div_ceil(8)];
        for i in 0..n {
            if (v >> i) & 1 == 1 {
        ...
        circ.sim_load_reg_bytes_shot(&c, &bytes, 0);
        for (i, lq) in lenders.iter().enumerate() {
            if (lender_init >> i) & 1 == 1 {
                circ.sim_load_reg_bytes_shot(std::slice::from_ref(lq), &[1u8], 0);
            }
        }
        let c_refs: Vec<&QReg> = c.iter().collect();
        let res_refs: Vec<&QReg> = res.iter().collect();
        ...
        let target_ref = &target;
        // The body records the firing index into `res` while `target` is in
        // its detector-flipped state. With flip_after=false the detector has
        // already flipped target before body runs; with flip_after=true the
        // body runs before the flip. To get a flip_after-agnostic spec we key
        // `res` off the index whenever target is asserted *at body time*: that
        // matches the production swap, where the body reads `active`. We test
        // the net invariant (target final state + counter/lender restore)
        // which is order-independent, plus `res` recorded via the detector.
        unary_iterate_log_star_ladder_fused_cx(
            ...
            flip_after,
            |circ, i| {
                // record i into res, controlled on target (the live detector
                // flag) — for flip_after=false target already holds AND(c).
                if !flip_after {
                    for bit in 0..n {
                        if (i >> bit) & 1 == 1 {
                            circ.cx(target_ref, res_refs[bit]);
                        }
                    }
                }
            },
        ...
        let c_d = &det[..n];
        let res_d = &det[n..2 * n];
        let target_d = &det[2 * n];
        let lender_d = &det[2 * n + 1..2 * n + 1 + n_lenders];
        let in_range = (v as usize) < n_iters;
        // Net target: flipped once per iteration where AND(c)=1, i.e. exactly
        // once (at i==v) when v in range, so target == in_range.
        assert_eq!(
            sim.qubit_mask(target_d) & 1,
            in_range as u64,
            "uls-fused n={} v={} flip_after={} target net",
            n,
            v,
            flip_after
        );
        ...
        assert_eq!(
            c_out, v,
            "uls-fused n={} v={} counter not restored",
            n, v
        );
        for (i, q) in lender_d.iter().enumerate() {
            assert_eq!(
                sim.qubit_mask(q) & 1,
                (lender_init >> i) & 1,
                "uls-fused n={} v={} lender{} not restored",
                n,
                v,
                i
            );
        ...
        assert_eq!(
            sim.phase_mask(),
            0,
            "uls-fused n={} v={} flip_after={} phase",
            n,
            v,
            flip_after
        );
    ...
    #[test]
    fn unary_iterate_log_star_ladder_fused_full_range() {
        for n in 3..=6 {
            let l = 1usize << n;
            for v in 0..(1u64 << n) {
                for &flip_after in &[false, true] {
                    let lender_init = (v.wrapping_mul(2654435761) ^ 0xAB) & ((1u64 << (n - 2)) - 1);
                    run_unary_ls_ladder_fused_case(n, v, l, lender_init, flip_after);
                }
            }
        }
    }
    ...
    #[test]
    fn unary_iterate_log_star_ladder_fused_partial_range() {
        // n=9 (REFERENCE_LENGTH_WIDTH address), n_iters spanning the
        // physical_work_width+1 = 260 swap-scan window — the production
        // swap-step config; 7 borrowed lenders (l_q).
        for &(n, l) in &[(4usize, 11usize), (5, 20), (6, 50), (9, 260)] {
            for v in 0..(1u64 << n.min(9)) {
                for &flip_after in &[false, true] {
                    let lender_init = (v ^ 0x55) & ((1u64 << (n - 2)) - 1);
                    run_unary_ls_ladder_fused_case(n, v, l, lender_init, flip_after);
                }
            }
        }
    ...
    #[test]
    fn unary_iterate_log_star_dirty_full_range() {
        for n in 2..=6 {
            let l = 1usize << n;
            for v in 0..(1u64 << n) {
                for dirty_init in 0..2u64 {
                    run_unary_ls_dirty_case(n, v, l, dirty_init);
                }
            }
        }
    }
    ...
    #[test]
    fn unary_iterate_log_star_dirty_partial_range() {
        // n=9 (REFERENCE_LENGTH_WIDTH address), n_iters spanning the
        // physical_work_width+1 = 260 swap-scan window — the production
        // configuration of the register-shared EEA swap step.
        for &(n, l) in &[(4usize, 11usize), (5, 20), (6, 50), (9, 260)] {
            for v in 0..(1u64 << n.min(9)) {
                for dirty_init in 0..2u64 {
                    run_unary_ls_dirty_case(n, v, l, dirty_init);
