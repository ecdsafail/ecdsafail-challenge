...
pub fn mod_double(circ: &mut Circuit, a: &[QReg], p_bytes: &[u8; 32], flag: &QReg) {
    let p_val_pre = crate::point_add::trailmix_port::compat::num_bigint::BigUint::from_bytes_le(p_bytes);
    {
        let a_for_capture: Vec<&QReg> = a.iter().collect();
        let p_val = p_val_pre.clone();
        circ.contract_capture(
            "mod_arith.mod_double",
            move |view, shot| -> Result<crate::point_add::trailmix_port::compat::num_bigint::BigUint, String> {
