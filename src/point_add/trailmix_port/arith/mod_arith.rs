...
pub fn mod_double(circ: &mut Circuit, a: &[QReg], p_bytes: &[u8; 32], flag: &QReg) {
    let p_val_pre =
        crate::point_add::trailmix_port::compat::num_bigint::BigUint::from_bytes_le(p_bytes);
    {
            ...
            > {
                ...
                for (i, q) in a_for_capture.iter().enumerate() {
                    if view.contract_read_bit_shot(q, shot) {
                        v |= crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(
                            1u32,
                        ) << i;
                    }
                }
    ...
    crate::point_add::trailmix_port::arith::const_add::controlled_sub_const(circ, flag, a, p_bytes);
    {
        ...
        let p_val = p_val_pre;
        circ.contract_pop_and_check::<crate::point_add::trailmix_port::compat::num_bigint::BigUint, _>(
            "mod_arith.mod_double",
            move |a_pre, view, shot| -> Result<(), String> {
                ...
                let expected = (a_pre * crate::point_add::trailmix_port::compat::num_bigint::BigUint::from(2u32)) % &p_val;
                if a_post != expected {
                    return Err(format!(
...
pub fn mod_add_mbu(circ: &mut Circuit, a: &[QReg], b: &[QReg], _p_bytes: &[u8; 32]) {
    ...
    crate::point_add::trailmix_port::arith::compare::compare_lt_phase_correction_mbu(
        circ, a, b, &flag,
    );
}
