...
fn assert_compact_training_provenance() {
    assert_eq!(
        embedded_data::SELECTION_JOB_ID,
        Q949_ROBUST_SELECTION_JOB_ID
    );
    assert_eq!(
...
fn parse_envelope() -> Q949RobustEnvelope {
    ...
    assert_eq!(
        embedded_data::PROJECTION_SHA256,
    ...
    let phase_requirements =
        embedded_data::PHASE_REQUIREMENTS.map(|row| row.map(|phase| phase.map(usize::from)));
