...
fn lowq_q959_selective_borrow_enabled() -> bool {
    ...
    assert_eq!(
        ...
        684,
        "LOWQ_Q959_SELECTIVE_BORROW is sealed to Q_TARGET=684"
    );
    assert_eq!(
...
/// existing five-bit target. The MSB selectors are disjoint, and all dirty
/// lenders are restored after each selected constant update.
fn direct_bitlen_update(
    ...
    let candidates: Vec<&QReg> = peer
        ...
        .collect();
    let mut action: Vec<&QReg> = target.iter().collect();
    ...
    for k in lo..src.len() {
        ...
        // Materialize the mutually exclusive MSB selector once. Reusing this
        // borrowed clean gate across the five-bit constant update avoids
        // rebuilding the long selector for every carry bit.
...
fn q959_direct_bitlen_needed(
    ...
    circ.b.active_qubits as usize + hybrid_transcript_width(aw.max(bw)) > 959
}
...
pub fn division_substep_windowed(
    ...
    if q959_direct_bitlen_needed(circ, a, b, lo_a, lo_b) {
    ...
    if q959_direct_bitlen_needed(circ, a, b, lo_a, lo_a) {
...
fn borrowed_swap_in_place(
    ...
    let candidates: Vec<&QReg> = aa
        ...
        .collect();
    let action = [qz, anz, off];
    let prev = circ.push_section("p.swap.borrowed");

    // At every step boundary s_rot and off are clean. Retain the two
    // predicates in s_rot and materialize their active conjunction in off,
    // then restore all three lanes after the controlled swaps.
    ...
    let toggle_gate = |circ: &mut Circuit| {
        ...
        controls.push(qz);
        controls.push(anz);
        dirty_controlled_x(circ, &controls, off, &candidates, &action);
    ...
    toggle_gate(circ);
    for j in 0..aa.len() {
        circ.cswap(off, &aa[j], &bb[j]);
    }
    for j in 0..cca.len() {
        circ.cswap(off, &cca[j], &ccb[j]);
    }
    circ.cx(off, parity);
    toggle_gate(circ);
    toggle_nonzero_dirty(circ, aa, anz, &candidates, &action);
...
pub fn multiply_substep_windowed(
    ...
    if q959_direct_bitlen_needed(circ, a, b, ca_window, ca_window) {
    ...
    if q959_direct_bitlen_needed(circ, b, a, cb_window, ca_window) {
        direct_bitlen_diff_update(
            circ,
            b,
            a,
...
pub(crate) fn done_counter_fn(
    ...
    if lowq_q959_selective_borrow_enabled() {
        ...
        if inverse {
            cnz(c);
            dirty_controlled_inc_suffix(c, &[done], counter, 0, true, candidates);
            conv(c);
        } else {
            conv(c);
            dirty_controlled_inc_suffix(c, &[done], counter, 0, false, candidates);
            cnz(c);
        }
        return;
    }
...
pub fn exhaustive_selective_borrow_check() -> SelectiveBorrowExhaustiveReport {
    ...
    for width in 1..=3usize {
        for subtract_diff in [false, true] {
            ...
            assert_eq!(added, 0, "width={width}: direct bitlen allocated");
            ...
            for input in 0..(1u64 << external) {
                ...
                let got = apply(&builder.ops, input);
                assert_eq!(got, expected, "bitlen width={width} input={input}");
            }
        }
    }
    ...
    for width in 1..=3usize {
        ...
        set_bit_at_s_gated(&mut c, &q, &s, &active, &gate, &lenders);
