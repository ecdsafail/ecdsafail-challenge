...
use super::q945_local_hosts::{
    assert_q945_static_host_table, q945_carry_route, q945_hclz_route, Q945CarryRoute, Q945HclzForm,
    Q945HclzRoute, Q945Host, Q945StateRegister, Q945Substep, Q945_HCLZ_ROWS, Q945_NON_HCLZ_ROWS,
};
...
use crate::point_add::trailmix_port::inversion::q944_quotient_witness::{
    ...
    q944_reverse_park_sentinel, Q944_QUOTIENT_SENTINEL, Q944_QUOTIENT_WIDTH, Q944_SHIFT_WIDTH,
};
...
fn p_plus_1_bytes() -> Vec<u8> {
    vec![
        0x30, 0xfc, 0xff, 0xff, 0xfe, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
fn lowq_direct_prefix_bitlen_requested() -> bool {
    std::env::var("LOWQ_DIRECT_PREFIX_BITLEN").ok().as_deref() == Some("1")
}
...
fn lowq_direct_prefix_no_flag_requested() -> bool {
    std::env::var("LOWQ_DIRECT_PREFIX_NO_FLAG").ok().as_deref() == Some("1")
}
...
pub(crate) const DIRECT_PREFIX_COMPACT7_SCRATCH_LEN: usize = DIRECT_PREFIX_KG_SCRATCH_LEN + 2;
pub(crate) const DIRECT_PREFIX_COMPACT6_SCRATCH_LEN: usize = DIRECT_PREFIX_KG_SCRATCH_LEN + 1;
pub(crate) const DIRECT_PREFIX_COMPACT5_SCRATCH_LEN: usize = DIRECT_PREFIX_KG_SCRATCH_LEN;

fn assert_direct_prefix_full_scratch(src: &[&QReg], target: &[QReg], scratch: &[&QReg]) {
    assert_eq!(
        scratch.len(),
...
fn assert_direct_prefix_compact_scratch(src: &[&QReg], target: &[QReg], scratch: &[&QReg]) {
    assert_eq!(
        scratch.len(),
...
fn assert_direct_prefix_compact7_scratch(src: &[&QReg], target: &[QReg], scratch: &[&QReg]) {
    assert_eq!(
        scratch.len(),
...
fn assert_direct_prefix_compact5_scratch(src: &[&QReg], target: &[QReg], scratch: &[&QReg]) {
    assert_eq!(
        scratch.len(),
        DIRECT_PREFIX_COMPACT5_SCRATCH_LEN,
        "direct-prefix compact5 scratch requires exactly {DIRECT_PREFIX_COMPACT5_SCRATCH_LEN} lanes"
    );
    assert_eq!(
        target.len(),
        4,
        "direct-prefix compact5 scratch is sealed to four-bit targets"
    );
    for (index, lane) in scratch.iter().enumerate() {
        assert!(
            scratch[..index].iter().all(|other| other.id() != lane.id()),
            "direct-prefix compact5 scratch lane {index} aliases an earlier scratch lane"
        );
        assert!(
            src.iter().all(|source| source.id() != lane.id()),
            "direct-prefix compact5 scratch lane {index} aliases the source"
        );
        assert!(
            target.iter().all(|target| target.id() != lane.id()),
            "direct-prefix compact5 scratch lane {index} aliases the target"
        );
    }
}

fn assert_direct_prefix_compact6_scratch(src: &[&QReg], target: &[QReg], scratch: &[&QReg]) {
    assert_eq!(
        scratch.len(),
        DIRECT_PREFIX_COMPACT6_SCRATCH_LEN,
        "direct-prefix compact6 scratch requires exactly {DIRECT_PREFIX_COMPACT6_SCRATCH_LEN} lanes"
    );
    assert_eq!(
        target.len(),
        4,
        "direct-prefix compact6 scratch is sealed to four-bit targets"
    );
    for (index, lane) in scratch.iter().enumerate() {
        assert!(
            scratch[..index].iter().all(|other| other.id() != lane.id()),
            "direct-prefix compact6 scratch lane {index} aliases an earlier scratch lane"
        );
        assert!(
            src.iter().all(|source| source.id() != lane.id()),
            "direct-prefix compact6 scratch lane {index} aliases the source"
        );
        assert!(
            target.iter().all(|target| target.id() != lane.id()),
            "direct-prefix compact6 scratch lane {index} aliases the target"
        );
    }
}
...
fn assert_fused_zero_prefix_preconditions(
    ...
    for (index, lane) in target.iter().enumerate() {
        assert!(
            target[..index].iter().all(|other| other.id() != lane.id()),
    ...
    if let Some(scratch) = full_scratch {
        if scratch.len() == DIRECT_PREFIX_COMPACT5_SCRATCH_LEN {
            assert!(dirty_updates && no_materialized_flag);
            assert_eq!(
                omitted_high_bits, 5,
                "compact5 direct-prefix scratch is sealed to split-five bit length"
            );
            assert_direct_prefix_compact5_scratch(src, target, scratch);
        } else if scratch.len() == DIRECT_PREFIX_COMPACT6_SCRATCH_LEN {
            assert!(dirty_updates && no_materialized_flag);
            assert_eq!(
                omitted_high_bits, 5,
                "compact6 direct-prefix scratch is sealed to split-five bit length"
            );
            assert_direct_prefix_compact6_scratch(src, target, scratch);
...
fn add_const_with_borrowed_carry(circ: &mut Circuit, a: &[QReg], val: &[u8], carry: &QReg) {
    if a.is_empty() {
        return;
    }
    let has_nonzero = (0..a.len()).any(|bit| {
        val.get(bit / 8)
            .map(|byte| ((byte >> (bit % 8)) & 1) != 0)
            .unwrap_or(false)
    });
    if !has_nonzero {
        return;
    }
    crate::point_add::trailmix_port::arith::khattar_gidney::classical_quantum_add(
        circ, a, val, carry,
    );
}

fn sub_const_with_borrowed_carry(circ: &mut Circuit, a: &[QReg], val: &[u8], carry: &QReg) {
    if a.is_empty() {
        return;
    }
    let mut neg_bits = vec![false; a.len()];
    let mut carry_bit = true;
    for bit in 0..a.len() {
        let constant_bit = val
            .get(bit / 8)
            .map(|byte| ((byte >> (bit % 8)) & 1) != 0)
            .unwrap_or(false);
        let inv = !constant_bit;
        neg_bits[bit] = inv ^ carry_bit;
        carry_bit = inv && carry_bit;
    }
    let mut neg_val = vec![0u8; a.len().div_ceil(8)];
    for (bit, set) in neg_bits.into_iter().enumerate() {
        if set {
            neg_val[bit / 8] |= 1u8 << (bit % 8);
        }
    }
    add_const_with_borrowed_carry(circ, a, &neg_val, carry);
}
...
fn bit_length_lean_direct_prefix(
    ...
    use crate::point_add::trailmix_port::arith::khattar_gidney::{
        cdec_khattar_gidney_refs_with_anc_exact_reverse, cinc_khattar_gidney_refs_with_anc,
        kg_prefix_ancilla_count, KgPrefixAnd,
    };
    ...
    if let Some(scratch) = full_scratch {
        let compact5_scratch = scratch.len() == DIRECT_PREFIX_COMPACT5_SCRATCH_LEN;
        let compact6_scratch = scratch.len() == DIRECT_PREFIX_COMPACT6_SCRATCH_LEN;
        ...
        if compact5_scratch || compact6_scratch {
            assert!(
                dirty_updates && no_materialized_flag,
                "compact direct-prefix caller scratch uses dirty double-control updates"
            );
            assert_eq!(
                omitted_high_bits, 5,
                "compact direct-prefix scratch is sealed to split-five bit length"
            );
            if compact6_scratch {
                assert_direct_prefix_compact6_scratch(src, s, scratch);
            } else {
                assert_direct_prefix_compact5_scratch(src, s, scratch);
            }
        } else {
            assert!(
                !dirty_updates,
        ...
        } else if !compact5_scratch && !compact6_scratch {
    ...
    let full_increment_scratch = full_scratch.map(|scratch| {
        if scratch.len() == DIRECT_PREFIX_COMPACT5_SCRATCH_LEN
            || scratch.len() == DIRECT_PREFIX_COMPACT6_SCRATCH_LEN
        {
            return &scratch[DIRECT_PREFIX_KG_SCRATCH_LEN..DIRECT_PREFIX_KG_SCRATCH_LEN];
        }
    ...
    let borrowed_flag = full_scratch.map(|scratch| {
        ...
        assert!(DIRECT_PREFIX_KG_SCRATCH_LEN + kg_prefix_ancilla_count(s.len()) <= index);
        Some(scratch[index])
    });
    ...
    if let (Some(control), Some(constant_scratch)) =
        (borrowed_flag.flatten(), full_increment_scratch)
    {
        ...
        circ.x(control);
    } else if let Some(scratch) = full_scratch.filter(|scratch| {
        scratch.len() == DIRECT_PREFIX_COMPACT6_SCRATCH_LEN
    }) {
        let borrowed_carry = scratch[DIRECT_PREFIX_KG_SCRATCH_LEN];
        let n_bytes = n.to_le_bytes();
        if dec {
            sub_const_with_borrowed_carry(circ, s, &n_bytes, borrowed_carry);
        } else {
            add_const_with_borrowed_carry(circ, s, &n_bytes, borrowed_carry);
        }
    } else {
        let n_bytes = n.to_le_bytes();
        if dec {
    ...
    let anc_owned = if full_scratch.is_some() {
        ...
        circ.alloc_qreg_bits("bll.direct-prefix.kganc", kg_prefix_ancilla_count(n))
    };
    ...
    let sref: Vec<&QReg> = s.iter().collect();
    let dirty_candidates: Vec<&QReg> = qbits.iter().copied().chain(anc.iter().copied()).collect();
    let direct_double_control = dirty_updates
        && no_materialized_flag
        && dirty_candidates.len().saturating_sub(2) >= sref.len().saturating_sub(1);
    let borrowed_flag = borrowed_flag.flatten();
    ...
    let done = KgPrefixAnd::new(&qbits, &anc).forward(circ, |c, i, controls| {
        ...
        let update = |c: &mut Circuit, control: &QReg| {
            ...
            if dirty_updates && dirty_available >= sref.len().saturating_sub(2) {
                dirty_controlled_inc_suffix(c, &[control], &sref, 0, !dec, &dirty_candidates);
        ...
        match controls {
            ...
            [a, b] => {
                if direct_double_control {
                    dirty_controlled_inc_suffix(c, &[a, b], &sref, 0, !dec, &dirty_candidates);
                } else {
...
pub(crate) fn with_bit_length_callsite<R>(
    ...
    assert!(
        previous.is_none(),
        "nested bit-length call-site tracing is unsupported"
    );
    let result = body();
...
pub(crate) fn bit_length_lean_with_increment_scratch(
    ...
    bit_length_lean_impl(circ, src, s, dec, increment_scratch, None, false, 0);
}
...
pub(crate) fn bit_length_lean_with_full_prefix_scratch(
    ...
    bit_length_lean_impl(circ, src, s, dec, &[], Some(full_scratch), false, 0);
}
...
pub(crate) fn bit_length_lean_with_full_prefix_scratch_complemented_source(
    ...
    bit_length_lean_impl(circ, src, s, dec, &[], Some(full_scratch), true, 0);
}
...
/// Five-lane split-five variant used when the caller supplies only the KG
/// prefix ancillae and the four-bit target is updated with dirty double-control
/// callbacks instead of a materialized prefix flag.
pub(crate) fn bit_length_lean_with_compact5_prefix_scratch_split_five_high(
    circ: &mut Circuit,
    src: &[&QReg],
    s: &[QReg],
    dec: bool,
    ...
    assert!(
        lowq_direct_prefix_bitlen_requested(),
        "split-five-high compact5 scratch requires LOWQ_DIRECT_PREFIX_BITLEN=1"
    );
    assert_eq!(kg_scratch.len(), DIRECT_PREFIX_COMPACT5_SCRATCH_LEN);
    assert_eq!(s.len(), 4);
    let section = circ.push_section(bit_length_section());
    bit_length_lean_direct_prefix(
        circ,
...
/// Six-lane split-five variant: five KG prefix lanes plus one borrowed carry
/// for the fixed constant update. Prefix callbacks still avoid a materialized
/// flag via dirty double-control updates.
pub(crate) fn bit_length_lean_with_compact6_prefix_scratch_split_five_high(
    circ: &mut Circuit,
    src: &[&QReg],
    s: &[QReg],
    dec: bool,
    ...
    assert!(
        lowq_direct_prefix_bitlen_requested(),
        "split-five-high compact6 scratch requires LOWQ_DIRECT_PREFIX_BITLEN=1"
    );
    assert_eq!(full_scratch.len(), DIRECT_PREFIX_COMPACT6_SCRATCH_LEN);
    assert_eq!(s.len(), 4);
    let section = circ.push_section(bit_length_section());
    bit_length_lean_direct_prefix(
        circ,
...
fn lowq_hybrid_clz_prefix_parity_enabled() -> bool {
    std::env::var("LOWQ_HYBRID_CLZ_PREFIX_PARITY")
        .ok()
        .as_deref()
        == Some("1")
}
...
fn lowq_q959_selective_borrow_enabled() -> bool {
    ...
    assert_eq!(
        std::env::var("TRAILMIX_SIGN_PARITY_Q_REUSE")
            .ok()
            .as_deref(),
        Some("1"),
...
pub fn q954_srot_counter7_schedule_certificate() -> Q954ScheduleCertificate {
    use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::{
        SHRUNKEN_PZ_A, SHRUNKEN_PZ_A_LO, SHRUNKEN_PZ_B, SHRUNKEN_PZ_B_LO, SHRUNKEN_PZ_CA,
        SHRUNKEN_PZ_CA_LO, SHRUNKEN_PZ_CB, SHRUNKEN_PZ_CB_LO, SHRUNKEN_PZ_NSTEPS, SHRUNKEN_PZ_Q,
        SHRUNKEN_PZ_Q_LO, SHRUNKEN_PZ_S2, SHRUNKEN_PZ_SDIV,
    };

    static CERTIFICATE: std::sync::OnceLock<Q954ScheduleCertificate> = std::sync::OnceLock::new();
...
fn with_arithmetic_srot_view<'a, R>(
    ...
    if lowq_q954_srot_counter7_enabled() {
        ...
        let split = [&owned[0], &owned[1], &owned[2], &owned[3], &counter[7]];
        body(&split)
    } else {
...
pub(crate) fn shrunken_pz_product_undo(c: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg]) {
    if lowq_q955_off_canonical_enabled() {
        crate::point_add::trailmix_port::arith::rfold_mbu::mod_mul_canonical_mbu_undo(
            c, result, a, b,
        );
    } else {
        crate::point_add::trailmix_port::arith::rfold_mbu::mod_mul_rfold_mbu_undo(c, result, a, b);
    }
}

fn assert_q956_off_alias(off: &QReg, counter: &[QReg], s_rot: &[QReg]) {
    assert!(
        !counter.is_empty(),
        "Q956 off borrow requires a counter lane"
    );
    assert!(
    ...
    assert!(
        s_rot.len() >= 3,
        "Q956 boundary predicates require s_rot[0..3]"
    );
    assert!(
...
fn binary_clz_compute(circ: &mut Circuit, src: &[QReg], peer: &[QReg], transcript: &[&QReg]) {
...
fn binary_clz_uncompute(circ: &mut Circuit, src: &[QReg], peer: &[QReg], transcript: &[&QReg]) {
...
fn direct_bitlen_update(
    ...
    for k in (lo..src.len()).rev() {
        ...
        if lowq_q956_off_borrow_enabled() {
            ...
            dirty_controlled_inc_suffix(circ, &[selector_gate], target, 0, subtract, &candidates);
        }
        if k > lo {
            circ.x(&src[k]);
        }
...
pub const Q944_RESIDUAL_HCLZ_ROWS: [usize; 25] = [
    292, 293, 294, 301, 302, 303, 304, 319, 320, 321, 322, 323, 324, 325, 326, 334, 335, 336, 337,
    338, 343, 344, 349, 357, 385,
];
#[doc(hidden)]
...
#[doc(hidden)]
pub const Q945_DIRTY_PARITY_MICROKERNEL_COMMIT: &str = "8cd51b5df0ef18373d38fcee5ce77ddabf4e58cb";
#[doc(hidden)]
pub const Q945_DIRTY_PARITY_MICROKERNEL_TREE: &str = "c74ce2b47ff5a021396393f296ff8677f2e69244";
#[doc(hidden)]
pub const Q945_DIRTY_PARITY_MICROKERNEL_BLOB: &str = "ae3b46c5dc1b63546587066cad94846e8b1222c1";
    ...
    const fn dirty_parity_arithmetic(self) -> Option<Q945DirtyParityArithmetic<'a>> {
        match self {
            ...
            | Self::Row364DivisionLower80 {
                ...
                Some(lane) => Some(Q945DirtyParityArithmetic { lane, row, substep }),
                None => None,
            },
...
impl BorrowedTranscriptLoan<'_> {
    fn assert_disjoint(self, forbidden: &[&QReg]) {
        assert!(
            forbidden.iter().all(|lane| !std::ptr::eq(*lane, self.lane)),
...
fn with_hybrid_transcript<R>(
    ...
    assert!(
        owned_width >= 5,
        "hybrid CLZ requires five low transcript lanes"
    );
...
fn selective_direct_bitlen_needed(
    ...
    if !lowq_q948_direct_hclz_peak_guard_enabled() || !circ.lowq_q948_direct_hclz_peak_guard_active
    {
...
fn hybrid_bitlen_diff_update(
    ...
    assert_eq!(
        a.len(),
        b.len(),
        "LOWQ_HYBRID_CLZ requires equal full widths"
    );
...
fn hybrid_bitlen_diff_parity(
    ...
    let b_window = &b[lo_b.min(b.len() - 1)..];
    let logical_width = hybrid_transcript_width(a_window.len().max(b_window.len()));
    let forbidden: Vec<&QReg> = a.iter().chain(b.iter()).chain([out, active]).collect();
...
fn narrow_lt_controlled(
    ...
    match q945_carry {
        ...
        Some(Q945NarrowCarry::Row364DivisionLower80 {
            ...
            borrow_compare_gated_not_refs_with_carry(circ, &ar, &br, active, not_gate, out, carry);
...
fn q945_controlled_arithmetic(
    ...
    assert_eq!(
        a.len(),
        expected_width,
        "Q945 dirty-parity route width drift"
    );
...
pub(crate) fn multiply_substep_windowed(
    ...
    if selective_direct_bitlen_needed(
        ...
        direct_bitlen_diff_parity(circ, a, b, ca_window, ca_window, off, active, extra_lenders);
    } else if lowq_hybrid_clz_enabled() {
        hybrid_bitlen_diff_parity(
            circ,
            a,
...
fn q944_gate_hold_counter_zero_hosted(
    ...
    assert_eq!(
        q944_full_gate_route(row, substep),
        Q944FullGateRoute::Ordinary { host, peer }
    );
    assert_eq!(
        counter.len(),
        1,
        "Q944 hosted gate requires the affine done lane"
    );
    ...
    let toggle_gate = |c: &mut Circuit| {
        c.x(&counter[0]);
        strict_compare_gated_dirty_carry_refs(c, &xr, &yr, &counter[0], host_lane, parity);
        c.x(&counter[0]);
    };
    ...
    match q944_full_gate_route(row, substep) {
        Q944FullGateRoute::Ordinary { host, peer } => {
            ...
            q944_gate_hold_counter_zero_hosted(
                c, x, y, counter, parity, row, substep, inverse, host, peer, host_lane, peer_lane,
                body,
            );
        }
        Q944FullGateRoute::QuotientWitness => {
...
fn assert_borrowed_transcript_lender_certificate() {
    use super::q949_robust_envelope::{
        ...
        q949_robust_phase_requirements, q949_robust_row_requirements, Q949RobustPhaseRequirement,
...
fn borrowed_transcript_loan<'a>(
    ...
    let (lane, kind, preparation) = if row < Q949_FIRST_TERMINAL_ROW {
        ...
        assert_eq!(
            aa.len(),
            72,
            "borrowed transcript row-379 A allocation drift"
        );
        assert!(
        ...
        assert_eq!(
            cca.len(),
            256,
            "borrowed transcript row-380 ca allocation drift"
        );
        assert_eq!(
            ccb.len(),
            256,
            "borrowed transcript row-380 cb allocation drift"
        );
        if inverse {
...
fn q945_narrow_carry<'a>(
    ...
    if lowq_q944_residual_one_lane_cut_enabled() && Q944_RESIDUAL_NON_HCLZ_ROWS.contains(&row) {
...
pub(crate) fn shrunken_pz_pass_step(
    ...
    let direct_hclz_binding = if q947_passenger_direct_hclz_requested() {
        ...
        let q944_residual_binding =
            lowq_q944_residual_one_lane_cut_enabled() && Q944_RESIDUAL_HCLZ_ROWS.contains(&i);
    ...
    c.lowq_q948_direct_hclz_peak_guard_active =
        lowq_q948_direct_hclz_peak_guard_enabled() && direct_hclz_binding;
    if lowq_q954_srot_counter7_enabled() {
        assert_eq!(
            s_rot.len(),
            4,
            "Q954 boundary must see four owned shift lanes"
        );
    ...
    let reverse_relational_division =
        inverse && i == BORROWED_ROW_380 && lowq_reverse_ca255_relational_loan_enabled();
    ...
    let division_q945_carry =
        q945_narrow_carry(i, Q945Substep::Division, aa, bb, cca, ccb, qq, off, parity);
    let multiply_q945_carry =
        q945_narrow_carry(i, Q945Substep::Multiply, aa, bb, cca, ccb, qq, off, parity);
    ...
    c.lowq_trace_multiply_borrow =
        multiply_transcript_loans.update.is_some() || multiply_transcript_loans.parity.is_some();
    ...
    if lowq_q959_selective_borrow_enabled() {
        if inverse {
            ...
            let q944_full = lowq_q944_full_structural_enabled() && Q945_NON_HCLZ_ROWS.contains(&i);
...
fn restore_q955_canonical_lambda_top(c: &mut Circuit, lambda: &mut Vec<QReg>) {
    ...
    assert!(
        c.lowq_lambda_top_releases > 0,
        "lambda release state underflow"
    );
    c.lowq_lambda_top_releases -= 1;
}
...
struct ReleasedCanonicalPassengerTop {
    lane: Option<QReg>,
    physical_id: u32,
}
...
fn release_canonical_passenger_top(
    c: &mut Circuit,
...
fn restore_canonical_passenger_top(
    c: &mut Circuit,
    passenger: &mut Vec<QReg>,
    ...
    assert_eq!(
        passenger.len(),
        257,
        "canonical passenger API requires 257 lanes"
    );
    assert_eq!(
        passenger[256].id(),
        physical_id,
...
fn shrunken_pz_rebalance_pack(
    ...
    assert!(c.b.active_qubits as usize <= transition_peak_bound);
    assert_eq!([aa.len(), bb.len(), cca.len(), ccb.len(), qq.len()], widths);
...
pub(crate) fn shrunken_pz_invert_forward(
    ...
    for i in 0..SHRUNKEN_PZ_NSTEPS {
        ...
        shrunken_pz_rebalance_pack(c, aa, bb, cca, ccb, qq, widths, ["A", "B", "ca", "cb", "q"]);
...
pub fn q949_row_369_371_resize_gate_cleanup_check() -> Q949Row370CleanupReport {
    ...
    for row in FIRST_ROW..=LAST_ROW {
        ...
        let active_before = c.b.active_qubits;
        shrunken_pz_pass_step(
            &mut c, &aa, &bb, &cca, &ccb, &qq, &counter, &parity, &s_rot, &off, row, false,
        );
        c.flush_pending_frees();
        assert_eq!(
    ...
    for row in (FIRST_ROW..=LAST_ROW).rev() {
        let active_before = c.b.active_qubits;
        shrunken_pz_pass_step(
            &mut c, &aa, &bb, &cca, &ccb, &qq, &counter, &parity, &s_rot, &off, row, true,
        );
        c.flush_pending_frees();
        assert_eq!(
            c.b.active_qubits, active_before,
...
pub fn q949_robust_resize_ordering_check() -> Q949RobustResizeOrderingReport {
    ...
    let final_active_qubits = c.b.active_qubits as usize;
    assert_eq!(
        final_active_qubits, 0,
        "Q949 robust resize proof leaked ancillae"
    );
    let builder = c.into_builder();
...
pub fn shrunken_pz_divide_forward(
    ...
    assert_eq!(
        lambda.len(),
        257,
        "slope API and raw dy roundtrip require 257 lanes"
    );
...
pub fn exhaustive_gated_compare_check() -> GatedCompareExhaustiveReport {
            ...
            match op.kind {
                ...
                OperationType::R => {
                    assert!(
                        ...
                        "freed comparator carry was not zero"
                    );
                }
    ...
    for width in 1..=5usize {
        ...
        for input in 0..(1u64 << external) {
            ...
            assert_eq!(
                word(got, 2 + width, width),
                u_pre,
                "width={width}: u changed"
            );
        }
        ...
        let builder = c.into_builder();
        let extra = builder.peak_qubits as usize - external;
        max_borrowed_comparator_extra_qubits = max_borrowed_comparator_extra_qubits.max(extra);
        ...
        gate_hold(&mut c, &x, &y, &active, &g, Some(&carry), |c, gate| {
            c.cx(gate, &body)
        });
        ...
        for lo_a in 0..width {
            for lo_b in 0..width {
                ...
                for input in 0..(1u64 << update_external) {
                    ...
                    let got = apply(&update_builder.ops, input, &mut reset_operations_checked);
                    assert_eq!(
                        got, input,
                ...
                for input in 0..(1u64 << parity_external) {
                    ...
                    let got = apply(&parity_builder.ops, input, &mut reset_operations_checked);
                    assert_eq!(
                        got, input,
            ...
            for lo_b in 0..width {
                for subtract_diff in [false, true] {
                    ...
                    for input in 0..(1u64 << external) {
                        ...
                        let target_post = target_pre.wrapping_add(active_pre * delta) & target_mask;
    ...
    for width in 1..=3usize {
        for lo_a in 0..width {
            for lo_b in 0..width {
                ...
                let extra_refs: Vec<&QReg> = extra.iter().collect();
                direct_bitlen_diff_parity(&mut c, &a, &b, lo_a, lo_b, &out, &active, &extra_refs);
                let external = 4 + 2 * width;
                let builder = c.into_builder();
                let added = builder.peak_qubits as usize - external;
                max_bitlen_parity_extra_qubits = max_bitlen_parity_extra_qubits.max(added);
                assert_eq!(
                    added, 0,
    ...
    for width in 1..=3usize {
        for inverse in [false, true] {
            ...
            for input in 0..(1u64 << external) {
                ...
                assert_eq!(
                    got, expected,
                    "done width={width} inverse={inverse} input={input}"
                );
            }
        }
    }
        ...
        for op in ops {
            match op.kind {
                ...
                OperationType::R => {
                    assert!(
                        !bit(state, op.q_target.0),
                        "Q956 proof freed a nonzero lane"
                    );
                }
    ...
    for width in 2..=4usize {
        ...
        for input in 0..(1u64 << external) {
            ...
            if gate == 0 || value_pre >> (width - 1) == 0 {
                ...
                let expected = (input & !((s_mask << s_start) | (value_mask << value_start)))
                    | (s_post << s_start)
...
fn q944_catalytic_blocked_stream(row: usize, inverse: bool) -> Q944CatalyticStreamReport {
    use crate::point_add::trailmix_port::inversion::q944_dirty_catalytic_predicate::{
        q944_catalytic_cost, q944_classify_template_op, q944_select_catalytic_dirty_pair,
        Q944CatalyticGateCounts, Q944CatalyticKind,
    };
    use crate::point_add::trailmix_port::inversion::q949_robust_envelope::q949_robust_pair_symmetric_widths;
    use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::shift_bounds;
    ...
    let reverse_relational =
        inverse && row == BORROWED_ROW_380 && lowq_reverse_ca255_relational_loan_enabled();
    ...
    assert_eq!(
        final_active_qubits, input_qubits,
        "Q944 census body leaked qubits"
    );
...
pub fn q944_catalytic_blocked_operation_census() -> Q944CatalyticBlockedCensusReport {
    ...
    let first_failure = streams
        .iter()
        .find_map(|stream| stream.first_failure.clone());
...
fn q944_quotient_witness_candidate_stream(
    ...
    for index in body_start..body_end {
        ...
        body_q24_controls += usize::from(op.q_control1 == active_id || op.q_control2 == active_id);
        ...
        body_predicate_support_targets += usize::from(predicate_support.contains(&op.q_target));
    ...
    let demux_indices: Vec<usize> = phases
        .iter()
        .enumerate()
        .filter_map(|(index, phase)| phase.contains("/q944.qw.partial-demux").then_some(index))
        .collect();
    ...
    for index in crossed_start..crossed_end {
        ...
        if first_failure.is_none() && matches!(op.kind, OperationType::Hmr | OperationType::R) {
            first_failure = Some(Q944QuotientDependencyFailure {
                row,
                inverse,
                operation_index: index,
...
pub fn q944_quotient_witness_dependency_census() -> Q944QuotientDependencyCensusReport {
    ...
    Q944QuotientDependencyCensusReport {
        ...
        baseline_operations: streams
            .iter()
            .map(|stream| stream.baseline_operations)
            .sum(),
        candidate_operations: streams
            .iter()
            .map(|stream| stream.candidate_operations)
            .sum(),
...
pub fn q954_srot_counter7_roundtrip_check() -> Q954SrotCounter7Report {
    ...
    fn simulate(harness: &Harness, values: &[u64]) -> Snapshot {
        ...
        let registers = harness
            ...
            .collect();
        let internal_clean = (0..harness.builder.next_qubit)
            .all(|id| harness.external.contains(&id) || sim.qubit(QubitId(u64::from(id))) & 1 == 0);
        Snapshot {
            registers,
            phase: sim.phase & 1,
            internal_clean,
    ...
    fn build_body(split: bool, multiply: bool, mode: u8) -> Harness {
        ...
        let s_rot: Vec<&QReg> = if split {
            vec![&owned[0], &owned[1], &owned[2], &owned[3], &counter[7]]
        } else {
            owned.iter().collect()
        };
        let off = &counter[0];
    ...
    for mode in 0..=2u8 {
        ...
        assert_eq!(
            canonical_out.registers[1] & 0x7f,
            split_out.registers[1] & 0x7f
        );
        assert!(canonical_out.internal_clean && split_out.internal_clean);
        let canonical_t = toffoli(&canonical.builder);
        let split_t = toffoli(&split.builder);
        ...
        assert!(
            split_t <= canonical_t,
            "Q954 split carry increased Toffoli count"
        );
    ...
    for (case, harness) in [&late_forward, &late_roundtrip].into_iter().enumerate() {
        ...
        assert_eq!(
            out.registers[0], 16,
            "late counter[7] alias was not restored"
        );
...
pub fn borrowed_transcript_roundtrip_check() -> BorrowedTranscriptHelperReport {
    ...
    for (owned, borrowed, expected_reset_savings) in [
        ...
        (
            &owned_roundtrip.builder,
            &borrowed_roundtrip.builder,
            2usize,
        ),
    ] {
        assert_eq!(
...
pub fn reverse_ca255_relational_loan_roundtrip_check() -> ReverseCa255RelationalLoanReport {
    ...
    fn build(relational: bool, roundtrip: bool) -> Harness {
        ...
        hybrid_bitlen_diff_parity(&mut c, &a, &b, 0, 0, &out, &relation_control, loan);
        if roundtrip {
            hybrid_bitlen_diff_parity(&mut c, &a, &b, 0, 0, &out, &relation_control, loan);
        }
        let a_ids = ids(&a);
    ...
    for (owned, relational, repetitions) in [
        ...
        (
            &owned_roundtrip.builder,
            &relational_roundtrip.builder,
            2usize,
        ),
    ] {
        assert_eq!(owned.peak_qubits, relational.peak_qubits + 1);
        assert_eq!(
            operation_delta(OperationType::X, owned, relational),
            (2 * repetitions) as isize
        );
        assert_eq!(
            operation_delta(OperationType::CX, owned, relational),
            (2 * repetitions) as isize
        );
        assert_eq!(
            operation_delta(OperationType::R, owned, relational),
        ...
        for kind in 0..owned.counted_kind_ops.len() {
            ...
            assert_eq!(
                owned.counted_kind_ops[kind],
                relational.counted_kind_ops[kind]
            );
        }
    }
...
pub fn passenger_top_lifetime_roundtrip_check() -> PassengerTopLifetimeHelperReport {
    ...
    fn simulate(harness: &Harness, values: &[u64]) -> Snapshot {
        ...
        let registers = harness
            ...
            .collect();
        let internal_clean = (0..harness.builder.next_qubit)
            .all(|id| harness.external.contains(&id) || sim.qubit(QubitId(u64::from(id))) == 0);
        Snapshot {
            registers,
            phase: sim.phase,
            internal_clean,
...
pub fn q949_affine_counter_roundtrip_check() -> Q949AffineCounterReport {
    ...
    fn simulate(harness: &Harness, values: &[u64]) -> Snapshot {
        ...
        let internal_clean = (0..harness.builder.next_qubit).all(|id| {
            harness.external.contains(&id) || simulator.qubit(QubitId(u64::from(id))) & 1 == 0
        });
        Snapshot {
            registers,
    ...
    for terminal in [false, true] {
        for count in 0..=158u64 {
            ...
            for dirty in 0..=255u64 {
                let dirty_word =
                    dirty | ((dirty ^ 0xff) << 8) | ((dirty.rotate_left(3) & 0xff) << 16);
    ...
    for count in 1..=159u64 {
        let encoded = (SECP256K1_P_LOW_BYTE as u64) ^ count;
        for dirty in 0..=255u64 {
            let dirty_word = dirty | ((dirty ^ 0xff) << 8) | ((dirty.rotate_left(3) & 0xff) << 16);
...
pub fn q949_canonical_horner_roundtrip_check() -> Q949CanonicalHornerReport {
    ...
    for (shot, &(left, right)) in cases.iter().enumerate() {
        ...
        let passenger_value = passenger_ids
            .iter()
            .enumerate()
            .fold(0u64, |value, (bit, &id)| {
                value | (((simulator.qubit(QubitId(u64::from(id))) >> shot) & 1) << bit)
            });
    ...
    assert_eq!(
        simulator.phase, 0,
        "Q949 Horner regression left phase garbage"
    );
    for id in &external {
...
pub fn q955_off_canonical_lifetime_roundtrip_check() -> CanonicalLambdaLifetimeReport {
    ...
    fn load_u256<R: XofReader>(sim: &mut Simulator<'_, R>, reg: &[u32], value: U256, shot: usize) {
    ...
    assert!(
        !c.b.count_only,
        "Q955 lifetime proof requires emitted operations"
    );
    ...
    assert_eq!(
        c.b.active_qubits as usize, live_before_release,
    ...
    let external = a_ids.len() + b_ids.len() + lambda_ids.len() + roundtrip_temp_ids.len() + 2;
    assert_eq!(
        c.b.active_qubits as usize, external,
    ...
    for (shot, &(a_value, b_value, negate_value)) in cases.iter().enumerate() {
        ...
        assert_eq!(
            read_u256(&sim, &a_ids, shot),
            a_value,
            "a changed at shot {shot}"
        );
        assert_eq!(
            read_u256(&sim, &b_ids, shot),
            b_value,
            "b changed at shot {shot}"
        );
        assert_eq!(
    ...
    let controlled_zero_cases_checked = cases
        .iter()
        .filter(|(a, b, negate)| *negate && (*a).mul_mod(*b, SECP256K1_P) == U256::ZERO)
        .count();
    assert!(
