...
/// When set, the KG-only prefix path accepts a single borrowed clean-|0> flag
/// lane (appended to the loan as a 9th lane). The two-control prefix callbacks
/// then use the cheap `ccx; clean-CINC; clear_and` path instead of the
/// O(n)-firing dirty `dirty_controlled_inc_suffix`, recovering the op count
/// while keeping the qubit peak (the borrowed lane is already on the plateau).
pub(crate) fn lowq_kg_only_prefix_borrowed_flag_requested() -> bool {
    std::env::var("LOWQ_KG_ONLY_PREFIX_BORROWED_FLAG")
        .ok()
        .as_deref()
        == Some("1")
}
...
/// Flag-eliminated prefix loan: KG prefix ancillae + increment scratch, but no
/// materialized flag lane (the two-control prefix callbacks use a dirty
/// double-control instead). One lane fewer than the full nine-lane budget.
pub(crate) const DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN: usize =
    DIRECT_PREFIX_KG_SCRATCH_LEN + DIRECT_PREFIX_INCREMENT_SCRATCH_LEN;
/// KG-only loan plus one *borrowed* clean-|0> flag lane (the 9th lane). The flag
/// is supplied by an already-on-plateau idle lane (the swap promised-support
/// lane), so the qubit peak is unchanged, but the two-control prefix callbacks
/// regain the cheap `ccx; clean-CINC; clear_and` path (no O(n) dirty ladder).
pub(crate) const DIRECT_PREFIX_KG_ONLY_WITH_FLAG_LEN: usize =
    DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN + 1;
...
/// Disjointness validation for the KG-only prefix loan: the five loaned lanes
/// must be distinct and must not alias the source or target. (Unlike the
/// nine-lane full scratch, there is no flag/increment partition to check.)
fn assert_kg_only_prefix_scratch(
    src: &[&QReg],
    target: &[QReg],
    scratch: &[&QReg],
) {
    assert_eq!(
        ...
        "KG-only prefix scratch requires exactly {DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN} lanes"
    );
    for (index, lane) in scratch.iter().enumerate() {
        assert!(
            scratch[..index]
                .iter()
                .all(|other| other.id() != lane.id()),
            "KG-only prefix scratch lane {index} aliases an earlier scratch lane"
        );
        assert!(
            src.iter().all(|source| source.id() != lane.id()),
            "KG-only prefix scratch lane {index} aliases the source"
        );
        assert!(
            target.iter().all(|target| target.id() != lane.id()),
            "KG-only prefix scratch lane {index} aliases the target"
        );
    }
}
...
fn assert_fused_zero_prefix_preconditions(
    ...
    assert!(
        target_capacity.is_none_or(|capacity| src.len() < capacity),
        "fused zero-prefix target width {} cannot represent source width {}",
        target.len(),
        src.len()
    );
    // The materialized flag may be dropped either with the dirty increment
    // (LOWQ_DIRECT_PREFIX_DIRTY_UPDATE) or in KG-only mode (two-control prefix
    // callbacks handled by a dirty double-control while the increment stays clean).
    assert!(
        !no_materialized_flag || dirty_updates || kg_only_full_scratch,
        "LOWQ_DIRECT_PREFIX_NO_FLAG requires LOWQ_DIRECT_PREFIX_DIRTY_UPDATE=1 or KG-only mode"
    );
...
fn bit_length_lean_direct_prefix(
    ...
    bit_length_lean_direct_prefix_modes(
        circ,
        src,
        s,
        dec,
...
/// Internal entry that additionally exposes the `kg_only_full_scratch` mode.
///
/// In `kg_only_full_scratch` mode the caller-supplied `full_scratch` holds only
/// the [`DIRECT_PREFIX_KG_SCRATCH_LEN`] Khattar-Gidney prefix ancillae (NOT the
/// full nine-lane budget). The per-position unit update is then computed with a
/// `mcx_dirty_ladder`-class `dirty_controlled_inc_suffix` over the (restored)
/// source bits + KG ancillae, and the two-control prefix callback is handled by
/// a direct double-control dirty increment, so neither the three increment
/// scratch lanes nor the materialized flag lane is required. This is the lever
/// that lets the fused-prefix loan shrink from nine clean lanes to five: the
/// `dirty_updates`/`no_materialized_flag` switches are forced on internally.
#[allow(clippy::too_many_arguments, clippy::fn_params_excessive_bools)]
fn bit_length_lean_direct_prefix_modes(
    circ: &mut Circuit,
    src: &[&QReg],
    s: &[QReg],
    dec: bool,
    ...
    // The KG-only full-scratch mode eliminates ONLY the materialized prefix
    // flag (the 9th loaned lane), keeping the efficient clean log-depth
    // increment on the three loaned increment-scratch lanes. The rare two-control
    // prefix callbacks (O(log* n) of them, at layer boundaries) are handled by a
    // dirty double-control `mcx_dirty_ladder` over the source bits + KG ancillae,
    // so no flag lane is required. The hot one-control per-position increment
    // stays clean — there is NO O(n)-scale dirty inflation.
    let no_materialized_flag = no_materialized_flag || kg_only_full_scratch;
    ...
    if fuse_zero_prefix {
        // In KG-only mode the full_scratch is the five KG lanes, not the nine
        // lane budget; the generic precondition (which validates a nine-lane
        // full_scratch) is replaced by the KG-only validation below.
        assert_fused_zero_prefix_preconditions(
            ...
            no_materialized_flag,
            increment_scratch,
            if kg_only_full_scratch { None } else { full_scratch },
            kg_only_full_scratch,
        );
    }
    debug_assert!(
        (n as u128) < (1u128 << s.len()),
        "bit_length_lean_direct_prefix: target width {} too small for n={n}",
        s.len()
    );

    if let Some(scratch) = full_scratch {
        if kg_only_full_scratch {
            // Flag-eliminated loan: 5 KG prefix ancillae + 3 increment-scratch
            // lanes (clean log-depth increment), no materialized flag. The rare
            // two-control prefix callbacks use a dirty double-control instead.
            assert!(
                no_materialized_flag,
                "KG-only full scratch eliminates the materialized flag"
            );
            assert!(
                increment_scratch.is_empty(),
                "KG-only full scratch cannot be combined with increment-only scratch"
            );
            // The loan is either the 8-lane flag-eliminated form (dirty
            // two-control callbacks) or the 9-lane form with a borrowed clean-|0>
            // flag appended (cheap two-control callbacks, peak-neutral).
            assert!(
                scratch.len() == DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN
                    || scratch.len() == DIRECT_PREFIX_KG_ONLY_WITH_FLAG_LEN,
                "KG-only full scratch requires {DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN} or {DIRECT_PREFIX_KG_ONLY_WITH_FLAG_LEN} lanes, got {}",
                scratch.len()
            );
            assert_kg_only_prefix_scratch(src, s, &scratch[..DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN]);
            if scratch.len() == DIRECT_PREFIX_KG_ONLY_WITH_FLAG_LEN {
                let flag = scratch[DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN];
                assert!(
                    src.iter().all(|lane| lane.id() != flag.id()),
                    "KG-only borrowed flag aliases the source"
                );
                assert!(
                    s.iter().all(|lane| lane.id() != flag.id()),
                    "KG-only borrowed flag aliases the target"
                );
                assert!(
                    scratch[..DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN]
                        .iter()
                        .all(|lane| lane.id() != flag.id()),
                    "KG-only borrowed flag aliases a loan lane"
                );
            }
            assert!(
            ...
            assert!(
                !dirty_updates,
                "direct-prefix full scratch forbids dirty updates"
            );
            assert!(
                !no_materialized_flag,
                "direct-prefix full scratch requires its materialized flag"
            );
            assert!(
                increment_scratch.is_empty(),
    ...
    // Both modes carve the three increment-scratch lanes out of the loan at the
    // same offset; KG-only simply omits the trailing flag lane.
    let full_increment_scratch = full_scratch.map(|scratch| {
        &scratch[DIRECT_PREFIX_KG_SCRATCH_LEN
            ..DIRECT_PREFIX_KG_SCRATCH_LEN + DIRECT_PREFIX_INCREMENT_SCRATCH_LEN]
    });
    let borrowed_flag = if kg_only_full_scratch {
        // 9-lane KG-only loan: the appended lane is a borrowed clean-|0> flag,
        // restoring the cheap two-control prefix callback path.
        full_scratch.and_then(|scratch| {
            (scratch.len() == DIRECT_PREFIX_KG_ONLY_WITH_FLAG_LEN)
                .then(|| scratch[DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN])
        })
    } else {
        full_scratch.map(|scratch| scratch[DIRECT_PREFIX_FULL_SCRATCH_LEN - 1])
    };
    ...
    if let (Some(control), Some(constant_scratch)) = (borrowed_flag, full_increment_scratch) {
        ...
        circ.x(control);
    } else if kg_only_full_scratch {
        // KG-only mode: the constant `n` add/sub must NOT allocate a fresh
        // carry lane (which `add_const`/`sub_const` would for a multi-bit
        // constant). The loaned KG ancillae are clean-|0> and idle until the
        // prefix forward sweep; host the always-on control on KG anc[0] and use
        // the three loaned increment-scratch lanes as the CINC scratch —
        // allocation-free and restored before the prefix tree runs.
        let scratch = full_scratch.expect("KG-only mode requires loaned scratch");
        let control = scratch[0];
        let constant_scratch =
            full_increment_scratch.expect("KG-only mode has increment scratch");
        let s_refs: Vec<&QReg> = s.iter().collect();
        circ.x(control);
        for bit in 0..s.len() {
            if ((n >> bit) & 1) == 0 {
    ...
    let sref: Vec<&QReg> = s.iter().collect();
    // The dirty double-control MCX (`dirty_controlled_inc_suffix`) needs at most
    // `controls.len()-2` lenders, and `controls` is bounded by `2 + sref.len()`.
    // Collecting/scanning the full n-wide source as candidates is O(n) PER prefix
    // callback, which at the production width (n ~ 259) compounds across every
    // callback x swap x EEA iteration into a >50x op blow-up. Cap the candidate
    // pool to a small window that still over-covers the worst-case lender demand
    // (2 controls already excluded, plus slack). Restored dirty lenders are
    // interchangeable, so this is value-preserving; the cap engages at n>=8 and
    // is exercised by the 16320-case direct-prefix roundtrip proof.
    let dirty_candidate_budget = sref.len() + 4;
    let dirty_candidates: Vec<&QReg> = if kg_only_full_scratch || dirty_updates {
        // Put the KG ancillae FIRST (they are guaranteed disjoint from the per
        // callback prefix controls drawn from `qbits`), then top up from the low
        // source bits, capped to the budget.
        anc.iter()
            .copied()
            .chain(qbits.iter().copied())
            .take(dirty_candidate_budget.max(anc.len() + 2))
            .collect()
    } else {
        qbits
            .iter()
    ...
    // KG-only mode handles the two-control prefix callback with a dirty
    // double-control (no flag) while keeping the one-control increment clean.
    // When a borrowed flag lane is supplied (9-lane loan), the cheap
    // `ccx; clean-CINC; clear_and` path is used instead, so the dirty
    // double-control is disabled.
    let direct_double_control = (dirty_updates || kg_only_full_scratch)
        && no_materialized_flag
        && borrowed_flag.is_none()
...
fn direct_prefix_bit_length_roundtrip_check_mode(
    ...
    direct_prefix_bit_length_roundtrip_check_mode_inner(dirty_updates, no_materialized_flag, false)
}

fn direct_prefix_bit_length_roundtrip_check_mode_inner(
    dirty_updates: bool,
    no_materialized_flag: bool,
    kg_only: bool,
) -> DirectPrefixBitLengthProofReport {
    ...
    for dec in [false, true] {
        ...
        // KG-only mode: loan exactly the five Khattar-Gidney prefix ancillae
        // (clean-|0>). The proof's clean-internal-ancilla assertion below then
        // confirms the loan is restored and nothing else is allocated.
        let kg_scratch = if kg_only {
            Some(circuit.alloc_qreg_bits(
                "direct-bitlen-proof.kg-scratch",
                DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN,
            ))
        } else {
            None
        ...
        let kg_scratch_refs: Vec<&QReg> =
            kg_scratch.as_ref().map(|s| s.iter().collect()).unwrap_or_default();
        bit_length_lean_direct_prefix_modes(
            &mut circuit,
            &source_refs,
            &target,
            ...
            no_materialized_flag,
            &[],
            if kg_only { Some(&kg_scratch_refs) } else { None },
            false,
            kg_only,
        );
...
/// Exhaustive value proof for the KG-only prefix-scratch path: the prefix-AND
/// is driven with only the five loaned Khattar-Gidney ancillae (dirty increment,
/// no materialized flag). Asserts source preserved, target = target ± bitlen, and
/// every internal/loaned lane restored clean — for all nonzero eight-bit sources,
/// all five-bit targets, both directions. This is the standalone positive proof
/// that the lane-reduced length kernel computes the EXACT bit length and restores
/// its loaned lanes.
#[doc(hidden)]
pub fn direct_prefix_kg_only_bit_length_roundtrip_check() -> DirectPrefixBitLengthProofReport {
    let saved_direct = std::env::var_os("LOWQ_DIRECT_PREFIX_BITLEN");
    ...
    let report = direct_prefix_bit_length_roundtrip_check_mode_inner(true, true, true);
    match saved_direct {
...
fn bit_length_lean_impl_modes(
    ...
    assert!(
        !kg_only_full_scratch,
        "KG-only prefix scratch requires LOWQ_DIRECT_PREFIX_BITLEN=1"
    );
    assert!(
        !source_is_complemented,
...
pub(crate) fn bit_length_lean(circ: &mut Circuit, src: &[&QReg], s: &[QReg], dec: bool) {
    bit_length_lean_impl(circ, src, s, dec, &[], None, false);
}
...
pub(crate) fn bit_length_lean_complemented_source(
    ...
    bit_length_lean_impl(circ, src, s, dec, &[], None, true);
}
...
pub(crate) fn bit_length_lean_with_increment_scratch(
    ...
    bit_length_lean_impl(circ, src, s, dec, increment_scratch, None, false);
}
...
pub(crate) fn bit_length_lean_with_full_prefix_scratch(
    ...
    bit_length_lean_impl(circ, src, s, dec, &[], Some(full_scratch), false);
}
...
pub(crate) fn bit_length_lean_with_full_prefix_scratch_complemented_source(
    ...
    bit_length_lean_impl(circ, src, s, dec, &[], Some(full_scratch), true);
}

/// KG-only prefix-scratch variant: the loan provides only the five
/// Khattar-Gidney prefix ancillae; the unit update is computed with a dirty
/// `mcx_dirty_ladder`-class increment (no clean increment scratch, no flag).
/// This is the [`DIRECT_PREFIX_FULL_SCRATCH_LEN`] 9 -> [`DIRECT_PREFIX_KG_SCRATCH_LEN`] 5
/// lane reduction that lets the P2 fused-prefix loan drop its owned lanes.
pub(crate) fn bit_length_lean_with_kg_only_prefix_scratch(
    circ: &mut Circuit,
    src: &[&QReg],
    s: &[QReg],
    dec: bool,
    ...
    bit_length_lean_impl_modes(circ, src, s, dec, &[], Some(kg_scratch), false, true);
}

/// KG-only variant of [`bit_length_lean_with_full_prefix_scratch_complemented_source`].
pub(crate) fn bit_length_lean_with_kg_only_prefix_scratch_complemented_source(
    circ: &mut Circuit,
    src: &[&QReg],
    s: &[QReg],
    dec: bool,
    ...
    bit_length_lean_impl_modes(circ, src, s, dec, &[], Some(kg_scratch), true, true);
}
