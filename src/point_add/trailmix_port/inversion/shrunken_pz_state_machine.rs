...
pub fn shrunken_pz_divide_forward(
    ...
    assert!(
        dy.len() == 256 || dy.len() == 257,
        "dy passenger may omit the known-zero overflow bit"
    );
    // sgn = dx > p/2  <=>  dx >= (p+1)/2.
    let half_bytes = vec![
    ...
    c.zero_and_free(f);
    c.zero_and_free(lambda.pop().expect("lambda padded to 257"));
    ...
    compare_geq_const(c, &dx, &half_bytes, &sgn);
    c.zero_and_free(sgn);
    lambda.push(c.alloc_qreg("shpzdiv.lambda_ov"));
