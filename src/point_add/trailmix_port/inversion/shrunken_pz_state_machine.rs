...
pub fn division_substep_windowed(
    ...
    // mask s_rot = diff AND active. Under TRAILMIX_FUSE_DIV_CLZ_A, deposit pos_a
    // ONCE (A is unchanged between the two clz calls -- only B rotates) and reuse
    // it for both, removing one redundant A bit-length scan per step.
    let fuse_a = clz_fuse_div_a_enabled();
    let pa_hoist = if fuse_a { Some(clz_deposit_a(circ, a, w, lo_a)) } else { None };

    if let Some(pa) = &pa_hoist {
        clz_diff_use(circ, pa, b, w, lo_a, lo_b, borrowed_carry, |circ, diff| {
            for j in 0..w {
                circ.ccx(active, &diff[j], &s_rot[j]);
            }
        });
    } else {
        clz_diff_body_middle(circ, a, b, w, lo_a, lo_b, borrowed_carry, |circ, diff| {
            for j in 0..w {
                circ.ccx(active, &diff[j], &s_rot[j]);
            }
        });
    ...
    if let Some(pa) = &pa_hoist {
        clz_diff_use(circ, pa, b, w, lo_a, lo_a, borrowed_carry, |circ, diff| {
            circ.ccx(active, &diff[0], offset);
        });
    } else {
        clz_diff_body_middle(circ, a, b, w, lo_a, lo_a, borrowed_carry, |circ, diff| {
            circ.ccx(active, &diff[0], offset);
        });
    }
    if let Some(pa) = pa_hoist {
    ...
    // accumulator (freed before the next step; off the clz peak).
    {
        ...
        bit_length_lean(circ, &rev, &t, true, borrowed_carry);
        let srr: Vec<&QReg> = s_rot.iter().collect();
        let tr: Vec<&QReg> = t.iter().collect();
        ctrl_sub(circ, active, &srr, &tr);
        bit_length_lean(circ, &rev, &t, false, borrowed_carry);
        xor_const(circ, &t, n_pad);
        for q in t {
            circ.zero_and_free(q);
        }
...
pub fn division_substep_windowed_inv(
    ...
    // 8' offset clean (self-inverse, _middle); diff in the clz's pa. Mirror of the
    // forward hoist: deposit pos_a ONCE here, reuse across both inverse clz calls.
    let fuse_a = clz_fuse_div_a_enabled();
    let pa_hoist = if fuse_a { Some(clz_deposit_a(circ, a, w, lo_a)) } else { None };
    if let Some(pa) = &pa_hoist {
        clz_diff_use(circ, pa, b, w, lo_a, lo_a, borrowed_carry, |circ, diff| {
