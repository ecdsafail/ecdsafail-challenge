...
fn bit_length_lean_middle(
    ...
    let done = kg.forward(circ, |c, i, ctrls| bit_length_deposit_step(c, i, ctrls, pos, &flag, n)); // pos -> p
    let clean = body(circ);
    if clean {
        // 4n: re-run the deposit on the reverse, returning pos to |n>.
        done.reverse(circ, |c, i, ctrls| bit_length_deposit_step(c, i, ctrls, pos, &flag, n));
    } else {
...
fn held_prefix_clz_enabled() -> bool {
    std::env::var("TRAILMIX_HELD_PREFIX_CLZ")
        .ok()
        .as_deref()
        == Some("1")
}

fn clz_diff_body_middle_held(
    circ: &mut Circuit,
    a: &[QReg],
    b: &[QReg],
    w: usize,
    ...
    let aqbits: Vec<&QReg> = aw.iter().rev().copied().collect();
    let aanc_owned = circ.alloc_qreg_bits("clzm.ka", kg_prefix_ancilla_count(na));
    let aanc: Vec<&QReg> = aanc_owned.iter().collect();
    let aflag = circ.alloc_qreg("clzm.fa");
    let adone = KgPrefixAnd::new(&aqbits, &aanc)
        .forward(circ, |c, i, ctrls| bit_length_deposit_step(c, i, ctrls, &pa, &aflag, na));
    for q in &aw {
        circ.x(q);
    }
    ...
    let bqbits: Vec<&QReg> = bw.iter().rev().copied().collect();
    let banc_owned = circ.alloc_qreg_bits("clzm.kb", kg_prefix_ancilla_count(nb));
    let banc: Vec<&QReg> = banc_owned.iter().collect();
    let bflag = circ.alloc_qreg("clzm.fb");
    let bdone = KgPrefixAnd::new(&bqbits, &banc)
        .forward(circ, |c, i, ctrls| bit_length_deposit_step(c, i, ctrls, &pb, &bflag, nb));
    for q in &bw {
        circ.x(q);
    }
    ...
    bdone.reverse(circ, |c, i, ctrls| bit_length_deposit_step(c, i, ctrls, &pb, &bflag, nb));
    circ.zero_and_free(bflag);
    drop(banc);
    for q in banc_owned {
        circ.zero_and_free(q);
    }
    ...
    adone.reverse(circ, |c, i, ctrls| bit_length_deposit_step(c, i, ctrls, &pa, &aflag, na));
    circ.zero_and_free(aflag);
    drop(aanc);
    for q in aanc_owned {
        circ.zero_and_free(q);
    }
...
fn clz_diff_body_middle(
    ...
    if held_prefix_clz_enabled() {
        clz_diff_body_middle_held(circ, a, b, w, lo_a, lo_b, body);
        return;
    }
    use crate::point_add::trailmix_port::arith::ripple_add::add_const;
    let pbl = circ.push_section("p.bitlen");
...
pub(crate) fn shrunken_pz_invert_forward(
    ...
    for i in 0..SHRUNKEN_PZ_NSTEPS {
        ...
        shrunken_pz_resize(c, qq, trailmix_q_width(wq), "q");
...
pub fn shrunken_pz_divide_cancel(
    ...
    use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::{
        enter_cancel_profile, reg_widths,
    };
    use crate::point_add::trailmix_port::num_bigint::BigUint;
    let _cancel_profile = enter_cancel_profile();
    assert_eq!(dx.len(), 257);
    assert_eq!(dy.len(), 257);
