...
fn assert_fused_zero_prefix_preconditions(
    ...
    let allow_wrapped_target =
        std::env::var("LOWQ_Q842_BORROW_ROTATED_BITLEN_HIGH")
            .ok()
            .as_deref()
            == Some("1");
    assert!(
        allow_wrapped_target || target_capacity.is_none_or(|capacity| src.len() < capacity),
...
fn bit_length_lean_direct_prefix(
    ...
    let borrowed_flag = full_scratch.map(|scratch| scratch[DIRECT_PREFIX_FULL_SCRATCH_LEN - 1]);
