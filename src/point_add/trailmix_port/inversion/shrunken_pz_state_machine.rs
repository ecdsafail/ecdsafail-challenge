...
fn lowq_flagless_bitlen_enabled() -> bool {
    std::env::var("TRAILMIX_LOWQ_FLAGLESS_BITLEN")
        .ok()
        .as_deref()
        != Some("0")
}

fn lowq_direct_uncond_add_enabled() -> bool {
    std::env::var("TRAILMIX_LOWQ_DIRECT_UNCOND_ADD")
        .ok()
        .as_deref()
        == Some("1")
}
...
fn bit_length_lean_middle(
    ...
    let flag = (!lowq_flagless_bitlen_enabled()).then(|| circ.alloc_qreg("bll.flag"));
    // Deposit at layer i (position k = n-1-i): gray-XOR (k ^ (k+1)) into pos gated
    // on f_k = AND(ctrls). In the low-Q route, avoid the reusable materialized
    // flag and write two-control gray bits directly into pos. This spends extra
    // Toffoli when several gray bits flip, but saves one live qubit at the 983q
    // bitlength peak.
    fn deposit_step(
        circ: &mut Circuit,
        i: usize,
...
fn add_refs(circ: &mut Circuit, a: &[&QReg], b: &[&QReg]) {
    ...
    let n = a.len();
    assert_eq!(b.len(), n, "add_refs: a/b length mismatch");
    if n == 0 {
        return;
    }
    if n == 1 {
    ...
    let c = circ.alloc_qreg("sm.add.c");
