...
fn trailmix_q_width_at_step(wq: usize, step: usize) -> usize {
    let capped = trailmix_q_width(wq);
    let giveback_width = || {
        std::env::var("TRAILMIX_Q_GIVEBACK_WIDTH")
            .ok()
            .and_then(|s| s.parse::<usize>().ok())
            .unwrap_or(wq)
            .min(wq.max(1))
    };
    if std::env::var("TRAILMIX_Q_GIVEBACK_STEPS")
        .ok()
        .is_some_and(|steps| {
            steps
                .split(',')
    ...
    {
        return capped.max(giveback_width());
    }
    let Some(giveback_step) = std::env::var("TRAILMIX_Q_GIVEBACK_STEP")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
    else {
        return capped;
    };
    if giveback_step != step {
        return capped;
    }
    capped.max(giveback_width())
}
...
pub(crate) fn shrunken_pz_invert_forward(
    ...
    for i in 0..SHRUNKEN_PZ_NSTEPS {
        ...
        shrunken_pz_resize(c, qq, trailmix_q_width_at_step(wq, i), "q");
...
pub(crate) fn shrunken_pz_invert_backward(
    ...
    for i in (0..SHRUNKEN_PZ_NSTEPS).rev() {
        ...
        if i > 0 {
            ...
            shrunken_pz_resize(c, cca, wca.max(wcb), "ca");
            shrunken_pz_resize(c, ccb, wca.max(wcb), "cb");
            shrunken_pz_resize(c, qq, trailmix_q_width_at_step(wq, i - 1), "q");
        }
    }
}
