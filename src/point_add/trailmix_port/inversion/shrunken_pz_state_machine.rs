...
fn lowq_exact_window_compare_enabled() -> bool {
    if std::env::var("LOWQ_EXACT_WINDOW_COMPARE")
        .ok()
        .as_deref()
        != Some("1")
    {
    ...
    assert_eq!(
        ...
        683,
        "LOWQ_EXACT_WINDOW_COMPARE is sealed to the Q948 target683 route"
    );
    assert_eq!(
        std::env::var("LOWQ_Q948_DIRECT_HCLZ_PEAK_GUARD")
            ...
            .as_deref(),
        Some("1"),
        "LOWQ_EXACT_WINDOW_COMPARE requires the Q948 direct-HCLZ route"
    );
    true
}
...
fn lowq_passenger_top_lifetime_experiment_enabled() -> bool {
    ...
    let q947_fresh = std::env::var("LOWQ_Q947_FRESH_SUPPORT_CERTIFIED")
        ...
        .as_deref()
        == Some("1");
    let q946_fresh = std::env::var("LOWQ_Q946_FRESH_SUPPORT_CERTIFIED")
        .ok()
        .as_deref()
        == Some("1");
    if q947_route {
        ...
        let route_fresh = if q946_route {
            assert!(!q947_fresh, "Q946 route cannot inherit the Q947 certificate");
            q946_fresh
        } else {
            assert!(!q946_fresh, "Q947 route cannot inherit the Q946 certificate");
            q947_fresh
        };
        assert!(
            proof_mode ^ route_fresh,
            "passenger release requires exactly one of proof mode or its route-specific fresh certificate"
        );
        let parent_fresh = std::env::var("LOWQ_Q949_ROBUST_FRESH_SUPPORT_CERTIFIED")
            .ok()
            .as_deref()
        ...
        let borrowed_fresh =
            std::env::var("LOWQ_BORROWED_TRANSCRIPT_FRESH_SUPPORT_CERTIFIED")
                ...
                == Some("1");
        assert!(
            parent_fresh == route_fresh && borrowed_fresh == route_fresh,
            "passenger profile requires parent, borrowed-transcript, and route-specific certificates together"
        );
    } else {
        assert!(
            proof_mode && !q947_fresh && !q946_fresh,
...
fn selective_direct_bitlen_needed(
    ...
    let direct_target = if q946_second_ownership_release_requested() {
        946
...
fn narrow_lt(circ: &mut Circuit, a: &[QReg], b: &[QReg], out: &QReg, lo: usize) {
    ...
    let lo = if lowq_exact_window_compare_enabled() {
        0
    } else {
        lo.min(hi.saturating_sub(1))
    };
...
pub(crate) fn division_substep_windowed(
    ...
    if selective_direct_bitlen_needed(circ, a, b, lo_a, lo_b, "update") {
    ...
    if selective_direct_bitlen_needed(circ, a, b, lo_a, lo_a, "parity") {
...
pub(crate) fn multiply_substep_windowed(
    ...
    if selective_direct_bitlen_needed(circ, a, b, ca_window, ca_window, "parity") {
    ...
    if selective_direct_bitlen_needed(circ, b, a, cb_window, ca_window, "update") {
        direct_bitlen_diff_update(
            circ,
            b,
            a,
...
fn hardened_peak_trace_marker(c: &mut Circuit, row: usize, inverse: bool, substep: &str) {
    ...
    c.b.record_lowq_liveness_marker(format!(
        concat!(
            ...
            "off_counter_alias={};second_ownership_release={}"
        ),
        direction,
        row,
        substep,
...
pub(crate) fn shrunken_pz_pass_step(
    ...
    let direct_hclz_binding = if q947_passenger_direct_hclz_requested() {
        ...
        matches!(c.current_section.as_str(), "ec3.alt.cancel" | "ec3.inv_fwd")
            && (Q947_DIRECT_HCLZ_BINDING_ROWS.contains(&i) || q946_second_release_binding)
    } else {
    ...
    let division_transcript_loan = (!reverse_relational_division)
        .then(|| {
        ...
        .flatten();
    let multiply_transcript_loan = borrowed_transcript_loan(
        i,
        inverse,
    ...
    c.lowq_trace_division_borrow =
        division_transcript_loan.is_some() || reverse_relational_division;
    c.lowq_trace_multiply_borrow = multiply_transcript_loan.is_some();
                ...
                |c, g| {
                    ...
                    with_arithmetic_srot_view(s_rot, counter, |s_rot_arith| {
                        division_substep_windowed_inv(
                            ...
                            rb(sdb),
                            ctz_bits,
                            relation_loan.or(division_transcript_loan),
                        );
                    });
                },
            );
    ...
    if inverse {
        ...
        gate_hold(
            ...
            with_arithmetic_srot_view(s_rot, counter, |s_rot_arith| {
                division_substep_windowed_inv(
                    ...
                    ctz_bits, relation_loan.or(division_transcript_loan),
                );
            });
            },
        );
        ...
        gate_hold(
            ...
            with_arithmetic_srot_view(s_rot, counter, |s_rot_arith| {
                division_substep_windowed(
                    c, aa, bb, qq, s_rot_arith, off, g, &lenders, lo_a, lo_b, rb(sdb),
                    ctz_bits, division_transcript_loan,
                );
            });
            },
        );
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Q946ComposedEmittedGateReport {
    pub widths_checked: usize,
    pub window_pairs_checked: usize,
    pub update_active_clean_selector_states_checked: usize,
    pub update_inactive_clean_selector_states_checked: usize,
    pub update_inactive_dirty_selector_states_checked: usize,
    pub parity_active_states_checked: usize,
    pub parity_inactive_states_checked: usize,
    pub dirty_lender_patterns_checked: usize,
    pub phase_cleanup_states_checked: usize,
    pub ancilla_cleanup_states_checked: usize,
    pub lender_restoration_states_checked: usize,
    pub reset_operations_checked: usize,
    pub phase_sensitive_operations_observed: usize,
    pub max_update_extra_qubits: usize,
    pub max_parity_extra_qubits: usize,
    pub lifecycle_widths_checked: usize,
    pub lifecycle_shared_alias_circuits_checked: usize,
    pub lifecycle_boundary_states_checked: usize,
    pub lifecycle_roundtrip_states_checked: usize,
    pub lifecycle_terminal_transitions_checked: usize,
    pub lifecycle_active_before_states_checked: usize,
    pub lifecycle_active_after_states_checked: usize,
    pub lifecycle_inactive_dirty_before_states_checked: usize,
    pub lifecycle_inactive_dirty_after_states_checked: usize,
    pub lifecycle_dirty_lender_patterns_checked: usize,
    pub lifecycle_phase_cleanup_states_checked: usize,
    pub lifecycle_ancilla_cleanup_states_checked: usize,
    pub lifecycle_lender_restoration_states_checked: usize,
    pub max_lifecycle_extra_qubits: usize,
}

/// Compose the reduced-width direct-HCLZ update/parity gates with the Q946
/// selective-borrow ownership contract. The update selector is physically
/// `counter[0]`: it must be clean while active, but inactive states exhaustively
/// include both clean and dirty selector values. The lifecycle harness emits
/// direct-HCLZ, the affine ownership transition, direct-HCLZ again, and the
/// production export boundary in one circuit, with the same physical lane used
/// as affine `done`, borrowed `off`, and the direct selector. Every roundtrip
/// circuit emits the exact inverse schedule and compares all dirty lenders.
#[doc(hidden)]
pub fn q946_composed_emitted_gate_roundtrip_check() -> Q946ComposedEmittedGateReport {
    use crate::circuit::{Op, OperationType};

    assert!(q946_second_ownership_release_requested());
    assert!(q947_passenger_direct_hclz_requested());
    assert!(lowq_q948_direct_hclz_peak_guard_enabled());
            ...
            match op.kind {
                ...
                OperationType::R => {
                    assert!(
                        ...
                        "Q946 composed emitted-gate proof reset a dirty lane"
                    );
                    *resets_checked += 1;
                }
                other => panic!(
                    "Q946 composed emitted-gate proof emitted phase-sensitive gate {other:?}"
                ),
            }
        }
        state
    ...
    let mut window_pairs_checked = 0usize;
    let mut update_active_clean_selector_states_checked = 0usize;
    let mut update_inactive_clean_selector_states_checked = 0usize;
    let mut update_inactive_dirty_selector_states_checked = 0usize;
    let mut parity_active_states_checked = 0usize;
    let mut parity_inactive_states_checked = 0usize;
    let mut dirty_lender_patterns_checked = 0usize;
    let mut phase_cleanup_states_checked = 0usize;
    let mut ancilla_cleanup_states_checked = 0usize;
    let mut lender_restoration_states_checked = 0usize;
    let mut reset_operations_checked = 0usize;
    let mut max_update_extra_qubits = 0usize;
    let mut max_parity_extra_qubits = 0usize;
    ...
    for width in 1..=3usize {
        for lo_a in 0..width {
            for lo_b in 0..width {
                ...
                let build_update = |roundtrip: bool| {
                    let mut c = Circuit::new();
                    let active = c.alloc_qreg("q946-composed-update.active");
                    let counter = c.alloc_qreg_bits("q946-composed-update.counter", 2);
                    let selector = &counter[0];
                    let target = c.alloc_qreg_bits("q946-composed-update.target", 3);
                    let target_refs: Vec<&QReg> = target.iter().collect();
                    let a = c.alloc_qreg_bits("q946-composed-update.a", width);
                    let b = c.alloc_qreg_bits("q946-composed-update.b", width);
                    let extra = c.alloc_qreg_bits("q946-composed-update.extra", 2);
                    let extra_refs: Vec<&QReg> = counter
                        .iter()
                        .skip(1)
                        .chain(extra.iter())
                        .collect();
                ...
                let update_forward = build_update(false);
                let update_roundtrip = build_update(true);
                let update_external = 8 + 2 * width;
                for builder in [&update_forward, &update_roundtrip] {
                    let extra = builder.peak_qubits as usize - update_external;
                    max_update_extra_qubits = max_update_extra_qubits.max(extra);
                    assert_eq!(extra, 0, "Q946 composed direct update allocated a lane");
                }
                let target_start = 3;
                let a_start = target_start + 3;
                let b_start = a_start + width;
                let extra_start = b_start + width;
                for input in 0..(1u64 << update_external) {
                    let active = input & 1;
                    let selector = (input >> 1) & 1;
                    if active == 1 && selector == 1 {
                        continue;
                    }
                    let a = word(input, a_start, width);
                    let b = word(input, b_start, width);
                    if active == 1 && ((a >> lo_a) == 0 || (b >> lo_b) == 0) {
                        continue;
                    }
                    match (active, selector) {
                        (1, 0) => update_active_clean_selector_states_checked += 1,
                        (0, 0) => update_inactive_clean_selector_states_checked += 1,
                        (0, 1) => update_inactive_dirty_selector_states_checked += 1,
                        _ => unreachable!(),
                    }
                    if word(input, extra_start, 2) != 0 || (input >> 2) & 1 != 0 {
                        dirty_lender_patterns_checked += 1;
                    }
                    let target = word(input, target_start, 3);
                    let a_len = 64 - a.leading_zeros() as u64;
                    let b_len = 64 - b.leading_zeros() as u64;
                    let target_post = target.wrapping_add(active * a_len.wrapping_sub(b_len)) & 7;
                    let expected = (input & !(7u64 << target_start))
                    ...
                    assert_eq!(
                        ...
                        expected,
                        "Q946 composed direct update width={width} lo_a={lo_a} lo_b={lo_b} input={input}"
                    );
                    assert_eq!(
                        apply(
                            &update_roundtrip.ops,
                            input,
                            &mut reset_operations_checked,
                        ),
                        input,
                        "Q946 composed direct update roundtrip width={width} lo_a={lo_a} lo_b={lo_b} input={input}"
                    );
                    phase_cleanup_states_checked += 1;
                    ancilla_cleanup_states_checked += 1;
                    lender_restoration_states_checked += 1;
                }

                let mut parity = Circuit::new();
                let active = parity.alloc_qreg("q946-composed-parity.active");
                let out = parity.alloc_qreg("q946-composed-parity.out");
                let counter = parity.alloc_qreg_bits("q946-composed-parity.counter", 2);
                let a = parity.alloc_qreg_bits("q946-composed-parity.a", width);
                let b = parity.alloc_qreg_bits("q946-composed-parity.b", width);
                let extra = parity.alloc_qreg_bits("q946-composed-parity.extra", 2);
                let extra_refs: Vec<&QReg> = counter.iter().chain(extra.iter()).collect();
                direct_bitlen_diff_parity(
                    &mut parity,
                    &a,
                    &b,
                ...
                let parity_external = 6 + 2 * width;
                let parity_builder = parity.into_builder();
                let extra_qubits = parity_builder.peak_qubits as usize - parity_external;
                max_parity_extra_qubits = max_parity_extra_qubits.max(extra_qubits);
                assert_eq!(extra_qubits, 0, "Q946 composed direct parity allocated a lane");
                let a_start = 4;
                let b_start = a_start + width;
                for input in 0..(1u64 << parity_external) {
                    let active = input & 1;
                    ...
                    assert_eq!(
                        ...
                        input,
                        "Q946 composed direct parity roundtrip width={width} lo_a={lo_a} lo_b={lo_b} input={input}"
                    );
                    phase_cleanup_states_checked += 1;
                    ancilla_cleanup_states_checked += 1;
                    lender_restoration_states_checked += 1;
    ...
    fn write_word(mut state: u64, register: &[u32], value: u64) -> u64 {
    ...
    fn build_lifecycle(width: usize, lo_a: usize, lo_b: usize, roundtrip: bool) -> LifecycleHarness {
        let mut c = Circuit::new();
        let aa = c.alloc_qreg_bits("q946-lifecycle.aa", 1);
        let qq = c.alloc_qreg_bits("q946-lifecycle.qq", 1);
        let ca = c.alloc_qreg_bits("q946-lifecycle.ca", Q949_AFFINE_COUNTER_WIDTH);
        let done = c.alloc_qreg("q946-lifecycle.done-off-counter0");
        let saved = c.alloc_qreg_bits("q946-lifecycle.saved", Q949_AFFINE_COUNTER_WIDTH);
        let active = c.alloc_qreg("q946-lifecycle.active");
        let target = c.alloc_qreg_bits("q946-lifecycle.target", 3);
        let lhs = c.alloc_qreg_bits("q946-lifecycle.lhs", width);
        let rhs = c.alloc_qreg_bits("q946-lifecycle.rhs", width);
        let lenders = c.alloc_qreg_bits("q946-lifecycle.lenders", 12);
        let candidates: Vec<&QReg> = aa
            .iter()
            .chain(qq.iter())
            .chain(ca.iter())
            ...
            .chain(std::iter::once(&active))
            .collect();
        let direct_lenders: Vec<&QReg> = aa
            .iter()
            .chain(qq.iter())
            .chain(ca.iter())
            .chain(saved.iter())
            ...
            .collect();
        let target_refs: Vec<&QReg> = target.iter().collect();
        let direct = |c: &mut Circuit, subtract: bool| {
            c.x(&active);
            c.cx(&done, &active);
            direct_bitlen_diff_update(
                c,
    ...
    let mut lifecycle_boundary_states_checked = 0usize;
    let mut lifecycle_roundtrip_states_checked = 0usize;
    let mut lifecycle_shared_alias_circuits_checked = 0usize;
    let mut lifecycle_terminal_transitions_checked = 0usize;
    let mut lifecycle_active_before_states_checked = 0usize;
    let mut lifecycle_active_after_states_checked = 0usize;
    let mut lifecycle_inactive_dirty_before_states_checked = 0usize;
    let mut lifecycle_inactive_dirty_after_states_checked = 0usize;
    let mut lifecycle_dirty_lender_patterns_checked = 0usize;
    let mut lifecycle_phase_cleanup_states_checked = 0usize;
    let mut lifecycle_ancilla_cleanup_states_checked = 0usize;
    let mut lifecycle_lender_restoration_states_checked = 0usize;
    let mut max_lifecycle_extra_qubits = 0usize;
    ...
    let lender_patterns = [0u64, 1, 0x555, 0xaaa, 0xfff];
    for width in 1..=2usize {
        let value_mask = (1u64 << width) - 1;
        for lo_a in 0..width {
            for lo_b in 0..width {
                let boundary = build_lifecycle(width, lo_a, lo_b, false);
                let roundtrip = build_lifecycle(width, lo_a, lo_b, true);
                lifecycle_shared_alias_circuits_checked += 2;
                let external = boundary.aa.len()
                    + boundary.qq.len()
                    + boundary.ca.len()
                    + 1 // done/off/counter[0]
                    + boundary.saved.len()
                    + 1 // active
                    + boundary.target.len()
                    + boundary.lhs.len()
                    + boundary.rhs.len()
                    + boundary.lenders.len();
                for harness in [&boundary, &roundtrip] {
                    assert_eq!(harness.builder.next_qubit as usize, external);
                    let extra = harness.builder.peak_qubits as usize - external;
                    max_lifecycle_extra_qubits = max_lifecycle_extra_qubits.max(extra);
                    assert_eq!(extra, 0, "Q946 affine/off/direct lifecycle allocated a lane");
                }
                assert_eq!(boundary.aa, roundtrip.aa);
                assert_eq!(boundary.qq, roundtrip.qq);
                assert_eq!(boundary.ca, roundtrip.ca);
                assert_eq!(boundary.done, roundtrip.done);
                assert_eq!(boundary.saved, roundtrip.saved);
                assert_eq!(boundary.active, roundtrip.active);
                assert_eq!(boundary.target, roundtrip.target);
                assert_eq!(boundary.lhs, roundtrip.lhs);
                assert_eq!(boundary.rhs, roundtrip.rhs);
                assert_eq!(boundary.lenders, roundtrip.lenders);

                for (count, aa, qq) in lifecycle_states {
                    let done = u64::from(count != 0);
                    let terminal_transition = count == 0 && aa == 0 && qq == 0;
                    let done_after = done ^ u64::from(terminal_transition);
                    let count_after = count + done_after;
                    assert!(count_after <= 159);
                    for lhs in 1u64..=value_mask {
                        for rhs in 1u64..=value_mask {
                            if (lhs >> lo_a) == 0 || (rhs >> lo_b) == 0 {
                                continue;
                            }
                            let lhs_len = u64::from(64 - lhs.leading_zeros());
                            let rhs_len = u64::from(64 - rhs.leading_zeros());
                            let delta = lhs_len.wrapping_sub(rhs_len);
                            for target in [0u64, 3, 7] {
                                for lender_pattern in lender_patterns {
                                    let mut input = 0u64;
                                    input = write_word(input, &boundary.aa, aa);
                                    input = write_word(input, &boundary.qq, qq);
                                    input = write_word(
                                        input,
                                        &boundary.ca,
                                        (SECP256K1_P_LOW_BYTE as u64) ^ count,
                                    );
                                    input |= done << boundary.done;
                                    input = write_word(input, &boundary.target, target);
                                    input = write_word(input, &boundary.lhs, lhs);
                                    input = write_word(input, &boundary.rhs, rhs);
                                    input = write_word(input, &boundary.lenders, lender_pattern);

                                    let active_before = done == 0;
                                    let active_after = done_after == 0;
                                    let target_after = target
                                        .wrapping_add(u64::from(active_before) * delta)
                                        .wrapping_add(u64::from(active_after) * delta)
                                        & 7;
                                    let mut expected_boundary = input;
                                    expected_boundary = write_word(
                                        expected_boundary,
                                        &boundary.ca,
                                        SECP256K1_P_LOW_BYTE as u64,
                                    );
                                    expected_boundary &= !(1u64 << boundary.done);
                                    expected_boundary = write_word(
                                        expected_boundary,
                                        &boundary.saved,
                                        count_after,
                                    ...
                                    assert_eq!((input >> boundary.active) & 1, 0);
                                    assert_eq!(
                                        apply(
                                            &boundary.builder.ops,
                                            input,
                                            &mut reset_operations_checked,
                                        ),
                                        expected_boundary,
                                        "Q946 affine/off/direct boundary width={width} lo_a={lo_a} lo_b={lo_b} count={count} aa={aa} qq={qq} lhs={lhs} rhs={rhs} target={target} lenders={lender_pattern}"
                                    );
                                    assert_eq!(
                                        apply(
                                            &roundtrip.builder.ops,
                                            input,
                                            &mut reset_operations_checked,
                                        ),
                                        input,
                                        "Q946 affine/off/direct roundtrip width={width} lo_a={lo_a} lo_b={lo_b} count={count} aa={aa} qq={qq} lhs={lhs} rhs={rhs} target={target} lenders={lender_pattern}"
                                    );
                                    lifecycle_boundary_states_checked += 1;
                                    lifecycle_roundtrip_states_checked += 1;
                                    lifecycle_terminal_transitions_checked +=
                                        usize::from(terminal_transition);
                                    lifecycle_active_before_states_checked +=
                                        usize::from(active_before);
                                    lifecycle_active_after_states_checked +=
                                        usize::from(active_after);
                                    lifecycle_inactive_dirty_before_states_checked +=
                                        usize::from(!active_before && done == 1);
                                    lifecycle_inactive_dirty_after_states_checked +=
                                        usize::from(!active_after && done_after == 1);
                                    lifecycle_dirty_lender_patterns_checked +=
                                        usize::from(lender_pattern != 0);
                                    lifecycle_phase_cleanup_states_checked += 1;
                                    lifecycle_ancilla_cleanup_states_checked += 1;
                                    lifecycle_lender_restoration_states_checked += 1;
                                }
                            }
                        }
                    }
    ...
    assert!(lifecycle_boundary_states_checked > 0);
    assert!(lifecycle_shared_alias_circuits_checked > 0);
    assert_eq!(lifecycle_boundary_states_checked, lifecycle_roundtrip_states_checked);
    assert!(lifecycle_terminal_transitions_checked > 0);
    assert!(lifecycle_active_before_states_checked > 0);
    assert!(lifecycle_active_after_states_checked > 0);
    assert!(lifecycle_inactive_dirty_before_states_checked > 0);
    assert!(lifecycle_inactive_dirty_after_states_checked > 0);
    assert!(lifecycle_dirty_lender_patterns_checked > 0);
    assert_eq!(
        lifecycle_phase_cleanup_states_checked,
        lifecycle_ancilla_cleanup_states_checked
    );
    assert_eq!(
        lifecycle_phase_cleanup_states_checked,
        lifecycle_lender_restoration_states_checked
    );

    assert!(update_active_clean_selector_states_checked > 0);
    assert!(update_inactive_clean_selector_states_checked > 0);
    assert!(update_inactive_dirty_selector_states_checked > 0);
    assert!(parity_active_states_checked > 0 && parity_inactive_states_checked > 0);
    assert!(dirty_lender_patterns_checked > 0);
    assert_eq!(phase_cleanup_states_checked, ancilla_cleanup_states_checked);
    assert_eq!(phase_cleanup_states_checked, lender_restoration_states_checked);
    Q946ComposedEmittedGateReport {
        widths_checked: 3,
        window_pairs_checked,
        update_active_clean_selector_states_checked,
        update_inactive_clean_selector_states_checked,
        update_inactive_dirty_selector_states_checked,
        parity_active_states_checked,
        parity_inactive_states_checked,
        dirty_lender_patterns_checked,
        phase_cleanup_states_checked,
        ancilla_cleanup_states_checked,
        lender_restoration_states_checked,
        reset_operations_checked,
        phase_sensitive_operations_observed: 0,
        max_update_extra_qubits,
        max_parity_extra_qubits,
        lifecycle_widths_checked: 2,
        lifecycle_shared_alias_circuits_checked,
        lifecycle_boundary_states_checked,
        lifecycle_roundtrip_states_checked,
        lifecycle_terminal_transitions_checked,
        lifecycle_active_before_states_checked,
        lifecycle_active_after_states_checked,
        lifecycle_inactive_dirty_before_states_checked,
