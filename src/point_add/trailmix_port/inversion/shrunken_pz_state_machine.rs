...
/// transiently one bit at a time, FOLDS the diff
...
fn clz_diff_body_middle(
    ...
    let na = aw.len();
    // Deposit pos_a. The b position is folded into pa one bit at a time below so
    // no full pb register is live at the bitlen peak.
