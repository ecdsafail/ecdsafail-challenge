...
fn lowq_dirty_demux_enabled() -> bool {
    if std::env::var("LOWQ_DIRTY_DEMUX").ok().as_deref() != Some("1") {
        return false;
    }
    let target = env_usize("TRAILMIX_Q_TARGET", 0);
    assert!(
        matches!(target, 684 | 685 | 686),
        "LOWQ_DIRTY_DEMUX is sealed to TRAILMIX_Q_TARGET=684/685/686"
    );
    assert_eq!(
        env_usize("TRAILMIX_SROT_W", 0),
        5,
        "LOWQ_DIRTY_DEMUX is proved only for the five-bit shift register"
    );
    assert_eq!(
        std::env::var("TRAILMIX_THIN_CLZ_WINDOW").ok().as_deref(),
        Some("78"),
        "LOWQ_DIRTY_DEMUX is sealed to the audited 78-bit CLZ windows"
    );
    true
}
...
/// Exact large-control toggle with no clean allocation. Every borrowed lane
/// may contain arbitrary data and is restored by `mcx_dirty_ladder`.
fn dirty_controlled_x(
    ...
    assert!(
        dirty.len() >= need,
        "LOWQ_DIRTY_DEMUX lender shortage: controls={} need={} have={}",
        controls.len(),
        need,
        dirty.len()
    );
...
/// Controlled increment/decrement of `target[lo..]`, conditioned on an exact
/// MSB selector. High-to-low toggles make every carry/borrow predicate read the
/// pre-update lower bits. No carry register is allocated.
fn dirty_controlled_inc_suffix(
    ...
    let action: Vec<&QReg> = target.iter().collect();
    for i in (lo + 1..target.len()).rev() {
        let lower: Vec<&QReg> = target[lo..i].iter().collect();
        if subtract {
            for q in &lower {
                circ.x(q);
            }
...
/// Add/subtract a classical constant under a large exact selector. Decomposing
/// into powers of two needs at most `w(w+1)/2` dirty-ladder toggles.
...
/// Apply `target +=/-= active*bitlen(src)` without materializing CLZ output.
/// Selector `k` is `active & src[k] & AND(!src[k+1..])`. The peer and all
/// non-control source lanes are arbitrary restored workspace.
fn direct_bitlen_update(
    ...
    for k in lo..src.len() {
        ...
        for q in src[k + 1..].iter().rev() {
            circ.x(q);
        }
    }
}

/// `target += sign*active*(bitlen(a)-bitlen(b))` modulo `2^w`.
...
/// Toggle by the parity of the bit-length difference. Modulo two, subtraction
/// and addition coincide, so the two exact MSB selectors XOR directly.
...
fn toggle_low_block_zero(
    ...
    let width = width.min(q.len());
    for bit in &q[..width] {
        circ.x(bit);
    }
    let mut controls = Vec::with_capacity(width + 1);
    controls.push(active);
    controls.extend(q[..width].iter());
    dirty_controlled_x(circ, &controls, out, lenders, &[out]);
    for bit in q[..width].iter().rev() {
        circ.x(bit);
    }
}

/// Deposit `active*ctz(q)` into a zero `s`, or erase it by the exact inverse.
/// Each high-to-low stage records whether the current low `2^b`-bit block is
/// zero, then right-normalizes `q`. Reverse controlled shifts restore `q` while
/// preserving `s`; the erase orientation reverses every gate.
fn direct_ctz_binary(
    circ: &mut Circuit,
    q: &[QReg],
    s: &[QReg],
    active: &QReg,
    ...
    use crate::point_add::trailmix_port::arith::qshift_sub::controlled_shift_fixed_inplace;

    if !erase {
        for bit in (0..s.len()).rev() {
            let distance = 1usize << bit;
            toggle_low_block_zero(circ, q, &s[bit], active, distance, lenders);
            controlled_shift_fixed_inplace(circ, q, &s[bit], distance, false);
        }
        for bit in 0..s.len() {
            controlled_shift_fixed_inplace(circ, q, &s[bit], 1usize << bit, true);
        }
    } else {
        for bit in (0..s.len()).rev() {
            controlled_shift_fixed_inplace(circ, q, &s[bit], 1usize << bit, false);
        }
        for bit in 0..s.len() {
            let distance = 1usize << bit;
            controlled_shift_fixed_inplace(circ, q, &s[bit], distance, true);
            toggle_low_block_zero(circ, q, &s[bit], active, distance, lenders);
        }
    }
}
...
fn set_bit_at_s_gated(
    ...
    if lowq_dirty_demux_enabled() {
        let action: Vec<&QReg> = q_div.iter().collect();
        for (i, target) in q_div.iter().enumerate() {
            for (j, q) in s.iter().enumerate() {
                if (i >> j) & 1 == 0 {
                    circ.x(q);
                }
            }
            ...
            controls.push(active);
            controls.extend(s.iter());
            dirty_controlled_x(circ, &controls, target, lenders, &action);
            for (j, q) in s.iter().enumerate().rev() {
                if (i >> j) & 1 == 0 {
                    circ.x(q);
                }
            }
...
pub fn division_substep_windowed(
    ...
    if lowq_dirty_demux_enabled() {
        // s_rot starts at zero: arithmetic deposit of the exact CLZ difference.
        ...
        // diff = bitlen(A)-bitlen(B), masked by active into s_rot.
    ...
    set_bit_at_s_gated(circ, q_div, s_rot, active, &lenders); // q_div ^= active·(1<<s_rot)
    ...
    if lowq_dirty_demux_enabled() {
        direct_ctz_binary(circ, q_div, s_rot, active, &lenders, true);
    } else {
...
pub fn division_substep_windowed_inv(
    ...
    if lowq_dirty_demux_enabled() {
        direct_ctz_binary(circ, q_div, s_rot, active, &lenders, false);
    } else {
        let t = circ.alloc_qreg_bits("dg.ctz", w);
        xor_const(circ, &t, n_pad);
        let rev: Vec<&QReg> = q_div.iter().rev().collect();
...
pub fn multiply_substep_windowed(
    ...
    if lowq_dirty_demux_enabled() {
        direct_ctz_binary(circ, q_mul, s_rot, active, &lenders, false);
    } else {
    ...
    set_bit_at_s_gated(circ, q_mul, s_rot, active, &lenders); // q_mul ^= active·(1<<s_rot)
...
pub fn multiply_substep_windowed_inv(
    ...
    if lowq_dirty_demux_enabled() {
        direct_ctz_binary(circ, q_mul, s_rot, active, &lenders, true);
    } else {
        let t = circ.alloc_qreg_bits("mg.ctz", w);
        let rev: Vec<&QReg> = q_mul.iter().rev().collect();
        xor_const(circ, &t, n_pad);
