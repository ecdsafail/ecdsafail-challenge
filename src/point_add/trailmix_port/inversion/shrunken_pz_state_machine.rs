...
///
/// With `fuse_zero_prefix=false`, PRE: src nonzero (EEA gcd / nonzero quotient
/// pad); an all-zero source historically returns one. The fused mode includes
/// the final complemented-prefix term and maps zero to an intermediate -1, so
/// the caller's `pos + 1` arithmetic returns the exact bit length zero.
...
fn bit_length_lean_middle(
    ...
    src: &[&QReg],
    pos: &[QReg],
    fuse_zero_prefix: bool,
    bracket_source_complement: bool,
    ...
    // ~src (X-bracket); the prefix-AND reads the complemented bits. A paired
    // caller may keep this source complemented across a disjoint USE block.
    if bracket_source_complement {
        for q in src {
            circ.x(q);
        }
    }
    ...
    fn deposit_step(
        ...
        if i > n || (i == n && !fuse_zero_prefix) {
            return;
        }
        if i == n {
            // For an all-zero source the historical gray deposit leaves pos=0,
            // which made the surrounding `pos + 1` arithmetic return one.
            // The final complemented-prefix term maps that zero to -1 instead,
            // so `pos + 1` is exactly zero. Nonzero inputs do not activate the
            // final prefix control. The reverse traversal applies the same
            // self-inverse toggle and restores the position register.
            match ctrls {
                [] => {
                    for bit in pos {
                        circ.x(bit);
                ...
                [left, right] => {
                    circ.ccx(left, right, flag);
                    for bit in pos {
                        circ.cx(flag, bit);
                    }
                    circ.clear_and(flag, left, right);
                }
    ...
    let done = kg.forward(circ, |c, i, ctrls| {
        deposit_step(c, i, ctrls, pos, &flag, n, fuse_zero_prefix)
    }); // pos -> p, or -1 for zero on the fused route
    let clean = body(circ);
    if clean {
...
fn bit_length_lean_with_source_state(
    circ: &mut Circuit,
    src: &[&QReg],
    s: &[QReg],
    dec: bool,
...
pub(crate) fn bit_length_lean(circ: &mut Circuit, src: &[&QReg], s: &[QReg], dec: bool) {
    bit_length_lean_with_source_state(circ, src, s, dec, false);
}

/// Apply the bit-length update while `src` is already bitwise complemented.
/// The source remains complemented on return. This is intentionally crate-local:
/// callers must prove that every operation between paired calls is source-disjoint.
pub(crate) fn bit_length_lean_complemented_source(
    ...
    bit_length_lean_with_source_state(circ, src, s, dec, true);
}
...
fn clz_diff_body_middle(
    ...
    bit_length_lean_middle(circ, &aw, &pa, false, true, |_| false); // pa = pos_a
    xor_const(circ, &pb, nb);
    bit_length_lean_middle(circ, &bw, &pb, false, true, |_| false); // pb = pos_b
    ...
    bit_length_lean_middle(circ, &bw, &pb, false, true, |_| false); // pb -> nb
    xor_const(circ, &pb, nb); // pb -> 0
    bit_length_lean_middle(circ, &aw, &pa, false, true, |_| false); // pa -> na
