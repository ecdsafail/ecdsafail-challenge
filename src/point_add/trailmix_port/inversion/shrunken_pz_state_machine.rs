...
const Q953_COUNTER7_THRESHOLD: usize = 1 << 7;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Q953OwnershipRows {
    pub first_terminal_capable_row: usize,
    pub counter6_cutover_row: usize,
    pub counter7_cutover_row: usize,
}

/// Derive Q953's ownership rows from the static support metadata covered by the
/// certified schedule digest. Callers still need the runtime certificate to pin
/// the generated or loaded thin-width payload.
#[doc(hidden)]
pub fn q953_srot_counter67_ownership_rows() -> Q953OwnershipRows {
    use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::{
        SHRUNKEN_PZ_B_LO, SHRUNKEN_PZ_NSTEPS,
    };
    ...
    let derive_cutover = |threshold: usize, bit: usize| {
        first_terminal_capable_row
            ...
            .unwrap_or_else(|| panic!("Q953 counter[{bit}] cutover row overflow"))
    };
    let counter6_cutover_row = derive_cutover(Q953_COUNTER6_THRESHOLD, 6);
    let counter7_cutover_row = derive_cutover(Q953_COUNTER7_THRESHOLD, 7);
    assert!(
        counter6_cutover_row < counter7_cutover_row
            && counter7_cutover_row < SHRUNKEN_PZ_NSTEPS,
        "Q953 derived cutovers fall outside the schedule"
    );
    Q953OwnershipRows {
        first_terminal_capable_row,
...
fn q953_pinned_schedule_sha3_256() -> [u8; 32] {
    let raw = std::env::var("LOWQ_Q953_SCHEDULE_SHA3_256")
        .expect("LOWQ_Q953_SCHEDULE_SHA3_256 must pin the generated schedule");
    let raw = raw.strip_prefix("0x").unwrap_or(&raw);
    assert_eq!(raw.len(), 64, "Q953 schedule SHA3-256 must have 64 hex digits");
    let mut digest = [0u8; 32];
    for (index, byte) in digest.iter_mut().enumerate() {
        *byte = u8::from_str_radix(&raw[index * 2..index * 2 + 2], 16)
            .expect("Q953 schedule SHA3-256 contains non-hexadecimal data");
    }
    digest
}
...
/// Both ownership cutovers are derived from the certified first terminal row:
/// C6=F+64 and C7=F+128.
#[doc(hidden)]
pub fn q953_srot_counter67_schedule_certificate() -> Q953ScheduleCertificate {
    use crate::point_add::trailmix_port::inversion::shrunken_pz_schedule::{
        ...
        static_schedule_sha3_256, SHRUNKEN_PZ_NSTEPS,
    };
    ...
    *CERTIFICATE.get_or_init(|| {
        ...
        let ownership = q953_srot_counter67_ownership_rows();
        let first_terminal_capable_row = ownership.first_terminal_capable_row;
        let counter6_cutover_row = ownership.counter6_cutover_row;
        let counter7_cutover_row = ownership.counter7_cutover_row;
        ...
        let max_counter7_alias_value = last_counter7_alias_row - first_terminal_capable_row;
        let max_final_counter = SHRUNKEN_PZ_NSTEPS - first_terminal_capable_row;
        assert_eq!(
            max_counter6_alias_value + 1,
            Q953_COUNTER6_THRESHOLD,
            "Q953 counter[6] alias bound drift"
        );
        assert_eq!(
            max_counter7_alias_value + 1,
            Q953_COUNTER7_THRESHOLD,
            "Q953 counter[7] alias bound drift"
        );
...
pub struct Q953CounterRecurrenceReport {
    ...
    pub aliased_forward_states_checked: [usize; 2],
    pub aliased_reverse_states_checked: [usize; 2],
    pub forward_threshold_transitions: [usize; 2],
    pub reverse_threshold_transitions: [usize; 2],
}

/// Exhaust every counter trajectory admitted by the schedule-derived terminal
/// lower bound and prove counter[6] and counter[7] remain zero in every body
/// that aliases them. A trajectory is identified by its first terminal row,
/// with `rows` representing no convergence during the fixed schedule.
#[doc(hidden)]
pub fn q953_counter_recurrence_check() -> Q953CounterRecurrenceReport {
    let certificate = q953_srot_counter67_schedule_certificate();
    let thresholds = [
        (Q953_COUNTER6_THRESHOLD, certificate.last_counter6_alias_row),
        (Q953_COUNTER7_THRESHOLD, certificate.last_counter7_alias_row),
    ];
    let mut report = Q953CounterRecurrenceReport {
        terminal_onsets_checked: 0,
        forward_states_checked: 0,
        reverse_states_checked: 0,
        aliased_forward_states_checked: [0; 2],
        aliased_reverse_states_checked: [0; 2],
        forward_threshold_transitions: [0; 2],
        reverse_threshold_transitions: [0; 2],
    };
        ...
        for row in 0..certificate.rows {
            ...
            for (index, &(threshold, last_alias_row)) in thresholds.iter().enumerate() {
                if row <= last_alias_row {
                    assert!(
                        counter < threshold,
                        "Q953 aliased counter bit is live in forward row {row}"
                    );
        ...
        for row in (0..certificate.rows).rev() {
            ...
            for (index, &(threshold, last_alias_row)) in thresholds.iter().enumerate() {
                if row <= last_alias_row {
                    assert!(
                        counter < threshold,
                        "Q953 aliased counter bit is live in reverse row {row}"
                    );
...
fn q953_owned_srot_width_at_row(certificate: &Q953ScheduleCertificate, row: usize) -> usize {
    if row < certificate.counter6_cutover_row {
        3
...
fn with_arithmetic_srot_view<'a, R>(
    ...
    if lowq_q953_srot_counter67_enabled() {
        ...
        assert_eq!(split.len(), 5, "Q953 logical shift-lane width drift");
        body(&split)
...
fn selective_direct_bitlen_needed(
    ...
    let peak_target = if lowq_q953_srot_counter67_enabled() {
        953
    } else if lowq_q954_srot_counter7_enabled() {
        954
...
pub(crate) fn shrunken_pz_pass_step(
    ...
    if lowq_srot_counter_alias_enabled() {
        let expected_owned = if lowq_q953_srot_counter67_enabled() {
            let certificate = q953_srot_counter67_schedule_certificate();
            q953_owned_srot_width_at_row(&certificate, i)
        } else {
            4
        };
        assert_eq!(
        ...
        assert!(
            s_rot.iter().all(|lane| {
                !std::ptr::eq(lane, &counter[6]) && !std::ptr::eq(lane, &counter[7])
            }),
...
fn q953_prepare_forward_srot(c: &mut Circuit, s_rot: &mut Vec<QReg>, row: usize) {
    if !lowq_q953_srot_counter67_enabled() {
        return;
    }
    let certificate = q953_srot_counter67_schedule_certificate();
    let cutovers = [certificate.counter6_cutover_row, certificate.counter7_cutover_row];
    if cutovers.contains(&row) {
        let expected_before = q953_owned_srot_width_at_row(&certificate, row - 1);
        assert_eq!(s_rot.len(), expected_before, "Q953 forward cutover ownership");
        let active_before = c.b.active_qubits;
        let lane = s_rot.len();
    ...
    assert_eq!(
        s_rot.len(),
        q953_owned_srot_width_at_row(&certificate, row),
        "Q953 forward ownership schedule drift at row {row}"
    );
}

fn q953_prepare_reverse_srot(c: &mut Circuit, s_rot: &mut Vec<QReg>, row: usize) {
    if !lowq_q953_srot_counter67_enabled() {
        return;
    }
    let certificate = q953_srot_counter67_schedule_certificate();
    ...
    if cutovers.iter().any(|&cutover| row + 1 == cutover) {
        let expected_before = q953_owned_srot_width_at_row(&certificate, row + 1);
        assert_eq!(s_rot.len(), expected_before, "Q953 reverse cutover ownership");
        let active_before = c.b.active_qubits;
        c.zero_and_free(s_rot.pop().expect("Q953 transient owned shift lane"));
        assert_eq!(
            c.b.active_qubits + 1,
            active_before,
...
pub(crate) fn shrunken_pz_invert_forward(
    ...
    for i in 0..SHRUNKEN_PZ_NSTEPS {
        ...
        q953_prepare_forward_srot(c, s_rot, i);
...
pub(crate) fn shrunken_pz_invert_backward(
    ...
    for i in (0..SHRUNKEN_PZ_NSTEPS).rev() {
        q953_prepare_reverse_srot(c, s_rot, i);
        shrunken_pz_pass_step(
...
pub struct Q953SrotCounter67Report {
    ...
    pub ownership_qubits_saved: [usize; 3],
    pub max_helper_qubit_increase: usize,
    pub max_helper_operation_increase: usize,
    pub max_helper_toffoli_increase: usize,
}

/// Exhaustively validate all three Q953 ownership widths as a five-bit logical
/// register, then check production arithmetic bodies and both counter cutovers.
#[doc(hidden)]
pub fn q953_srot_counter67_roundtrip_check() -> Q953SrotCounter67Report {
    ...
    assert_eq!(
        ...
        certificate.last_counter7_alias_row + 1
    );
    assert_eq!(
    ...
    assert_eq!(
        certificate.max_counter7_alias_value + 1,
        Q953_COUNTER7_THRESHOLD
    );
    ...
    fn audit_helper(
        ...
        assert_eq!(
            ...
            canonical.builder.peak_qubits,
            "Q953 helper physical-lane saving drift"
        );
    ...
    fn build_alias(owned_lanes: usize, mode: u8) -> Harness {
        let mut c = Circuit::new();
        let owned = c.alloc_qreg_bits("q953-alias.srot", owned_lanes);
        ...
        let candidates: Vec<&QReg> = owned
            ...
            .chain(lenders.iter())
            .collect();
        with_arithmetic_srot_view(&owned, &counter, |s_rot| match mode {
    ...
    fn build_body(owned_lanes: usize, multiply: bool, mode: u8) -> Harness {
        let mut c = Circuit::new();
        let a = c.alloc_qreg_bits("q953-body.a", 20);
        let b = c.alloc_qreg_bits("q953-body.b", 20);
        let q = c.alloc_qreg_bits("q953-body.q", 17);
        let owned = c.alloc_qreg_bits("q953-body.srot", owned_lanes);
    ...
    fn owned_width_for_counter(counter: usize, inverse: bool) -> usize {
        ...
        if before < Q953_COUNTER6_THRESHOLD {
            3
        } else if before < Q953_COUNTER7_THRESHOLD {
            4
        } else {
            5
        }
                ...
                for active in 0..=1u64 {
                    for off in 0..=1u64 {
                        for lenders in [0u64, 0xa5] {
                            ...
                            assert_eq!(
                                route_out.registers,
                                [expected_s, expected_route_counter, active, lenders]
                            );
    ...
    for multiply in [false, true] {
        ...
        for owned_lanes in 3..=5usize {
            for mode in 0..=2u8 {
                let canonical = build_body(5, multiply, mode);
                let route = build_body(owned_lanes, multiply, mode);
                audit_helper(
                    &route,
                    &canonical,
                    (5 - owned_lanes) as u32,
    ...
    for mode in 0..=2u8 {
        ...
        for input_counter in counters {
            ...
            let harness = &harnesses[owned_lanes - 3];
    ...
    for (index, &cutover) in cutovers.iter().enumerate() {
        q953_prepare_forward_srot(&mut lifecycle, &mut owned, cutover - 1);
        assert_eq!(owned.len(), index + 3);
        q953_prepare_forward_srot(&mut lifecycle, &mut owned, cutover);
        assert_eq!(owned.len(), index + 4);
        transient_ids.push(owned[index + 3].id());
    }
    assert_eq!(lifecycle.b.active_qubits, baseline + 2);
    for (index, &cutover) in cutovers.iter().enumerate().rev() {
        q953_prepare_reverse_srot(&mut lifecycle, &mut owned, cutover);
        assert_eq!(owned.len(), index + 4);
        q953_prepare_reverse_srot(&mut lifecycle, &mut owned, cutover - 1);
        assert_eq!(owned.len(), index + 3);
    }
    assert_eq!(lifecycle.b.active_qubits, baseline);
    let lifecycle_builder = lifecycle.into_builder();
    assert_eq!(lifecycle_builder.ops.len(), 2);
