...
fn build_hosted_lifecycle(width: usize) -> B {
    ...
    let toggle_gate = |circ: &mut Circuit| {
        circ.x(&counter);
        strict_compare_gated_dirty_carry_refs(circ, &vr, &ur, &counter, &host, &parity);
        circ.x(&counter);
    };
    ...
    for width in 1..=5 {
        ...
        for input in 0..(1u64 << qubits) {
            ...
            operand_restoration_checks +=
                usize::from(((output >> 5) & mask) == v && ((output >> (5 + width)) & mask) == u);
            body_semantic_checks +=
                usize::from(((output >> 4) & 1) == (body_target ^ expected_toggle));
        }
