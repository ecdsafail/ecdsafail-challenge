...
const SHIFT_WIDTH: usize = 10;
const AUX_WIDTH: usize = 22;
const LOCAL_WIDTH: usize = 581;
const SCHEDULE_STEPS: usize = 1_616;
const STREAM_X_PER_TRAVERSAL: usize = 47_308_944;
const STREAM_CX_PER_TRAVERSAL: usize = 47_351_234;
const STREAM_CCX_PER_TRAVERSAL: usize = 59_599_489;
const STREAM_HMR_PER_TRAVERSAL: usize = 3_591_352;
const STREAM_CZ_PER_TRAVERSAL: usize = 3_591_352;
...
const STREAM_CHUNKS: [&[u8]; 36] = [
    include_bytes!("paper2607_data/chunk-0001-0045.zst"),
    include_bytes!("paper2607_data/chunk-0046-0090.zst"),
    include_bytes!("paper2607_data/chunk-0091-0135.zst"),
    include_bytes!("paper2607_data/chunk-0136-0180.zst"),
    include_bytes!("paper2607_data/chunk-0181-0225.zst"),
    include_bytes!("paper2607_data/chunk-0226-0270.zst"),
    include_bytes!("paper2607_data/chunk-0271-0315.zst"),
    include_bytes!("paper2607_data/chunk-0316-0360.zst"),
    include_bytes!("paper2607_data/chunk-0361-0405.zst"),
    include_bytes!("paper2607_data/chunk-0406-0450.zst"),
    include_bytes!("paper2607_data/chunk-0451-0495.zst"),
    include_bytes!("paper2607_data/chunk-0496-0540.zst"),
    include_bytes!("paper2607_data/chunk-0541-0585.zst"),
    include_bytes!("paper2607_data/chunk-0586-0630.zst"),
    include_bytes!("paper2607_data/chunk-0631-0675.zst"),
    include_bytes!("paper2607_data/chunk-0676-0720.zst"),
    include_bytes!("paper2607_data/chunk-0721-0765.zst"),
    include_bytes!("paper2607_data/chunk-0766-0810.zst"),
    include_bytes!("paper2607_data/chunk-0811-0855.zst"),
    include_bytes!("paper2607_data/chunk-0856-0900.zst"),
    include_bytes!("paper2607_data/chunk-0901-0945.zst"),
    include_bytes!("paper2607_data/chunk-0946-0990.zst"),
    include_bytes!("paper2607_data/chunk-0991-1035.zst"),
    include_bytes!("paper2607_data/chunk-1036-1080.zst"),
    include_bytes!("paper2607_data/chunk-1081-1125.zst"),
    include_bytes!("paper2607_data/chunk-1126-1170.zst"),
    include_bytes!("paper2607_data/chunk-1171-1215.zst"),
    include_bytes!("paper2607_data/chunk-1216-1260.zst"),
    include_bytes!("paper2607_data/chunk-1261-1305.zst"),
    include_bytes!("paper2607_data/chunk-1306-1350.zst"),
    include_bytes!("paper2607_data/chunk-1351-1395.zst"),
    include_bytes!("paper2607_data/chunk-1396-1440.zst"),
    include_bytes!("paper2607_data/chunk-1441-1485.zst"),
    include_bytes!("paper2607_data/chunk-1486-1530.zst"),
    include_bytes!("paper2607_data/chunk-1531-1575.zst"),
    include_bytes!("paper2607_data/chunk-1576-1616.zst"),
];
...
fn local_wires(core: &Core) -> Vec<&QReg> {
...
fn emit_forward(circ: &mut Circuit, core: &Core) {
    if count_stub_enabled(circ) {
        emit_count_stub(circ);
        return;
    }
    let wires = local_wires(core);
...
fn emit_reverse(circ: &mut Circuit, core: &Core) {
    if count_stub_enabled(circ) {
        emit_count_stub(circ);
        return;
    }
...
fn expose_terminal_shift_count(circ: &mut Circuit, core: &Core) {
    use super::register_shared_eea_microkernels::increment_mod_2n;

    increment_mod_2n(circ, &core.l_s, &core.aux[..SHIFT_WIDTH - 1]);
}

fn restore_terminal_shift_encoding(circ: &mut Circuit, terminal: &Terminal) {
    use super::register_shared_eea_microkernels::decrement_mod_2n_exact_reverse_increment;

    let carries = circ.alloc_qreg_bits("paper2607.terminal-shift-carries", SHIFT_WIDTH - 1);
    decrement_mod_2n_exact_reverse_increment(circ, &terminal.l_s, &carries);
    free_clean(circ, carries);
}
...
fn initialize(circ: &mut Circuit, mut dx: Vec<QReg>) -> Core {
    ...
    let l_rp = circ.alloc_qreg_bits("paper2607.l-rp", LENGTH_WIDTH);
    let source: Vec<&QReg> = dx.iter().take(VALUE_WIDTH).collect();
    bit_length_lean(circ, &source, &l_rp, false);
    let length_scratch = circ.alloc_qreg_bits("paper2607.length-decrement", LENGTH_WIDTH - 1);
    ...
    let l_t = circ.alloc_qreg_bits("paper2607.l-t", LENGTH_WIDTH);
    let l_q = circ.alloc_qreg_bits("paper2607.l-q", LENGTH_WIDTH);
    let l_s = circ.alloc_qreg_bits("paper2607.l-s", SHIFT_WIDTH);
    toggle_constant(circ, &l_q, (1 << LENGTH_WIDTH) - 1);
    toggle_constant(circ, &l_s, (1 << SHIFT_WIDTH) - 1);
...
fn release_terminal(circ: &mut Circuit, core: Core) -> Terminal {
    ...
    toggle_constant(circ, &core.l_q, (1 << LENGTH_WIDTH) - 1);
    free_clean(circ, core.l_q);
    toggle_constant(circ, &core.l_rp, (1 << LENGTH_WIDTH) - 1);
...
fn rebuild_terminal(circ: &mut Circuit, terminal: Terminal) -> Core {
    ...
    let l_t = circ.alloc_qreg_bits("paper2607.l-t.rebuilt", LENGTH_WIDTH);
    toggle_constant(circ, &l_t, VALUE_WIDTH - 1);
    let l_q = circ.alloc_qreg_bits("paper2607.l-q.rebuilt", LENGTH_WIDTH);
    toggle_constant(circ, &l_q, (1 << LENGTH_WIDTH) - 1);
    let l_rp = circ.alloc_qreg_bits("paper2607.l-rp.rebuilt", LENGTH_WIDTH);
    toggle_constant(circ, &l_rp, (1 << LENGTH_WIDTH) - 1);
    Core {
...
fn finish(circ: &mut Circuit, mut core: Core) -> Vec<QReg> {
    ...
    toggle_constant(circ, &core.l_q, (1 << LENGTH_WIDTH) - 1);
    free_clean(circ, core.l_q);
    toggle_constant(circ, &core.l_s, (1 << SHIFT_WIDTH) - 1);
    ...
    let length_scratch = circ.alloc_qreg_bits("paper2607.length-increment", LENGTH_WIDTH - 1);
...
pub fn divide_forward(
    ...
    use super::shrunken_pz_state_machine::{
        release_canonical_passenger_top, release_q955_canonical_lambda_top,
        restore_canonical_passenger_top, restore_q955_canonical_lambda_top,
    };
    use crate::point_add::trailmix_port::arith::rfold_mbu::mod_mul_canonical_mbu;

    assert_eq!(dx.len(), FIELD_WIDTH);
    assert_eq!(dy.len(), FIELD_WIDTH);
    let released_dy_top = release_canonical_passenger_top(circ, &mut dy, "paper2607 forward dy");
    let core = initialize(circ, dx);
    emit_forward(circ, &core);
    expose_terminal_shift_count(circ, &core);
    let mut terminal = release_terminal(circ, core);
    canonicalize_terminal_work2(circ, &terminal);
    toggle_inverse_sign(circ, &terminal);

    restore_canonical_passenger_top(circ, &mut dy, released_dy_top, "paper2607 forward dy");
    ...
    restore_terminal_shift_encoding(circ, &terminal);
...
pub fn divide_cancel(
    ...
    use super::shrunken_pz_state_machine::{
        release_canonical_passenger_top, restore_canonical_passenger_top,
    };
    ...
    let released_forward_dy_top =
        release_canonical_passenger_top(circ, &mut dy, "paper2607 cancel-forward dy");
    let core = initialize(circ, dx);
    emit_forward(circ, &core);
    expose_terminal_shift_count(circ, &core);
    let mut terminal = release_terminal(circ, core);
    ...
    restore_canonical_passenger_top(
        circ,
        &mut dy,
        released_forward_dy_top,
        "paper2607 cancel-forward dy",
    );
    ...
    restore_terminal_work2_rotation(circ, &terminal);
    restore_terminal_shift_encoding(circ, &terminal);
    let released_reverse_dy_top =
        release_canonical_passenger_top(circ, &mut dy, "paper2607 cancel-reverse dy");
    let core = rebuild_terminal(circ, terminal);
    emit_reverse(circ, &core);
    let dx = finish(circ, core);
    restore_canonical_passenger_top(
        circ,
        &mut dy,
        released_reverse_dy_top,
        "paper2607 cancel-reverse dy",
    );
    (dx, dy)
}
