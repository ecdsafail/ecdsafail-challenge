...
/// Explicit proof/profile override for the Q873 decrement stream.
pub const Q873_REVERSE_DECREMENT_STREAM_ENV: &str = "LOWQ_Q873_REVERSE_DECREMENT_STREAM";
...
pub const Q873_REVERSE_DECREMENT_STREAM_SOURCE_BAKE: bool = false;
...
#[must_use]
pub fn q873_production_decrement_stream() -> ReversibleDecrementStream {
    static STREAM: std::sync::OnceLock<ReversibleDecrementStream> = std::sync::OnceLock::new();

    *STREAM.get_or_init(|| {
        let enabled = match std::env::var(Q873_REVERSE_DECREMENT_STREAM_ENV) {
            Ok(value) => match value.as_str() {
                "0" => false,
                "1" => true,
                _ => panic!(
                    "{Q873_REVERSE_DECREMENT_STREAM_ENV} must be exactly 0 or 1, got {value:?}"
                ),
            },
            Err(std::env::VarError::NotPresent) => Q873_REVERSE_DECREMENT_STREAM_SOURCE_BAKE,
            Err(error) => panic!("invalid {Q873_REVERSE_DECREMENT_STREAM_ENV}: {error}"),
        };
        if enabled {
...
pub fn production_decrement_mod_2n(circ: &mut Circuit, register: &[QReg], scratch: &[QReg]) {
    decrement_mod_2n_with_stream(circ, register, scratch, q873_production_decrement_stream());
}
...
pub fn production_controlled_decrement_mod_2n(
    ...
    controlled_decrement_mod_2n_with_stream(
        ...
        register,
        scratch,
        q873_production_decrement_stream(),
    );
}
...
pub fn exhaustive_exact_reverse_decrement_check() -> RegisterSharedReverseDecrementProofReport {
    ...
    assert!(!Q873_REVERSE_DECREMENT_STREAM_SOURCE_BAKE);
