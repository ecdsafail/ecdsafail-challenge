...
/// Controlled add one modulo `2^n` using RESTORED DIRTY lenders instead of
/// clean carry lanes.
///
/// Semantics (identical to [`controlled_increment_mod_2n`]):
///   `register += control (mod 2^n)`.
///
/// Implementation: increment carries out of bit `j` exactly when `control` and
/// every lower bit `register[0..j]` is set, so
///   `register[j] ^= control AND register[0] AND ... AND register[j-1]`.
/// Emitting these high-to-low leaves each lower bit at its pre-increment value
/// while it is still read as a control. Each toggle is an `mcx_dirty_ladder`
/// over the borrowed `lenders`, which the double-Barenco cascade restores
/// exactly -- so the lenders may hold arbitrary (dirty) state. The widest toggle
/// has `1 + (width-1)` controls and therefore needs `width-1` lenders.
pub fn controlled_increment_mod_2n_dirty_lenders(
    circ: &mut Circuit,
    control: &QReg,
    register: &[QReg],
    lenders: &[&QReg],
    ...
    for j in (0..width).rev() {
        let mut controls: Vec<&QReg> = Vec::with_capacity(j + 1);
        controls.push(control);
        controls.extend(register[..j].iter());
        mcx_dirty_ladder(circ, &controls, &register[j], lenders);
    }
}

/// Controlled subtract one modulo `2^n` using RESTORED DIRTY lenders.
///
/// Semantics (identical to [`controlled_decrement_mod_2n`]):
///   `register -= control (mod 2^n)`.
///
/// Decrement borrows out of bit `j` exactly when `control` is set and every
/// lower bit `register[0..j]` is ZERO, so
///   `register[j] ^= control AND !register[0] AND ... AND !register[j-1]`.
/// The X-bracket turns the low bits into their complements, reusing the
/// increment carry shape; high-to-low order again preserves lower bits during
/// evaluation. Lenders are restored exactly.
pub fn controlled_decrement_mod_2n_dirty_lenders(
    circ: &mut Circuit,
    control: &QReg,
    register: &[QReg],
    lenders: &[&QReg],
    ...
    for j in (0..width).rev() {
        ...
        controls.extend(register[..j].iter());
        mcx_dirty_ladder(circ, &controls, &register[j], lenders);
        for q in register[..j].iter().rev() {
            circ.x(q);
        }
    }
}

/// Unconditional `+1 (mod 2^w)` on a register SLICE using restored dirty
/// lenders. Bit `j` flips iff every lower bit `slice[0..j]` is set (carry
/// propagation), emitted high-to-low so lower bits are still their pre-increment
/// value when read. `j == 0` flips the LSB unconditionally (`mcx_dirty_ladder`
/// with zero controls degenerates to `X`).
fn increment_subregister_dirty_lenders(
    circ: &mut Circuit,
    slice: &[&QReg],
    lenders: &[&QReg],
) {
    ...
    for j in (0..w).rev() {
        let controls: Vec<&QReg> = slice[..j].to_vec();
        mcx_dirty_ladder(circ, &controls, slice[j], lenders);
    }
}

/// Add a CLASSICAL constant `value (mod 2^width)` to `register` in place, using
/// restored dirty lenders and NO clean scratch buffer.
///
/// Modular addition is associative, so `register += value` decomposes into one
/// modular `+= 2^b` for every set bit `b` of `value`. Each `+= 2^b` is an
/// unconditional increment of the sub-slice `register[b..]`. Every toggle runs
/// on `mcx_dirty_ladder`, restoring the lenders exactly. The widest toggle (from
/// `b == 0`) has `width-1` controls, so `width-1` lenders suffice.
///
/// This replaces the `cuccaro_(sub|add)_mod_2n` constant path that required a
/// clean-|0> `width`-lane operand buffer (the swap-step cursor-scratch).
pub fn add_constant_mod_2n_dirty_lenders(
    circ: &mut Circuit,
    register: &[&QReg],
    value: usize,
    lenders: &[&QReg],
    ...
    let modulus = if width >= usize::BITS as usize {
        usize::MAX
    } else {
        (1usize << width) - 1
    };
    ...
    for b in 0..width {
        if (value >> b) & 1 != 0 {
            increment_subregister_dirty_lenders(circ, &register[b..], lenders);
        }
    }
}
