...
def t_tail_zero_toggle_gate(*, n: int, len_width: int, shift_width: int,
                            name: str = "T_TAIL_ZERO_S835_FAST",
                            lower_endpoint_prepared: bool = False) -> Gate:
    ...
    def transform_endpoints() -> None:
        ...
        if not lower_endpoint_prepared:
...
def t_tail_zero_toggle_prepared_gate(*, n: int, len_width: int, shift_width: int,
                                     name: str = "T_TAIL_ZERO_PREPARED_S835_FAST") -> Gate:
    """Toggle the dynamic tail predicate when l_t already stores A=l_t+2."""
    return t_tail_zero_toggle_gate(
        n=n,
        len_width=len_width,
        shift_width=shift_width,
        name=name,
        lower_endpoint_prepared=True,
    )


@lru_cache(maxsize=None)
def t_prefix_addsub_midpoint_gate(*, n: int, k: int, K: int,
                                  len_width: int, shift_width: int,
                                  mode: Literal["add", "sub"], name: str) -> Gate:
    """Prefix add/sub with exact tail and borrow/carry capture at its midpoint.

    After the first Cuccaro pass, l_t already holds the physical endpoint A,
    the selected Work1[A-1] lane is the lower borrow/carry, and Work2[A..B]
    is untouched.  The prepared tail scan can therefore share the clean unary
    path while the arithmetic carry and accumulator remain live on dedicated
    wires.  The second pass restores every temporary except Tail/Neg, which the
    paired T-add midpoint clears on valid Algorithm-3 states.
    """
    if k > K:
        raise ValueError("need k <= K")
    work_size = n + 3
    M = K - k + 1
    depth = _e.unary_depth(M)
    tail_gate = t_tail_zero_toggle_prepared_gate(
        n=n, len_width=len_width, shift_width=shift_width
    )
    tail_need = tail_gate.num_qubits - (
        2 + 2 * work_size + len_width + shift_width + len_width
    )
    shared_size = max(tail_need, depth, len_width)
    scratch_size = shared_size + 3
    ...
    qc = _e._block_circuit(
        ArithCtrl, TailCtrl, Tail, Neg, Work1, Work2, l_t, l_s, l_rp,
        Scratch, name=name,
    )
    path = list(Scratch[:depth])
    carry = Scratch[shared_size]
    acc = Scratch[shared_size + 1]
    cell_tmp = Scratch[shared_size + 2]
    const_scratch = list(Scratch[:len_width]) + [carry]
    ...
    qc.cx(ArithCtrl[0], acc)

    def leaf_first(j: int, ej: Qubit) -> None:
        addend, target = qpair(j)
        carry_in = carry if j == k else Work1[j - 2]
        ctrl = ArithCtrl[0] if j == k else acc
        _apply_cell_dirty(qc, mode, "first", ctrl, addend, target, carry_in, cell_tmp)
        # At the selected endpoint this is exactly the lower subtraction
        # borrow, or the restoring-add carry.  ej already includes ArithCtrl.
        qc.ccx(ej, addend, Neg[0])
        qc.cx(ej, acc)
    ...
    tail_args = (
        [TailCtrl[0], Tail[0]] + list(Work1) + list(Work2)
        + list(l_t) + list(l_s) + list(l_rp) + list(Scratch[:tail_need])
    )
    ...
    def leaf_second(j: int, ej: Qubit) -> None:
        ...
        carry_in = carry if j == k else Work1[j - 2]
        ctrl = ArithCtrl[0] if j == k else acc
        _apply_cell_dirty(qc, mode, "second", ctrl, addend, target, carry_in, cell_tmp)
...
def append_one_step_T(qc: QuantumCircuit, *, T: int, n: int, len_width: int, shift_width: int,
    ...
    _make_condition(qc, [(Phase1[0], 0), (Phase2[0], 1)], ctrl, scratch)
    # Compute the mathematical underflow at the arithmetic midpoints.  The
    # first pass has already prepared A=l_t+2 and exposes the selected lower
    # borrow/carry in Work1[A-1], so the tail scan shares its clean unary path.
    tail_zero = scratch[-2]
    neg = scratch[-1]
    ...
    _make_condition(qc, [(Phase2[0], 0), (Sign[0], 1)], tmp, scratch[1:])
    tsub = t_prefix_addsub_midpoint_gate(
        n=n, k=k3, K=K3, len_width=len_width, shift_width=shift_width,
        mode="sub", name="T_SUB_MIDPOINT_S835_FAST",
    )
    tsub_need = tsub.num_qubits - (4 + 2 * work_size + 2 * len_width + shift_width)
    _e._append_with_optional_clbits(
        qc, tsub,
        [ctrl, Phase1[0], tail_zero, neg] + list(Work1) + list(Work2)
        + list(l_t) + list(l_s) + list(l_rp) + scratch[:-2][:tsub_need],
    )
    _make_condition(qc, [(Phase2[0], 0), (Sign[0], 1)], tmp, scratch[1:])
    _make_condition(qc, [(Phase1[0], 1), (tmp, 0)], ctrl, scratch[1:])
    _make_condition(qc, [(Phase2[0], 0), (Sign[0], 1)], tmp, scratch[1:])
    qc.cx(Phase1[0], Sign[0])
    qc.ccx(tail_zero, neg, Sign[0])
    _make_condition(qc, [(Phase1[0], 1)], ctrl, scratch)
    tadd = t_prefix_addsub_midpoint_gate(
        n=n, k=k3, K=K3, len_width=len_width, shift_width=shift_width,
        mode="add", name="T_ADD_MIDPOINT_S835_FAST",
    )
    tadd_need = tadd.num_qubits - (4 + 2 * work_size + 2 * len_width + shift_width)
    _e._append_with_optional_clbits(
        qc, tadd,
        [ctrl, Phase1[0], tail_zero, neg] + list(Work1) + list(Work2)
        + list(l_t) + list(l_s) + list(l_rp) + scratch[:-2][:tadd_need],
    )
    _make_condition(qc, [(Phase1[0], 1)], ctrl, scratch)
    # Post-shift
