...
impl Q845SwapOnlyCoefficientGuard {
        ...
        F: FnMut(&mut Circuit, usize, &QReg),
    {
        ...
        // `active` is a live lane that is not a detector control (those are the
        // `address` counter bits) and not the internal gate, so it is a valid
        // dirty lender for the detector MCX: borrowing it removes the peak-top
        // clean ancilla. `mcx_dirty` restores it before the body's cx reads it.
        ...
        F: FnMut(&mut Circuit, usize, &QReg),
    {
        ...
        let address = self.address.iter().collect::<Vec<_>>();
        // See `for_each_forward`: `active` is a valid dirty lender (not a
        // detector control, not the gate), restored by `mcx_dirty` each edge.
        unary_iterate_log_star_borrowed(
            circ,
            &address,
            if truncated {
