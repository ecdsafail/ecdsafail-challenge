...
pub const Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV: &str =
    "LOWQ_Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT";
pub const Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_SOURCE_BAKE: bool = false;

fn q873_paired_bitlen_source_complement_requested() -> bool {
    match std::env::var(Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV) {
        Ok(value) => match value.as_str() {
            "0" => false,
            "1" => true,
            _ => panic!(
                "{Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV} must be exactly 0 or 1, got {value:?}"
            ),
        },
        Err(std::env::VarError::NotPresent) => {
            Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_SOURCE_BAKE
        }
        Err(error) => panic!(
            "invalid {Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV}: {error}"
        ),
    }
}

/// Explicit proof/profile override for the Q873 equality-mask sweep.
pub const Q873_CHAINED_EQUALITY_MASK_ENV: &str = "LOWQ_Q873_CHAINED_EQUALITY_MASK";

/// Source-bake point for equality-mask chaining.
///
/// The production route sets this feature's default before generation. Keep
/// the lower-level fallback off so focused proof binaries can test both modes.
pub const Q873_CHAINED_EQUALITY_MASK_SOURCE_BAKE: bool = false;

/// One forward and one inverse sweep occur in each of the two production
/// register-shared divisions.
pub const Q873_PRODUCTION_EQUALITY_MASK_SWEEPS: usize = 4;

#[must_use]
pub fn q873_chained_equality_mask_requested() -> bool {
    match std::env::var(Q873_CHAINED_EQUALITY_MASK_ENV) {
        Ok(value) => match value.as_str() {
            "0" => false,
            "1" => true,
            _ => panic!("{Q873_CHAINED_EQUALITY_MASK_ENV} must be exactly 0 or 1, got {value:?}"),
        },
        Err(std::env::VarError::NotPresent) => Q873_CHAINED_EQUALITY_MASK_SOURCE_BAKE,
        Err(error) => panic!("invalid {Q873_CHAINED_EQUALITY_MASK_ENV}: {error}"),
    }
}

/// Explicit proof/profile override for the coefficient nonnegative-predicate cut.
pub const Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL_ENV: &str =
    "LOWQ_Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL";

/// The production route sets this feature's default before generation. Keep
/// the lower-level fallback off so focused proof binaries can test both modes.
pub const Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL_SOURCE_BAKE: bool = false;

/// Explicit proof/profile override for the coefficient compare/arithmetic fusion.
pub const Q873_FUSED_COEFFICIENT_COMPARE_ENV: &str = "LOWQ_Q873_FUSED_COEFFICIENT_COMPARE";

/// The zero-spill packed-window theorem is source-baked into production.
pub const Q873_FUSED_COEFFICIENT_COMPARE_SOURCE_BAKE: bool = true;

#[must_use]
pub fn q873_fused_coefficient_compare_requested() -> bool {
    Q873_FUSED_COEFFICIENT_COMPARE_SOURCE_BAKE
}
...
fn production_coefficient_nonnegative_bracket() -> CoefficientNonnegativeBracket {
    static BRACKET: std::sync::OnceLock<CoefficientNonnegativeBracket> = std::sync::OnceLock::new();

    *BRACKET.get_or_init(|| {
        let enabled = match std::env::var(Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL_ENV) {
            Ok(value) => match value.as_str() {
                "0" => false,
                "1" => true,
                _ => panic!(
                    "{Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL_ENV} must be exactly 0 or 1, got {value:?}"
                ),
            },
            Err(std::env::VarError::NotPresent) => {
                Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL_SOURCE_BAKE
            }
            Err(error) => panic!(
                "invalid {Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL_ENV}: {error}"
            ),
        };
        if enabled {
...
#[must_use]
pub fn q873_coefficient_nonnegative_x_cancel_enabled() -> bool {
    production_coefficient_nonnegative_bracket() == CoefficientNonnegativeBracket::CancelMiddleX
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Q873EqualityMaskSweepAudit {
    pub steps: usize,
    pub length_width: usize,
    pub scheduled_indices: usize,
    pub zero_mask_bit_occurrences: usize,
    pub legacy_mask_x: usize,
    pub chained_forward_mask_x: usize,
    pub chained_inverse_mask_x: usize,
    pub x_removed_per_sweep: usize,
    pub production_sweeps: usize,
    pub production_x_removed: usize,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Q873EqualityMaskChainingProofReport {
    pub sweep_parameter_sets_checked: usize,
    pub sweep_basis_states_checked: usize,
    pub location_work_widths_checked: usize,
    pub location_basis_states_checked: usize,
    pub control_states_checked: usize,
    pub legacy_chained_permutation_checks: usize,
    pub forward_inverse_pair_checks: usize,
    pub boundary_restoration_checks: usize,
    pub scratch_clean_checks: usize,
    pub phase_clean_streams_checked: usize,
    pub transition_arithmetic_checks: usize,
    pub default_off_stream_identity_checks: usize,
    pub audit: Q873EqualityMaskSweepAudit,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Q873CoefficientNonnegativeXCancelProofReport {
    pub cursor_widths_checked: usize,
    pub bracket_body_kinds_checked: usize,
    pub body_target_safety_checks: usize,
    pub bracket_basis_states_checked: usize,
    pub predicate_cases_checked: usize,
    pub legacy_candidate_equivalence_checks: usize,
    pub control_off_identity_checks: usize,
    pub control_on_checks: usize,
    pub forward_inverse_composition_checks: usize,
    pub cursor_preservation_checks: usize,
    pub sign_preservation_checks: usize,
    pub phase_clean_checks: usize,
    pub ancilla_clean_checks: usize,
    pub kernel_work_widths_checked: usize,
    pub kernel_basis_states_checked: usize,
    pub kernel_equivalence_checks: usize,
    pub kernel_inverse_pair_checks: usize,
    pub default_off_stream_identity_checks: usize,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Q873FusedCoefficientCompareProofReport {
    pub inherited_count_only_counterexample_checks: usize,
    pub guard_length_widths_checked: usize,
    pub guard_predicate_basis_states_checked: usize,
    pub guard_predicate_scratch_clean_checks: usize,
    ...
    pub scratch_clean_checks: usize,
    pub source_baked_stream_identity_checks: usize,
}
...
fn equality_zero_mask_weight(width: usize, value: usize) -> usize {
    (0..width).filter(|&bit| ((value >> bit) & 1) == 0).count()
}

fn equality_mask_transition_weight(width: usize, from: usize, to: usize) -> usize {
    (0..width)
        .filter(|&bit| ((from ^ to) >> bit) & 1 != 0)
        .count()
}

#[must_use]
pub fn q873_equality_mask_sweep_audit() -> Q873EqualityMaskSweepAudit {
    let mut scheduled_indices = 0usize;
    let mut zero_mask_bit_occurrences = 0usize;
    let mut chained_forward_mask_x = 0usize;
    let mut chained_inverse_mask_x = 0usize;

    for step in 1..=REFERENCE_STEPS {
        let window = reference_active_windows(256, step).quotient_swap;
        let first = window.0 - 1;
        let end = window.1;
        let last = end - 1;
        scheduled_indices += end - first;
        zero_mask_bit_occurrences += (first..end)
            .map(|index| equality_zero_mask_weight(REFERENCE_LENGTH_WIDTH, index))
            .sum::<usize>();

        chained_forward_mask_x += equality_zero_mask_weight(REFERENCE_LENGTH_WIDTH, first);
        chained_forward_mask_x += (first..last)
            .map(|index| equality_mask_transition_weight(REFERENCE_LENGTH_WIDTH, index, index + 1))
            .sum::<usize>();
        chained_forward_mask_x += equality_zero_mask_weight(REFERENCE_LENGTH_WIDTH, last);

        chained_inverse_mask_x += equality_zero_mask_weight(REFERENCE_LENGTH_WIDTH, last);
        chained_inverse_mask_x += (first..last)
            .rev()
            .map(|index| equality_mask_transition_weight(REFERENCE_LENGTH_WIDTH, index + 1, index))
            .sum::<usize>();
        chained_inverse_mask_x += equality_zero_mask_weight(REFERENCE_LENGTH_WIDTH, first);
    }

    assert_eq!(chained_forward_mask_x, chained_inverse_mask_x);
    let legacy_mask_x = 4 * zero_mask_bit_occurrences;
    let x_removed_per_sweep = legacy_mask_x - chained_forward_mask_x;
    Q873EqualityMaskSweepAudit {
        steps: REFERENCE_STEPS,
        length_width: REFERENCE_LENGTH_WIDTH,
        scheduled_indices,
        zero_mask_bit_occurrences,
        legacy_mask_x,
        chained_forward_mask_x,
        chained_inverse_mask_x,
        x_removed_per_sweep,
        production_sweeps: Q873_PRODUCTION_EQUALITY_MASK_SWEEPS,
        production_x_removed: Q873_PRODUCTION_EQUALITY_MASK_SWEEPS * x_removed_per_sweep,
    }
}
...
fn toggle_equality_zero_mask(circ: &mut Circuit, register: &[QReg], value: usize) {
    for (index, bit) in register.iter().enumerate() {
        if ((value >> index) & 1) == 0 {
            circ.x(bit);
        }
...
fn transition_equality_zero_mask(circ: &mut Circuit, register: &[QReg], from: usize, to: usize) {
    for (index, bit) in register.iter().enumerate() {
        if (((from ^ to) >> index) & 1) != 0 {
            circ.x(bit);
        }
    }
}

fn toggle_complemented_equality_flag(
    circ: &mut Circuit,
    control: &QReg,
    register: &[QReg],
    flag: &QReg,
...
fn equality_flag(
    ...
    toggle_equality_zero_mask(circ, register, value);
    toggle_complemented_equality_flag(circ, control, register, flag, chain);
    toggle_equality_zero_mask(circ, register, value);
}

#[allow(clippy::too_many_arguments)]
fn controlled_dynamic_swaps_legacy(
    circ: &mut Circuit,
    active: &QReg,
    sign: &QReg,
    work1_window: &[QReg],
...
#[allow(clippy::too_many_arguments)]
fn controlled_dynamic_swaps_chained(
    circ: &mut Circuit,
    active: &QReg,
    sign: &QReg,
    work1_window: &[QReg],
    ...
    if work1_window.is_empty() {
        return;
    }

    let first_local_index = if reverse { work1_window.len() - 1 } else { 0 };
    let mut current_global_index = first_global_index + first_local_index;
    toggle_equality_zero_mask(circ, sum, current_global_index);

    if reverse {
        for (local_index, work_bit) in work1_window.iter().enumerate().rev() {
            let global_index = first_global_index + local_index;
            debug_assert_eq!(global_index, current_global_index);
            toggle_complemented_equality_flag(circ, active, sum, flag, chain);
            circ.cswap(flag, work_bit, sign);
            toggle_complemented_equality_flag(circ, active, sum, flag, chain);
            if local_index != 0 {
                let next_global_index = global_index - 1;
                transition_equality_zero_mask(circ, sum, global_index, next_global_index);
                current_global_index = next_global_index;
            }
        }
    } else {
        for (local_index, work_bit) in work1_window.iter().enumerate() {
            ...
            circ.cswap(flag, work_bit, sign);
            toggle_complemented_equality_flag(circ, active, sum, flag, chain);
            if local_index + 1 != work1_window.len() {
                let next_global_index = global_index + 1;
                transition_equality_zero_mask(circ, sum, global_index, next_global_index);
                current_global_index = next_global_index;
            }
...
fn controlled_dynamic_swaps(
    ...
    if chained {
        controlled_dynamic_swaps_chained(
            circ,
            active,
            sign,
            work1_window,
        ...
        controlled_dynamic_swaps_legacy(
            circ,
            active,
            sign,
            work1_window,
...
pub fn location_controlled_swap_one_hot(
    ...
    location_controlled_swap_one_hot_with_mask_mode(
        circ,
        phase1,
        phase2,
        sign,
        ...
        l_q,
        scratch,
        q873_chained_equality_mask_requested(),
    );
}

#[allow(clippy::too_many_arguments)]
fn location_controlled_swap_one_hot_with_mask_mode(
    circ: &mut Circuit,
    phase1: &QReg,
    phase2: &QReg,
    sign: &QReg,
...
pub fn location_controlled_swap_one_hot_inverse(
    ...
    location_controlled_swap_one_hot_inverse_with_mask_mode(
        circ,
        phase1,
        phase2,
        sign,
...
#[allow(clippy::too_many_arguments)]
fn location_controlled_swap_one_hot_inverse_with_mask_mode(
    circ: &mut Circuit,
    phase1: &QReg,
    phase2: &QReg,
    sign: &QReg,
...
fn toggle_control_and_nonnegative_legacy(
    circ: &mut Circuit,
    control: &QReg,
    signed_length: &[QReg],
    active: &QReg,
...
fn begin_control_and_nonnegative_bracket(
    circ: &mut Circuit,
    control: &QReg,
    signed_length: &[QReg],
    active: &QReg,
    ...
    if bracket == CoefficientNonnegativeBracket::Legacy {
        toggle_control_and_nonnegative_legacy(circ, control, signed_length, active);
        return;
    }
...
fn end_control_and_nonnegative_bracket(
    circ: &mut Circuit,
    control: &QReg,
    signed_length: &[QReg],
    active: &QReg,
...
pub fn coefficient_add_single(
    ...
    coefficient_add_single_with_bracket(
        circ,
        phase1,
        sign,
        work1,
        ...
        l_t,
        scratch,
        production_coefficient_nonnegative_bracket(),
    );
}

#[allow(clippy::too_many_arguments)]
fn coefficient_add_single_with_bracket(
    circ: &mut Circuit,
    phase1: &QReg,
    sign: &QReg,
    work1: &[QReg],
    ...
    for index in 0..work1.len() {
        begin_control_and_nonnegative_bracket(circ, phase1, l_t, active, bracket);
        emit_q873_coefficient_bracket_body(
            circ,
            Q873CoefficientBracketBody::AddForward,
            active,
            carry,
            &work1[index],
            &work2[index],
        ...
        end_control_and_nonnegative_bracket(circ, phase1, l_t, active, bracket);
    ...
    for index in (0..work1.len()).rev() {
        ...
        emit_q873_coefficient_bracket_body(
            circ,
            Q873CoefficientBracketBody::AddReverse,
            active,
            carry,
            &work1[index],
            &work2[index],
...
pub fn coefficient_add_single_inverse(
    ...
    coefficient_add_single_inverse_with_bracket(
        circ,
        phase1,
        sign,
        work1,
...
#[allow(clippy::too_many_arguments)]
fn coefficient_add_single_inverse_with_bracket(
    circ: &mut Circuit,
    phase1: &QReg,
    sign: &QReg,
    work1: &[QReg],
    ...
    for index in 0..work1.len() {
        ...
        emit_q873_coefficient_bracket_body(
            circ,
            Q873CoefficientBracketBody::SubForward,
            active,
            carry,
            &work1[index],
            &work2[index],
    ...
    for index in (0..work1.len()).rev() {
        ...
        emit_q873_coefficient_bracket_body(
            circ,
            Q873CoefficientBracketBody::SubReverse,
            active,
            carry,
            &work1[index],
            &work2[index],
...
pub fn coefficient_sub_single(
    ...
    coefficient_sub_single_with_bracket(
        circ,
        phase1,
        phase2,
        sign,
...
#[allow(clippy::too_many_arguments)]
fn coefficient_sub_single_with_bracket(
    circ: &mut Circuit,
    phase1: &QReg,
    phase2: &QReg,
    sign: &QReg,
    ...
    for index in 0..work1.len() {
        begin_control_and_nonnegative_bracket(circ, enable, l_t, active, bracket);
        emit_q873_coefficient_bracket_body(
            circ,
            Q873CoefficientBracketBody::SubForward,
            active,
        ...
        end_control_and_nonnegative_bracket(circ, enable, l_t, active, bracket);
        if index + 1 != work1.len() {
            decrement_mod_2n(circ, l_t, length_chain);
        }
    }
...
pub fn coefficient_sub_single_inverse(
    ...
    coefficient_sub_single_inverse_with_bracket(
        circ,
        phase1,
        phase2,
        sign,
...
#[allow(clippy::too_many_arguments)]
fn coefficient_sub_single_inverse_with_bracket(
    circ: &mut Circuit,
    phase1: &QReg,
    phase2: &QReg,
    sign: &QReg,
...
fn controlled_xor_saturating_bit_length_difference(
    ...
    use super::shrunken_pz_state_machine::{
        bit_length_lean_complemented_source, lowq_fused_zero_prefix_bitlen_requested,
    };
    ...
    // Baseline is X_S K_add X_S V X_S K_sub X_S. The USE block V is
    // source-disjoint (enforced below), so X_S V X_S = V and the middle pair
    // cancels exactly: X_S K_add V K_sub X_S.
    let paired_source_complement = q873_paired_bitlen_source_complement_requested();
    if paired_source_complement {
        assert!(
            lowq_fused_zero_prefix_bitlen_requested(),
            "paired bit-length source complement requires fused zero-prefix bit length"
        );
        for source_lane in source {
            assert_ne!(source_lane.id(), control.id(), "source aliases control");
            for body_lane in boundary.iter().chain(output) {
                assert_ne!(
                    source_lane.id(),
                    body_lane.id(),
                    "source aliases paired bit-length USE lane"
                );
            }
            circ.x(source_lane);
        }
    ...
    if paired_source_complement {
        bit_length_lean_complemented_source(circ, source, &length, false);
    } else {
    ...
    if paired_source_complement {
        bit_length_lean_complemented_source(circ, source, &length, true);
        for source_lane in source {
            circ.x(source_lane);
        }
    } else {
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct Q873PairedBitLengthLocalResources {
    pub extra_qubits: usize,
    pub emitted_ops: usize,
    pub emitted_x: usize,
    pub emitted_toffoli: usize,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Q873PairedBitLengthSourceComplementProofReport {
    pub widths_checked: usize,
    ...
    pub local_baseline: Q873PairedBitLengthLocalResources,
    pub local_optimized: Q873PairedBitLengthLocalResources,
}

struct Q873PairedBitLengthProofCircuit {
    builder: B,
    source_ids: Vec<u32>,
    control_id: u32,
...
fn build_q873_paired_bitlength_proof_circuit(
    source_width: usize,
    boundary_width: usize,
    enabled: Option<bool>,
) -> Q873PairedBitLengthProofCircuit {
    std::env::set_var("LOWQ_FUSED_ZERO_PREFIX_BITLEN", "1");
    match enabled {
        Some(true) => std::env::set_var(Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV, "1"),
        Some(false) => std::env::set_var(Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV, "0"),
        None => std::env::remove_var(Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV),
    }
    let mut circuit = Circuit::new();
    let source = circuit.alloc_qreg_bits("q873-paired-bitlen.source", source_width);
    let control = circuit.alloc_qreg("q873-paired-bitlen.control");
    let boundary = circuit.alloc_qreg_bits("q873-paired-bitlen.boundary", boundary_width);
    let output = circuit.alloc_qreg_bits("q873-paired-bitlen.output", boundary_width);
    let source_refs: Vec<&QReg> = source.iter().collect();
...
fn q873_paired_bitlength_resources(
    circuit: &Q873PairedBitLengthProofCircuit,
) -> Q873PairedBitLengthLocalResources {
    use crate::circuit::OperationType;

    let external = circuit.source_ids.len() + circuit.boundary_ids.len()
        + circuit.output_ids.len() + 1;
    Q873PairedBitLengthLocalResources {
        extra_qubits: circuit.builder.peak_qubits as usize - external,
        emitted_ops: circuit.builder.ops.len(),
        emitted_x: circuit.builder.counted_kind_ops[OperationType::X as usize],
        emitted_toffoli: circuit.builder.counted_kind_ops[OperationType::CCX as usize]
            + circuit.builder.counted_kind_ops[OperationType::CCZ as usize],
    }
}

fn q873_paired_bitlength_external_ids(
    circuit: &Q873PairedBitLengthProofCircuit,
) -> Vec<u32> {
    circuit.source_ids.iter()
        .chain(std::iter::once(&circuit.control_id))
        .chain(&circuit.boundary_ids)
        .chain(&circuit.output_ids)
        .copied()
        .collect()
}

fn q873_assert_paired_bitlength_internal_clean<R: sha3::digest::XofReader>(
    simulator: &crate::sim::Simulator<'_, R>,
    circuit: &Q873PairedBitLengthProofCircuit,
    live: u64,
    shots: usize,
    ...
    let external = q873_paired_bitlength_external_ids(circuit);
    ...
    for id in 0..circuit.builder.next_qubit {
        if !external.contains(&id) {
            assert_eq!(
                simulator.qubit(QubitId(u64::from(id))) & live,
                0,
                "paired bit-length proof left internal q{id} dirty"
            );
            lanes += 1;
        }
    }
...
fn verify_q873_paired_bitlength_equivalence(
    baseline: &Q873PairedBitLengthProofCircuit,
    optimized: &Q873PairedBitLengthProofCircuit,
    source_width: usize,
    boundary_width: usize,
    ...
    assert_eq!(baseline.source_ids, optimized.source_ids);
    assert_eq!(baseline.control_id, optimized.control_id);
    assert_eq!(baseline.boundary_ids, optimized.boundary_ids);
    assert_eq!(baseline.output_ids, optimized.output_ids);
    let state_bits = source_width + 1 + 2 * boundary_width;
    let cases = 1usize << state_bits;
    let source_mask = if source_width == 0 { 0 } else { (1u64 << source_width) - 1 };
    let value_mask = (1u64 << boundary_width) - 1;
    let mut ancilla_checks = 0usize;

    for (batch, first) in (0..cases).step_by(64).enumerate() {
        let count = (cases - first).min(64);
        let mut seed = Shake128::default();
        seed.update(b"q873-paired-bitlength-source-complement");
        seed.update(&(source_width as u64).to_le_bytes());
        seed.update(&(batch as u64).to_le_bytes());
        let mut baseline_xof = seed.clone().finalize_xof();
        let mut optimized_xof = seed.finalize_xof();
        let mut baseline_simulator = Simulator::new(
            baseline.builder.next_qubit as usize,
            baseline.builder.next_bit as usize,
            &mut baseline_xof,
        ...
        for shot in 0..count {
            let case = (first + shot) as u64;
            let source = case & source_mask;
            let control = (case >> source_width) & 1;
            let boundary = (case >> (source_width + 1)) & value_mask;
            let output = (case >> (source_width + 1 + boundary_width)) & value_mask;
            for (bit, &id) in baseline.source_ids.iter().enumerate() {
                if (source >> bit) & 1 == 1 {
                    *baseline_simulator.qubit_mut(QubitId(u64::from(id))) |= 1u64 << shot;
                    *optimized_simulator.qubit_mut(QubitId(u64::from(id))) |= 1u64 << shot;
                }
            }
            if control == 1 {
                *baseline_simulator.qubit_mut(QubitId(u64::from(baseline.control_id))) |=
                    1u64 << shot;
                *optimized_simulator.qubit_mut(QubitId(u64::from(optimized.control_id))) |=
                    1u64 << shot;
            }
            for (value, ids) in [
                (boundary, &baseline.boundary_ids),
                (output, &baseline.output_ids),
            ] {
        ...
        let live = if count == 64 { u64::MAX } else { (1u64 << count) - 1 };
        assert_eq!(baseline_simulator.phase & live, 0);
        ...
        for shot in 0..count {
            ...
            let boundary = (case >> (source_width + 1)) & value_mask;
            let output = (case >> (source_width + 1 + boundary_width)) & value_mask;
            let bit_length = if source == 0 { 0 } else { u64::from(64 - source.leading_zeros()) };
            let expected = output ^ if control == 1 {
                bit_length.saturating_sub(boundary)
            } else {
                0
            };
            for (simulator, circuit) in [
                (&baseline_simulator, baseline),
                (&optimized_simulator, optimized),
            ] {
                assert_eq!(read_fused_zero_prefix_register(
                    simulator, &circuit.source_ids, shot), source);
                assert_eq!((simulator.qubit(QubitId(u64::from(circuit.control_id))) >> shot) & 1,
                    control);
                assert_eq!(read_fused_zero_prefix_register(
                    simulator, &circuit.boundary_ids, shot), boundary);
                assert_eq!(read_fused_zero_prefix_register(
                    simulator, &circuit.output_ids, shot), expected);
            }
        }
        ancilla_checks += q873_assert_paired_bitlength_internal_clean(
            &baseline_simulator, baseline, live, count);
        ancilla_checks += q873_assert_paired_bitlength_internal_clean(
            &optimized_simulator, optimized, live, count);
    }
    (cases, 2 * cases, cases, 2 * cases, 2 * cases, ancilla_checks)
}

/// Exhaustively prove reduced-width equivalence and cleanup for retaining the
/// source complement across a bit-length compute/use/uncompute pair.
#[doc(hidden)]
#[must_use]
pub fn q873_paired_bitlength_source_complement_check(
) -> Q873PairedBitLengthSourceComplementProofReport {
    use crate::circuit::OperationType;

    const MAX_SOURCE_WIDTH: usize = 5;
    const BOUNDARY_WIDTH: usize = 3;
    const LOCAL_SOURCE_WIDTH: usize = 259;
    const LOCAL_BOUNDARY_WIDTH: usize = 9;
    let saved_fused = std::env::var_os("LOWQ_FUSED_ZERO_PREFIX_BITLEN");
    let saved_feature = std::env::var_os(Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV);
    ...
    for source_width in 0..=MAX_SOURCE_WIDTH {
        let default = build_q873_paired_bitlength_proof_circuit(
            source_width, BOUNDARY_WIDTH, None);
        let baseline = build_q873_paired_bitlength_proof_circuit(
            source_width, BOUNDARY_WIDTH, Some(false));
        let optimized = build_q873_paired_bitlength_proof_circuit(
            source_width, BOUNDARY_WIDTH, Some(true));
        assert_eq!(default.builder.ops, baseline.builder.ops);
        assert_eq!(default.builder.next_qubit, baseline.builder.next_qubit);
        assert_eq!(default.builder.next_bit, baseline.builder.next_bit);
        ...
        for kind in 0..baseline.builder.counted_kind_ops.len() {
            if kind != OperationType::X as usize {
                assert_eq!(baseline.builder.counted_kind_ops[kind],
                    optimized.builder.counted_kind_ops[kind]);
            }
        }
        assert_eq!(baseline.builder.ops.len() - optimized.builder.ops.len(),
            2 * source_width);
        assert_eq!(baseline.builder.counted_kind_ops[OperationType::X as usize]
            - optimized.builder.counted_kind_ops[OperationType::X as usize],
            2 * source_width);
        let (cases, oracles, equivalents, restores, phases, ancillas) =
            verify_q873_paired_bitlength_equivalence(
                &baseline, &optimized, source_width, BOUNDARY_WIDTH);
        basis_states_checked += cases;
        oracle_checks += oracles;
        equivalence_checks += equivalents;
        source_restore_checks += restores;
        phase_clean_checks += phases;
        ancilla_clean_checks += ancillas;
    }

    let local_baseline = q873_paired_bitlength_resources(
        &build_q873_paired_bitlength_proof_circuit(
            LOCAL_SOURCE_WIDTH, LOCAL_BOUNDARY_WIDTH, Some(false)));
    let local_optimized = q873_paired_bitlength_resources(
        &build_q873_paired_bitlength_proof_circuit(
            LOCAL_SOURCE_WIDTH, LOCAL_BOUNDARY_WIDTH, Some(true)));
    assert_eq!(local_baseline.extra_qubits, local_optimized.extra_qubits);
    assert_eq!(local_baseline.emitted_ops - local_optimized.emitted_ops,
        2 * LOCAL_SOURCE_WIDTH);
    assert_eq!(local_baseline.emitted_x - local_optimized.emitted_x,
        2 * LOCAL_SOURCE_WIDTH);
    assert_eq!(local_baseline.emitted_toffoli, local_optimized.emitted_toffoli);
    match saved_fused {
        Some(value) => std::env::set_var("LOWQ_FUSED_ZERO_PREFIX_BITLEN", value),
        None => std::env::remove_var("LOWQ_FUSED_ZERO_PREFIX_BITLEN"),
    }
    match saved_feature {
        Some(value) => std::env::set_var(Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV, value),
        None => std::env::remove_var(Q873_PAIRED_BITLEN_SOURCE_COMPLEMENT_ENV),
    }
    Q873PairedBitLengthSourceComplementProofReport {
        widths_checked: MAX_SOURCE_WIDTH + 1,
        ...
        oracle_checks,
        baseline_equivalence_checks: equivalence_checks,
...
/// Toggle `target` when `control` is set and the cached target magnitude is
/// wider than the shifted source plus its arithmetic guard bit.
///
/// The fused coefficient pass runs the Cuccaro majority half across the source
/// support and one zero source bit. A borrow leaving that guard is a global
/// underflow unless the target has still higher support, which this predicate
/// records.
fn toggle_coefficient_length_above_guarded_boundary(
    circ: &mut Circuit,
    control: &QReg,
    target_length: &[QReg],
    l_t: &[QReg],
    ...
    let length_width = l_t.len();
    let wide = length_width + 1;
    let difference = circ.alloc_qreg_bits("rs.coeff-fused.difference", wide);
    let boundary = circ.alloc_qreg_bits("rs.coeff-fused.boundary", wide);
    for (source, destination) in target_length.iter().zip(&difference) {
        circ.cx(source, destination);
    }
    ...
    let boundary_carry = circ.alloc_qreg("rs.coeff-fused.boundary-carry");
    let boundary_overflow = circ.alloc_qreg("rs.coeff-fused.boundary-overflow");
    cuccaro_add_mod_2n(
        circ,
        l_s,
    ...
    let predicate_chain =
        circ.alloc_qreg_bits("rs.coeff-fused.predicate-chain", wide.saturating_sub(2));
    controlled_add_one(circ, &[], &boundary, &predicate_chain);

    let compare_carry = circ.alloc_qreg("rs.coeff-fused.compare-carry");
    let compare_overflow = circ.alloc_qreg("rs.coeff-fused.compare-overflow");
    cuccaro_sub_mod_2n(
        circ,
        &boundary,
        &difference,
    ...
    let nonzero = circ.alloc_qreg("rs.coeff-fused.nonzero");
    compute_nonzero(circ, &difference, &nonzero, &predicate_chain);
    let sign = &difference[wide - 1];
    circ.x(sign);
    multi_controlled_x_vchain(circ, &[control, &nonzero, sign], target, target_chain);
    circ.x(sign);
    uncompute_nonzero(circ, &difference, &nonzero, &predicate_chain);
    circ.zero_and_free(nonzero);
    cuccaro_add_mod_2n(
        circ,
        &boundary,
    ...
    controlled_sub_one(circ, &[], &boundary, &predicate_chain);
    free_clean(circ, predicate_chain);
    cuccaro_sub_mod_2n(
        circ,
        l_s,
        &boundary[..length_width],
...
fn toggle_coefficient_less_than(
    ...
    for index in 0..work1.len() {
        begin_control_and_nonnegative_bracket(circ, control, &cursor, &active, bracket);
        emit_q873_coefficient_bracket_body(
            circ,
            Q873CoefficientBracketBody::SubForward,
            &active,
        ...
        end_control_and_nonnegative_bracket(circ, control, &cursor, &active, bracket);
        if index + 1 != work1.len() {
            decrement_mod_2n(circ, &cursor, &cursor_scratch);
        }
    }
...
#[allow(clippy::too_many_arguments)]
fn coefficient_fused_data_and_sign(
    circ: &mut Circuit,
    phase1: &QReg,
    phase2: &QReg,
    sign: &QReg,
    ...
    // Write x = |t'| = 2^l_s Q + R. The Work2 right rotation maps Q to the
    // low lanes and R to the high wrapped lanes. With a = l_t + 1, this pass
    // acts only on Q mod 2^a; above_guard records Q >= 2^a, so its corrected
    // borrow is exactly [x < 2^l_s t]. On add-only support Q < 2^(a-1), hence
    // Q + t < 2^a and the clean Work1 separator at lane a-1 is sufficient.
    let cursor = circ.alloc_qreg_bits("rs.coeff-fused.cursor", l_t.len());
    for (source, destination) in l_t.iter().zip(&cursor) {
        circ.cx(source, destination);
    }
    let cursor_scratch =
        circ.alloc_qreg_bits("rs.coeff-fused.cursor-scratch", l_t.len().saturating_sub(1));
    let carry = circ.alloc_qreg("rs.coeff-fused.carry");
    let active = circ.alloc_qreg("rs.coeff-fused.active");
    let tmp = circ.alloc_qreg("rs.coeff-fused.tmp");

    if inverse {
        // Inverse of the shared add-unmajority half. On the promised support,
        // carry is the old local borrow on subtract branches and zero on
        // add-only branches.
        for index in 0..work1.len() {
            if index != 0 {
                decrement_mod_2n(circ, &cursor, &cursor_scratch);
            }
            begin_control_and_nonnegative_bracket(circ, phase1, &cursor, &active, bracket);
            emit_q873_coefficient_bracket_body(
                circ,
                Q873CoefficientBracketBody::SubForward,
                &active,
            ...
            end_control_and_nonnegative_bracket(circ, phase1, &cursor, &active, bracket);
        }

        circ.x(above_guard);
        circ.ccx(&carry, above_guard, borrow);
        circ.x(above_guard);
        toggle_output_coefficient_enable(circ, phase1, phase2, sign, borrow, enable, chain);
        circ.x(enable);
        ...
        for index in (0..work1.len()).rev() {
            ...
            begin_control_and_nonnegative_bracket(circ, add_only, &cursor, &active, bracket);
            emit_q873_coefficient_bracket_body(
                circ,
                Q873CoefficientBracketBody::SubReverse,
                &active,
            ...
            end_control_and_nonnegative_bracket(circ, add_only, &cursor, &active, bracket);
            begin_control_and_nonnegative_bracket(circ, enable, &cursor, &active, bracket);
            emit_q873_coefficient_bracket_body(
                circ,
                Q873CoefficientBracketBody::AddReverse,
                &active,
            ...
            end_control_and_nonnegative_bracket(circ, enable, &cursor, &active, bracket);
        }
        circ.x(enable);
        circ.ccx(phase1, enable, add_only);
        circ.x(enable);
...
#[allow(clippy::too_many_arguments)]
fn coefficient_phase_block_fused(
    circ: &mut Circuit,
    phase1: &QReg,
    phase2: &QReg,
    sign: &QReg,
    ...
    let enable = circ.alloc_qreg("rs.coeff-fused.enable");
    let above_guard = circ.alloc_qreg("rs.coeff-fused.above-guard");
    let borrow = circ.alloc_qreg("rs.coeff-fused.borrow");
    let add_only = circ.alloc_qreg("rs.coeff-fused.add-only");
    let chain = circ.alloc_qreg_bits("rs.coeff-fused.chain", 2);
    let bracket = production_coefficient_nonnegative_bracket();

    if inverse {
        toggle_coefficient_length_above_guarded_boundary(
            circ,
            phase1,
            l_t_prime,
            l_t,
        ...
        controlled_xor_raw_t_prime_bit_length(circ, phase1, l_s, l_r_prime, full_work2, l_t_prime);
        coefficient_fused_data_and_sign(
            circ,
            phase1,
            phase2,
            sign,
...
fn coefficient_phase_block(
    ...
    if q873_fused_coefficient_compare_requested() {
        coefficient_phase_block_fused(
    ...
    if inverse {
        toggle_coefficient_less_than(
            circ, phase1, work1, work2, l_t, l_s, l_t_prime, &less_than, bracket,
        );
        ...
        coefficient_add_data_only(circ, &add_only, work1, work2, l_t, true, bracket);
        controlled_xor_raw_t_prime_bit_length(
            circ, &add_only, l_s, l_r_prime, full_work2, l_t_prime,
        );
        circ.x(&enable);
        ...
        circ.cx(phase1, sign);
        toggle_coefficient_less_than(
            circ, &enable, work1, work2, l_t, l_s, l_t_prime, &less_than, bracket,
        );
        toggle_initial_coefficient_enable(circ, phase1, phase2, sign, &enable, &chain);
    } else {
        toggle_initial_coefficient_enable(circ, phase1, phase2, sign, &enable, &chain);
        ...
        coefficient_add_data_only(circ, &add_only, work1, work2, l_t, false, bracket);
        controlled_xor_raw_t_prime_bit_length(
            circ, &add_only, l_s, l_r_prime, full_work2, l_t_prime,
        );
        circ.x(&enable);
...
fn build_coefficient_kernel(
    ...
    build_coefficient_kernel_with_bracket(
        work_width,
        length_width,
        kernel,
        production_coefficient_nonnegative_bracket(),
...
fn build_coefficient_kernel_with_bracket(
    work_width: usize,
    length_width: usize,
    kernel: CoefficientKernel,
    bracket: CoefficientNonnegativeBracket,
    ...
    match kernel {
        CoefficientKernel::Add => coefficient_add_single_with_bracket(
            &mut circ, &phase1, &sign, &work1, &work2, &l_t, &scratch, bracket,
        ),
        CoefficientKernel::AddInverse => coefficient_add_single_inverse_with_bracket(
            &mut circ, &phase1, &sign, &work1, &work2, &l_t, &scratch, bracket,
        ),
        CoefficientKernel::Sub => coefficient_sub_single_with_bracket(
            &mut circ, &phase1, &phase2, &sign, &work1, &work2, &l_t, &scratch, bracket,
        ),
        CoefficientKernel::SubInverse => coefficient_sub_single_inverse_with_bracket(
            &mut circ, &phase1, &phase2, &sign, &work1, &work2, &l_t, &scratch, bracket,
        ),
    }
    circ.into_builder()
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum Q873CoefficientBracketBody {
    AddForward,
    AddReverse,
    SubForward,
    SubReverse,
...
impl Q873CoefficientBracketBody {
    fn inverse(self) -> Self {
        match self {
            ...
            Self::PredicateProbe => Self::PredicateProbe,
        }
    }
}

const Q873_COEFFICIENT_DATA_BODIES: [Q873CoefficientBracketBody; 4] = [
    Q873CoefficientBracketBody::AddForward,
    Q873CoefficientBracketBody::AddReverse,
    Q873CoefficientBracketBody::SubForward,
    Q873CoefficientBracketBody::SubReverse,
...
#[allow(clippy::too_many_arguments)]
fn emit_q873_coefficient_bracket_body(
    circ: &mut Circuit,
    body: Q873CoefficientBracketBody,
    active: &QReg,
    carry: &QReg,
    work1: &QReg,
    work2: &QReg,
    ...
    match body {
        ...
        Q873CoefficientBracketBody::PredicateProbe => circ.cx(active, work1),
    }
}

struct Q873CoefficientBracketProofCircuit {
    builder: B,
    control_id: u32,
    cursor_ids: Vec<u32>,
    active_id: u32,
...
fn build_q873_coefficient_bracket_proof_circuit(
    cursor_width: usize,
    body: Q873CoefficientBracketBody,
    bracket: CoefficientNonnegativeBracket,
) -> Q873CoefficientBracketProofCircuit {
    assert!(cursor_width > 0);
    let mut circ = Circuit::new();
    let control = circ.alloc_qreg("q873.coeff-xcancel.control");
    let cursor = circ.alloc_qreg_bits("q873.coeff-xcancel.cursor", cursor_width);
    let active = circ.alloc_qreg("q873.coeff-xcancel.active");
    let carry = circ.alloc_qreg("q873.coeff-xcancel.carry");
    let work1 = circ.alloc_qreg("q873.coeff-xcancel.work1");
    let work2 = circ.alloc_qreg("q873.coeff-xcancel.work2");
    let tmp = circ.alloc_qreg("q873.coeff-xcancel.tmp");

    begin_control_and_nonnegative_bracket(&mut circ, &control, &cursor, &active, bracket);
    let body_start = circ.b.ops.len();
    emit_q873_coefficient_bracket_body(&mut circ, body, &active, &carry, &work1, &work2, &tmp);
    let body_end = circ.b.ops.len();
    end_control_and_nonnegative_bracket(&mut circ, &control, &cursor, &active, bracket);
...
fn q873_id_mask(ids: &[u32]) -> u64 {
    ids.iter().fold(0u64, |mask, &id| mask | (1u64 << id))
}

fn assert_q873_classical_gate_stream(builder: &B) {
    use crate::circuit::OperationType;

    assert!(builder.ops.iter().all(|operation| matches!(
        operation.kind,
        OperationType::X | OperationType::CX | OperationType::CCX
    )));
}

fn apply_q873_classical_with_clean_resets(ops: &[crate::circuit::Op], mut state: u64) -> u64 {
    use crate::circuit::OperationType;
    ...
    for operation in ops {
        match operation.kind {
            ...
            OperationType::R => assert!(
                ...
                "Q873 prototype reset a dirty ancilla q{}",
                operation.q_target.0
            ),
            other => panic!("Q873 prototype emitted phase/nonclassical operation {other:?}"),
        }
    }
    state
}

fn assert_q873_only_x_removed(legacy: &B, candidate: &B, expected: usize) {
    let legacy_counts = gate_counts(&legacy.ops);
    let candidate_counts = gate_counts(&candidate.ops);
    assert_eq!(legacy_counts.cx, candidate_counts.cx);
    assert_eq!(legacy_counts.ccx, candidate_counts.ccx);
    assert_eq!(legacy_counts.x - candidate_counts.x, expected);
    assert_eq!(legacy_counts.total - candidate_counts.total, expected);
}

fn assert_q873_builder_stream_identity(default: &B, legacy: &B) {
    assert_eq!(default.ops, legacy.ops);
    assert_eq!(default.next_qubit, legacy.next_qubit);
    assert_eq!(default.next_bit, legacy.next_bit);
    assert_eq!(default.active_qubits, legacy.active_qubits);
    assert_eq!(default.peak_qubits, legacy.peak_qubits);
    assert_eq!(default.free_qubits, legacy.free_qubits);
    assert_eq!(default.allocation_serial, legacy.allocation_serial);
}

#[must_use]
pub fn exhaustive_q873_coefficient_nonnegative_x_cancel_check(
) -> Q873CoefficientNonnegativeXCancelProofReport {
    use crate::circuit::QubitId;

    const CURSOR_WIDTHS: std::ops::RangeInclusive<usize> = 1..=4;
    const TEST_LENGTH_WIDTH: usize = 3;
    assert!(!Q873_COEFFICIENT_NONNEGATIVE_X_CANCEL_SOURCE_BAKE);
    assert!(
        !q873_coefficient_nonnegative_x_cancel_enabled(),
        "focused proof must begin from the default-off stream"
    );

    let mut body_target_safety_checks = 0usize;
    let mut bracket_basis_states_checked = 0usize;
    let mut predicate_cases_checked = 0usize;
    let mut legacy_candidate_equivalence_checks = 0usize;
    let mut control_off_identity_checks = 0usize;
    let mut control_on_checks = 0usize;
    let mut forward_inverse_composition_checks = 0usize;
    let mut cursor_preservation_checks = 0usize;
    let mut sign_preservation_checks = 0usize;
    let mut phase_clean_checks = 0usize;
    let mut ancilla_clean_checks = 0usize;
    let mut kernel_basis_states_checked = 0usize;
    let mut kernel_equivalence_checks = 0usize;
    let mut kernel_inverse_pair_checks = 0usize;
    let mut default_off_stream_identity_checks = 0usize;

    for cursor_width in CURSOR_WIDTHS.clone() {
        let probe_legacy = build_q873_coefficient_bracket_proof_circuit(
            cursor_width,
            Q873CoefficientBracketBody::PredicateProbe,
            CoefficientNonnegativeBracket::Legacy,
        );
        let probe_candidate = build_q873_coefficient_bracket_proof_circuit(
            cursor_width,
            Q873CoefficientBracketBody::PredicateProbe,
            CoefficientNonnegativeBracket::CancelMiddleX,
        );
        let probe_default = build_q873_coefficient_bracket_proof_circuit(
            cursor_width,
            Q873CoefficientBracketBody::PredicateProbe,
            production_coefficient_nonnegative_bracket(),
        );
        assert_q873_builder_stream_identity(&probe_default.builder, &probe_legacy.builder);
        default_off_stream_identity_checks += 1;
        assert_q873_only_x_removed(&probe_legacy.builder, &probe_candidate.builder, 2);
        let active_mask = 1u64 << probe_legacy.active_id;
        let tmp_mask = 1u64 << probe_legacy.tmp_id;
        let control_mask = 1u64 << probe_legacy.control_id;
        let sign_mask = 1u64 << probe_legacy.cursor_ids[cursor_width - 1];
        let probe_mask = 1u64 << probe_legacy.work1_id;
        for input in 0..(1u64 << probe_legacy.builder.next_qubit) {
            if input & (active_mask | tmp_mask) != 0 {
                continue;
            }
            let predicate = input & control_mask != 0 && input & sign_mask == 0;
            let expected = if predicate { input ^ probe_mask } else { input };
            assert_eq!(apply_scalar(&probe_legacy.builder.ops, input), expected);
            assert_eq!(apply_scalar(&probe_candidate.builder.ops, input), expected);
            predicate_cases_checked += 1;
        }

        for body in Q873_COEFFICIENT_DATA_BODIES {
            let legacy = build_q873_coefficient_bracket_proof_circuit(
                cursor_width,
                body,
                CoefficientNonnegativeBracket::Legacy,
            );
            let candidate = build_q873_coefficient_bracket_proof_circuit(
                cursor_width,
                body,
                CoefficientNonnegativeBracket::CancelMiddleX,
            );
            let legacy_inverse = build_q873_coefficient_bracket_proof_circuit(
                cursor_width,
                body.inverse(),
                CoefficientNonnegativeBracket::Legacy,
            );
            let candidate_inverse = build_q873_coefficient_bracket_proof_circuit(
                cursor_width,
                body.inverse(),
                CoefficientNonnegativeBracket::CancelMiddleX,
            );
            let default = build_q873_coefficient_bracket_proof_circuit(
                cursor_width,
                body,
                production_coefficient_nonnegative_bracket(),
            );
            assert_q873_builder_stream_identity(&default.builder, &legacy.builder);
            default_off_stream_identity_checks += 1;
            assert_q873_only_x_removed(&legacy.builder, &candidate.builder, 2);
            assert_q873_classical_gate_stream(&legacy.builder);
            assert_q873_classical_gate_stream(&candidate.builder);

            let protected_mask = (1u64 << legacy.control_id) | q873_id_mask(&legacy.cursor_ids);
            for circuit in [&legacy, &candidate] {
                for operation in &circuit.builder.ops[circuit.body_start..circuit.body_end] {
                    assert_eq!(
                        protected_mask & (1u64 << operation.q_target.0),
                        0,
                        "coefficient bracket body targeted control or cursor"
                    );
                    assert_ne!(operation.q_target, QubitId(u64::from(circuit.active_id)));
                    body_target_safety_checks += 1;
                }
            }

            let cursor_mask = q873_id_mask(&legacy.cursor_ids);
            let sign_mask = 1u64 << legacy.cursor_ids[cursor_width - 1];
            let control_mask = 1u64 << legacy.control_id;
            let active_mask = 1u64 << legacy.active_id;
            let tmp_mask = 1u64 << legacy.tmp_id;
            for input in 0..(1u64 << legacy.builder.next_qubit) {
                let legacy_output = apply_scalar(&legacy.builder.ops, input);
                let candidate_output = apply_scalar(&candidate.builder.ops, input);
                assert_eq!(candidate_output, legacy_output);
                legacy_candidate_equivalence_checks += 1;
                assert_eq!(legacy_output & cursor_mask, input & cursor_mask);
                assert_eq!(candidate_output & cursor_mask, input & cursor_mask);
                cursor_preservation_checks += 2;
                assert_eq!(legacy_output & sign_mask, input & sign_mask);
                assert_eq!(candidate_output & sign_mask, input & sign_mask);
                sign_preservation_checks += 2;
                assert_eq!(legacy_output & active_mask, input & active_mask);
                assert_eq!(candidate_output & active_mask, input & active_mask);
                assert_eq!(legacy_output & tmp_mask, input & tmp_mask);
                assert_eq!(candidate_output & tmp_mask, input & tmp_mask);
                ancilla_clean_checks += 4;
                assert_eq!(
                    apply_scalar(&legacy_inverse.builder.ops, legacy_output),
                    input
                );
                assert_eq!(
                ...
                forward_inverse_composition_checks += 2;
                phase_clean_checks += 2;
                if input & control_mask == 0 && input & (active_mask | tmp_mask) == 0 {
                    assert_eq!(legacy_output, input);
                    assert_eq!(candidate_output, input);
                    control_off_identity_checks += 2;
                }
                if input & control_mask != 0 {
                    control_on_checks += 1;
                }
                bracket_basis_states_checked += 1;
            }
        }
    }
    ...
    for work_width in 1..=4 {
        let legacy = [
            ...
            build_coefficient_kernel_with_bracket(
                work_width,
                TEST_LENGTH_WIDTH,
                CoefficientKernel::AddInverse,
                CoefficientNonnegativeBracket::Legacy,
            ),
            build_coefficient_kernel_with_bracket(
                work_width,
                TEST_LENGTH_WIDTH,
                CoefficientKernel::Sub,
                CoefficientNonnegativeBracket::Legacy,
            ),
            build_coefficient_kernel_with_bracket(
                work_width,
                TEST_LENGTH_WIDTH,
                CoefficientKernel::SubInverse,
                CoefficientNonnegativeBracket::Legacy,
            ),
        ];
        let candidate = [
        ...
        let default = [
            build_coefficient_kernel(work_width, TEST_LENGTH_WIDTH, CoefficientKernel::Add),
            build_coefficient_kernel(work_width, TEST_LENGTH_WIDTH, CoefficientKernel::AddInverse),
            build_coefficient_kernel(work_width, TEST_LENGTH_WIDTH, CoefficientKernel::Sub),
            build_coefficient_kernel(work_width, TEST_LENGTH_WIDTH, CoefficientKernel::SubInverse),
        ];
        for index in 0..legacy.len() {
            assert_q873_builder_stream_identity(&default[index], &legacy[index]);
            assert_q873_only_x_removed(&legacy[index], &candidate[index], 4 * work_width);
            assert_q873_classical_gate_stream(&legacy[index]);
            assert_q873_classical_gate_stream(&candidate[index]);
            default_off_stream_identity_checks += 1;
        }

        let data_width = 3 + 2 * work_width + TEST_LENGTH_WIDTH;
        let length_offset = 3 + 2 * work_width;
        let length_mask = ((1u64 << TEST_LENGTH_WIDTH) - 1) << length_offset;
        for input in 0..(1u64 << data_width) {
            let mut legacy_outputs = [0u64; 4];
            let mut candidate_outputs = [0u64; 4];
            for index in 0..legacy.len() {
                legacy_outputs[index] = apply_scalar(&legacy[index].ops, input);
                candidate_outputs[index] = apply_scalar(&candidate[index].ops, input);
                assert_eq!(candidate_outputs[index], legacy_outputs[index]);
                assert_eq!(legacy_outputs[index] >> data_width, 0);
                assert_eq!(candidate_outputs[index] >> data_width, 0);
                assert_eq!(legacy_outputs[index] & length_mask, input & length_mask);
                assert_eq!(candidate_outputs[index] & length_mask, input & length_mask);
                kernel_equivalence_checks += 1;
                ancilla_clean_checks += 2;
                phase_clean_checks += 2;
                if input & 1 == 0 {
                    assert_eq!(legacy_outputs[index], input);
                    assert_eq!(candidate_outputs[index], input);
                    control_off_identity_checks += 2;
                }
            }
            for (forward, inverse) in [(0usize, 1usize), (2, 3)] {
                assert_eq!(
                    apply_scalar(&legacy[inverse].ops, legacy_outputs[forward]),
                    input
                );
                assert_eq!(
                    apply_scalar(&candidate[inverse].ops, candidate_outputs[forward]),
                    input
                );
                kernel_inverse_pair_checks += 2;
            }
            kernel_basis_states_checked += 1;
        }
    }

    let schedule = exhaustive_reference_schedule_check();
    let active_coefficient_positions = schedule.t_window_sum;
    let bracket_pairs_per_phase_block_position = 6;
    // One forward and one reverse phase block occur in each of the two divider
    // lifecycles used by the complete point-add builder.
    let coefficient_phase_blocks_per_point_add = 4;
    ...
    Q873CoefficientNonnegativeXCancelProofReport {
        ...
        bracket_body_kinds_checked: Q873_COEFFICIENT_DATA_BODIES.len(),
        body_target_safety_checks,
        bracket_basis_states_checked,
        predicate_cases_checked,
        legacy_candidate_equivalence_checks,
        control_off_identity_checks,
        control_on_checks,
        forward_inverse_composition_checks,
        cursor_preservation_checks,
        sign_preservation_checks,
        phase_clean_checks,
        ancilla_clean_checks,
        kernel_work_widths_checked: 4,
        kernel_basis_states_checked,
        kernel_equivalence_checks,
        kernel_inverse_pair_checks,
...
struct Q873FusedCoefficientCoreCircuit {
    builder: B,
    phase1_id: u32,
    phase2_id: u32,
    sign_id: u32,
...
struct Q873GuardPredicateCircuit {
    builder: B,
    control_id: u32,
...
fn q873_full_stream_proof_circuit() -> Circuit {
    let mut circ = Circuit::new();
    // The sealed count/support wrapper exports POINT_ADD_COUNT_ONLY=1. These
    // reduced proof circuits must retain their gates for classical replay.
    circ.b.count_only = false;
    circ.b.fiat_hash = None;
    circ
}

fn build_q873_guard_predicate(length_width: usize) -> Q873GuardPredicateCircuit {
    let mut circ = q873_full_stream_proof_circuit();
    let control = circ.alloc_qreg("q873.coeff-guard-proof.control");
    let target_length = circ.alloc_qreg_bits("q873.coeff-guard-proof.target-length", length_width);
    let source_length = circ.alloc_qreg_bits("q873.coeff-guard-proof.source-length", length_width);
    let shift = circ.alloc_qreg_bits("q873.coeff-guard-proof.shift", length_width);
    let target = circ.alloc_qreg("q873.coeff-guard-proof.target");
    let chain = circ.alloc_qreg_bits("q873.coeff-guard-proof.chain", 1);
    toggle_coefficient_length_above_guarded_boundary(
        &mut circ,
        &control,
        &target_length,
    ...
    let data_mask = (1u64 << control_id)
        | q873_id_mask(&target_length_ids)
        | q873_id_mask(&source_length_ids)
        | q873_id_mask(&shift_ids)
        | (1u64 << target_id);
    Q873GuardPredicateCircuit {
        builder: circ.into_builder(),
        control_id,
        target_length_ids,
        source_length_ids,
...
fn build_q873_fused_coefficient_core(
    work_width: usize,
    length_width: usize,
    inverse: bool,
) -> Q873FusedCoefficientCoreCircuit {
    let mut circ = q873_full_stream_proof_circuit();
    let phase1 = circ.alloc_qreg("q873.coeff-fused-proof.phase1");
    let phase2 = circ.alloc_qreg("q873.coeff-fused-proof.phase2");
    let sign = circ.alloc_qreg("q873.coeff-fused-proof.sign");
    let work1 = circ.alloc_qreg_bits("q873.coeff-fused-proof.work1", work_width);
    let work2 = circ.alloc_qreg_bits("q873.coeff-fused-proof.work2", work_width);
    let l_t = circ.alloc_qreg_bits("q873.coeff-fused-proof.l-t", length_width);
    let above_guard = circ.alloc_qreg("q873.coeff-fused-proof.above-guard");
    let enable = circ.alloc_qreg("q873.coeff-fused-proof.enable");
    let borrow = circ.alloc_qreg("q873.coeff-fused-proof.borrow");
    let add_only = circ.alloc_qreg("q873.coeff-fused-proof.add-only");
    let chain = circ.alloc_qreg_bits("q873.coeff-fused-proof.chain", 2);
    let bracket = CoefficientNonnegativeBracket::CancelMiddleX;
    ...
    if inverse {
        ...
        toggle_initial_coefficient_enable(&mut circ, &phase1, &phase2, &sign, &enable, &chain);
        coefficient_fused_data_and_sign(
            &mut circ,
            &phase1,
            &phase2,
    ...
    let data_mask = (1u64 << phase1_id)
        | (1u64 << phase2_id)
        | (1u64 << sign_id)
        | q873_id_mask(&work1_ids)
        | q873_id_mask(&work2_ids)
        | q873_id_mask(&length_ids)
        | (1u64 << above_guard_id);
    Q873FusedCoefficientCoreCircuit {
        builder: circ.into_builder(),
        phase1_id,
...
fn q873_set_register(mut value: u64, ids: &[u32], register: usize) -> u64 {
...
fn build_q873_fused_dispatch_stream(direct: bool) -> B {
    let mut circ = q873_full_stream_proof_circuit();
    let phase1 = circ.alloc_qreg("q873.coeff-fused-stream.phase1");
    let phase2 = circ.alloc_qreg("q873.coeff-fused-stream.phase2");
    let sign = circ.alloc_qreg("q873.coeff-fused-stream.sign");
    let work1 = circ.alloc_qreg_bits("q873.coeff-fused-stream.work1", 4);
    let work2 = circ.alloc_qreg_bits("q873.coeff-fused-stream.work2", 4);
    let l_t = circ.alloc_qreg_bits("q873.coeff-fused-stream.l-t", 3);
    let l_t_prime = circ.alloc_qreg_bits("q873.coeff-fused-stream.l-t-prime", 3);
    let l_s = circ.alloc_qreg_bits("q873.coeff-fused-stream.l-s", 3);
    let l_r_prime = circ.alloc_qreg_bits("q873.coeff-fused-stream.l-r-prime", 3);
    if direct {
        coefficient_phase_block_fused(
...
/// Exhaustively prove the fused three-half-sweep coefficient core on every
/// reduced-width promised basis state. The promise is structural: the source
/// guard bit is zero and the add-only branch has no carry out of that guard.
#[must_use]
pub fn exhaustive_q873_fused_coefficient_compare_check() -> Q873FusedCoefficientCompareProofReport {
    const LENGTH_WIDTH: usize = 4;
    assert!(Q873_FUSED_COEFFICIENT_COMPARE_SOURCE_BAKE);
    assert_eq!(
        std::env::var("POINT_ADD_COUNT_ONLY").ok().as_deref(),
        Some("1"),
        "the Q873 proof must exercise the sealed inherited count-only environment"
    );
    assert!(std::env::var_os(Q873_FUSED_COEFFICIENT_COMPARE_ENV).is_none());
    assert!(q873_fused_coefficient_compare_requested());
    let production_stream = build_q873_fused_dispatch_stream(false);
    let direct_fused_stream = build_q873_fused_dispatch_stream(true);
    assert_q873_builder_stream_identity(&production_stream, &direct_fused_stream);
    let source_baked_stream_identity_checks = 1;

    // Regression for sealed WMI job 86986. Before proof circuits forced a
    // retained stream, POINT_ADD_COUNT_ONLY made this replay return 5 rather
    // than toggling target qubit 7 and returning 133.
    let regression = build_q873_guard_predicate(2);
    assert!(!regression.builder.count_only);
    assert!(!regression.builder.ops.is_empty());
    let regression_input = 5u64;
    let regression_expected = 133u64;
    assert_eq!(
        apply_q873_classical_with_clean_resets(&regression.builder.ops, regression_input)
            & regression.data_mask,
        regression_expected
    );
    let inherited_count_only_counterexample_checks = 1;

    let mut guard_predicate_basis_states_checked = 0usize;
    let mut guard_predicate_scratch_clean_checks = 0usize;
    for length_width in 2..=4 {
        let predicate = build_q873_guard_predicate(length_width);
        let all_mask = (1u64 << predicate.builder.next_qubit) - 1;
        let scratch_mask = all_mask & !predicate.data_mask;
                ...
                for target_length in 0..modulus {
                    for source_length in 0..modulus {
                        for shift in 0..modulus {
                            if source_length + shift + 1 >= modulus {
                                continue;
                            }
                            let mut input = 0u64;
                            input |= u64::from(control) << predicate.control_id;
                            input |= u64::from(target_bit) << predicate.target_id;
                            input = q873_set_register(
                                input,
                                &predicate.target_length_ids,
                                target_length,
                            );
                            input = q873_set_register(
                                input,
                                &predicate.source_length_ids,
                                source_length,
                            );
                            input = q873_set_register(input, &predicate.shift_ids, shift);
                            let output = apply_q873_classical_with_clean_resets(
                                &predicate.builder.ops,
                                input,
                            );
                            let toggle = control && target_length > source_length + shift + 1;
                            let expected = if toggle {
                                input ^ (1u64 << predicate.target_id)
                            } else {
                                input
                            };
                            assert_eq!(output & predicate.data_mask, expected);
                            assert_eq!(output & scratch_mask, 0);
                            guard_predicate_basis_states_checked += 1;
                            guard_predicate_scratch_clean_checks += 1;
                        }
                    }
                }
            }
    ...
    for work_width in 2..=5 {
        let forward = build_q873_fused_coefficient_core(work_width, LENGTH_WIDTH, false);
        let inverse = build_q873_fused_coefficient_core(work_width, LENGTH_WIDTH, true);
                        ...
                        for work1 in 0..(1usize << work_width) {
                            ...
                            for target in 0..(1usize << work_width) {
                                ...
                                for above_guard in [false, true] {
                                    ...
                                    input = q873_set_register(input, &forward.work1_ids, work1);
                                    input = q873_set_register(input, &forward.work2_ids, target);
                                    input = q873_set_register(
                                        input,
                                        &forward.length_ids,
                                        active_width - 1,
                                    );
                                    input |= u64::from(above_guard) << forward.above_guard_id;
                                    ...
                                    assert_eq!(
                                        apply_q873_classical_with_clean_resets(
                                            &inverse.builder.ops,
                                            output,
                                        ),
                                        input
    ...
    Q873FusedCoefficientCompareProofReport {
        inherited_count_only_counterexample_checks,
        guard_length_widths_checked: 3,
        guard_predicate_basis_states_checked,
        guard_predicate_scratch_clean_checks,
        ...
        add_headroom_checks,
        scratch_clean_checks,
        source_baked_stream_identity_checks,
    }
}
...
#[derive(Clone, Copy)]
enum EqualityMaskSweepMode {
    Production,
    Legacy,
    Chained,
}

fn build_location_swap_with_mode(
    work_width: usize,
    length_width: usize,
    inverse: bool,
    mode: EqualityMaskSweepMode,
) -> B {
    let mut circ = Circuit::new();
    ...
    match (inverse, mode) {
        (false, EqualityMaskSweepMode::Production) => location_controlled_swap_one_hot(
            &mut circ, &phase1, &phase2, &sign, &work1, 0, &l_t, &l_q, &scratch,
        ),
        (true, EqualityMaskSweepMode::Production) => location_controlled_swap_one_hot_inverse(
            &mut circ, &phase1, &phase2, &sign, &work1, 0, &l_t, &l_q, &scratch,
        ),
        (false, mode) => location_controlled_swap_one_hot_with_mask_mode(
            &mut circ,
            &phase1,
            &phase2,
            &sign,
            ...
            &l_q,
            &scratch,
            matches!(mode, EqualityMaskSweepMode::Chained),
        ),
        (true, mode) => location_controlled_swap_one_hot_inverse_with_mask_mode(
            &mut circ,
            &phase1,
            &phase2,
            &sign,
...
fn build_location_swap(work_width: usize, length_width: usize, inverse: bool) -> B {
    build_location_swap_with_mode(
        work_width,
        length_width,
        inverse,
        EqualityMaskSweepMode::Production,
    )
}
...
fn build_dynamic_swap_sweep(
    ...
    assert!(first_global_index + work_width <= (1usize << length_width));
    let mut circ = Circuit::new();
    let active = circ.alloc_qreg("q873-mask-proof.active");
    let sign = circ.alloc_qreg("q873-mask-proof.sign");
    let work = circ.alloc_qreg_bits("q873-mask-proof.work", work_width);
    let sum = circ.alloc_qreg_bits("q873-mask-proof.sum", length_width);
    let flag = circ.alloc_qreg("q873-mask-proof.flag");
    let chain = circ.alloc_qreg_bits("q873-mask-proof.chain", length_width.saturating_sub(1));
    controlled_dynamic_swaps(
        &mut circ,
        &active,
        &sign,
...
fn assert_equality_mask_phase_clean(builder: &B) {
    use crate::circuit::OperationType;
...
fn assert_equality_mask_stream_identity(default: &B, legacy: &B) {
    assert_eq!(default.ops, legacy.ops);
    assert_eq!(default.count_only, legacy.count_only);
    assert_eq!(default.counted_ops, legacy.counted_ops);
    assert_eq!(default.counted_kind_ops, legacy.counted_kind_ops);
    assert_eq!(default.next_qubit, legacy.next_qubit);
    assert_eq!(default.next_bit, legacy.next_bit);
    assert_eq!(default.next_register, legacy.next_register);
    assert_eq!(default.free_qubits, legacy.free_qubits);
    assert_eq!(default.active_qubits, legacy.active_qubits);
    assert_eq!(default.peak_qubits, legacy.peak_qubits);
    assert_eq!(default.peak_ops_idx, legacy.peak_ops_idx);
    assert_eq!(default.allocation_serial, legacy.allocation_serial);
    assert_eq!(
        default.peak_allocation_serial,
        legacy.peak_allocation_serial
    );
}

fn expected_dynamic_swap_sweep(
    input: u64,
    work_width: usize,
    length_width: usize,
    first_global_index: usize,
) -> u64 {
    let active = (input & 1) != 0;
    let mut sign = ((input >> 1) & 1) != 0;
    let work_mask = (1u64 << work_width) - 1;
    let length_mask = (1u64 << length_width) - 1;
    let mut work = (input >> 2) & work_mask;
    let sum = (input >> (2 + work_width)) & length_mask;
    let end = first_global_index as u64 + work_width as u64;
    if active && sum >= first_global_index as u64 && sum < end {
        let index = (sum - first_global_index as u64) as usize;
        let work_bit = ((work >> index) & 1) != 0;
        if work_bit != sign {
            work ^= 1u64 << index;
            sign = work_bit;
        }
    }
    u64::from(active) | (u64::from(sign) << 1) | (work << 2) | (sum << (2 + work_width))
}

/// Exhaustively compare the historical and chained equality-mask sweeps.
///
/// The isolated sweeps cover lower and upper windows for length widths one
/// through four. The complete location-swap harness additionally covers every
/// phase/sign control state, every three-bit length state, work widths one
/// through six, and both forward and inverse entry points.
#[doc(hidden)]
#[must_use]
pub fn q873_equality_mask_chaining_roundtrip_check() -> Q873EqualityMaskChainingProofReport {
    assert_ne!(
        std::env::var("POINT_ADD_COUNT_ONLY").ok().as_deref(),
        Some("1"),
        "the exhaustive proof requires materialized local operation streams"
    );

    let mut sweep_parameter_sets_checked = 0usize;
    let mut sweep_basis_states_checked = 0usize;
    let mut location_basis_states_checked = 0usize;
    let mut legacy_chained_permutation_checks = 0usize;
    let mut forward_inverse_pair_checks = 0usize;
    let mut boundary_restoration_checks = 0usize;
    let mut scratch_clean_checks = 0usize;
    let mut phase_clean_streams_checked = 0usize;
    let mut transition_arithmetic_checks = 0usize;

    for length_width in 1..=4 {
        let modulus = 1usize << length_width;
        for work_width in 1..=modulus.min(6) {
            let upper_first = modulus - work_width;
            let mut first_indices = vec![0usize];
            if upper_first != 0 {
                first_indices.push(upper_first);
            }
            for first_global_index in first_indices {
                let legacy_forward = build_dynamic_swap_sweep(
                    work_width,
                    length_width,
                    first_global_index,
                    false,
                ...
                let legacy_inverse = build_dynamic_swap_sweep(
                    work_width,
                    length_width,
                    first_global_index,
                    true,
                ...
                let chained_forward = build_dynamic_swap_sweep(
                    work_width,
                    length_width,
                    first_global_index,
                    false,
                ...
                let chained_inverse = build_dynamic_swap_sweep(
                    work_width,
                    length_width,
                    first_global_index,
                    true,
                ...
                for builder in [
                    ...
                    assert_equality_mask_phase_clean(builder);
                    phase_clean_streams_checked += 1;
                }

                let end = first_global_index + work_width;
                let last = end - 1;
                let zero_occurrences = (first_global_index..end)
                    .map(|index| equality_zero_mask_weight(length_width, index))
                    .sum::<usize>();
                let legacy_x = 4 * zero_occurrences;
                let chained_x = equality_zero_mask_weight(length_width, first_global_index)
                    + (first_global_index..last)
                        .map(|index| {
                            equality_mask_transition_weight(length_width, index, index + 1)
                        })
                        .sum::<usize>()
                    + equality_zero_mask_weight(length_width, last);
                let legacy_forward_counts = gate_counts(&legacy_forward.ops);
                let legacy_inverse_counts = gate_counts(&legacy_inverse.ops);
                let chained_forward_counts = gate_counts(&chained_forward.ops);
                let chained_inverse_counts = gate_counts(&chained_inverse.ops);
                assert_eq!(legacy_forward_counts.x, legacy_x);
                assert_eq!(legacy_inverse_counts.x, legacy_x);
                assert_eq!(chained_forward_counts.x, chained_x);
                assert_eq!(chained_inverse_counts.x, chained_x);
                assert_eq!(legacy_forward_counts.cx, chained_forward_counts.cx);
                assert_eq!(legacy_forward_counts.ccx, chained_forward_counts.ccx);
                assert_eq!(legacy_inverse_counts.cx, chained_inverse_counts.cx);
                assert_eq!(legacy_inverse_counts.ccx, chained_inverse_counts.ccx);
                transition_arithmetic_checks += 1;

                let data_width = 2 + work_width + length_width;
                let sum_offset = 2 + work_width;
                let length_mask = (1u64 << length_width) - 1;
                for input in 0..(1u64 << data_width) {
                    let expected = expected_dynamic_swap_sweep(
                        input,
                        work_width,
                        length_width,
                        first_global_index,
                    );
                    let legacy_forward_output = apply_scalar(&legacy_forward.ops, input);
                    let legacy_inverse_output = apply_scalar(&legacy_inverse.ops, input);
                    let chained_forward_output = apply_scalar(&chained_forward.ops, input);
                    let chained_inverse_output = apply_scalar(&chained_inverse.ops, input);
                    assert_eq!(legacy_forward_output, expected);
                    assert_eq!(legacy_inverse_output, expected);
                    assert_eq!(chained_forward_output, legacy_forward_output);
                    assert_eq!(chained_inverse_output, legacy_inverse_output);
                    legacy_chained_permutation_checks += 2;

                    assert_eq!(
                        apply_scalar(&legacy_inverse.ops, legacy_forward_output),
                        input
                    );
                    assert_eq!(
                        apply_scalar(&chained_inverse.ops, chained_forward_output),
                        input
                    );
                    forward_inverse_pair_checks += 2;

                    let input_sum = (input >> sum_offset) & length_mask;
                    assert_eq!(
                        (chained_forward_output >> sum_offset) & length_mask,
                        input_sum
                    );
                    assert_eq!(
                        (chained_inverse_output >> sum_offset) & length_mask,
                        input_sum
                    );
                    boundary_restoration_checks += 2;

                    for output in [
                        legacy_forward_output,
                        legacy_inverse_output,
                        chained_forward_output,
                        chained_inverse_output,
                    ] {
                        assert_eq!(output >> data_width, 0);
                        scratch_clean_checks += 1;
                    }
                    sweep_basis_states_checked += 1;
                }
                sweep_parameter_sets_checked += 1;
            }
        }
    }

    const LOCATION_LENGTH_WIDTH: usize = 3;
    let mut control_states_seen = [false; 8];
    for work_width in 1..=6 {
        let legacy_forward = build_location_swap_with_mode(
            work_width,
            LOCATION_LENGTH_WIDTH,
            false,
            EqualityMaskSweepMode::Legacy,
        );
        let legacy_inverse = build_location_swap_with_mode(
            work_width,
            LOCATION_LENGTH_WIDTH,
            true,
            EqualityMaskSweepMode::Legacy,
        );
        let chained_forward = build_location_swap_with_mode(
            work_width,
            LOCATION_LENGTH_WIDTH,
            false,
            EqualityMaskSweepMode::Chained,
        );
        let chained_inverse = build_location_swap_with_mode(
            work_width,
            LOCATION_LENGTH_WIDTH,
            true,
            EqualityMaskSweepMode::Chained,
        ...
        let work_mask = (1u64 << work_width) - 1;
        let length_mask = (1u64 << LOCATION_LENGTH_WIDTH) - 1;
        let data_width = 3 + work_width + 2 * LOCATION_LENGTH_WIDTH;
        for input in 0..(1u64 << data_width) {
            control_states_seen[(input & 7) as usize] = true;
            let phase1 = (input & 1) != 0;
            let phase2 = ((input >> 1) & 1) != 0;
            let mut sign = ((input >> 2) & 1) != 0;
            let mut work = (input >> 3) & work_mask;
            let l_t = (input >> (3 + work_width)) & length_mask;
            let mut l_q = (input >> (3 + work_width + LOCATION_LENGTH_WIDTH)) & length_mask;
            let active = phase1 ^ phase2;
            if active && !phase1 {
                l_q = l_q.wrapping_add(1) & length_mask;
            }
            ...
            let expected = u64::from(phase1)
                ...
                | (work << 3)
                | (l_t << (3 + work_width))
                | (l_q << (3 + work_width + LOCATION_LENGTH_WIDTH));
            ...
            let chained_inverse_output = apply_scalar(&chained_inverse.ops, input);
            assert_eq!(legacy_forward_output, expected);
            assert_eq!(chained_forward_output, expected);
            assert_eq!(chained_forward_output, legacy_forward_output);
            assert_eq!(chained_inverse_output, legacy_inverse_output);
            legacy_chained_permutation_checks += 2;
            ...
            assert_eq!(
                apply_scalar(&legacy_forward.ops, legacy_inverse_output),
                input
            );
            assert_eq!(
                apply_scalar(&chained_forward.ops, chained_inverse_output),
                input
            );
            forward_inverse_pair_checks += 4;
            ...
            location_basis_states_checked += 1;
        }
    }
    assert!(control_states_seen.into_iter().all(|seen| seen));

    let original_feature = std::env::var_os(Q873_CHAINED_EQUALITY_MASK_ENV);
    std::env::remove_var(Q873_CHAINED_EQUALITY_MASK_ENV);
    assert!(!q873_chained_equality_mask_requested());
    let mut default_off_stream_identity_checks = 0usize;
    for work_width in 1..=6 {
        for inverse in [false, true] {
            let default = build_location_swap_with_mode(
                work_width,
                LOCATION_LENGTH_WIDTH,
                inverse,
                EqualityMaskSweepMode::Production,
            );
            let legacy = build_location_swap_with_mode(
                work_width,
                LOCATION_LENGTH_WIDTH,
                inverse,
                EqualityMaskSweepMode::Legacy,
            );
            assert_equality_mask_stream_identity(&default, &legacy);
            default_off_stream_identity_checks += 1;
        }
    }
    if let Some(value) = original_feature {
        std::env::set_var(Q873_CHAINED_EQUALITY_MASK_ENV, value);
    }

    Q873EqualityMaskChainingProofReport {
        sweep_parameter_sets_checked,
        sweep_basis_states_checked,
        location_work_widths_checked: 6,
        location_basis_states_checked,
        control_states_checked: control_states_seen.len(),
        legacy_chained_permutation_checks,
        forward_inverse_pair_checks,
        boundary_restoration_checks,
        scratch_clean_checks,
        phase_clean_streams_checked,
        transition_arithmetic_checks,
        default_off_stream_identity_checks,
        audit: q873_equality_mask_sweep_audit(),
    }
}
