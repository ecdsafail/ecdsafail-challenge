...
fn cuccaro_add_mod_2n_refs_target_refs(
    circ: &mut Circuit,
    a: &[&QReg],
    b: &[&QReg],
    carry: &QReg,
...
fn cuccaro_sub_mod_2n_refs_target_refs(
    circ: &mut Circuit,
    a: &[&QReg],
    b: &[&QReg],
    carry: &QReg,
...
fn q843_borrow_rotated_bitlen_sign_requested() -> bool {
    std::env::var("LOWQ_Q843_BORROW_ROTATED_BITLEN_SIGN")
        .ok()
        .as_deref()
        == Some("1")
}

fn q842_borrow_rotated_bitlen_high_requested() -> bool {
    std::env::var("LOWQ_Q842_BORROW_ROTATED_BITLEN_HIGH")
        .ok()
        .as_deref()
        == Some("1")
}

fn xor_bitlength_high_bit_from_source(
    circ: &mut Circuit,
    source: &[&QReg],
    low_width: usize,
    target: &QReg,
    ...
    let Some(threshold) = 1usize.checked_shl(low_width as u32) else {
        return;
    };
    if threshold == 0 {
        return;
    ...
    let controls = &source[start..];
    assert!(
        controls.len() <= 4,
        "q842 bit-length high-bit helper only covers the current four-lane tail"
    );
    for (index, lane) in temps.iter().enumerate() {
        assert_ne!(lane.id(), target.id());
        assert!(
            temps[..index].iter().all(|other| other.id() != lane.id()),
            "q842 high-bit temp lane aliases an earlier temp"
        );
        assert!(
            source.iter().all(|source_lane| source_lane.id() != lane.id()),
            "q842 high-bit temp lane aliases the source"
        );
    }
    assert!(
        source.iter().all(|source_lane| source_lane.id() != target.id()),
        "q842 high-bit target aliases the source"
    );
    if !source_is_complemented {
    ...
    match controls.len() {
        ...
        2 => {
            ...
            circ.ccx(controls[0], controls[1], temps[0]);
            circ.cx(temps[0], target);
            circ.ccx(controls[0], controls[1], temps[0]);
        }
        3 => {
            assert!(temps.len() >= 2);
            circ.ccx(controls[0], controls[1], temps[0]);
            circ.ccx(temps[0], controls[2], temps[1]);
            circ.cx(temps[1], target);
            circ.ccx(temps[0], controls[2], temps[1]);
            circ.ccx(controls[0], controls[1], temps[0]);
        }
        4 => {
            assert!(temps.len() >= 3);
            circ.ccx(controls[0], controls[1], temps[0]);
            circ.ccx(controls[2], controls[3], temps[1]);
            circ.ccx(temps[0], temps[1], temps[2]);
            circ.cx(temps[2], target);
            circ.ccx(temps[0], temps[1], temps[2]);
            circ.ccx(controls[2], controls[3], temps[1]);
            circ.ccx(controls[0], controls[1], temps[0]);
        }
...
fn controlled_xor_saturating_difference_inplace_boundary(
    ...
    output: &[QReg],
    scratch: &[&QReg],
    borrowed_sign: Option<&QReg>,
) {
    if borrowed_sign.is_some() {
    ...
    let sign;
    let low_length;
    if let Some(borrowed_sign) = borrowed_sign {
        assert_ne!(borrowed_sign.id(), control.id());
        assert!(boundary.iter().all(|lane| lane.id() != borrowed_sign.id()));
        assert!(source.iter().all(|lane| lane.id() != borrowed_sign.id()));
        assert!(output.iter().all(|lane| lane.id() != borrowed_sign.id()));
        assert!(length.iter().all(|lane| lane.id() != borrowed_sign.id()));
        assert!(scratch[..3]
            .iter()
            .all(|lane| lane.id() != borrowed_sign.id()));
        low_length = length;
        sign = borrowed_sign;
    } else {
        let (low, sign_lane) = length.split_at(boundary.len());
        low_length = low;
        sign = sign_lane[0];
    }
    ...
    cuccaro_sub_mod_2n_refs_target_refs(circ, &boundary_refs, low_length, carry, sign);
    circ.x(sign);
    circ.ccx(control, sign, enabled);
    ...
    circ.ccx(control, sign, enabled);
    circ.x(sign);
    cuccaro_add_mod_2n_refs_target_refs(circ, &boundary_refs, low_length, carry, sign);
...
fn controlled_xor_saturating_difference_inplace_mixed_boundary(
    ...
    if borrowed_sign.is_some() {
        assert_eq!(length.len(), output.len());
    } else {
    ...
    if let Some(borrowed_sign) = borrowed_sign {
        ...
        let (low, sign_lane) = length.split_at(output.len());
        low_length = low;
        sign = sign_lane[0];
    }
    cuccaro_sub_mod_2n_refs_target_refs(circ, &extended_boundary, low_length, carry, sign);
    circ.x(sign);
    circ.ccx(control, sign, enabled);
    for (difference_bit, output_bit) in low_length.iter().zip(output) {
        circ.ccx(enabled, difference_bit, output_bit);
    ...
    circ.ccx(control, sign, enabled);
    circ.x(sign);
    cuccaro_add_mod_2n_refs_target_refs(circ, &extended_boundary, low_length, carry, sign);
...
fn controlled_xor_saturating_bit_length_difference_with_route(
    ...
    let borrow_mixed_sign = q843_borrow_rotated_bitlen_sign_requested()
        && inplace
        && (boundary.len() == output.len() || boundary.len() + 1 == output.len())
        && scratch.len() >= 4;
    let borrow_high_bit = q842_borrow_rotated_bitlen_high_requested()
        && borrow_mixed_sign
        && output.len() > 1
        && scratch.len() >= 8
        && source.len() > ((1usize << (output.len() - 1)) - 1)
        && source.len() - ((1usize << (output.len() - 1)) - 1) <= 4;
    ...
    let length_width = if borrow_high_bit {
        output.len() - 1
    } else if borrow_mixed_sign {
        output.len()
    } else {
        signed_width
    };
    ...
    let borrowed_high_bit = borrow_high_bit.then_some(scratch[4]);
    if let Some(high_bit) = borrowed_high_bit {
        xor_bitlength_high_bit_from_source(
            circ,
            source,
            length_width,
            high_bit,
    ...
    if inplace {
        let length_refs: Vec<&QReg> = length
            .iter()
            .chain(borrowed_high_bit.into_iter())
            .collect();
        if boundary.len() == output.len() {
            let borrowed_sign = if borrow_mixed_sign {
                Some(scratch[3])
            } else {
                None
            };
        ...
        assert!(
            !borrow_high_bit,
            "q842 borrowed high bit is only implemented for in-place rotated bit length"
        );
...
fn controlled_xor_rotated_suffix_with_zero_s_lender(
    ...
    assert!(scratch.len() >= l_s.len().saturating_sub(2));
    uncompute_zero(circ, l_s, zero_s, scratch);
    controlled_xor_rotated_suffix_bit_length_with_prefix_scratch(
        circ,
        control,
...
fn conditional_work_and_length_swap_promised_l_q_zero(
    ...
    with_bit_length_callsite(
        ...
        || {
            if predicate_prefix_scratch.is_empty() {
                controlled_xor_rotated_suffix_bit_length(
                    circ,
                    control,
                    l_t,
...
fn coefficient_fused_data_and_sign_q845_swap_only(
    ...
    if inverse {
        ...
        guard.toggle_nonzero(circ, phase1, l_t_prime, above_guard, &cursor_scratch);
        ...
        circ.x(above_guard);
        circ.cx(phase1, sign);
        guard.toggle_nonzero(circ, enable, l_t_prime, above_guard, &cursor_scratch);
...
fn build_mixed_rotated_boundary_harness(
    ...
    assert_eq!(output_width, boundary_width + 1);
    assert!(source_width <= (1usize << output_width) - 1);
    assert!(scratch_lanes == 0 || scratch_lanes >= 3);
    let mut circ = Circuit::new();
    let control = circ.alloc_qreg("rs.mixed-boundary-proof.control");
    let boundary = circ.alloc_qreg_bits("rs.mixed-boundary-proof.boundary", boundary_width);
    let source = circ.alloc_qreg_bits("rs.mixed-boundary-proof.source", source_width);
    let output = circ.alloc_qreg_bits("rs.mixed-boundary-proof.output", output_width);
    let scratch = circ.alloc_qreg_bits("rs.mixed-boundary-proof.scratch", scratch_lanes);
    let source_refs: Vec<&QReg> = source.iter().collect();
    let scratch_refs: Vec<&QReg> = scratch.iter().collect();
    let data_ids: Vec<u32> = std::iter::once(&control)
        .chain(&boundary)
...
pub fn exhaustive_inplace_rotated_bitlen_boundary_check() -> InplaceRotatedBoundaryProofReport {
    ...
    let modes = [RawBitLengthZeroMode::Legacy, RawBitLengthZeroMode::Fused];
    let scratch_modes = [0usize, 3usize, 4usize];
    ...
    assert!(candidate_local.peak_qubits < baseline_local.peak_qubits);
...
/// Reduced proof for the Q843 borrowed sign lane on the mixed
/// boundary-width route used by the sub-800 q844 path.
#[doc(hidden)]
pub fn exhaustive_q843_borrowed_mixed_rotated_bitlen_sign_check() -> (usize, usize, usize, usize) {
    assert_eq!(
        std::env::var("LOWQ_Q843_BORROW_ROTATED_BITLEN_SIGN")
            .ok()
            .as_deref(),
    ...
    let _environment = RawBitLengthProofEnvironment::capture();
    let configurations = [(1usize, 2usize, 1usize), (2, 3, 3), (3, 4, 4)];
    let modes = [RawBitLengthZeroMode::Legacy, RawBitLengthZeroMode::Fused];
    let scratch_modes = [4usize, 5usize];
    ...
    for mode in modes {
        configure_inplace_rotated_boundary_proof(mode);
        for scratch_lanes in scratch_modes {
            for &(boundary_width, output_width, source_width) in &configurations {
                let baseline = build_mixed_rotated_boundary_harness(
                    boundary_width,
                    output_width,
                    source_width,
                    scratch_lanes,
                ...
                let candidate = build_mixed_rotated_boundary_harness(
                    boundary_width,
                    output_width,
                    source_width,
                    scratch_lanes,
                ...
                for value in 0..data_states {
                    ...
                    assert_inplace_rotated_boundary_clean(
                        &baseline,
                        baseline_output,
                        "mixed materialized boundary",
                    );
                    assert_inplace_rotated_boundary_clean(
                        &candidate,
                        candidate_output,
                        "mixed borrowed-sign boundary",
                    );
                    assert_eq!(
                        candidate_output & candidate.preserved_mask,
                        input & candidate.preserved_mask
    ...
    (
        configurations.len() * modes.len() * scratch_modes.len(),
        basis_states_checked,
        simulator_equivalence_checks,
        phase_clean_checks + inverse_pair_checks,
    )
}
