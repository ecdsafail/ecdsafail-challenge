...
impl<'a> Q828LsParityBridge<'a> {
    ...
    fn prepare(
        ...
        assert_eq!(l_q_dirty.len(), SUB820_L_Q_WIDTH);
        ...
        let full_l_s = std::iter::once(host.borrowed_alias())
            ...
            .collect::<Vec<_>>();
        let full_refs = full_l_s.iter().collect::<Vec<_>>();
        let clean_scratch = count.iter().collect::<Vec<_>>();
        ...
        let address = std::iter::once(remainder_low.borrowed_alias())
            .chain(l_s_high.iter().map(QReg::borrowed_alias))
            .collect::<Vec<_>>();
        let address_ids = address.iter().map(QReg::id).collect::<Vec<_>>();
    ...
    fn prepare_reverse_boundary(&self, circ: &mut Circuit, l_q_dirty: &[QReg]) {
        ...
        let dirty = l_q_dirty[..6].iter().collect::<Vec<_>>();
...
fn build_q828_ls_parity_bridge_harness(
    ...
    let l_r_prime_ids = l_r_prime.iter().map(QReg::id).collect::<Vec<_>>();
    let l_q_dirty_ids = l_q_dirty.iter().map(QReg::id).collect::<Vec<_>>();
    let scratch_ids = count
        .iter()
        .chain(std::iter::once(&carry))
        .map(QReg::id)
...
fn build_q828_terminal_rotation_harness(
    ...
    let low_id = low.id();
    let high_ids = high.iter().map(QReg::id).collect::<Vec<_>>();
    let baseline_ids = baseline.iter().map(QReg::id).collect::<Vec<_>>();
    let candidate_ids = candidate.iter().map(QReg::id).collect::<Vec<_>>();
    let builder = circ.into_builder();
    let mut external = vec![false; builder.next_qubit as usize];
...
fn q828_terminal_one_hot_position<R: sha3::digest::XofReader>(
    simulator: &crate::sim::Simulator<'_, R>,
    ids: &[u32],
    shot: usize,
) -> usize {
...
fn q828_terminal_rotate_high(circ: &mut Circuit, l_s_high: &[QReg], register: &[QReg]) {
    ...
    for current in 1..register.len() {
        circ.swap(&register[0], &register[current]);
    }
