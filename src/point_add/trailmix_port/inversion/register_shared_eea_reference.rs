...
use super::register_shared_eea_microkernels::{
    add_constant_mod_2n_dirty_lenders, apply_scalar, controlled_add_one,
    controlled_decrement_mod_2n_dirty_lenders, controlled_increment_mod_2n,
    controlled_increment_mod_2n_dirty_lenders, controlled_sub_one,
...
// --- cursor-scratch lifetime probe (additive; only active when a probe is installed) ---
thread_local! {
    static CURSOR_SCRATCH_PROBE: std::cell::RefCell<Option<CursorScratchProbe>> =
        const { std::cell::RefCell::new(None) };
}

#[derive(Default, Clone)]
struct CursorScratchProbe {
    cursor_ids: Vec<u32>,
    reverse_boundary_ops_idx: usize,
    saw_reverse_boundary: bool,
}

fn cursor_scratch_probe_record_ids(ids: &[u32]) {
    CURSOR_SCRATCH_PROBE.with(|cell| {
        if let Some(probe) = cell.borrow_mut().as_mut() {
            probe.cursor_ids = ids.to_vec();
        }
    });
}

fn cursor_scratch_probe_record_reverse_boundary(ops_idx: usize) {
    CURSOR_SCRATCH_PROBE.with(|cell| {
        if let Some(probe) = cell.borrow_mut().as_mut() {
            if !probe.saw_reverse_boundary {
                probe.reverse_boundary_ops_idx = ops_idx;
                probe.saw_reverse_boundary = true;
            }
        }
    });
}
...
/// sub-846 FRONT P2: shrink the bit-length direct-prefix-AND loan from the
/// nine-lane full scratch (5 KG + 3 increment + 1 flag) to only the five clean
/// Khattar-Gidney prefix ancillae, by computing the per-position unit update
/// with a dirty `mcx_dirty_ladder`-class increment (option (c): in-place dirty
/// update, no fresh increment scratch, no materialized flag). The four removed
/// clean lanes are exactly the owned `rs.dynamic-bitlen.fused-prefix-scratch`
/// allocation on the P2 plateau, so this drops the P2 co-peak by up to its
/// owned-lane count. Additive flag; OFF => bit-identical nine-lane clean loan.
pub const KG_ONLY_PREFIX_SCRATCH_FLAG: &str = "LOWQ_KG_ONLY_PREFIX_SCRATCH";
pub const KG_ONLY_PREFIX_BORROWED_FLAG_ENV: &str = "LOWQ_KG_ONLY_PREFIX_BORROWED_FLAG";
...
/// C1 (sub-851): route the swap-scan unary-iterate prefix-AND MCX through a
/// borrowed DIRTY ancilla instead of allocating the clean `mcxk_t3` lane that
/// sets peak=851. The borrowed lane is an idle caller register (`add_only`),
/// restored exactly by the dirty-MCX construction. Additive flag; OFF => the
/// baked 851 behaviour is bit-identical.
pub const Q845_SWAP_DIRTY_MCX_FLAG: &str = "LOWQ_Q845_SWAP_DIRTY_MCX";
/// C1 extension (sub-850): drop the persistent Khattar-Gidney prefix-AND tree
/// (`uls_anc`, 3 clean qubits on the n=9 swap scan) by emitting the swap-scan
/// all-ones detector as a single full-width `mcx_dirty_ladder` over all 9
/// `address` bits, borrowing n-2 = 7 dirty lenders from the idle `l_q` length
/// register. `l_q` is NEVER threaded into the swap step (`coefficient_phase_block`
/// has no l_q parameter), so it is provably untouched there; `mcx_dirty_ladder`
/// restores every lender exactly, so l_q's live bit-length value is preserved.
/// Additive flag; OFF => the LOWQ_Q845_SWAP_DIRTY_MCX (peak 850) behaviour is
/// bit-identical. Requires LOWQ_Q845_SWAP_DIRTY_MCX (shares the `phase2` borrow
/// path for the single-lane fallback).
pub const Q845_SWAP_LADDER_DETECTOR_FLAG: &str = "LOWQ_Q845_SWAP_LADDER_DETECTOR";

/// uls_gate fusion (sub-847 -> 846): on the ladder-detector path, the swap-scan
/// all-ones detector is XORed DIRECTLY into the swap `active` flag via the
/// `mcx_dirty_ladder` (`unary_iterate_log_star_ladder_fused_cx`), instead of
/// being computed into a fresh clean `uls_gate` ancilla and then driving
/// `cx(uls_gate, active)`. The body consumes the detector as exactly one
/// `cx(gate, active)`, so fusing `ladder(c -> gate); cx(gate, active)` into
/// `ladder(c -> active)` is bit-identical net while saving the `uls_gate` qubit
/// (peak 847 -> 846). Requires LOWQ_Q845_SWAP_LADDER_DETECTOR (needs the same
/// n-2 idle `l_q` lenders). Additive flag; OFF => the ladder-detector (peak 847)
/// behaviour with the clean `uls_gate` is restored bit-identically.
pub const Q846_ULS_GATE_FUSE_FLAG: &str = "LOWQ_Q846_ULS_GATE_FUSE";

/// sub-846 FRONT P2: host ONE lane of the `scheduled.swap-condition`
/// `condition_scratch` (the 10-lane [zero_q,zero_s,control,chain(7)] register
/// that co-binds the 846 peak with P1) on the idle persistent `phase2` flag,
/// dropping the fresh allocation 10 -> 9. The `chain` is consumed by
/// `compute_zero`/`uncompute_zero` and the swap support v-chain; both are
/// converted to `mcx_dirty_ladder` here so the borrowed `phase2` lane (live,
/// non-zero, restored exactly by the cascade) is a legal dirty lender. The
/// bit-length swap kernels already borrow their prefix scratch dirty-tolerantly.
/// Additive; OFF => the clean 10-lane condition_scratch is bit-identical.
pub const Q845_SWAP_CONDITION_BORROW_FLAG: &str = "LOWQ_Q845_SWAP_CONDITION_BORROW";

/// EXPERIMENTAL (sub-846 cursor-scratch borrow probe). Host the swap-step
/// `cursor-scratch` (8 lanes, lanes 839..845 of the peak climb) on borrowed
/// `l_q` lanes instead of a fresh clean allocation, eliminating the +8
/// transient (target peak 846 -> 838). The dirty-tolerant uses (Bennett
/// v-chains, controlled add/sub-one v-chains) restore any borrowed content; the
/// CLEAN-ZERO uses (the reverse-boundary constant-operand buffer in
/// `prepare_reverse_boundary` / `for_each_reverse`) do NOT. This flag exists to
/// obtain BUILD evidence of exactly which uses survive a dirty borrow.
/// OFF (default) => fresh clean cursor-scratch, bit-identical to the 846 build.
pub const Q838_CURSOR_BORROW_FLAG: &str = "LOWQ_Q838_CURSOR_BORROW";

/// sub-846 lever 2 (note-sub851 UPDATE 4): rewrite `prepare_reverse_boundary`
/// to do an IN-PLACE register add of `l_s` / `l_r_prime` directly onto
/// `address` instead of staging through the `source` buffer (which IS the
/// swap-step `cursor-scratch`). The old path does
/// `cx(addend -> source); cuccaro_add(source, address); cx(addend -> source)`,
/// requiring `source` (cursor-scratch) clean-|0> so the cx-staged operand
/// equals the addend. Cuccaro leaves its first operand unchanged, so the same
/// `address += addend` is obtained by passing `addend` DIRECTLY as the cuccaro
/// addend -- no `source` buffer, no clean-zero requirement on cursor-scratch at
/// the boundary. The l_r_prime add (8 lanes) is zero-extended to address width
/// (9) by appending a single clean lane (`coefficient_active`, idle+clean at the
/// boundary, supplied as the last `source` lane). This removes the boundary's
/// clean-zero pin on cursor-scratch. Additive flag; OFF => the staged
/// `source`-buffer path is bit-identical.
pub const Q838_INPLACE_BOUNDARY_FLAG: &str = "LOWQ_Q838_INPLACE_BOUNDARY";
...
pub struct Q845SwapOnlyCoefficientProofReport {
    ...
    /// Number of (data input ⊗ dirty l_q value) basis states for which the
    /// production `mcx_dirty_ladder` l_q-lender swap-detector path was exercised
    /// AND proven data-transparent + l_q-restored + reversible. Nonzero == the
    /// dirty-borrow path is now covered by the small-n value proof.
    pub lq_borrow_dirty_path_checks: usize,
    pub lq_restore_checks: usize,
    pub lq_data_transparent_checks: usize,
    ...
    /// sub-846 FRONT P2: number of (swap input ⊗ dirty borrowed-lane value)
    /// basis states for which the swap-condition borrow (host one chain lane on
    /// an external dirty lane) was exercised AND proven data-transparent vs the
    /// clean path + borrowed-lane-restored + scratch-clean + reversible. Nonzero
    /// == the P2 phase2-borrow is covered by the small-n value proof.
    pub swap_condition_borrow_checks: usize,
    /// sub-846 FRONT P2 (positive): number of swap basis states for which the
    /// KG-only fused-prefix loan (the lane-reduced bit-length kernel) produced
    /// output BIT-IDENTICAL to the clean nine-lane reference. Nonzero == the
    /// dirty-tolerant / lane-reduced length kernel is value-correct and the P2
    /// loan can shrink. (Complements the negative swap-condition-borrow result.)
    pub kg_only_prefix_checks: usize,
    /// Peak qubits of the swap harness with the clean nine-lane prefix loan vs
    /// the KG-only eight-lane loan, at the SAME work width. The delta is the
    /// per-swap qubit saving the KG-only loan realises (expected -1: the freed
    /// owned fused-prefix lane).
    pub kg_only_clean_peak_qubits: usize,
    pub kg_only_kg_peak_qubits: usize,
    /// Op inflation: emitted ops of the KG-only harness vs the clean harness.
    pub kg_only_clean_ops: usize,
    pub kg_only_kg_ops: usize,
}
...
fn compute_zero(circ: &mut Circuit, register: &[QReg], zero: &QReg, chain: &[QReg]) {
    let chain_refs: Vec<&QReg> = chain.iter().collect();
    compute_zero_refs(circ, register, zero, &chain_refs);
}

/// `zero ^= (register == 0)` with a BORROWED chain (`&[&QReg]`). When the
/// swap-condition borrow is active the chain may host a dirty external lane
/// (phase2), so the all-ones detector uses the dirty-tolerant `mcx_dirty_ladder`
/// (restores every lender) instead of the clean Bennett v-chain. Bit-identical
/// net on `zero` and on the chain in both modes.
fn compute_zero_refs(circ: &mut Circuit, register: &[QReg], zero: &QReg, chain: &[&QReg]) {
    assert!(!register.is_empty());
    for bit in register {
        circ.x(bit);
    }
    let controls: Vec<&QReg> = register.iter().collect();
    if q845_swap_condition_borrow_requested() {
        mcx_dirty_ladder(circ, &controls, zero, chain);
    } else {
        let chain_owned: Vec<&QReg> = chain.to_vec();
        // multi_controlled_x_vchain takes &[QReg]; reconstruct the owned form is
        // impossible (chain may be external refs), so on the clean path call the
        // ref-based vchain equivalent via the borrowed variant (identical gates).
        multi_controlled_x_vchain_borrowed(circ, &controls, zero, &chain_owned);
    }
    for bit in register {
        circ.x(bit);
    }
...
fn uncompute_zero_refs(circ: &mut Circuit, register: &[QReg], zero: &QReg, chain: &[&QReg]) {
    compute_zero_refs(circ, register, zero, chain);
}
...
pub fn normalized_phase_update(
    ...
    assert_l_r_prime_metadata_width(l_q.len(), l_r_prime.len());
...
fn kg_only_prefix_scratch_requested() -> bool {
    std::env::var(KG_ONLY_PREFIX_SCRATCH_FLAG).ok().as_deref() == Some("1")
}
...
fn bit_length_lean_allow_zero_with_borrowed_scratch_impl(
    ...
    borrowed_prefix_flag: Option<&QReg>,
) {
    use super::shrunken_pz_state_machine::{
        ...
        bit_length_lean_with_kg_only_prefix_scratch,
        bit_length_lean_with_kg_only_prefix_scratch_complemented_source,
        lowq_fused_zero_prefix_bitlen_requested,
        lowq_kg_only_prefix_borrowed_flag_requested, DIRECT_PREFIX_FULL_SCRATCH_LEN,
        DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN, DIRECT_PREFIX_KG_ONLY_WITH_FLAG_LEN,
    };

    let prefix_loan = fused_prefix_scratch_loan_requested();
    let kg_only = kg_only_prefix_scratch_requested();
    // A borrowed clean-|0> flag (the swap promised-support lane) lets the
    // KG-only two-control prefix callbacks use the cheap clean-CINC path while
    // keeping owned=1 (peak-neutral): the loan becomes 9 lanes = 1 owned + 7
    // chain + 1 borrowed flag.
    let kg_only_borrowed_flag = kg_only
        && prefix_loan
        && lowq_kg_only_prefix_borrowed_flag_requested()
        && borrowed_prefix_flag.is_some();
    if prefix_loan {
        assert!(
            lowq_fused_zero_prefix_bitlen_requested(),
    ...
    if full_zero_carry_prefix_scratch_requested() {
        ...
        // KG-only mode reduces the prefix-AND clean-lane requirement from the
        // full nine-lane budget (5 KG + 3 increment + 1 flag) to only the five
        // Khattar-Gidney prefix ancillae; the per-position unit update runs as a
        // dirty mcx_dirty_ladder over the (restored) source bits + KG ancillae.
        //
        // KG-only mode only pays off where it saves an owned lane on the binding
        // 845 plateau -- i.e. the swap promised-support site, which supplies the
        // borrowed flag. Off-plateau callers (no borrowed flag) instead use the
        // original full nine-lane clean materialized-flag path: that path is the
        // long-proven cheap construction and its extra owned flag lane is
        // peak-neutral away from the plateau. This keeps every two-control prefix
        // callback on the cheap clean-CINC path (no O(n) dirty ladder) without
        // relying on the unvalidated owned-flag KG-only variant.
        let use_full_clean_offplateau = kg_only
            && !kg_only_borrowed_flag
            && lowq_kg_only_prefix_borrowed_flag_requested();
        let use_kg_only = kg_only && !use_full_clean_offplateau;
        let required_lanes = if kg_only_borrowed_flag {
            DIRECT_PREFIX_KG_ONLY_WITH_FLAG_LEN
        } else if use_kg_only {
            DIRECT_PREFIX_KG_ONLY_SCRATCH_LEN
        } else {
            DIRECT_PREFIX_FULL_SCRATCH_LEN
        };
        // The appended borrowed flag (when present) occupies the final lane; the
        // chain + owned lanes fill the remaining `required_lanes - flag` slots,
        // so the owned-lane count (and thus the plateau peak) is unchanged from
        // the 8-lane KG-only loan.
        let flag_lanes = usize::from(kg_only_borrowed_flag);
        let chain_target = required_lanes - flag_lanes;
        let borrowed_lanes = if prefix_loan {
            borrowed_prefix_scratch.len().min(chain_target)
        } else {
            0
        };
        let owned_lanes = chain_target - borrowed_lanes;
        ...
        let scratch_refs: Vec<&QReg> = scratch
            ...
            .chain(
                borrowed_prefix_flag
                    .iter()
                    .copied()
                    .take(flag_lanes),
            )
            .collect();
        assert_eq!(scratch_refs.len(), required_lanes);
        if use_kg_only {
...
fn bit_length_lean_allow_zero_with_borrowed_scratch_and_flag(
    circ: &mut Circuit,
    source: &[&QReg],
    output: &[QReg],
    decrement: bool,
...
fn bit_length_lean_allow_zero_with_borrowed_scratch_complemented_source_and_flag(
    circ: &mut Circuit,
    source: &[&QReg],
    output: &[QReg],
    decrement: bool,
...
fn q845_swap_dirty_mcx_requested() -> bool {
    std::env::var(Q845_SWAP_DIRTY_MCX_FLAG).ok().as_deref() == Some("1")
}

fn q845_swap_ladder_detector_requested() -> bool {
    q845_swap_dirty_mcx_requested()
        && std::env::var(Q845_SWAP_LADDER_DETECTOR_FLAG).ok().as_deref() == Some("1")
}

fn q846_uls_gate_fuse_requested() -> bool {
    q845_swap_ladder_detector_requested()
        && std::env::var(Q846_ULS_GATE_FUSE_FLAG).ok().as_deref() == Some("1")
}

/// sub-846 FRONT P2: host one `scheduled.swap-condition` chain lane on the idle
/// `phase2` (dirty lender), shrinking condition_scratch 10 -> 9. Routes
/// compute_zero / swap-support to `mcx_dirty_ladder`.
fn q845_swap_condition_borrow_requested() -> bool {
    std::env::var(Q845_SWAP_CONDITION_BORROW_FLAG).ok().as_deref() == Some("1")
}

/// Reserved hook for the cursor-scratch borrow (sub-846 investigation). The
/// borrow is BLOCKED (see note-sub851 UPDATE 3): the swap-step plateau is a
/// co-peak with the dynamic-bitlen plateau (both = 846), so freeing the 8
/// cursor lanes alone cannot lower the measured peak; additionally the
/// reverse-boundary constant-operand buffer requires clean-zero scratch.
fn q838_cursor_borrow_requested() -> bool {
    std::env::var(Q838_CURSOR_BORROW_FLAG).ok().as_deref() == Some("1")
}

/// When the cursor-scratch borrow is active AND a swap-detector dirty bank
/// (`l_q`) with enough idle lenders is available, return the lender slice the
/// dirty-ladder consumer should borrow. Otherwise return `None`, leaving the
/// caller on its clean-|0> cursor-scratch path (bit-identical fallback).
///
/// `controls` is the FULL control count of the widest MCX the consumer emits;
/// an `mcx_dirty_ladder` over `controls` controls needs `controls - 2` restored
/// lenders. The swap-detector restores all `l_q` lenders before the iterate body
/// runs (the `mcx_dirty_ladder` detector is compute-uncompute), so during the
/// body every `l_q` lane is an available dirty lender.
fn cursor_borrow_lenders<'a, 'b>(
    dirty_bank: Option<&'a [&'b QReg]>,
    controls: usize,
) -> Option<&'a [&'b QReg]> {
    if !q838_cursor_borrow_requested() {
        return None;
    }
    let needed = controls.saturating_sub(2);
    dirty_bank.filter(|bank| bank.len() >= needed)
}

/// sub-846 lever 2: emit the in-place `prepare_reverse_boundary` (direct cuccaro
/// add onto `address`, no `source` staging buffer) instead of the staged path.
fn q838_inplace_boundary_requested() -> bool {
    std::env::var(Q838_INPLACE_BOUNDARY_FLAG).ok().as_deref() == Some("1")
}
...
fn controlled_xor_saturating_difference_inplace_boundary(
    ...
    assert_eq!(length.len(), boundary.len() + 1);
    trace_raw_bit_length_allocations(RawBitLengthAllocationTrace {
    ...
    let sign = &sign_lane[0];

    // On the zero-high-lane domain used here, the low word plus overflow of
    // this n-bit inverse-Cuccaro transform equals the low (n+1)-bit word of
    // the previous zero-extended transform. Store that overflow directly in
    // the existing sign lane instead of materializing an (n+1)-bit boundary.
...
fn controlled_xor_saturating_difference_inplace_mixed_boundary(
    ...
    assert_eq!(length.len(), output.len() + 1);
    trace_raw_bit_length_allocations(RawBitLengthAllocationTrace {
        rotated_inplace_boundary_uses: 1,
        ..RawBitLengthAllocationTrace::default()
    });
    ...
    // needs only eight lanes. Materialize its ninth zero only after the prefix
    // bit-length scratch has been released; this lane is restored locally.
    let zero_extension = circ.alloc_qreg("rs.l-r-prime.zero-extension");
...
fn controlled_xor_saturating_bit_length_difference_with_route(
    ...
    if paired_source_complement {
        bit_length_lean_allow_zero_with_borrowed_scratch_complemented_source_and_flag(
            circ,
            source,
            &length,
            false,
        ...
        bit_length_lean_allow_zero_with_borrowed_scratch_and_flag(
            circ,
            source,
            &length,
            false,
...
fn controlled_xor_rotated_prefix_bit_length(
    ...
    let scratch_refs: Vec<&QReg> = scratch.iter().copied().collect();
...
fn controlled_xor_rotated_suffix_bit_length(
    ...
    for lender in preserved_dy_top_scratch {
...
fn materialize_promised_l_q_swap_discrepancy(
    ...
    assert!(!l_q.is_empty());
    assert_eq!(l_q.len(), l_t_prime.len());
    assert_l_r_prime_metadata_width(l_q.len(), l_r_prime.len());
...
fn toggle_materialized_promised_l_q_swap_control(
    ...
    let support_ancillas: Vec<&QReg> = scratch
        .iter()
        .copied()
        .chain(support_workspace.iter())
        .take(required_ancillas)
        .collect();
    assert_eq!(support_ancillas.len(), required_ancillas);
    // The support v-chain is the ONLY clean-|0> consumer of the swap `scratch`
    // (the bit-length kernels borrow their prefix scratch dirty-tolerantly). When
    // the swap-condition borrow hosts one chain lane on a DIRTY external lane
    // (phase2), use the dirty-tolerant ladder so that lane need not be clean.
    if q845_swap_condition_borrow_requested() {
        mcx_dirty_ladder(circ, &controls, control, &support_ancillas);
    } else {
...
fn conditional_work_and_length_swap_under_zero_predicate(
    ...
    if q845_swap_only_t_prime_length_requested() {
        ...
        circ.ccx(zero_q, zero_s, control);
        // The freshly-allocated promised-support lane is clean-|0> and idle
        // during this bit-length scan; lend it as the KG-only prefix flag so the
        // two-control callbacks use the cheap clean-CINC path (peak-neutral: the
        // lane is already counted on the swap plateau). The bit-length restores
        // it to |0> before promised-support is consumed below.
    ...
    if q845_swap_only_t_prime_length_requested() {
        circ.ccx(zero_q, zero_s, control);
        // promised-support is clean-|0> here (restored by the (un)materialize
        // above, asserted clean by the free below); lend it as the KG-only
        // prefix flag (peak-neutral, restored to |0> by the bit-length).
        controlled_xor_rotated_prefix_bit_length(
            circ,
            control,
            l_r_prime,
...
/// counter, while one nine-lane address excludes the packed remainder suffix.
/// Existing coefficient scratch is time-shared for every carry chain.
struct Q845SwapOnlyCoefficientGuard {
    address: Vec<QReg>,
...
fn toggle_register_geq_constant_vchain(
    ...
    // The widest pattern below has `width` controls. When the cursor-borrow is
    // active, every MCX runs on the borrowed dirty `l_q` lenders (restored by
    // the Bennett cascade); otherwise on the clean cursor-scratch v-chain.
    let borrow_lenders = cursor_borrow_lenders(dirty_bank, width);
    if borrow_lenders.is_none() {
    ...
    let mut toggle_pattern = |required: &[(usize, bool)]| {
        ...
        let controls = required
            ...
            .collect::<Vec<_>>();
        if let Some(lenders) = borrow_lenders {
            mcx_dirty_ladder(circ, &controls, target, lenders);
        } else {
...
impl Q845SwapOnlyCoefficientGuard {
    fn prepare(
        circ: &mut Circuit,
        physical_work_width: usize,
        count: &[QReg],
    ...
    fn accumulate(
        ...
        if let Some(lenders) = cursor_borrow_lenders(dirty_bank, count.len()) {
            controlled_increment_mod_2n_dirty_lenders(circ, condition, count, lenders);
        } else {
    ...
    fn unaccumulate(
        ...
        if let Some(lenders) = cursor_borrow_lenders(dirty_bank, count.len()) {
            controlled_decrement_mod_2n_dirty_lenders(circ, condition, count, lenders);
        } else {
    ...
    {
        use crate::point_add::trailmix_port::arith::khattar_gidney::{
            unary_iterate_log_star_dirty_bank, unary_iterate_log_star_ladder_fused_cx,
        };

        assert!(scan_width <= self.physical_work_width);
        circ.x(active);
        let address = self.address.iter().collect::<Vec<_>>();
        ...
        // uls_gate fusion: when the ladder-detector lenders are available, XOR
        // the all-ones detector DIRECTLY into `active` (no clean uls_gate).
        // flip_after=false => detector THEN body (matches `cx(gate,active)`,body).
        let fused_lenders: Option<&[&QReg]> = if q846_uls_gate_fuse_requested() {
            dirty_bank.filter(|bank| bank.len() >= address.len().saturating_sub(2))
        } else {
            None
        };
        if let Some(lenders) = fused_lenders {
            unary_iterate_log_star_ladder_fused_cx(
                circ,
                &address,
                n_iters,
                lenders,
            ...
            unary_iterate_log_star_dirty_bank(
                circ,
                &address,
                n_iters,
                dirty_bank,
        ...
        F: FnMut(&mut Circuit, usize, &QReg),
    {
        ...
        assert!(scan_width <= self.physical_work_width);
        // The clean constant-staging buffer is required only when the constant
        // subtract uses cuccaro; the cursor-borrow path subtracts in place on
        // `address` via dirty lenders and never touches `constant_scratch`.
        if cursor_borrow_lenders(dirty_bank, self.address.len()).is_none() {
            assert_eq!(constant_scratch.len(), self.address.len());
        }
        let truncated = q851_truncated_swap_only_guard_requested()
            && scan_width < self.physical_work_width;
        let delta = self.physical_work_width - scan_width;
        // The `address -= delta` (and `+= delta` on exit) constant subtractions
        // stage `delta` through the clean `constant_scratch` (= cursor-scratch ∪
        // coefficient_active) buffer. When the cursor-borrow is active that buffer
        // is gone, so emit the SAME modular constant subtract IN PLACE on
        // `address` using the borrowed `l_q` dirty lenders (restored exactly).
        let address_width = self.address.len();
        let constant_borrow = cursor_borrow_lenders(dirty_bank, address_width);
        if truncated {
            if let Some(lenders) = constant_borrow {
                let address_refs: Vec<&QReg> = self.address.iter().collect();
                let modulus = 1usize << address_width;
                add_constant_mod_2n_dirty_lenders(
                    circ,
                    &address_refs,
                    (modulus - (delta % modulus)) % modulus,
                    lenders,
                );
            } else {
        ...
        // uls_gate fusion (reverse): body THEN detector-into-active
        // (flip_after=true, matching the reverse closure body,`cx(gate,active)`).
        let fused_lenders: Option<&[&QReg]> = if q846_uls_gate_fuse_requested() {
            dirty_bank.filter(|bank| bank.len() >= address.len().saturating_sub(2))
        } else {
            None
    ...
    fn toggle_nonzero(
        ...
        if let Some(lenders) = cursor_borrow_lenders(dirty_bank, controls.len()) {
            mcx_dirty_ladder(circ, &controls, target, lenders);
        } else {
    ...
    fn prepare_reverse_boundary(
        ...
        if q838_inplace_boundary_requested() {
            // The in-place path consumes only `source.last()` (the clean high
            // lane = coefficient_active) to zero-extend l_r_prime; the cursor
            // lanes are not used, so a single-lane `source` is sufficient when the
            // cursor-scratch register has been eliminated by the borrow.
            assert!(!source.is_empty(), "nonempty boundary high lane");
            // In-place add: cuccaro leaves its first operand (the addend)
            // unchanged, so `address += addend` is obtained by passing the live
            // `l_s` / `l_r_prime` register DIRECTLY as the addend -- no `source`
            // staging buffer, no clean-zero requirement on cursor-scratch.
            //
            // `l_s` is address-width (9); pass it directly. `l_r_prime` is one
            // lane narrower (8); zero-extend its MSB with the single clean lane
            // `source[last]` (= coefficient_active, idle+|0> at the boundary),
            // exactly the lane the staged path left at 0 in its l_r_prime blocks.
            // This reproduces the staged path bit-for-bit on `address` AND on
            // `overflow` (each cuccaro contributes `cx(addend_msb, overflow)`:
            // 0, l_s[msb], l_s[msb], 0 in BOTH paths).
            let l_s_refs: Vec<&QReg> = l_s.iter().collect();
            assert_eq!(l_s_refs.len(), self.address.len());
            let high_lane = *source.last().expect("nonempty boundary source");
            let mut l_r_prime_ext: Vec<&QReg> = l_r_prime.iter().collect();
            assert!(l_r_prime_ext.len() <= self.address.len());
            // Zero-extend l_r_prime's high lanes to address width with the clean
            // `source` high lane(s). When l_r_prime is already address-width
            // (the r_width == full_width config) this loop is inert.
            while l_r_prime_ext.len() < self.address.len() {
                l_r_prime_ext.push(high_lane);
            }
            assert_eq!(l_r_prime_ext.len(), self.address.len());

            cuccaro_add_mod_2n_refs(circ, &l_r_prime_ext, &self.address, carry, overflow);
            cuccaro_add_mod_2n_refs(circ, &l_s_refs, &self.address, carry, overflow);
            toggle_constant(circ, &self.address, self.physical_work_width);
            cuccaro_add_mod_2n_refs(circ, &l_s_refs, &self.address, carry, overflow);
            cuccaro_add_mod_2n_refs(circ, &l_r_prime_ext, &self.address, carry, overflow);
            return;
        }
        ...
        cuccaro_add_mod_2n_refs(circ, source, &self.address, carry, overflow);
        for (bit, destination) in l_r_prime.iter().zip(source) {
            circ.cx(bit, destination);
        }
    ...
    fn finish(
        ...
        cuccaro_sub_mod_2n(circ, count, &self.address, carry, overflow);
        for (bit, destination) in l_r_prime.iter().zip(count) {
            circ.cx(bit, destination);
        }
...
fn toggle_q851_fixed_sign_event(
    ...
    let target = signed_length.last().expect("nonempty signed length");
    if let Some(lenders) = cursor_borrow_lenders(dirty_bank, controls.len()) {
        mcx_dirty_ladder(circ, &controls, target, lenders);
    } else {
        assert_eq!(scratch.len(), REFERENCE_LENGTH_WIDTH - 1);
        multi_controlled_x_vchain(circ, &controls, target, scratch);
    }
...
fn transition_q851_coefficient_cursor(
    ...
    if q851_fixed_sign_event_requested() && fixed_sign_domain {
        toggle_q851_fixed_sign_event(circ, signed_length, transition_index, scratch, dirty_bank);
        return;
    }
    // Non-fixed-sign ripple fallback: an UNCONDITIONAL +1 / -1 on the full
    // register. Production always takes the fixed-sign branch above, but the
    // small-n value-proof harness (length_width != REFERENCE_LENGTH_WIDTH)
    // exercises this branch, so it must also honour the cursor-borrow. A
    // constant +1 is `add_constant(1)`; -1 is `add_constant(2^w - 1)`, both on
    // the borrowed l_q dirty lenders with no clean scratch.
    if let Some(lenders) = cursor_borrow_lenders(dirty_bank, signed_length.len()) {
        let refs: Vec<&QReg> = signed_length.iter().collect();
        let width = refs.len();
        let modulus = 1usize << width;
        let k = match direction {
            Q851CoefficientCursorDirection::Increment => 1 % modulus,
            Q851CoefficientCursorDirection::Decrement => (modulus - 1) % modulus,
        };
        add_constant_mod_2n_dirty_lenders(circ, &refs, k, lenders);
        return;
    }
    match direction {
...
fn coefficient_fused_data_and_sign_q845_swap_only(
    ...
    // Swap-scan all-ones-detector dirty bank. The single-lane fallback borrows
    // the idle `phase2` flag (peak 850); the ladder path (Q845_SWAP_LADDER) uses
    // the idle `l_q` length register (7 lenders), dropping the `uls_anc` tree
    // (peak 847). `l_q` is NOT threaded into this swap step, so it is provably
    // untouched here; mcx_dirty_ladder restores each lender exactly.
    let phase2_lane = [phase2];
    // address (the unary-iterate counter) has l_t_prime.len() bits => the ladder
    // detector needs l_t_prime.len()-2 dirty lenders.
    let ladder_lenders_needed = l_t_prime.len().saturating_sub(2);
    let swap_dirty_bank: Option<&[&QReg]> = if q845_swap_ladder_detector_requested() {
        l_q_borrow.filter(|bank| bank.len() >= ladder_lenders_needed)
    } else if q845_swap_dirty_mcx_requested() {
        Some(&phase2_lane[..])
    } else {
        None
    };
    ...
    // Cursor-scratch borrow (sub-846 P1 cut): when active AND the swap-detector
    // dirty bank (l_q) is available, the swap-step coefficient consumers borrow
    // l_q dirty lenders for every clean v-chain / ripple / constant-add, so the
    // 8-lane clean cursor-scratch register is no longer allocated (peak -8). The
    // l_q lenders are restored before the iterate body runs (detector cascade is
    // compute-uncompute), so they are idle/available during the body.
    let cursor_scratch_width = if q838_cursor_borrow_requested() && swap_dirty_bank.is_some() {
        0
    } else {
        l_t.len().saturating_sub(1)
    };
    let cursor_scratch = circ.alloc_qreg_bits(
        ...
        cursor_scratch_width,
    );
    cursor_scratch_probe_record_ids(
        &cursor_scratch.iter().map(QReg::id).collect::<Vec<_>>(),
    );
    ...
    if inverse {
        ...
        guard.toggle_nonzero(circ, phase1, l_t_prime, above_guard, &cursor_scratch, swap_dirty_bank);
        ...
        circ.x(above_guard);
        circ.cx(phase1, sign);
        guard.toggle_nonzero(circ, enable, l_t_prime, above_guard, &cursor_scratch, swap_dirty_bank);

        cursor_scratch_probe_record_reverse_boundary(circ.b.ops.len());
        let reverse_source: Vec<&QReg> = cursor_scratch
            .iter()
...
pub fn register_shared_full_window_step(
    ...
    let l_q_borrow: Vec<&QReg> = l_q.iter().collect();
    coefficient_phase_block(
        circ, phase1, phase2, sign, work1, work2, work2, l_t, l_t_prime, l_s, l_r_prime,
        Some(&l_q_borrow), false,
    );
    ...
    if emit_length_swap {
        ...
        let chain_refs: Vec<&QReg> = chain.iter().collect();
        let empty_scratch: Vec<&QReg> = Vec::new();
        compute_zero(circ, l_q, zero_q, chain);
        compute_zero(circ, l_s, zero_s, chain);
...
fn register_shared_scheduled_step_with_preserved_dy_top(
    ...
    if step % 4 == 0 {
        // sub-846 FRONT P2: when the swap-condition borrow is active, allocate ONE
        // fewer chain lane (REFERENCE_LENGTH_WIDTH instead of +1) and host the
        // missing chain lane on the idle persistent `phase2` flag (dirty lender,
        // restored exactly by the ladder cascade). condition_scratch 10 -> 9.
        let borrow = q845_swap_condition_borrow_requested();
        let condition_width = if borrow {
            REFERENCE_LENGTH_WIDTH
        } else {
            REFERENCE_LENGTH_WIDTH + 1
        };
        let condition_scratch =
            circ.alloc_qreg_bits("rs.scheduled.swap-condition", condition_width);
        let zero_q = &condition_scratch[0];
        let zero_s = &condition_scratch[1];
        let control = &condition_scratch[2];
        let chain_fresh = &condition_scratch[3..];
        // The full chain the kernels consume: the fresh lanes plus (on the borrow
        // path) the idle `phase2`. compute_zero / swap-support use mcx_dirty_ladder
        // when borrowing, so the dirty phase2 lane is a legal lender.
        let chain_refs: Vec<&QReg> = chain_fresh
            .iter()
            .chain(borrow.then_some(phase2))
            .collect();
        compute_zero_refs(circ, l_q, zero_q, &chain_refs);
        compute_zero_refs(circ, l_s, zero_s, &chain_refs);
        ...
        uncompute_zero_refs(circ, l_s, zero_s, &chain_refs);
        uncompute_zero_refs(circ, l_q, zero_q, &chain_refs);
        free_clean(circ, condition_scratch);
    }
}
...
fn register_shared_scheduled_step_inverse_with_preserved_dy_top(
    ...
    if step % 4 == 0 {
        // sub-846 FRONT P2 (inverse): mirror the forward borrow so the inverse
        // allocates the same 9-lane condition_scratch and hosts the missing chain
        // lane on phase2 -- required for forward/inverse reversibility.
        let borrow = q845_swap_condition_borrow_requested();
        let condition_width = if borrow {
            REFERENCE_LENGTH_WIDTH
        } else {
        ...
        let condition_scratch =
            circ.alloc_qreg_bits("rs.scheduled-inverse.swap-condition", condition_width);
        let zero_q = &condition_scratch[0];
        let zero_s = &condition_scratch[1];
        let control = &condition_scratch[2];
        let chain_fresh = &condition_scratch[3..];
...
pub fn register_shared_divide_forward(
    ...
    register_shared_reverse(circ, &core, None);
...
/// Classical bit-vector replay of the first `prefix` ops (X/CX/CCX/R), returning
/// the register state. `R` resets are honoured as plain "force-to-zero" so that the
/// trajectory reflects the real lane contents (no clean-reset masking).
fn cursor_probe_replay_prefix(ops: &[crate::circuit::Op], mut state: u64, prefix: usize) -> u64 {
    use crate::circuit::OperationType;
    let bit = |word: u64, id: u64| ((word >> id) & 1) != 0;
    for operation in ops.iter().take(prefix) {
        match operation.kind {
            OperationType::X => state ^= 1u64 << operation.q_target.0,
            OperationType::CX => {
                if bit(state, operation.q_control1.0) {
            ...
            OperationType::CCX => {
                if bit(state, operation.q_control1.0) && bit(state, operation.q_control2.0) {
                    state ^= 1u64 << operation.q_target.0;
                }
            }
            OperationType::R => state &= !(1u64 << operation.q_target.0),
            other => panic!("cursor probe saw nonclassical op {other:?}"),
        }
    }
    state
}

/// Whole-trajectory probe: for a representative sweep of inputs, scan the cursor
/// value at EVERY op index of the swap-only step and report, per op index, whether
/// cursor-scratch is ever nonzero there. This distinguishes a clean-between-ops
/// transient (nonzero only inside v-chain/carry brackets, zero at every "rest"
/// point) from a value-bearing running register (nonzero across long op spans).
/// Returns (boundary_ops_idx, total_ops, count of op indices where cursor!=0 for
/// some input, the longest contiguous run of such op indices).
pub fn probe_cursor_scratch_trajectory() -> (usize, usize, usize, usize) {
    let physical_work_width = 5usize;
    let scan_width = 4usize;
    let length_width = 3usize;
    let r_length_width = 2usize;

    CURSOR_SCRATCH_PROBE.with(|cell| *cell.borrow_mut() = Some(CursorScratchProbe::default()));
    let harness = build_q845_swap_only_core_harness(
        physical_work_width,
        scan_width,
        scan_width,
        length_width,
    ...
    let probe = CURSOR_SCRATCH_PROBE
        .with(|cell| cell.borrow_mut().take())
        .expect("probe installed");
    let cursor_ids = probe.cursor_ids.clone();
    let boundary = probe.reverse_boundary_ops_idx;
    let ops = &harness.builder.ops;
    let n_ops = ops.len();

    // For each op index, does cursor become nonzero for ANY swept input?
    let mut nonzero_at = vec![false; n_ops + 1];
    let inputs: Vec<u64> = {
        let mut v = Vec::new();
        for phase1 in [false, true] {
            for sign in [false, true] {
                for active_width in 1..=scan_width {
                    for work1 in 0..(1usize << scan_width) {
                        for work2 in 0..(1usize << physical_work_width) {
                            let mut input = 0u64;
                            input |= u64::from(phase1) << harness.phase1_id;
                            input |= u64::from(sign) << harness.sign_id;
                            input = q845_fusion_set_register(input, &harness.work1_ids, work1);
                            input = q845_fusion_set_register(input, &harness.work2_ids, work2);
    ...
    for &input in &inputs {
        // incremental replay
        let mut state = input;
        if q845_fusion_read_register(state, &cursor_ids) != 0 {
            nonzero_at[0] = true;
        }
        for (idx, operation) in ops.iter().enumerate() {
            state = cursor_probe_replay_prefix(std::slice::from_ref(operation), state, 1);
            if q845_fusion_read_register(state, &cursor_ids) != 0 {
                nonzero_at[idx + 1] = true;
            }
        }
    }
    let nonzero_op_count = nonzero_at.iter().filter(|&&b| b).count();
    ...
    (boundary, n_ops, nonzero_op_count, longest_run)
}

/// Probe: is `cursor_scratch` clean (==0) at the forward->reverse boundary of the
/// q845 swap-only coefficient step, or does it carry a live value across it?
/// Returns the maximum nonzero cursor value seen over a sweep of input states, and
/// a count of inputs for which it was nonzero at the boundary. A clean-in/clean-out
/// transient ancilla MUST be 0 at the boundary for all inputs (count==0).
pub fn probe_cursor_scratch_lifetime_at_boundary() -> (usize, usize, usize) {
    // 5/4/3/2 is the established purely-classical proof config (matches the
    // existing q845 swap-only exhaustive harness layouts). Larger configs leave
    // the fixed-sign domain and emit non-classical ops, so they are not modelled.
    let physical_work_width = 5usize;
    let scan_width = 4usize;
    let length_width = 3usize;
    let r_length_width = 2usize;
    ...
    let harness = build_q845_swap_only_core_harness(
        ...
        length_width,
        r_length_width,
        false, // forward (inverse=false)
        true,  // swap_only
        true,  // truncated_guard
    );
    let probe = CURSOR_SCRATCH_PROBE
        .with(|cell| cell.borrow_mut().take())
        .expect("probe installed");
    assert!(
        probe.saw_reverse_boundary,
        "swap-only step did not reach a reverse boundary"
    );
    let cursor_ids = probe.cursor_ids.clone();
    assert!(!cursor_ids.is_empty(), "no cursor-scratch lanes recorded");
    let boundary = probe.reverse_boundary_ops_idx;
    let ops = &harness.builder.ops;
    ...
    // Sweep over a representative set of data inputs (phase/sign/work1/work2/l_t).
    for phase1 in [false, true] {
                ...
                for active_width in 1..=scan_width {
                    for work1 in 0..(1usize << scan_width) {
                        for work2 in 0..(1usize << physical_work_width) {
                            let mut input = 0u64;
                            input |= u64::from(phase1) << harness.phase1_id;
                            input |= u64::from(phase2) << harness.phase2_id;
                            input |= u64::from(sign) << harness.sign_id;
                            input = q845_fusion_set_register(input, &harness.work1_ids, work1);
                            input = q845_fusion_set_register(input, &harness.work2_ids, work2);
                            input = q845_fusion_set_register(
                            ...
                            let mid = cursor_probe_replay_prefix(ops, input, boundary);
                            let cursor = q845_fusion_read_register(mid, &cursor_ids);
                            if cursor != 0 {
                                nonzero_count += 1;
                                max_value = max_value.max(cursor);
                            }
                            total += 1;
                        }
                    }
...
struct Q845SwapOnlyCoreHarness {
    ...
    /// The dirty-lender bank threaded into the swap-scan all-ones detector when
    /// `lq_borrow` is requested. Empty in the `None` (single-lane `phase2`
    /// fallback) path. Exposed so the value proof can sweep a dirty l_q and
    /// assert the ladder restores every lender exactly.
    l_q_ids: Vec<u32>,
    above_guard_id: u32,
    data_mask: u64,
}
...
fn build_q845_swap_only_core_harness(
    ...
    build_q845_swap_only_core_harness_inner(
        physical_work_width,
        scan_width,
        coefficient_width,
        length_width,
...
/// Variant that threads a real `l_q` dirty bank through the swap-scan all-ones
/// detector (the `LOWQ_Q845_SWAP_LADDER_DETECTOR` production path), so the
/// `mcx_dirty_ladder` lender borrow is exercised — and value-proven — at small
/// n. The production swap step (peak 846) borrows `l_q` here; the original
/// `build_q845_swap_only_core_harness` passes `l_q_borrow = None`, which falls
/// back to the single-lane `phase2` detector and NEVER touches the dirty ladder.
/// This builder closes that coverage gap.
#[allow(clippy::too_many_arguments)]
fn build_q845_swap_only_core_harness_lq_borrow(
    physical_work_width: usize,
    scan_width: usize,
    coefficient_width: usize,
    length_width: usize,
...
#[allow(clippy::too_many_arguments)]
fn build_q845_swap_only_core_harness_inner(
    physical_work_width: usize,
    scan_width: usize,
    coefficient_width: usize,
    length_width: usize,
    ...
    assert!(!lq_borrow || swap_only);
    if truncated_guard {
        std::env::set_var(Q851_TRUNCATED_SWAP_ONLY_GUARD_FLAG, "1");
    } else {
        std::env::remove_var(Q851_TRUNCATED_SWAP_ONLY_GUARD_FLAG);
    }
    // When the proof threads a real l_q bank, force the production ladder-detector
    // path so the `mcx_dirty_ladder` lender borrow is the code under test. Saved
    // and restored around the build. Without the ladder flags `swap_dirty_bank`
    // stays None even with a non-None l_q_borrow, leaving the dirty path uncovered.
    let saved_swap_dirty_mcx = std::env::var_os(Q845_SWAP_DIRTY_MCX_FLAG);
    let saved_swap_ladder = std::env::var_os(Q845_SWAP_LADDER_DETECTOR_FLAG);
    let saved_uls_gate_fuse = std::env::var_os(Q846_ULS_GATE_FUSE_FLAG);
    let saved_cursor_borrow = std::env::var_os(Q838_CURSOR_BORROW_FLAG);
    let saved_inplace_boundary = std::env::var_os(Q838_INPLACE_BOUNDARY_FLAG);
    if lq_borrow {
        std::env::set_var(Q845_SWAP_DIRTY_MCX_FLAG, "1");
        std::env::set_var(Q845_SWAP_LADDER_DETECTOR_FLAG, "1");
        // Match the PRODUCTION peak-846 path: the fused uls_gate ladder (XORs the
        // all-ones detector directly into `active`, eliminating the clean gate).
        std::env::set_var(Q846_ULS_GATE_FUSE_FLAG, "1");
        // sub-846 P1 cut under test: route the swap-step coefficient consumers
        // (increment ripple, fixed-sign / nonzero / geq v-chains, constant
        // address-subtract) onto the borrowed l_q dirty lenders, eliminating the
        // 8-lane clean cursor-scratch. The in-place boundary (its UPDATE-5
        // prerequisite) is forced on too, since the boundary buffer is the other
        // clean-zero pin on cursor-scratch. The lq-dirty proof loop then
        // value-proves this exact path (data-transparent, l_q restored, clean).
        std::env::set_var(Q838_CURSOR_BORROW_FLAG, "1");
        std::env::set_var(Q838_INPLACE_BOUNDARY_FLAG, "1");
    }
    let mut circ = q845_fusion_proof_circuit();
    ...
    // The dirty-lender bank for the swap-scan ladder detector. Allocated only on
    // the lq_borrow path; the detector needs `l_t_prime.len()-2` lenders for the
    // widest MCX, so length_width lanes cover it with margin. NOTE: allocating it
    // shifts above_guard/enable/add_only/chain ids, so the proof compares the
    // None and l_q candidates on the shared DATA-register ids (which are
    // allocated before l_q and stay id-stable), not on the raw data_mask.
    let l_q = if lq_borrow {
        circ.alloc_qreg_bits("q845.swap-only-proof.l-q", length_width)
    } else {
        Vec::new()
    };
    ...
    if swap_only {
        let l_q_refs: Vec<&QReg> = l_q.iter().collect();
        let l_q_borrow: Option<&[&QReg]> = if lq_borrow {
            Some(&l_q_refs[..])
        } else {
            None
        };
    ...
    // l_q is a borrowed dirty bank, NOT a data register: it is deliberately
    // excluded from data_mask so the proof's data comparison ignores it, and is
    // surfaced separately so the proof can sweep dirty values and check the
    // ladder restored every lender exactly. The lanes are allocated on every
    // path (id-stable), but only reported when actually threaded as a borrow —
    // on the None path they stay |0> and are not part of the circuit's logic.
    let l_q_ids = if lq_borrow {
        l_q.iter().map(QReg::id).collect::<Vec<_>>()
    } else {
        Vec::new()
    };
    let above_guard_id = above_guard.id();
    let data_mask = (1u64 << phase1_id)
        ...
        | (1u64 << above_guard_id);
    // Restore the swap-detector flags the lq_borrow path forced on, so this
    // builder leaves no global env side effects for the surrounding proof.
    match saved_swap_dirty_mcx {
        Some(value) => std::env::set_var(Q845_SWAP_DIRTY_MCX_FLAG, value),
        None => std::env::remove_var(Q845_SWAP_DIRTY_MCX_FLAG),
    }
    match saved_swap_ladder {
        Some(value) => std::env::set_var(Q845_SWAP_LADDER_DETECTOR_FLAG, value),
        None => std::env::remove_var(Q845_SWAP_LADDER_DETECTOR_FLAG),
    }
    match saved_uls_gate_fuse {
        Some(value) => std::env::set_var(Q846_ULS_GATE_FUSE_FLAG, value),
        None => std::env::remove_var(Q846_ULS_GATE_FUSE_FLAG),
    }
    match saved_cursor_borrow {
        Some(value) => std::env::set_var(Q838_CURSOR_BORROW_FLAG, value),
        None => std::env::remove_var(Q838_CURSOR_BORROW_FLAG),
    }
    match saved_inplace_boundary {
        Some(value) => std::env::set_var(Q838_INPLACE_BOUNDARY_FLAG, value),
        None => std::env::remove_var(Q838_INPLACE_BOUNDARY_FLAG),
    }
    Q845SwapOnlyCoreHarness {
        builder: circ.into_builder(),
        phase1_id,
...
struct Q845EphemeralSwapHarness {
    ...
    l_s_ids: Vec<u32>,
    l_r_prime_ids: Vec<u32>,
    // sub-846 FRONT P2: the external DIRTY lane (phase2 stand-in) hosting one
    // swap-condition chain lane when LOWQ_Q845_SWAP_CONDITION_BORROW is on. The
    // proof sweeps its dirty value and asserts the swap restores it exactly and
    // produces bit-identical data to the clean (None) path. None when not
    // borrowing.
    borrowed_lane_id: Option<u32>,
}
...
fn build_q845_ephemeral_swap_harness(
    ...
    build_q845_ephemeral_swap_harness_inner(
        work_width,
        length_width,
        r_length_width,
        inverse,
        swap_only,
        fuse_support_lifetime,
        false,
    )
}

/// Variant that hosts one swap-condition chain lane on an EXTERNAL dirty lane
/// (the FRONT-P2 borrow under test) by forcing LOWQ_Q845_SWAP_CONDITION_BORROW.
/// The lane id is surfaced so the proof can sweep its dirty value and assert the
/// swap restores it and stays data-transparent vs the clean path.
fn build_q845_ephemeral_swap_harness_condition_borrow(
    work_width: usize,
    length_width: usize,
    r_length_width: usize,
    inverse: bool,
...
/// Force the full production fused-prefix LOAN (so the bit-length prefix-AND
/// borrows its clean scratch from the swap chain). `kg_only` additionally
/// activates the lane-reduced (eight-lane, flag-eliminated) loan. Both the
/// clean reference and the KG-only candidate go through here so they differ
/// ONLY by the KG-only flag, making the positive comparison non-vacuous.
fn build_q845_ephemeral_swap_harness_prefix_loan(
    work_width: usize,
    length_width: usize,
    r_length_width: usize,
    inverse: bool,
    ...
    let saved: Vec<_> = loan_flags
        .iter()
        .map(|name| (*name, std::env::var_os(name)))
        .collect();
    for name in loan_flags {
        std::env::set_var(name, "1");
    }
    let saved_kg_only = std::env::var_os(KG_ONLY_PREFIX_SCRATCH_FLAG);
    let saved_borrowed_flag = std::env::var_os(KG_ONLY_PREFIX_BORROWED_FLAG_ENV);
    if kg_only {
        std::env::set_var(KG_ONLY_PREFIX_SCRATCH_FLAG, "1");
        // Force the production borrowed-flag path so this proof covers the
        // peak-neutral cheap two-control prefix callbacks (the op-count cut),
        // not just the dirty 8-lane variant.
        std::env::set_var(KG_ONLY_PREFIX_BORROWED_FLAG_ENV, "1");
    } else {
        std::env::remove_var(KG_ONLY_PREFIX_SCRATCH_FLAG);
        std::env::remove_var(KG_ONLY_PREFIX_BORROWED_FLAG_ENV);
    }
    let harness = build_q845_ephemeral_swap_harness_inner(
        work_width,
        length_width,
        r_length_width,
        inverse,
    ...
    match saved_kg_only {
        Some(value) => std::env::set_var(KG_ONLY_PREFIX_SCRATCH_FLAG, value),
        None => std::env::remove_var(KG_ONLY_PREFIX_SCRATCH_FLAG),
    }
    match saved_borrowed_flag {
        Some(value) => std::env::set_var(KG_ONLY_PREFIX_BORROWED_FLAG_ENV, value),
        None => std::env::remove_var(KG_ONLY_PREFIX_BORROWED_FLAG_ENV),
    }
...
#[allow(clippy::too_many_arguments)]
fn build_q845_ephemeral_swap_harness_inner(
    work_width: usize,
    length_width: usize,
    r_length_width: usize,
    inverse: bool,
    ...
    let saved_condition_borrow = std::env::var_os(Q845_SWAP_CONDITION_BORROW_FLAG);
    if condition_borrow {
        std::env::set_var(Q845_SWAP_CONDITION_BORROW_FLAG, "1");
    } else {
        std::env::remove_var(Q845_SWAP_CONDITION_BORROW_FLAG);
    }
    let mut circ = q845_fusion_proof_circuit();
    ...
    // The borrowed external dirty lane (phase2 stand-in). Allocated ONLY on the
    // borrow path; it stands in for the persistent flag the production borrow
    // hosts the chain lane on. It is NOT a data lane (excluded from data_ids).
    let borrowed_lane = if condition_borrow {
        Some(circ.alloc_qreg("q845.ephemeral-swap.borrowed-lane"))
    } else {
        None
    };
    let predicate_chain_width = 3usize.max(length_width.saturating_sub(2));
    // On the borrow path allocate ONE fewer chain lane (the missing lane is the
    // external borrowed lane), matching the production 10 -> 9 cut.
    let fresh_condition_width = if condition_borrow {
        3 + predicate_chain_width - 1
    } else {
        3 + predicate_chain_width
    };
    let condition_scratch =
        circ.alloc_qreg_bits("q845.ephemeral-swap.condition", fresh_condition_width);
    let zero_q = &condition_scratch[0];
    let zero_s = &condition_scratch[1];
    let control = &condition_scratch[2];
    let chain_fresh = &condition_scratch[3..];
    let chain_refs: Vec<&QReg> = chain_fresh
        .iter()
        .chain(borrowed_lane.as_ref())
        .collect();
    compute_zero_refs(&mut circ, &l_q, zero_q, &chain_refs);
    compute_zero_refs(&mut circ, &l_s, zero_s, &chain_refs);
    conditional_work_and_length_swap_under_zero_predicate(
        &mut circ,
        zero_q,
        zero_s,
    ...
    uncompute_zero_refs(&mut circ, &l_s, zero_s, &chain_refs);
    uncompute_zero_refs(&mut circ, &l_q, zero_q, &chain_refs);
    free_clean(&mut circ, condition_scratch);
    let borrowed_lane_id = borrowed_lane.as_ref().map(QReg::id);
    if let Some(lane) = borrowed_lane {
        // Free the id WITHOUT a clean assertion: it is a borrowed dirty lane
        // (the swap restores it to its dirty entry value, not |0>). zero_and_free
        // just deallocates the id; the proof reads the restored value from the
        // circuit output before this matters.
        circ.zero_and_free(lane);
    }
    match saved_condition_borrow {
        Some(value) => std::env::set_var(Q845_SWAP_CONDITION_BORROW_FLAG, value),
        None => std::env::remove_var(Q845_SWAP_CONDITION_BORROW_FLAG),
    }
...
fn build_q851_range_comparator_harness(
    ...
    toggle_register_geq_constant_vchain(&mut circ, &register, value, &target, &scratch, None);
    free_clean(&mut circ, scratch);
...
pub fn exhaustive_q845_swap_only_coefficient_check() -> Q845SwapOnlyCoefficientProofReport {
    ...
    // ============================================================================
    // l_q DIRTY-BORROW LADDER PATH COVERAGE (note-sub851 UPDATE 5/6 enabler).
    //
    // The loop above exercises only the swap step with `l_q_borrow = None`, which
    // takes the single-lane `phase2` detector fallback — the `mcx_dirty_ladder`
    // l_q lender borrow (the PRODUCTION peak-846 path) is never touched, so any
    // sub-846 cut that borrows l_q was un-value-provable at small n.
    //
    // Here we build the SAME swap-only coefficient step with a REAL l_q dirty
    // bank threaded through the ladder detector (forcing LOWQ_Q845_SWAP_DIRTY_MCX
    // + LOWQ_Q845_SWAP_LADDER_DETECTOR), then exhaustively prove:
    //   (1) DATA-TRANSPARENT: for every data input AND every dirty l_q value, the
    //       data-register output is bit-identical to the `None`-path candidate
    //       (the ladder detector cannot be observed on the data lanes).
    //   (2) l_q RESTORED: the borrowed l_q bank exits EXACTLY equal to its dirty
    //       entry value (the double-Barenco cascade restores every lender).
    //   (3) SCRATCH CLEAN: all non-data, non-l_q lanes return to |0>.
    //   (4) INVERSE-PAIR: forward∘inverse is identity on (data ⊕ l_q), so the
    //       dirty-borrow swap step is reversible with the dirty bank live.
    // A nonzero `lq_borrow_dirty_path_checks` is the assertion that the dirty
    // path actually executed (it is otherwise structurally unreachable here).
    let mut lq_borrow_dirty_path_checks = 0usize;
    let mut lq_restore_checks = 0usize;
    let mut lq_data_transparent_checks = 0usize;
    for &(shift, r_length) in &layouts {
        let coefficient_width = physical_work_width - shift - r_length;
        assert!(coefficient_width > 0);
        // Reference (None path) and candidate (l_q ladder path), both swap-only.
        let none_forward = build_q845_swap_only_core_harness(
            physical_work_width,
            scan_width,
            scan_width,
            length_width,
        ...
        let lq_forward = build_q845_swap_only_core_harness_lq_borrow(
            physical_work_width,
            scan_width,
            scan_width,
            length_width,
        ...
        let lq_inverse = build_q845_swap_only_core_harness_lq_borrow(
            physical_work_width,
            scan_width,
            scan_width,
            length_width,
        ...
        assert!(lq_forward.builder.next_qubit < 64);
        // The DATA registers are allocated before l_q in both harnesses, so their
        // ids are identical; only above_guard/enable/add_only/chain shift by the
        // l_q width. We therefore compare on the shared data-register ids, not on
        // the raw data_mask (which includes the shifted above_guard).
        assert_eq!(none_forward.phase1_id, lq_forward.phase1_id);
        assert_eq!(none_forward.phase2_id, lq_forward.phase2_id);
        assert_eq!(none_forward.sign_id, lq_forward.sign_id);
        assert_eq!(none_forward.work1_ids, lq_forward.work1_ids);
        assert_eq!(none_forward.work2_ids, lq_forward.work2_ids);
        assert_eq!(none_forward.l_t_ids, lq_forward.l_t_ids);
        assert_eq!(none_forward.l_t_prime_ids, lq_forward.l_t_prime_ids);
        assert_eq!(none_forward.l_s_ids, lq_forward.l_s_ids);
        assert_eq!(none_forward.l_r_prime_ids, lq_forward.l_r_prime_ids);
        // The candidate actually threaded a real l_q bank (else this proof would
        // be vacuous — the whole point of the harness extension).
        assert_eq!(lq_forward.l_q_ids.len(), length_width);
        assert!(none_forward.l_q_ids.is_empty());
        let lq_mask = q845_fusion_id_mask(&lq_forward.l_q_ids);
        let all_mask = (1u64 << lq_forward.builder.next_qubit) - 1;
        // Scratch = everything that is neither data nor the borrowed l_q bank.
        let scratch_mask = all_mask & !lq_forward.data_mask & !lq_mask;
        // Shared data-register id mask (NO above_guard — it differs by the l_q
        // width). data_mask minus above_guard, and identical in both harnesses.
        let shared_data_mask =
            lq_forward.data_mask & !(1u64 << lq_forward.above_guard_id);
        debug_assert_eq!(
            shared_data_mask,
            none_forward.data_mask & !(1u64 << none_forward.above_guard_id)
        );
        // Sweep the SAME valid-input domain as the 33424 loop above (the kernel's
        // contractual precondition: enable/add_only derived from phase/sign, and
        // the add_only no-overflow / no-above-guard guards), crossed with EVERY
        // dirty l_q value. Outside this domain neither path is contractually
        // clean, so the comparison is only meaningful inside it.
        for active_width in 1..=coefficient_width.min(scan_width) {
            let active_mask = (1usize << active_width) - 1;
            let source_mask = (1usize << active_width.saturating_sub(1)) - 1;
            for phase1 in [false, true] {
                for phase2 in [false, true] {
                    for sign in [false, true] {
                        ...
                        for work1 in 0..(1usize << scan_width) {
                            ...
                            for work2 in 0..(1usize << physical_work_width) {
                                let target = work2
                                    & ((1usize << coefficient_width.min(scan_width)) - 1);
                                let target_low = target & active_mask;
                                let above_guard = target >> active_width != 0;
                                if add_only
                                    && (above_guard
                                ...
                                let mut data_input = 0u64;
                                data_input |= u64::from(phase1) << lq_forward.phase1_id;
                                data_input |= u64::from(phase2) << lq_forward.phase2_id;
                                data_input |= u64::from(sign) << lq_forward.sign_id;
                                data_input = q845_fusion_set_register(
                                    data_input,
                                    &lq_forward.work1_ids,
                                    work1,
                                );
                                data_input = q845_fusion_set_register(
                                    data_input,
                                    &lq_forward.work2_ids,
                                    work2,
                                );
                                data_input = q845_fusion_set_register(
                                    data_input,
                                ...
                                data_input = q845_fusion_set_register(
                                    data_input,
                                    &lq_forward.l_r_prime_ids,
                                    r_length,
                                );
                                // None-path reference output on the data lanes.
                                let none_output =
                                    apply_q845_fusion_classical_with_clean_resets(
                                        &none_forward.builder.ops,
                                        data_input,
                                    );
                                // The whole dirty range of the borrowed bank.
                                for l_q_value in 0..(1usize << length_width) {
                                    let lq_input = q845_fusion_set_register(
                                        data_input,
                                        &lq_forward.l_q_ids,
                                        l_q_value,
                                    );
                                    let lq_output =
                                        apply_q845_fusion_classical_with_clean_resets(
                                            &lq_forward.builder.ops,
                                            lq_input,
                                        );
                                    // (2) l_q restored exactly to its dirty entry.
                                    assert_eq!(
                                        q845_fusion_read_register(
                                            lq_output,
                                            &lq_forward.l_q_ids,
                                        ),
                                        l_q_value,
                                        "ladder did not restore dirty l_q bank"
                                    );
                                    lq_restore_checks += 1;
                                    // (3) all scratch clean (above_guard returns
                                    // to |0> like every other non-data lane).
                                    assert_eq!(lq_output & scratch_mask, 0);
                                    assert_eq!(
                                        (lq_output >> lq_forward.above_guard_id) & 1,
                                        0
                                    );
                                    // (1) data lanes match the None path exactly,
                                    // independent of the dirty l_q value. Compared
                                    // on the SHARED data ids (above_guard differs
                                    // by the l_q width and is checked separately).
                                    assert_eq!(
                                        lq_output & shared_data_mask,
                                        none_output & shared_data_mask,
                                        "l_q ladder perturbed a data lane"
                                    );
                                    lq_data_transparent_checks += 1;
                                    // (4) forward then inverse is identity on the
                                    // full (data ⊕ l_q) register set.
                                    let round_trip =
                                        apply_q845_fusion_classical_with_clean_resets(
                                            &lq_inverse.builder.ops,
                                            lq_output,
                                        );
                                    assert_eq!(
                                        round_trip,
                                        lq_input,
                                        "l_q dirty-borrow swap step not reversible"
                                    );
                                    lq_borrow_dirty_path_checks += 1;
                                }
                            }
                        }
                    }
    ...
    assert!(
        lq_borrow_dirty_path_checks > 0,
        "l_q dirty-borrow ladder path was never exercised — the harness \
         extension is vacuous"
    );
    assert_eq!(lq_restore_checks, lq_borrow_dirty_path_checks);
    assert_eq!(lq_data_transparent_checks, lq_borrow_dirty_path_checks);
    ...
    // ============ sub-846 FRONT P2: DECISIVE NEGATIVE value result ==========
    // FRONT P2 asks to host one `scheduled.swap-condition` chain lane on a
    // borrowed DIRTY lane (the idle `phase2`), shrinking condition_scratch
    // 10 -> 9 (measured peak 846 -> 845 under count-only).
    //
    // This proof drives the borrow swap (LOWQ_Q845_SWAP_CONDITION_BORROW forced)
    // crossed with every dirty borrowed-lane value, against the clean reference,
    // and finds it is VALUE-INCORRECT: the swap `scratch` (= the chain) is
    // LOANED IN FULL to the bit-length prefix-AND kernel
    // (LOWQ_LOAN_FUSED_PREFIX_SCRATCH; `bit_length_lean_with_full_prefix_scratch`
    // borrows all 7 chain lanes as Khattar-Gidney prefix scratch and REQUIRES
    // them clean-|0>). compute_zero and the swap-support v-chain were made
    // dirty-tolerant (mcx_dirty_ladder), but the prefix-AND is NOT: a dirty
    // borrowed lane corrupts the swapped-in bit length (the l_t_prime output
    // lane diverges whenever the borrowed lane enters |1>).
    //
    // The count is the number of basis states for which the dirty-borrow output
    // DIVERGES from the clean path -- nonzero == the cut is decisively rejected
    // by the value gate. This is the floor evidence for FRONT P2: the swap
    // workspace is conserved (the chain is fully clean-pinned by the bit-length
    // loan), so phase2-borrowing it is value-incorrect, not merely peak-neutral.
    let mut swap_condition_borrow_checks = 0usize;
    {
        let clean_forward = build_q845_ephemeral_swap_harness(
            swap_work_width,
            length_width,
            r_length_width,
            false,
        ...
        let borrow_forward = build_q845_ephemeral_swap_harness_condition_borrow(
            swap_work_width,
            length_width,
            r_length_width,
            false,
        );
        assert_eq!(clean_forward.data_ids, borrow_forward.data_ids);
        assert_eq!(clean_forward.external_mask, borrow_forward.external_mask);
        let borrowed_id = borrow_forward
            .borrowed_lane_id
            .expect("condition-borrow harness must thread a borrowed lane");
        assert!(clean_forward.borrowed_lane_id.is_none());
            ...
            for r in 0..=swap_mask {
                ...
                for t_prime in 0..=swap_mask {
                    for r_prime in 0..=swap_mask {
                        ...
                        let work2 = q845_test_pack_work2(swap_work_width, t_prime, r_prime);
                        for iteration in [false, true] {
                            let mut base = u64::from(iteration) << borrow_forward.iteration_id;
                            base = q845_fusion_set_register(base, &borrow_forward.work1_ids, work1);
                            base = q845_fusion_set_register(base, &borrow_forward.work2_ids, work2);
                            base = q845_fusion_set_register(base, &borrow_forward.l_t_ids, l_t);
                            base = q845_fusion_set_register(
                                base,
                                &borrow_forward.l_r_prime_ids,
                                l_r_prime,
                            );
                            let clean_output = apply_scalar(&clean_forward.builder.ops, base);
                            // The dirty entry (borrowed lane = |1>) is the case the
                            // bit-length prefix-AND mishandles.
                            let borrow_input = base | (1u64 << borrowed_id);
                            let borrow_output =
                                apply_scalar(&borrow_forward.builder.ops, borrow_input);
                            if borrow_output & borrow_forward.external_mask
                                != clean_output & clean_forward.external_mask
                            {
                                divergences += 1;
                            }
                            swap_condition_borrow_checks += 1;
                        }
                    }
                }
            }
        }
        // The cut is VALUE-INCORRECT: at least one dirty-entry basis state diverges
        // from the clean path. (If this ever became zero, the bit-length kernel
        // would have become dirty-tolerant and the P2 cut would be revisitable.)
        assert!(
            divergences > 0,
            "FRONT P2 swap-condition borrow unexpectedly matched the clean path — \
             the bit-length prefix-AND may have become dirty-tolerant; re-evaluate P2"
        );
    }
    assert!(
        swap_condition_borrow_checks > 0,
        "swap-condition borrow path was never exercised — the P2 result is vacuous"
    );
    // ============== end sub-846 FRONT P2 negative value result ===============

    // ============== sub-846 FRONT P2 POSITIVE value result ===================
    // The KG-only fused-prefix loan shrinks the bit-length direct-prefix-AND
    // clean-lane requirement from nine to eight (drops the materialized flag),
    // freeing the owned `fused-prefix-scratch` lanes on the P2 plateau. This
    // proof drives the EXACT production swap with LOWQ_KG_ONLY_PREFIX_SCRATCH=1
    // and asserts its output is BIT-IDENTICAL to the clean nine-lane reference
    // across every swap basis state — i.e. the lane-reduced length kernel
    // computes the EXACT swapped-in bit length and restores every loaned lane.
    // (If this ever diverged the cut would be value-incorrect and rejected.)
    let mut kg_only_prefix_checks = 0usize;
    let mut kg_only_clean_peak_qubits = 0usize;
    let mut kg_only_kg_peak_qubits = 0usize;
    let mut kg_only_clean_ops = 0usize;
    let mut kg_only_kg_ops = 0usize;
    {
        let clean_forward = build_q845_ephemeral_swap_harness_prefix_loan(
            swap_work_width,
            length_width,
            r_length_width,
            false,
        ...
        let kg_forward = build_q845_ephemeral_swap_harness_prefix_loan(
            swap_work_width,
            length_width,
            r_length_width,
            false,
        ...
        kg_only_clean_peak_qubits = clean_forward.builder.peak_qubits as usize;
        kg_only_kg_peak_qubits = kg_forward.builder.peak_qubits as usize;
        kg_only_clean_ops = clean_forward.builder.ops.len();
        kg_only_kg_ops = kg_forward.builder.ops.len();
        assert_eq!(clean_forward.data_ids, kg_forward.data_ids);
        assert_eq!(clean_forward.external_mask, kg_forward.external_mask);
        for t in 1..=swap_mask {
            for r in 0..=swap_mask {
                let l_t = q845_test_bit_length(t);
                let l_r = q845_test_bit_length(r);
                ...
                for t_prime in 0..=swap_mask {
                    for r_prime in 0..=swap_mask {
                        ...
                        let work2 = q845_test_pack_work2(swap_work_width, t_prime, r_prime);
                        for iteration in [false, true] {
                            let mut base = u64::from(iteration) << kg_forward.iteration_id;
                            base = q845_fusion_set_register(base, &kg_forward.work1_ids, work1);
                            base = q845_fusion_set_register(base, &kg_forward.work2_ids, work2);
                            base = q845_fusion_set_register(base, &kg_forward.l_t_ids, l_t);
                            base = q845_fusion_set_register(
                                base,
                                &kg_forward.l_r_prime_ids,
                                l_r_prime,
                            );
                            let clean_output = apply_scalar(&clean_forward.builder.ops, base);
                            let kg_output = apply_scalar(&kg_forward.builder.ops, base);
                            assert_eq!(
                                kg_output & kg_forward.external_mask,
                                clean_output & clean_forward.external_mask,
                                "KG-only fused-prefix loan DIVERGED from the clean reference \
                                 (t={t} r={r} t'={t_prime} r'={r_prime} iter={iteration}) — \
                                 the lane-reduced bit-length kernel is value-incorrect"
                            );
                            kg_only_prefix_checks += 1;
                        }
                    }
                }
            }
    ...
    assert!(
        kg_only_prefix_checks > 0,
        "KG-only fused-prefix positive path was never exercised — the P2 cut is unproven"
    );
    // ============== end sub-846 FRONT P2 positive value result ===============
    ...
    Q845SwapOnlyCoefficientProofReport {
        ...
        lq_borrow_dirty_path_checks,
        lq_restore_checks,
        lq_data_transparent_checks,
        default_off_stream_identity_checks: 1,
        ...
        swap_condition_borrow_checks,
        kg_only_prefix_checks,
        kg_only_clean_peak_qubits,
        kg_only_kg_peak_qubits,
        kg_only_clean_ops,
        kg_only_kg_ops,
    }
}
...
/// layout. The three- and seven-lender shapes are the exact production shapes
/// in the coefficient and conditional-swap callers, respectively.
#[doc(hidden)]
pub fn exhaustive_fused_prefix_scratch_loan_check() -> FusedPrefixScratchLoanProofReport {
    ...
    const MAX_SOURCE_WIDTH: usize = 8;
    const ACCUMULATOR_WIDTH: usize = 5;
    let lender_modes = [3usize, 7usize];
    let mut basis_states_checked = 0usize;
    let mut scalar_equivalence_checks = 0usize;
    let mut simulator_equivalence_checks = 0usize;
    let mut phase_clean_checks = 0usize;
        ...
        for lender_count in lender_modes {
            ...
            if source_width != 0 {
                ...
                if source_width == MAX_SOURCE_WIDTH {
                    if lender_count == 3 {
                        three_lender_trace = Some(candidate_add.trace);
                    } else {
                        seven_lender_trace = Some(candidate_add.trace);
                    }
                }
            }
    ...
    let (baseline_local_builder, baseline_local_trace) =
        build_fused_prefix_scratch_loan_local_builder(259, 10, 7, false);
    configure_fused_prefix_scratch_loan_proof(true);
    ...
    assert_eq!(candidate_local.peak_qubits + 7, baseline_local.peak_qubits);
    assert_eq!(
        candidate_local.emitted_toffoli,
        baseline_local.emitted_toffoli
    );
    assert_eq!(candidate_local.emitted_ops + 7, baseline_local.emitted_ops);
...
fn build_work_and_length_swap(work_width: usize, length_width: usize, inverse: bool) -> B {
    ...
    if inverse {
        conditional_work_and_length_swap_inverse(
            ...
            &l_r_prime, window, window, &scratch_refs,
        );
    } else {
        conditional_work_and_length_swap(
            &mut circ, &control, &iteration, &work1, &work2, &l_t, &l_t_prime, &l_q, &l_s,
...
fn build_promised_l_q_swap_proof_harness_with_widths(
    ...
    let l_q = circ.alloc_qreg_bits("q847.swap.l-q", length_width);
