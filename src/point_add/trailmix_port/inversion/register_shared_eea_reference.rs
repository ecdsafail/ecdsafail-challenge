...
pub const SUB800_BORROW_PROMISED_SUPPORT_LENDER_FLAG: &str =
    "LOWQ_SUB800_BORROW_PROMISED_SUPPORT_LENDER";
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct BorrowPromisedSupportLenderProofReport {
    pub configurations_checked: usize,
    pub directions_checked: usize,
    pub basis_states_checked: usize,
    pub scalar_equivalence_checks: usize,
    pub simulator_equivalence_checks: usize,
    pub inverse_pair_checks: usize,
    pub preserved_lender_restore_checks: usize,
    pub phase_clean_checks: usize,
    pub ancilla_clean_checks: usize,
    pub baseline_local: Q847LifetimeLocalResources,
    pub candidate_local: Q847LifetimeLocalResources,
...
fn sub800_borrow_promised_support_lender_requested() -> bool {
    std::env::var(SUB800_BORROW_PROMISED_SUPPORT_LENDER_FLAG)
        .ok()
        .as_deref()
        == Some("1")
}
...
fn conditional_work_and_length_swap_under_zero_predicate(
    ...
    let borrow_promised_support = sub800_borrow_promised_support_lender_requested()
        && support_workspace_width == 1
        && !preserved_dy_top_scratch.is_empty();
    let support_workspace_owned = if borrow_promised_support {
        Vec::new()
    } else {
        circ.alloc_qreg_bits("rs.swap-length.promised-support", support_workspace_width)
    };
    let effective_preserved_dy_top_scratch: Vec<&QReg> = if borrow_promised_support {
        Vec::new()
    } else {
        preserved_dy_top_scratch.to_vec()
    };
    let support_workspace_refs: Vec<&QReg> = if borrow_promised_support {
        vec![preserved_dy_top_scratch[0]]
    } else {
        support_workspace_owned.iter().collect()
    };
    let raw_prefix_scratch = if sub800_raw_prefix_preserved_lender_requested() {
        effective_preserved_dy_top_scratch.as_slice()
    } else {
        &[]
    };
    if q845_swap_only_t_prime_length_requested() {
    ...
    let predicate_swap_lenders = if sub800_raw_prefix_predicate_lender_requested() {
        ...
        uncompute_zero(circ, l_s, zero_s, scratch);
        vec![zero_s, support_workspace_refs[0]]
    } else {
        Vec::new()
    };
    match (inverse, route) {
    ...
    free_clean(circ, support_workspace_owned);
}
...
impl RawBitLengthProofEnvironment {
    fn capture() -> Self {
        const NAMES: [&str; 31] = [
            ...
            SUB800_BORROW_PROMISED_SUPPORT_LENDER_FLAG,
...
/// Exhaustively compare borrowing the promised-support lane from the preserved
/// canonical dy/top lender against the current owned-support route at reduced
/// widths. This proves the support-lender substitution only; full q838 route
/// validation still requires the trusted 9024-shot evaluator.
#[doc(hidden)]
#[must_use]
pub fn exhaustive_borrow_promised_support_lender_check(
) -> BorrowPromisedSupportLenderProofReport {
    assert!(
        std::env::var_os(SUB800_BORROW_PROMISED_SUPPORT_LENDER_FLAG).is_none(),
        "the promised-support lender borrow must default off"
    );
    let _environment = RawBitLengthProofEnvironment::capture();
    configure_q847_lifetime_prerequisites(true);
    std::env::set_var(PROMISED_LQ_SWAP_BORROW_FLAG, "1");
    std::env::set_var(PROMISED_SWAP_SUPPORT_LIFETIME_FUSION_FLAG, "1");
    std::env::set_var(Q845_SWAP_ONLY_T_PRIME_LENGTH_FLAG, "1");
    std::env::set_var(SUB800_RAW_PREFIX_PRESERVED_LENDER_FLAG, "1");
    std::env::set_var(SUB800_RAW_PREFIX_PREDICATE_LENDER_FLAG, "1");

    let configurations = [(1usize, 1usize, 1usize), (2, 2, 2)];
    let mut basis_states_checked = 0usize;
    let mut scalar_equivalence_checks = 0usize;
    let mut simulator_equivalence_checks = 0usize;
    let mut inverse_pair_checks = 0usize;
    let mut preserved_lender_restore_checks = 0usize;
    let mut phase_clean_checks = 0usize;
    let mut ancilla_clean_checks = 0usize;
    ...
    for &(work_width, length_width, r_length_width) in &configurations {
        ...
        for &inverse in &[false, true] {
            std::env::remove_var(SUB800_BORROW_PROMISED_SUPPORT_LENDER_FLAG);
            let baseline = build_promised_l_q_swap_proof_harness_with_widths(
                work_width,
                length_width,
                r_length_width,
            ...
            std::env::set_var(SUB800_BORROW_PROMISED_SUPPORT_LENDER_FLAG, "1");
            let candidate = build_promised_l_q_swap_proof_harness_with_widths(
                work_width,
                length_width,
                r_length_width,
            ...
            assert_eq!(baseline.data_ids, candidate.data_ids);
            assert_eq!(baseline.external_mask, candidate.external_mask);
            assert_eq!(baseline.preserved_dy_top_mask, candidate.preserved_dy_top_mask);

            let label = if inverse {
                b"q838-borrow-support-inverse".as_slice()
            } else {
                b"q838-borrow-support-forward".as_slice()
            };
            let (simulator_cases, phases, ancillas) = verify_q847_simulator_equivalence(
                label,
                &baseline.builder,
            ...
            for value in 0..(1usize << baseline.data_ids.len()) {
                ...
                basis_states_checked += 1;
                scalar_equivalence_checks += 1;
                preserved_lender_restore_checks += 1;
            }
        ...
        for value in 0..(1usize << candidate_forward.data_ids.len()) {
            ...
            assert_eq!(
                ...
                input,
                "borrowed promised-support forward/inverse pair failed"
            );
            inverse_pair_checks += 1;
        }
    }

    std::env::set_var(MIXED_WIDTH_L_R_PRIME_FLAG, "1");
    std::env::remove_var(SUB800_BORROW_PROMISED_SUPPORT_LENDER_FLAG);
    let baseline_local = q847_lifetime_local_resources(
        &build_promised_l_q_swap_proof_harness_with_widths(
            259,
    ...
    BorrowPromisedSupportLenderProofReport {
        ...
        simulator_equivalence_checks,
        inverse_pair_checks,
        preserved_lender_restore_checks,
        phase_clean_checks,
        ancilla_clean_checks,
        baseline_local,
        candidate_local,
        local_qubit_delta: candidate_local.peak_qubits as i64
            - baseline_local.peak_qubits as i64,
        local_ops_delta: candidate_local.emitted_ops as i64 - baseline_local.emitted_ops as i64,
        local_toffoli_delta: candidate_local.emitted_toffoli as i64
            - baseline_local.emitted_toffoli as i64,
    }
