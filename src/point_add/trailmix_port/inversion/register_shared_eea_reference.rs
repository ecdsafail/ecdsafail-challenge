...
impl Q845SwapOnlyCoefficientGuard {
    ...
    {
        use crate::point_add::trailmix_port::arith::khattar_gidney::unary_iterate_log_star_borrowed;
        ...
        // `range_scratch` is already-allocated cursor scratch, disjoint from the
        // `address` counter bits and the detector gate. Borrowing it as dirty
        // lenders lets the flat all-ones detector run with NO clean prefix-AND
        // ancilla (`uls_anc` eliminated). Each `mcx_dirty_ladder` edge restores
        // every lender, so the body's own use of `range_scratch` is unaffected.
        let detector_lenders: Vec<&QReg> = range_scratch.iter().collect();
        // Gateless direct-target mode: the detector toggles `active` in place
        // (no `uls_gate`). Forward order is toggle-then-body, matching the
        // gate-mode closure (`cx(gate, active)` then body).
        unary_iterate_log_star_borrowed(
            circ,
            &address,
            if truncated {
                scan_width + 1
        ...
        F: FnMut(&mut Circuit, usize, &QReg),
    {
        ...
        let address = self.address.iter().collect::<Vec<_>>();
        // See `for_each_forward`: borrow `range_scratch` as dirty lenders so the
        // flat all-ones detector needs no clean prefix-AND ancilla.
        let detector_lenders: Vec<&QReg> = range_scratch.iter().collect();
        unary_iterate_log_star_borrowed(
            circ,
            &address,
            ...
            &detector_lenders,
            // Reverse sweep: body-first (consume `active` before this edge's
            // toggle), matching the gate-mode order (body then cx(gate, active)).
            Some((active, true)),
            |circ, reverse_index, _gate| {
