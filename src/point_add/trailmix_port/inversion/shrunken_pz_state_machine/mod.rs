...
fn p_plus_1_bytes() -> Vec<u8> {
    let mut b = crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE.to_vec();
    let mut carry = 1u16;
    for byte in &mut b {
...
fn shrunken_pz_srot_bits() -> usize {
    std::env::var("SHRUNKEN_PZ_SROT_BITS")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(6)
}

fn shrunken_pz_counter_bits() -> usize {
    std::env::var("SHRUNKEN_PZ_COUNTER_BITS")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(10)
}
...
/// Canary for the low-Q cursor-pack route: derive a moving prefix boundary from
/// the same PZ bitlength primitive used by the state machine, use it to drive a
/// co-moving arithmetic step, then reverse and clean the boundary.
pub fn run_cursor_pack_pz_bitlen_canary() {
    use crate::point_add::trailmix_port::arith::cuccaro::{
        controlled_add_cuccaro_3n_refs, controlled_add_cuccaro_3n_reverse_refs,
    };
    use crate::point_add::trailmix_port::arith::khattar_gidney::unary_iterate_log_star;
    use crate::point_add::trailmix_port::circuit::ContractSimView;

    fn load_bits(circ: &mut Circuit, regs: &[QReg], mut value: u64, shot: usize) {
        for q in regs {
            if (value & 1) == 1 {
    ...
    fn prefix_add_by_msb_pos(
        ...
        let pos_refs: Vec<&QReg> = msb_pos.iter().collect();
        unary_iterate_log_star(circ, &pos_refs, acc.len(), |circ, pos, gate| {
            let prefix_len = pos + 1;
            let ar: Vec<&QReg> = acc[..prefix_len].iter().collect();
            let br: Vec<&QReg> = addend[..prefix_len].iter().collect();
            if reverse {
                controlled_add_cuccaro_3n_reverse_refs(circ, gate, &ar, &br);
            } else {
                controlled_add_cuccaro_3n_refs(circ, gate, &ar, &br);
            }
        });
    }

    let mut circ = Circuit::new();
    circ.set_max_qubit_peak(160);
    let n = 16usize;
    let pos_w = 5usize;
    let a: Vec<QReg> = (0..n)
        .map(|i| circ.alloc_input_qreg(&format!("pz_a{i}")))
        .collect();
    let ca: Vec<QReg> = (0..n)
        .map(|i| circ.alloc_input_qreg(&format!("pz_ca{i}")))
        .collect();
    let cb: Vec<QReg> = (0..n)
        .map(|i| circ.alloc_input_qreg(&format!("pz_cb{i}")))
        .collect();

    for shot in 0..64 {
        let msb = shot % n;
        let low_mask = if msb == 0 { 0 } else { (1u64 << msb) - 1 };
        let av = (1u64 << msb) | (((shot as u64).wrapping_mul(0x9e37)) & low_mask);
        let cv_mask = (1u64 << n) - 1;
        let cav = ((shot as u64).wrapping_mul(0x4d) ^ 0x5a5a) & cv_mask;
        let cbv = ((shot as u64).wrapping_mul(0x73) ^ 0xa5a5) & cv_mask;
        load_bits(&mut circ, &a, av, shot);
        load_bits(&mut circ, &ca, cav, shot);
        load_bits(&mut circ, &cb, cbv, shot);
    }

    let a_pre: Vec<&QReg> = a.iter().collect();
    let ca_pre: Vec<&QReg> = ca.iter().collect();
    let cb_pre: Vec<&QReg> = cb.iter().collect();
    circ.contract_capture(
        "cursor_pack.pz_bitlen.roundtrip_pre",
        move |view, shot| -> Result<(u64, u64, u64), String> {
            Ok((
                read_bits(&view, &a_pre, shot),
                read_bits(&view, &ca_pre, shot),
                read_bits(&view, &cb_pre, shot),
            ))
        },
    );

    let pos = circ.alloc_qreg_bits("cursor_pack.pz_pos", pos_w);
    xor_const(&mut circ, &pos, n);
    let aref: Vec<&QReg> = a.iter().collect();
    bit_length_lean_middle(&mut circ, &aref, &pos, |circ| {
        let a_mid: Vec<&QReg> = a.iter().collect();
        let ca_mid: Vec<&QReg> = ca.iter().collect();
        let cb_mid: Vec<&QReg> = cb.iter().collect();
        let pos_mid: Vec<&QReg> = pos.iter().collect();
        circ.contract_capture(
            "cursor_pack.pz_bitlen.forward_pre",
            move |view, shot| -> Result<(u64, u64, u64, u64), String> {
                Ok((
                    read_bits(&view, &a_mid, shot),
                    read_bits(&view, &ca_mid, shot),
                    read_bits(&view, &cb_mid, shot),
                    read_bits(&view, &pos_mid, shot),
                ))
            },
        );

        prefix_add_by_msb_pos(circ, &pos, &ca, &cb, false);

        let a_after: Vec<&QReg> = a.iter().collect();
        let ca_after: Vec<&QReg> = ca.iter().collect();
        let cb_after: Vec<&QReg> = cb.iter().collect();
        let pos_after: Vec<&QReg> = pos.iter().collect();
        circ.contract_pop_and_check::<(u64, u64, u64, u64), _>(
            "cursor_pack.pz_bitlen.forward_pre",
            move |cap, view, shot| -> Result<(), String> {
                let (a0, ca0, cb0, pos0) = cap;
                let a1 = read_bits(&view, &a_after, shot);
                let ca1 = read_bits(&view, &ca_after, shot);
                let cb1 = read_bits(&view, &cb_after, shot);
                let pos1 = read_bits(&view, &pos_after, shot);
                if a1 != *a0 || cb1 != *cb0 || pos1 != *pos0 {
                    return Err(format!(
                        "pz-bitlen source/addend/boundary drift on shot {shot}: a {a0:#x}->{a1:#x}, cb {cb0:#x}->{cb1:#x}, pos {pos0}->{pos1}"
                    ));
                }
                let prefix = (*pos0 as usize + 1).min(16);
                let high_mask = !((1u64 << prefix) - 1) & ((1u64 << 16) - 1);
                if (ca1 & high_mask) != (*ca0 & high_mask) {
                    return Err(format!(
                        "pz-bitlen prefix carry escaped above {prefix} on shot {shot}: ca {ca0:#x}->{ca1:#x}"
                    ));
                }
                Ok(())
            },
        ...
        prefix_add_by_msb_pos(circ, &pos, &ca, &cb, true);
        true
    });
    xor_const(&mut circ, &pos, n);
    for q in pos {
    ...
    let a_final: Vec<&QReg> = a.iter().collect();
    let ca_final: Vec<&QReg> = ca.iter().collect();
    let cb_final: Vec<&QReg> = cb.iter().collect();
    circ.contract_pop_and_check::<(u64, u64, u64), _>(
        "cursor_pack.pz_bitlen.roundtrip_pre",
        move |cap, view, shot| -> Result<(), String> {
            let (a0, ca0, cb0) = cap;
            let a1 = read_bits(&view, &a_final, shot);
            let ca1 = read_bits(&view, &ca_final, shot);
            let cb1 = read_bits(&view, &cb_final, shot);
            if a1 != *a0 || ca1 != *ca0 || cb1 != *cb0 {
                return Err(format!(
                    "pz-bitlen cursor-pack reverse failed on shot {shot}: pre=({a0:#x},{ca0:#x},{cb0:#x}) now=({a1:#x},{ca1:#x},{cb1:#x})"
                ));
            }
            Ok(())
        },
    ...
    eprintln!(
        "trailmix_cursor_pack_pz_bitlen_canary: PASS peak={} ops={} ccx={}",
        circ.peak_qubits,
        circ.total_ops(),
        circ.ccx_count(),
    );
...
fn load_ghost_temp(
    circ: &mut Circuit,
    ghosts: &[crate::point_add::trailmix_port::tracker::ghost::Ghost],
    name: &str,
) -> Vec<QReg> {
    let temp = circ.alloc_qreg_bits(name, ghosts.len());
    for (q, g) in temp.iter().zip(ghosts.iter()) {
        circ.x_if_bit(q, g.bit);
    }
    temp
}

fn unload_ghost_temp(
    circ: &mut Circuit,
    temp: Vec<QReg>,
    ghosts: &[crate::point_add::trailmix_port::tracker::ghost::Ghost],
) {
    for (q, g) in temp.iter().zip(ghosts.iter()).rev() {
        circ.x_if_bit(q, g.bit);
    }
    for q in temp {
        circ.zero_and_free(q);
...
/// value is `parity ? cb : p-cb`), with A=p, B=|x| at the EEA terminal. `s_rot`
/// is fixed-width per build (`SHRUNKEN_PZ_SROT_BITS`, default 6), while `off`,
/// `parity`, and `counter` (`SHRUNKEN_PZ_COUNTER_BITS`, default 10) are fixed-width.
#[allow(clippy::too_many_arguments)]
...
pub fn shrunken_pz_divide_forward(
    ...
    let early_ghost_dy = std::env::var("TRAILMIX_LOWQ_EARLY_GHOST_DY")
        .ok()
        .as_deref()
        == Some("1");
    let mut dy_live = dy;
    let mut dy_ghosts = Vec::with_capacity(dy_live.len());
    if early_ghost_dy {
        for q in &dy_live {
            dy_ghosts.push(c.hmr_ghost(q));
        }
        for q in std::mem::take(&mut dy_live) {
            c.zero_and_free(q);
        }
    }
    let p =
        BigUint::from_bytes_le(&crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE);
    // sgn = dx > p/2  <=>  dx >= (p+1)/2.
    let half = (&p + 1u32) >> 1u32;
    let mut half_bytes = half.to_bytes_le();
    half_bytes.resize(33, 0);
    let p_bytes = crate::point_add::trailmix_port::arith::mod_arith::SECP256K1_P_LE;
    ...
    let s_rot = c.alloc_qreg_bits("shpzdiv.srot", shrunken_pz_srot_bits());
    let off = c.alloc_qreg("shpzdiv.off");
    let parity = c.alloc_qreg("shpzdiv.par");
    let counter = c.alloc_qreg_bits("shpzdiv.ctr", shrunken_pz_counter_bits());
    ...
    if early_ghost_dy {
        let dy_tmp = load_ghost_temp(c, &dy_ghosts, "shpzdiv.dy_tmp");
        mod_mul_rfold_mbu(c, &lambda, &ccb[..257], &dy_tmp); // lambda_raw = dy * cb
        unload_ghost_temp(c, dy_tmp, &dy_ghosts);
    } else {
        mod_mul_rfold_mbu(c, &lambda, &ccb[..257], &dy_live); // lambda_raw = dy * cb
    }
    ...
    let mut lambda_ghosts = Vec::with_capacity(lambda.len());
    if early_ghost_dy {
        for q in &lambda {
            lambda_ghosts.push(c.hmr_ghost(q));
        }
        for q in std::mem::take(&mut lambda) {
            c.zero_and_free(q);
        }
    }
    ...
    if early_ghost_dy {
        lambda = load_ghost_temp(c, &lambda_ghosts, "shpzdiv.lambda");
        for (g, q) in lambda_ghosts.into_iter().zip(lambda.iter()) {
            c.resolve_ghost(g, q);
        }
    }
    ...
    for (g, q) in dy_ghosts.into_iter().zip(dy_new.iter()) {
        c.resolve_ghost(g, q);
    }
...
pub fn shrunken_pz_divide_cancel(
    ...
    let lowq_ghost_dy = std::env::var("TRAILMIX_LOWQ_EARLY_GHOST_DY")
        .ok()
        .as_deref()
        == Some("1");
    let mut dy_ghosts = Vec::with_capacity(dy.len());
    if lowq_ghost_dy {
        for q in &dy {
            dy_ghosts.push(c.hmr_ghost(q));
        }
        for q in std::mem::take(&mut dy) {
            c.zero_and_free(q);
        }
    }
    ...
    let s_rot = c.alloc_qreg_bits("shpzcan.srot", shrunken_pz_srot_bits());
    let off = c.alloc_qreg("shpzcan.off");
    let parity = c.alloc_qreg("shpzcan.par");
    let counter = c.alloc_qreg_bits("shpzcan.ctr", shrunken_pz_counter_bits());
    let load_p = |c: &mut Circuit, reg: &[QReg]| {
        for (j, q) in reg.iter().enumerate() {
            if j < 256 && (p_bytes[j / 8] >> (j % 8)) & 1 == 1 {
                c.x(q);
    ...
    let dy_tmp = if lowq_ghost_dy {
        load_ghost_temp(c, &dy_ghosts, "shpzcan.dy_tmp")
    } else {
        Vec::new()
    };
    let dy_refs = if lowq_ghost_dy { &dy_tmp } else { &dy };
    mod_mul_rfold_mbu(c, &temp, &ccb[..257], dy_refs); // temp_raw = dy * cb
    ...
    c.cx(&sgn, &f); // uncompute f
    c.zero_and_free(f);
    mod_mul_rfold_mbu_undo(c, &temp, &ccb[..257], dy_refs); // temp -> 0
    if lowq_ghost_dy {
        unload_ghost_temp(c, dy_tmp, &dy_ghosts);
    }
    for q in temp {
    ...
    if lowq_ghost_dy {
        dy = load_ghost_temp(c, &dy_ghosts, "shpzcan.dy");
        for (g, q) in dy_ghosts.into_iter().zip(dy.iter()) {
            c.resolve_ghost(g, q);
        }
    }
