...
pub fn q944_reverse_materialize_non_sentinel_index(circ: &mut Circuit, q: &[QReg], s: &[&QReg]) {
    assert_eq!(q.len(), Q944_QUOTIENT_WIDTH);
    assert_eq!(s.len(), Q944_SHIFT_WIDTH);
...
fn build_harness() -> Harness {
    ...
    let expected_qubits = 1 + Q944_QUOTIENT_WIDTH + Q944_SHIFT_WIDTH + (Q944_SHIFT_WIDTH - 1) + 1;
        ...
        for shift in 0..Q944_QUOTIENT_WIDTH {
            for target in 0..=1u64 {
                for lender_word in 0..(1u64 << harness.lenders.len()) {
                    ...
                    let (hosted_boundary, hosted_boundary_phase) =
                        apply_scalar(&harness.builder.ops[..harness.hosted_body_end], initial);
