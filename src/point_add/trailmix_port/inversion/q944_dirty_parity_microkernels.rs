...
fn assert_comparator_layout(gate: &QReg, carry: &QReg, v: &[&QReg], u: &[&QReg], out: &QReg) {
    assert!(
        !v.is_empty(),
        "Q944 dirty-parity comparator requires n >= 1"
    );
    assert_eq!(
        v.len(),
        u.len(),
        "Q944 dirty-parity comparator width mismatch"
    );
...
fn build_arithmetic(width: usize, kind: ArithmeticKind) -> B {
    ...
    match kind {
        ...
        ArithmeticKind::Add => controlled_add_dirty_carry_refs(&mut circ, &gate, &carry, &ar, &br),
        ArithmeticKind::Sub => controlled_sub_dirty_carry_refs(&mut circ, &gate, &carry, &ar, &br),
    }
...
fn build_comparator(width: usize, raw: bool) -> B {
    ...
    if raw {
        strict_compare_gated_dirty_carry_raw_refs(&mut circ, &vr, &ur, &gate, &out, &carry);
    } else {
...
fn assert_raw_comparator_gate_shape(ops: &[Op], width: usize) {
    ...
    let next_ccx = |cursor: &mut usize, control1: u64, control2: u64, target: u64| {
        let op = &ops[*cursor];
        assert_eq!(
            (op.kind, op.q_control2.0, op.q_control1.0, op.q_target.0,),
