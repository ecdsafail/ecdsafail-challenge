...
pub struct RegisterSharedStepObservation {
    ...
    pub coefficient_l_t: usize,
    pub coefficient_l_s: usize,
    pub coefficient_t_prime_trailing_zeros_before: usize,
    pub coefficient_t_prime_trailing_zeros_after: usize,
    pub coefficient_source_guard_clean: bool,
    pub coefficient_adjacent_work1_before: bool,
    pub coefficient_adjacent_work2_before: bool,
    pub coefficient_adjacent_work2_after: bool,
...
impl State {
    fn work1_packed_bit(&self, index: usize) -> bool {
        let width = self.n + 3;
        assert!(index < width);
        if index < self.work1.l_t {
            return !((self.work1.t.magnitude >> index) & U512::from(1u64)).is_zero();
        }
        if index == self.work1.l_t {
            return false;
        }
        if index < self.work1.l_t + 1 + self.work1.l_q {
            let physical_index = index - self.work1.l_t - 1;
            let q_width = bit_len(self.work1.q);
            assert!(q_width >= self.work1.l_q);
            let active_q = self.work1.q >> (q_width - self.work1.l_q);
            let source_index = self.work1.l_q - 1 - physical_index;
            return !((active_q >> source_index) & U512::from(1u64)).is_zero();
        }
        let remainder_bit = width - 1 - index;
        !((self.work1.r.magnitude >> remainder_bit) & U512::from(1u64)).is_zero()
    }

    fn work2_packed_bit(&self, physical_index: usize) -> bool {
        let width = self.n + 3;
        assert!(physical_index < width);
        let raw_index = (physical_index + self.work2.l_s) % width;
        let t_prime = !((self.work2.t_prime.magnitude >> raw_index) & U512::from(1u64)).is_zero();
        let remainder_bit = width - 1 - raw_index;
        let r_prime = remainder_bit < self.work2.l_r_prime
            && !((self.work2.r_prime.magnitude >> remainder_bit) & U512::from(1u64)).is_zero();
        assert!(!(t_prime && r_prime), "Work2 packed components overlap");
        t_prime || r_prime
    }
    ...
    fn step(&mut self, step: usize) -> RegisterSharedStepObservation {
        ...
        if self.control.phase1 {
            ...
            let scheduled_end = ((step + 3) / 4).min(self.n) + 1;
            ...
            observation.coefficient_l_t = self.work1.l_t;
            observation.coefficient_l_s = self.work2.l_s;
            observation.coefficient_t_prime_trailing_zeros_before =
                self.work2.t_prime.magnitude.trailing_zeros() as usize;
            observation.coefficient_source_guard_clean = !self.work1_packed_bit(self.work1.l_t);
            observation.coefficient_adjacent_work1_before = self.work1_packed_bit(scheduled_end);
            observation.coefficient_adjacent_work2_before = self.work2_packed_bit(scheduled_end);
        ...
        if self.control.phase1 {
            ...
            observation.coefficient_t_prime_trailing_zeros_after =
                self.work2.t_prime.magnitude.trailing_zeros() as usize;
            observation.coefficient_adjacent_work2_after =
                self.work2_packed_bit(((step + 3) / 4).min(self.n) + 1);
        }
